...
// metadata for AI
ifdef::ai[]
Summary: Adds hyperlinks to a chart dataset. Use `setParameterField` to specify values to pass as parameters in the link and `setTargetFrame` to specify the link’s target frame. See the example below.
endif::ai[]
