= y2Title.visible
%experimental%
include::ROOT:partial$buttonSnippets.adoc[]

// metadata for AI
ifdef::ai[]
Summary: The `y2Title.visible` property (Boolean) gets or sets the visibility of the secondary (right) Y-axis title on a Chart component in Dashboard script. The default is `true` if a value has been provided for the y2Title text, otherwise `false`.
endif::ai[]



Sets the visibility of the secondary (right) Y-axis. The default is `true` (visible) if a value has been provided for xref:AxisY2TitleText.adoc[], otherwise `false`.

== Type
Boolean






== Example
[source,javascript]
Chart1.y2Title.visible = false;




[TIP]
====
Either syntax below can be used in  xref:AddScriptToDashboard.adoc#AddComponentScript[component script]:
[source,javascript]
y2Title.visible             // unqualified
Chart1.y2Title.visible      // qualified

{secondForm}
====

