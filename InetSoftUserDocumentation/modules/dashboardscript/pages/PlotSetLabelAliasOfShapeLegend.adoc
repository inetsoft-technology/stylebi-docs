= setLabelAliasOfShapeLegend(value, alias)
:navtitle: setLabelAliasOfShapeLegend()
%experimental%
include::ROOT:partial$buttonSnippets.adoc[]

// metadata for AI
ifdef::ai[]
Summary: Replace the specified label in the shape legend with an alias. {noGetter}
endif::ai[]


Replace the specified label in the shape legend with an alias. {noGetter}



== Parameters
value::			Value to be replaced
alias::			Replacement string




== Examples

[source,javascript]
Chart1.setLabelAliasOfShapeLegend('NJ','New Jersey');
Chart1.setLabelAliasOfShapeLegend('NY','New York');


image::commonscript:setLabelAliasOfShapeLegend.png[alt="chart shape legend with label values replaced by custom aliases"]

[TIP]
====
Either syntax below can be used in xref:dashboardscript:AddScriptToDashboard.adoc#AddComponentScript[component script]:
[source,javascript]
setLabelAliasOfShapeLegend(value, alias)           // unqualified
Chart1.setLabelAliasOfShapeLegend(value, alias)    // qualified

{secondForm}
====