= query
%experimental%
include::ROOT:partial$buttonSnippets.adoc[]

// metadata for AI
ifdef::ai[]
Summary: xref:ChartBindingInfoXFields.adoc[] and xref:ChartBindingInfoYFields.adoc[] to specify the fields to display in the Chart.
endif::ai[]

[WARNING,caption=See Also]
xref:ChartBindingInfoXFields.adoc[] and xref:ChartBindingInfoYFields.adoc[] to specify the fields to display in the Chart.


Specifies the data block to be used by the Chart component. The available data blocks appear at the top of the *Toolbox* panel in Visual Composer.

image::dataworksheet:dataBlocks.png[alt="Toolbox panel showing available data blocks for chart query binding"]

== Type
String




== Example
[source,javascript]
Chart1.query = "All Sales";
Chart1.bindingInfo.xFields = [["Employee",Chart.STRING]];
Chart1.bindingInfo.yFields = [["Total",Chart.NUMBER]];

image::commonscript:bindingInfoXFields.png[alt="bar chart with Employee on X-axis and Total on Y-axis"]


[TIP]
====
Either syntax below can be used in  xref:AddScriptToDashboard.adoc#AddComponentScript[component script]:
[source,javascript]
query              // unqualified
Calendar1.query    // qualified

{secondForm}
====







