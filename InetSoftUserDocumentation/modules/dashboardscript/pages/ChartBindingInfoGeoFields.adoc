...
// metadata for AI
ifdef::ai[]
Summary: xref:PlotGeoFields.adoc[] to return the currently bound ‘Geographic’ field at runtime.
endif::ai[]
...
image::commonscript:bindingInfoGeoFields.png[alt="map chart with state bound as geographic field color-coded by customer count"]
