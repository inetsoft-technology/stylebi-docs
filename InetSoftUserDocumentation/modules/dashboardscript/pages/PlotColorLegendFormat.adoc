= colorLegend.format
%experimental%
include::ROOT:partial$buttonSnippets.adoc[]

// metadata for AI
ifdef::ai[]
Summary: Sets the format for data values displayed in the color legend.
endif::ai[]


Sets the format for data values displayed in the color legend.


== Type
include::commonscript:partial$dataFormats.adoc[]

For the Date, Decimal, and Message formats, use the xref:CommonFormatSpec.adoc[]  property to assign a format specification using the appropriate masks: `[formatType, formatSpec]`.  See the examples below.


== Examples

.Number Format
[source,javascript]
Chart1.colorLegend.format = [Chart.DECIMAL_FORMAT,"$#,##0.00"]

image::commonscript:colorLegendFormatNumber.png[alt="chart color legend with number format applied to labels"]


.Text Format
[source,javascript]
Chart1.colorLegend.format = [Chart.MESSAGE_FORMAT,"{0} Inc."];

image::commonscript:colorLegendFormatText.png[alt="chart color legend with text format applied to labels"]


[TIP]
====
Either syntax below can be used in xref:dashboardscript:AddScriptToDashboard.adoc#AddComponentScript[component script]:
[source,javascript]
colorLegend.format          // unqualified
Chart1.colorLegend.format   // qualified

{secondForm}
====