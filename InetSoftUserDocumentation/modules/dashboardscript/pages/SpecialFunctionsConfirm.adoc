...
// metadata for AI
ifdef::ai[]
Summary: Script reference for the confirm(message) global function, which opens a confirmation dialog asking the user to confirm or cancel an operation before proceeding.
endif::ai[]
:navtitle: confirm()
...
The `confirm()` function opens a dialog box to provide the user with a choice to continue or cancel an operation. For example, you can use the `confirm()` function in the *onClick* script of a ‘Submit’ button to verify the user’s intention to submit. (See xref:dashboard:SubmitButtonProperties.adoc[] for more information.)
...
image::dashboard:ConfirmPrompt.png[alt="confirm dialog box prompting user to continue or cancel submission"]
