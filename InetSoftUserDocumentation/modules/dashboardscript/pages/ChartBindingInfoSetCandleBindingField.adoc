...
:navtitle: setCandleBindingField()
%experimental%
include::ROOT:partial$buttonSnippets.adoc[]

// metadata for AI
ifdef::ai[]
Summary: xref:ChartBindingInfoGetCandleBindingField.adoc[] to obtain existing binding information.
endif::ai[]
...
Configure the binding for a xref:dashboard:CandleChart.adoc[]. A candlestick chart displays four different measures, “low,” “high,” “opening,” and “closing,” and is most often used to plot trading information.
...
+
image::commonscript:setCandleBinding2.png[alt="candle chart displaying average high low open close prices by quarter"]
+
...
+
image::commonscript:setCandleBinding3.png[alt="candle chart filtered to year 2004 showing quarterly OHLC structure"]
...
TIP: Typically, different formulas are used for the different measures.  For example, *max* for the "high", *min* for the "low", *first* for the open, and *last* for the close.  See xref:dashboard:CandleChart.adoc[] for an example that uses this approach.
