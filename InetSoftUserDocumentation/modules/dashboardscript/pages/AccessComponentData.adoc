...
// metadata for AI
ifdef::ai[]
Summary: This page explains how to access component data and filter selections in Dashboard script. It covers: (1) Form components — using selectedObject/selectedObjects and selectedLabel/selectedLabels to read the state of Slider, Spinner, RadioButton, ComboBox, CheckBox, and Tabbed Interface components; (2) Filter components — using selectedObjects/selectedLabels on Selection List, Selection Tree, Range Slider, and Calendar, and drillMember on Selection Tree to get the next hierarchy level below the user's lowest selection; (3) Output components — using the value property on Gauge, Text, and Image components; (4) Chart components — using the data or table properties (covered in full in the Chart Script tutorial); (5) Table/Crosstab components — using table[rowIx][colIx] (displayed data, including headers) and data[rowIx][colIx] (raw pre-summarized data), along with table.length/table.size (row/column counts as displayed), data.length/data.size (raw counts), the field['col_name'] keyword (value in current row of a named column), the value keyword (current cell value), and the row/col variables for relative cell references. Applies to Dashboard (Viewsheet) script context.
endif::ai[]

include::dashboardscript:partial$seeAlsoComponentData.adoc[]

You can access  component data and filter selections using script. The following sections explain how to do this. For information on how to detect the most recent user interaction, see xref:dashboard:WriteBackToStorage.adoc#AccessUserModifiedData[Access  User-Modified Data] in xref:dashboard:WriteBackToStorage.adoc[].
...
xref:dashboardscript:CommonSelectedObject.adoc[], xref:dashboardscript:CommonSelectedLabel.adoc[]:: The selected value or label of the component: Applies to *single-value* input elements (Slider, Spinner, RadioButton, ComboBox) and the tabs of co-located components in a Tabbed Interface.  For example, if RadioButton1 has a value '`NY`' with corresponding label '`New York`', you can choose this value and display the label as follows:
+
...
xref:dashboardscript:CommonSelectedObjects.adoc[], xref:dashboardscript:CommonSelectedLabels.adoc[]::
...
image::dashboard:ComponentSelectedObject.png[alt="script editor component tree showing selectedObject node"]

The less-commonly used xref:dashboardscript:TextInput.adoc[]   provides a `value` property to return the current text.
...
+
image::dashboard:Object Access RadioButton.png[alt="radio button with two label and value entries added"]
...
+
image::dashboard:Object Access Viewsheet.png[alt="dashboard with radio button selection list and range slider"]
...
+
image::dashboard:Selection List Properties Expression.png[alt="selection list properties with enabled set to expression"]
...
xref:dashboardscript:SelectionListTreeDrillMember.adoc[]:: For Selection Trees, the name of the field that resides one level in the hierarchy below the user’s lowest selected level:
+
.Example: drillMember
[source,javascript]
...
+
image::commonscript:DrillMemberExample1.png[alt="selection tree with region state and city hierarchy fields"]
...
+
image::commonscript:DrillMemberExample2.png[alt="chart editor with Region dimension set to expression type"]

.. Press the {buttonEditFormula} to open the Script Editor.
+
image::commonscript:DrillMemberExample6.png[alt="edit formula button next to Region dimension expression"]
...
+
image::commonscript:DrillMemberExample4.png[alt="chart editor with drillMember expression assigned to X-axis binding"]
...
+
image::commonscript:DrillMemberExample5.png[alt="chart filtered by state selection showing city dimension on X-axis"]
====
...
xref:dashboard:WriteBackToStorage.adoc#AccessUserModifiedData[Access  User-Modified Data] in xref:dashboard:WriteBackToStorage.adoc[] information on how to access an *Editable Table*.

The key properties for accessing the values in a Table or Crosstab are xref:dashboardscript:TableData.adoc[] and xref:dashboardscript:TableTable.adoc[].
...
image::dashboard:RelativeReferenceEditor.png[alt="script editor component tree showing table data and field nodes"]
...
NOTE:  You can apply conditional formatting without using script. See xref:dashboard:ConditionalFormat.adoc[] in the xref:dashboard:VisualizeYourData.adoc[].
...
+
image::dashboard:ProductInfoDataBlock.png[alt="ProductInfo data block table added to the dashboard"]

. Click to select any data cell in the ‘Price’ column. Then select the *Format* tab at the bottom of the left panel.
+
image::dashboard:Capture_009.png[alt="Price column cell selected with Format tab visible"]


. Press the {buttonValueType} next to the ‘Color’ property, and select ‘Expression’ option.
+
image::Color Tab Formula Editor Button.png[alt="Color property with expression option selected in Format tab"]
...
+
image::formulaEditor1.png[alt="formula editor with color expression script using value greater than 1000"]
...
+
image::dashboard:Capture_011.png[alt="table with Price column cells colored red for values over 1000"]
...
. Press btn:[OK] to exit the Formula Editor.
+
image::dashboardscript:Set Property Using Field.png[alt="Name column highlighted based on corresponding Price column values"]
...
image::dashboard:Expression Access Relative Cell.png[alt="table with cells highlighted yellow where price is less than row above"]
...
. Press btn:[OK] to exit the Formula Editor.
+
image::dashboard:Expression Access Absolute Cell.png[alt="Price column header cell shown red because a price exceeds 2000"]
...
+
image::dashboard:Expression Drag Price Slider.png[alt="Range Slider bound to Price field added to the dashboard"]
