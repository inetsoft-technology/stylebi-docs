...
// metadata for AI
ifdef::ai[]
This page explains how to add JavaScript to a Dashboard in Visual Composer. It covers three levels at which script can be attached: (1) property-level expressions, which dynamically set individual component property values (e.g., visibility, color) via the Formula Editor; (2) component-level script, which runs whenever a component refreshes and can modify component properties including color, font, visibility, and alignment, or execute on user click via an onClick handler; and (3) dashboard-level script in the onInit handler (runs once on load, for initialization) or the onRefresh handler (runs on every Dashboard refresh). The page also describes script evaluation order (onInit → onRefresh → component scripts), how to access Java packages from script, and how to add Chart script using the Chart API.
endif::ai[]

If you need to modify a Dashboard at runtime, or implement  more complex business logic, you can add script to your Dashboard. There are three levels at which you can attach script in a Dashboard:
...
[square]
* For information on using simple scriptable Chart properties, see xref:Chart.adoc[].  This approach produces scripts that are easier to read and maintain.
...
Note that scripted Charts are not good candidates for user-modification, so you should *deselect* ‘Enable Ad Hoc Editing’ in the Chart Properties dialog box for scripted Charts.
====
...
1: Execute the *onInit* handler:: The onInit handler executes only once, when the Dashboard is first loaded. It can be used to initialize variables and define parameters. It *cannot* access user input or component data. See xref:#AddDashboardScript[] for more information.
...
3: Execute all *component-level* scripts:: Component scripts and property expressions are re-executed whenever the particular component is refreshed due to a user operation (on the component itself, or on a related component).  See xref:#AddPropertyScript[] xref:#AddComponentScript[] and for more information. For a given component, property expressions are  evaluated prior to the component script.
+
...
image::dashboard:Expression Select Popup.png[alt="property field showing the value type button with Expression option"]

This is especially useful when you want to automatically and dynamically adjust a property based on some other option that the user selects.  *Example:* If the user selects a particular measure for a Chart from a ComboBox (e.g., '`Products Sold`'), use the {buttonValueType} with  ‘Expression’ option for  measure`'s '`Aggregate`' property to add script to that returns an appropriate summarization method (e.g., '`Count`') for the '`Aggregate`' property.
...
+
image::dashboard:Right Click Properties.png[alt="right-click context menu showing Properties option for a component"]
...
+
image::dashboard:Expression Option.png[alt="Visible property with Expression selected from value type menu"]

. Press the {buttonEditFormula} to open the Formula Editor.
+
image::dashboard:Edit Expression Button.png[alt="edit formula button enabled next to Expression property field"]
...
+
image::dashboard:Formula Editor Function.png[alt="formula editor with Functions list and Components panel visible"]
+
See xref:dashboardscript:DashboardObjectReference.adoc[] and xref:commonscript:UserFunctions.adoc[] for a full description of available properties and functions.
+
...
+
image::dashboard:ScriptEditorText.png[alt="script editor Script tab with Script and onClick buttons at bottom"]
+
...
+
Select functions and operators from the top-right panel or use the ‘Search’ box. See xref:dashboardscript:DashboardObjectReference.adoc[] and xref:commonscript:UserFunctions.adoc[] for available functions. To enter properties such as `RadioButton1.selectedObject`, parameters such as `pass:[parameter._USER_]`, or data such as `data['Count(Order Number)']`,  select these options in the top-left panel of the Editor, or use the ‘Search’ box.
+
...
image::dashboard:scriptComponentSymbol.png[alt="component with JS symbol in top-left corner indicating attached script"]
...
|Underline|Draw an underline below the text. The line style should be an available xref:commonscript:StyleConstantLineStyle.adoc[].
...
[square]
* xref:commonscript:StyleConstant.adoc[] for commonly-used constant values.
* xref:dashboard:TextProperties.adoc[], for more information about Text properties.
====
The Alignment property controls both the horizontal and vertical alignment of the component. The horizontal alignment is specified by the alignment constants `H_LEFT`, `H_CENTER`, and `H_RIGHT`. (See xref:commonscript:StyleConstantAlignment.adoc[] for more details.)
...
Dashboard-level script allows you to perform high-level Dashboard logic such as checking and setting parameters or validating user input. To create a Dashboard script, follow the steps below:

. Press {buttonOptions} in the toolbar to open the ‘Dashboard Options’ panel.

. Select the *Script* tab. This opens the Script Editor.
+
image::dashboard:ViewsheetOptionsDialog.png[alt="Dashboard Options dialog box with Script tab showing script editor"]
