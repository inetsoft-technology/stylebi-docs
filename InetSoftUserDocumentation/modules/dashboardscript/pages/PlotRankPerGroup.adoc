...
// metadata for AI
ifdef::ai[]
Summary: When `true`, specifies that the top-N filter for a dimension (see xref:ChartBindingInfoSetTopN.adoc[]) should be relative to the parent dimension in the dimension hierarchy.
endif::ai[]
...
image::commonscript:rankPerGroup.png[alt="chart with data ranked separately within each group"]
