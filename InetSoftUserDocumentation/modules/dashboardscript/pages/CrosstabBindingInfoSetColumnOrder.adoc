...
// metadata for AI
ifdef::ai[]
Summary: Sets the sorting order for row or column header. This is equivalent to setting the sorting for a header in the Crosstab Editor. The `measure` parameter is required only when a “by value” sort is requested.
endif::ai[]
...
image::commonscript:bindingInfoSetColumnOrder.png[alt="crosstab editor showing sort method options for a header field"]
...
image::commonscript:bindingInfoSetColumnOrder2.png[alt="crosstab with Name rows sorted descending by Sum of Total value"]
