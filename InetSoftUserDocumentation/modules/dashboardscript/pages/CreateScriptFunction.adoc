...
// metadata for AI
ifdef::ai[]
Summary: In addition to using predefined JavaScript functions (see xref:commonscript:UserFunctions.adoc[]), you also can create your own functions. A custom function is a reusable component that can be applied in multiple Dashboards. By saving common script tasks to the script library, you can keep your main scrips short, simple, and easy to maintain. The following sections explain how to create and use a custom script function.
endif::ai[]
...
+
image::dashboard:portalDesignTab.png[alt="User Portal showing Create button and Dashboard or Worksheet option"]

. Select the *Assets* tab in the left panel (if not already selected).

. Expand the '`Library`' folder, and right-click the '`Script`' folder. {rightClickOptions} Select '`New Script Function`'.
+
image::customFunction1.png[alt="Assets panel with Script folder and New Script Function option selected"]
...
+
image::customFunction2.png[alt="script library editor showing custom function with parameter and body"]
+
...
+
.Example 1: tableTotal function (iteration)
[source,javascript]
----
// Parameter 'col' is a numerical index OR column name
...
----
+
TIP: Note in the above function that the property `table` is not qualified by a component ID (e.g., `Table1.table`). Functions are scoped dynamically; therefore, the property `table` refers to whichever Table component called the `tableTotal()` function. You can therefore call this `tableTotal()` function from within any Table xref:dashboardscript:AddScriptToDashboard.adoc#AddComponentScript[component script] to operate on that particular Table component. See xref:#FunctionScope[] below   for more information.
+
In the above  example, if you plan to call `tableTotal()`  with a *column name* as parameter (NOT a numerical index), the built-in xref:commonscript:CALC_sum.adoc[CALC.sum()] function provides a simpler script (below).  Use  built-in CALC functions whenever you can.
+
.Example 2: tableTotal function (CALC function)
[source,javascript]
----
// Parameter 'col' is a column name ONLY
function tableTotal(col) {
  return CALC.sum(table[col]);
}
----
...
+
image::customFunction3.png[alt="Script library folder expanded with existing function available to edit"]
...
[#FunctionScope]
== Function Scope
Function scoping is dynamic. This means that the scope in which the function executes is the scope of the *function caller*. For example, if a function is called from the scope of a xref:dashboardscript:AddScriptToDashboard.adoc#AddComponentScript[component script] on the   "`Table1`", then all the properties of  Table1 are available to the function. Properties that are not qualified with an element ID (e.g., `table`, `visible`, as opposed to `TableX.table`, `TableX.visible`) refer to the *caller*, Table1.
