...
// metadata for AI
ifdef::ai[]
Summary: Script reference for the setupGoogleMapsPlot() function, which creates a Google Maps background for a Chart component. Requires a Google Maps API key.
endif::ai[]
...
var heightPx = 400;
graph = new EGraph();
var urlPrefix = "https://maps.googleapis.com/maps/api/staticmap?&amp;key=<your API key>";
...
image::commonscript:GoogleMap.png[alt="chart showing Google Map background with labeled location points"]
