...
// metadata for AI
ifdef::ai[]
Summary: Returns the sorting order for a row or column header (as a xref:commonscript:StyleConstant.adoc[] integer) specified by xref:CrosstabBindingInfoSetColumnOrder.adoc[] or set by the ‘Sort’ option in the Crosstab Editor.
endif::ai[]
...
image::commonscript:bindingInfoSetColumnOrder_1.png[alt="crosstab editor showing column sort order setting for a row header"]
