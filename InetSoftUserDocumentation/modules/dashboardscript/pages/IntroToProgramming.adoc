...
// metadata for AI
ifdef::ai[]
Summary: You can add advanced interactivity to your Dashboards by attaching scripts to components or events (e.g., Dashboard loading). Dashboard scripts use JavaScript syntax (https://www.ecma-international.org/publications/standards/Ecma-262.htm[ECMA-262]) and support most standard JavaScript functions. The JavaScript function libraries are enhanced with additional functions that provide programmatic access to Dashboard properties and behaviors.
endif::ai[]
...
The following sections provide a brief overview of Dashboard programming with JavaScript. A basic JavaScript reference is provided in the xref:commonscript:UserFunctions.adoc[] section. To find more complete information about the JavaScript language, please refer to a JavaScript book or tutorial website (e.g., https://www.w3schools.com/js). If you are already familiar with the JavaScript language, you can skip ahead to xref:dashboardscript:AddScriptToDashboard.adoc[], which explains the different areas of a Dashboard to which you can add script.
...
.Example: Variable definitions and assignments
[source,javascript]
----
...
Note that these operators *cannot* be used to compare Date objects. Use date functions instead, such as `dateDiff()` or `CALC.dateValue()`. See xref:commonscript:DateObjectFunctions.adoc[] and xref:#UsefulDateFunctions[] below for more information about date functions.
...
If a value concatenated to a string is of a different type, it is converted to a string automatically. Strings have many methods. Some common ones are shown in xref:#UsefulTextFunctions[].
...
The date format is described in xref:dashboard:DataFormat.adoc[].
...
.Example 2: if/else-if/else multiple test
[source,javascript]
----
...
=== Custom Functions
As a general rule, use built-in CALC/Excel-style functions  whenever your calculation can be expressed as a single formula. (See xref:commonscript:CalcObjectFunctions.adoc[]). You can also create your own custom functions to help modularize your scripts. Once you define such a function, you can  call it from other parts of your script to perform the specified operations, as needed. See xref:dashboardscript:CreateScriptFunction.adoc[] for more details on how to define a function.
...
.Example 1: Function to return maximum of two values
[source,javascript]
----
// Define function 'max'
function max(a,b) {  //<1>
...
----
<1> See xref:dashboardscript:CreateScriptFunction.adoc[] for more details on how to define a function.




.Example 2: Function to sum values in a numerical array
[source,javascript]
----
...
Other common string search functions include xref:commonscript:StringObjectSearch.adoc[], xref:commonscript:StringObjectMatch.adoc[], and xref:commonscript:CALC_find.adoc[].
...
Because scripts are executed when the Dashboard is generated on the server, a script error can cause Dashboard generation to fail. To prevent this from happening, you should wrap error-prone code inside a “try-catch” block, which allows you to trap errors before they affect Dashboard execution. See xref:#TryCatch[]  for more information.

== Debug a Script

If a Dashboard generates errors, you can use the Visual Composer Console (see xref:dashboard:VisualComposerConsole.adoc[]) to analyze the errors.  Errors will also be recorded in the server logs. See xref:administration:Logging.adoc[] for more information.


When you attempt to debug a Dashboard script, it is often helpful to view the current values of variables and objects. To do this, use the xref:dashboardscript:SpecialFunctionsAlert.adoc[] function, which opens a "`toast`" message box to display a specified string.
[source,javascript]
...
image::administration:alertFunction.png[alt="alert dialog box showing comma-separated array values from script"]

The `alert()` dialog box is also useful for displaying critical information to a user (for example, if the user’s selections have resulted in an empty dataset).  You can also use the  xref:dashboardscript:SpecialFunctionsConfirm.adoc[] function to display information to a user, which also prompts the user to either continue or abort the operation.
