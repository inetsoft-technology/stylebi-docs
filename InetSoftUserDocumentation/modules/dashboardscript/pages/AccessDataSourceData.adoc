...
// metadata for AI
ifdef::ai[]
Summary: How to access and aggregate data from the Dashboard data source within Dashboard scripts. Covers syntax for referencing data block fields by name or index, using the viewsheet keyword for names with spaces, accessing date/time field parts, and applying aggregation methods (Sum, Average, Max, Min, Count, First, Last, and others) to data block columns.
endif::ai[]
...
image::insertData.png[alt="script editor Data folder with available data fields listed"]
...
image::insertDatePart.png[alt="script editor Data folder showing date field parts like Year and Month"]
