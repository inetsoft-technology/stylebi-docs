= bindingInfo.setPercentageType(col,type)
:navtitle: setPercentageType()
%experimental%
include::ROOT:partial$buttonSnippets.adoc[]

// metadata for AI
ifdef::ai[]
Summary: xref:dashboardscript:ChartBindingInfoGetPercentageType.adoc[] to obtain the binding information.
endif::ai[]


[WARNING,caption=See Also]
xref:dashboardscript:ChartBindingInfoGetPercentageType.adoc[]  to obtain the binding information.

Specifies the basis for computing percentages, grand total or group total.



== Parameter
col::			Field for which to set percentage type
type::			One of the following constants:
+
[source,javascript]
Chart.PERCENTAGE_OF_GRANDTOTAL
Chart.PERCENTAGE_OF_GROUP
Chart.PERCENTAGE_NONE








== Example

[source,javascript]
--
Chart1.bindingInfo.setPercentageType("Total", Chart.PERCENTAGE_OF_GROUP)

--

image::commonscript:setPercentageTypeGroup.png[alt="chart with Total displayed as percentage of group total"]

include::commonscript:partial$bindingInfoNote.adoc[]
