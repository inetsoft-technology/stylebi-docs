...
// metadata for AI
ifdef::ai[]
Summary: Once you have executed the query (xref:RunQueryFromScript.adoc[]), you can access specific ranges of the query result set. The data-referencing syntax explained in the following sections allows you to also group and filter the results, and to create expressions.
endif::ai[]
...
Typically, when you access query data using the methods described below, you will display this data within the cells of a Freehand Table.  (See xref:dashboard:AddFreehandTable.adoc[]) for more information about this component. The following sections provide several examples of how to do this.
...
image::dashboard:Freehand Table Cell Reference_1.png[alt="freehand table cell with name assigned in Cell Name field"]

You can insert this reference easily by selecting the field name from the ‘Cell’ folder in the editor.

image::dashboard:Freehand Table Cell Reference2_1.png[alt="formula editor showing cell reference inserted from Cell folder"]
...
+
image::dashboard:freehandExp1.png[alt="Dashboard Options Script tab with runQuery entered in onInit script"]
+
...
+
image::dashboard:freehandExp2.png[alt="freehand table editor with Formula option selected for cell"]
...
+
image::dashboard:freehandExp3.png[alt="freehand table with Expand Cell set to Vertical and state column data populated"]
+
Observe that the table is populated with all the records from the ‘state’ column of the ‘customers’ query.
+
image::dashboard:freehandExp4.png[alt="freehand table populated with all state values from customers query"]
+
To return just the unique values in the ‘state’ column, change the script from `q['state']` to `toList(q['state'])`.
+
image::dashboard:freehandExp5.png[alt="freehand table showing unique state values using toList function"]
====
...
. Select the ‘Formula’ option and press the btn:[Edit] button.
+
image::dashboard:freehandExp2_1.png[alt="freehand table editor with Formula selected for derived column expression"]
...
. Select ‘Expand Cell’ and choose ‘Vertical’. (This causes the extracted data to fill down vertically.)
+
image::dashboard:freehandExp6.png[alt="freehand table editor showing concatenation formula for state and zip fields"]
+
Observe that the table is populated with the concatenated values of ‘state’ and ‘zip’.
+
image::dashboard:freehandExp7.png[alt="freehand table populated with concatenated state and zip values"]
====
...
+
image::dashboard:freehandExp8.png[alt="freehand table structure with four rows and three columns inserted"]
+
...
. Select the ‘Formula’ option and press the btn:[Edit] button.
+
image::dashboard:freehandExp2_2.png[alt="freehand table editor with Formula selected for second example cell"]
...
. Select ‘Expand Cell’ and choose ‘Vertical’. (This causes the extracted data to fill down vertically.)
+
image::dashboard:freehandExp3_1.png[alt="freehand table with Expand Cell Vertical set for state column"]

. In the ‘Cell Name’ field, enter ‘`st`’ as the cell name.
+
image::dashboard:freehandExp9.png[alt="freehand table cell name field showing st entered as cell name"]
...
+
image::dashboard:freehandExp10.png[alt="freehand table formula editor showing city column extraction formula"]

. Select ‘Expand Cell’ and choose ‘Vertical’.

. In the ‘Cell name’ field, enter ‘`ct`’ as the cell name.
+
image::dashboard:freehandExp11.png[alt="freehand table cell name field showing ct entered as cell name"]

. {finishEditing} Observe that the table is now populated with states and cities.
+
image::dashboard:freehandExp12.png[alt="freehand table showing all states with all cities in adjacent column"]
+
...
+
image::dashboard:freehandExp13.png[alt="freehand table formula with field filtering to show cities matching current state"]
+
...
+
image::dashboard:freehandExp14.png[alt="freehand table showing only cities that correspond to each state row"]
+
...
+
image::dashboard:freehandExp15.png[alt="freehand table editor with customer count formula and Expand Cell Vertical set"]

. {finishEditing} The Table now displays a count of customer IDs within each city and state.
+
image::dashboard:freehandExp16.png[alt="freehand table showing customer count per city within each state"]
====
...
+
image::dashboard:freehandExp17.png[alt="freehand table with state group numbers displayed in first column"]
+
...
+
image::dashboard:freehandExp18.png[alt="freehand table Option tab with Merge expanded cells enabled and st row group selected"]
...
+
image::dashboard:freehandExp19.png[alt="freehand table Option tab with Merge expanded cells enabled for city column"]

. {finishEditing} Note how repeated states and group numbers have been merged together.
+
image::dashboard:freehandExp20.png[alt="freehand table with repeated state names and group numbers merged into single cells"]
...
+
image::dashboard:freehandExp21.png[alt="Visual Composer Format tab showing Middle vertical alignment applied to cells"]
====
...
+
image::dashboard:freehandExp22.png[alt="Dashboard Options Script tab with Sales Explore runQuery in onInit"]
+
...
. Select the ‘Formula’ option and press the btn:[Edit] button.
+
image::dashboard:freehandExp23.png[alt="freehand table editor with Formula selected for year group header cell"]
...
. Select ‘Expand Cell’ and choose ‘Vertical’. (This causes the extracted data to fill down vertically.)
+
image::dashboard:freehandExp24.png[alt="freehand table with year column set to Expand Cell Vertical"]

. In the ‘Cell Name’ field, enter ‘`yr`’ as the cell name.
+
image::dashboard:freehandExp25.png[alt="freehand table cell name field showing yr entered as cell name"]
...
+
image::dashboard:freehandExp26.png[alt="freehand table cell name field showing tot entered for annual totals cell"]
...
+
image::dashboard:freehandExp27.png[alt="freehand table editor showing relative cell reference formula for year-over-year difference"]

. {finishEditing} The resulting table displays totals by year, as well as the difference between successive years’ totals.
+
image::dashboard:freehandExp28.png[alt="freehand table displaying annual totals and year-over-year differences"]
====
...
+
image::dashboard:freehandExp22_1.png[alt="Dashboard Options Script tab with Sales Explore runQuery entered for absolute reference example"]
+
...
. Select the ‘Formula’ option and press the btn:[Edit] button.
+
image::dashboard:freehandExp23_1.png[alt="freehand table editor with Formula selected for second absolute reference walkthrough"]
...
. Select ‘Expand Cell’ and choose ‘Vertical’. (This causes the extracted data to fill down vertically.)
+
image::dashboard:freehandExp24_1.png[alt="freehand table with year column Expand Cell Vertical set for second walkthrough"]

. In the ‘Cell Name’ field, enter ‘`yr`’ as the cell name.
+
image::dashboard:freehandExp25_1.png[alt="freehand table cell name field showing yr for second absolute reference walkthrough"]
...
. In the ‘Cell Name’ field, enter ‘`tot`’ as the cell name.
+
image::dashboard:freehandExp26_1.png[alt="freehand table cell name field showing tot for second absolute reference walkthrough"]
...
This formula uses absolute cell referencing to calculate the difference between the total revenue (computed in the column named ‘tot’) of the current year and the year 2017. The table should appear as shown below:
+
image::dashboard:freehandExp30.png[alt="freehand table editor showing absolute year reference formula for 2017 comparison"]

. {finishEditing} The resulting table displays totals by year, as well as the difference between each year’s total and the total for 2017.
+
image::dashboard:freehandExp29.png[alt="freehand table showing annual totals and difference from 2017 baseline"]
====
