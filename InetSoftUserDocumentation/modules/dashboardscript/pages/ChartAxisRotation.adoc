= axis.rotation
%experimental%
include::ROOT:partial$buttonSnippets.adoc[]

// metadata for AI
ifdef::ai[]
Summary: Sets the rotation of the dimension or measure axis labels.
endif::ai[]







Sets the rotation of the dimension or measure axis labels.
include::partial$axisShortcut.adoc[]


== Type
Number (degrees)



== Example
[source,javascript]
Chart1.axis['Company'].rotation = 30;

image::commonscript:dimensionRotation.png[alt="chart with dimension axis labels rotated 30 degrees"]



[TIP]
====
Either syntax below can be used in  xref:AddScriptToDashboard.adoc#AddComponentScript[component script]:
[source,javascript]
axis['...'].rotation             // unqualified
Chart1.axis['...'].rotation      // qualified

{secondForm}
====

