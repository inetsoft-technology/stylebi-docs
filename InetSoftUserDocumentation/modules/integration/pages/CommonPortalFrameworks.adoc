...
// metadata for AI
ifdef::ai[]
Integration guidance for common portal frameworks, specifically Microsoft IIS Web Server and SharePoint Portal Framework. Explains that StyleBI must run on a J2EE-compliant server and cannot be directly deployed on IIS. StyleBI can be embedded in SharePoint Web Parts or IIS-hosted pages via URL. For SharePoint, seamless integration (including interactive operations) requires the StyleBI Server to run on the same host as SharePoint; if on a different host, browser cross-domain restrictions will disable interactive operations.
endif::ai[]
...
SharePoint is a popular portal framework developed by Microsoft (usually deployed on an IIS server). The ‘Contents’ of a SharePoint portal consists of one or more components referred to as ‘Web Parts’. A Web Part may retrieve its content from any third party source by simply specifying the appropriate URL. Therefore, by adding a new ‘Web Part’ and specifying the appropriate content URL you can display an InetSoft Dashboard  as a ‘Web Part’. (See xref:LoadAssetsViaURL.adoc[]. For example:
