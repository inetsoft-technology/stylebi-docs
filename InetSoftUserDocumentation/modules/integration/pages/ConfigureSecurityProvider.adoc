...
// metadata for AI
ifdef::ai[]
How to choose and configure a security provider for SSO integration in StyleBI, covering when to use the default file-based provider, LDAP provider (not available when multi-tenancy is enabled), or a custom DB/API-based provider. For custom providers, the recommended approach is the composite security provider, which splits responsibility by implementing a custom authentication provider alongside the built-in file-based authorization provider. Includes reference tables listing the key security provider functions for authentication and authorization, and showing which functions are invoked or bypassed during SSO authentication.
endif::ai[]
...
+
Implement a custom security provider if your setup requires the security provider to perform authentication of user credentials or user role/group mappings against an _external source_. For example, if your security data store is a database or another security framework and you have partial session-based SSO, use a custom security provider. The best approach is the _composite security provider_, which splits the responsibility for authentication and authorization tasks. To do this, implement your own authentication provider and use the built-in file-based authorization provider. See xref:administration:SpecifySecurityProvider.adoc[] in xref:administration:ManageServer.adoc[] for more information.
