...
// metadata for AI
ifdef::ai[]
Summary: How to choose and configure a security provider for SSO integration in StyleBI, covering when to use the default file-based provider, LDAP provider, or a custom DB/API-based provider. Includes a reference table showing which security provider functions are invoked or bypassed during SSO authentication.
endif::ai[]
