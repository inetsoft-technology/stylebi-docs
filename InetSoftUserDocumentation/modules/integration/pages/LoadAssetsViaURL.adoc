...
// metadata for AI
ifdef::ai[]
URL syntax reference for accessing StyleBI components from external web applications. Topics covered: displaying the User Portal, individual portal tabs (Repository, Dashboard, Schedule, custom tabs), Dashboards (public and private, both standalone and within Portal tabs), Pinboards deployed to the Dashboard tab, exporting Dashboards (PDF, Excel, PowerPoint, PNG, HTML, CSV, VSO formats), passing parameters to Dashboards via URL, displaying interactive charts (via IFrame or the inetsoft-chart web component including shadow DOM), displaying static charts as PNG or SVG images, opening Visual Composer, displaying Portal Preferences, and pinging the server. Also covers IFrame embedding details: cross-page communication via MessageEvent, adapting IFrame size to browser size, and controlling Dashboard toolbar buttons from the parent page via postMessage. Open Graph tag behavior for Dashboard titles and descriptions is also noted.
endif::ai[]
...
* xref:#DisplayPinboard[] to display a Dashboard that has been deployed as a Pinboard.
====
...
TIP: For more information about hyperlinks, see xref:dashboard:DrillDownIntoData.adoc[] in xref:dashboard:VisualizeYourData.adoc[], and  xref:dashboardscript:TextHyperlink.adoc[] and xref:dashboardscript:TableSetHyperlink.adoc[] in xref:dashboardscript:AdvancedDashboardScripting.adoc[].


=== Adapt Size of IFrame to Browser Size
A JSON message will be automatically triggered when the Dashboard has the ‘Scale to Screen’ option enabled (see xref:dashboard:DashboardOptions.adoc[]) and the `scrollHeight` of the dashboard changes, for example, `{ scrollHeight: 123 }`.  This message can be used to dynamically resize the IFrame to prevent the introduction of scroll bars.  For example (place script inside `<script></script>` tags):
...
To pass a message into the Dashboard, use `postMessage` as shown below, where `<buttonName>` is the name of a button, such as ‘Email’. (See xref:dashboardscript:thisViewsheet.adoc[] for a list of button names.)
...
You can set additional CSV options as well: ‘Delimiter’, ‘Quote’, ‘Tab Delimited’, and ‘Keep Header’. (See xref:dashboard:ExportDashboard.adoc[] for more information about these options.)
...
The *Dashboard* tab of the User Portal provides quick access to Dashboards. These Dashboards can be deployed either by a user (see xref:user:DeployDashboard.adoc[]) or by an administrator (see xref:administration:Repository.adoc#Pinboards[Pinboards]).
...
To display a Dashboard that has not been deployed as a Pinboard, see xref:#DisplayDashboard[].
...
====
[square]
* xref:dashboard:AddConditions.adoc[] in xref:dashboard:VisualizeYourData.adoc[] for information about parameters in  Dashboard conditions.
...
The transmitted parameter(s) will be available in the Dashboard through the xref:dashboardscript:parameter.adoc[] script object and in parameterized filtering conditions. See xref:#DisplayDashboard[] for additional information about opening a Dashboard.
