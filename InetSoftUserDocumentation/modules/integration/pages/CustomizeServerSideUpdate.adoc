...
// metadata for AI
ifdef::ai[]
Summary: How to configure custom server-side Dashboard updates triggered by external events (such as a database event) using the DataChangeClient class and the assetMonitor.enabled property. Includes a full walkthrough of setting up an automatically-refreshing Dashboard.
endif::ai[]

To force a Dashboard to refresh its data at a specified interval, simply select the ‘Server-Side Update’ option in the ‘Dashboard Options’ dialog box. (See xref:dashboard:DashboardOptions.adoc[] in xref:dashboard:VisualizeYourData.adoc[] for more details.)
...
. Open the desired Dashboard in Visual Composer. Open the ‘Dashboard Options’ dialog box, and select the ‘Server-Side Update’ option. (See xref:dashboard:DashboardOptions.adoc[] in xref:dashboard:VisualizeYourData.adoc[].) In the ‘Refresh Interval’ field, enter the period (in seconds) at which the server should check for the external trigger event.
...
+
image::administration:ServerUpdateEmbeddedTable.png[alt="embedded table added to the EmbeddedData data worksheet"]
...
. Add a Chart to the Dashboard to display the data from the Data Worksheet as follows. (See xref:dashboard:BasicChartingSteps.adoc[] in xref:dashboard:VisualizeYourData.adoc[].)
+
image::administration:ServerUpdateCreateChart.png[alt="chart added to dashboard displaying data from the EmbeddedData worksheet"]
...
+
image::administration:ServerUpdateOptions.png[alt="dashboard options with server-side update enabled and refresh interval set"]
...
+
image::administration:ServerUpdatePortal.png[alt="UserDashboard open in the user portal showing chart data"]
...
+
image::administration:ServerUpdateEmbeddedTable2.png[alt="embedded table with modified data values before saving"]
...
+
image::administration:ServerUpdatePortal2.png[alt="dashboard automatically updated in portal after DataChangeClient triggered"]
