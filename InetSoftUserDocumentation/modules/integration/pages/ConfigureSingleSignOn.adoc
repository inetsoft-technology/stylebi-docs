...
// metadata for AI
ifdef::ai[]
How to configure Single Sign-On (SSO) in StyleBI Enterprise, covering three methods: OpenID Connect, SAML, and Custom Filter (Java class or inline Groovy). Includes step-by-step instructions and configuration parameters for each SSO method. Important: SSO only provides authentication — for authorization (permissions and security actions), separate configuration is required. The Custom Filter method works by injecting an SRPrincipal object into the HTTP request as part of the security filter chain.
endif::ai[]
...
+
image::administration:SSO2.png[alt="security SSO tab with OpenID Connect selected as the SSO provider"]
...
+
image::administration:SSO1.png[alt="security SSO tab with SAML selected as the SSO provider"]
...
public class SampleFilter implements Filter {
   public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
      ...
      // Replace with your own business logic to determine the user
...
+
image::administration:SSO4.png[alt="security SSO tab with Custom selected as the SSO provider"]
...
+
image::administration:SSO6.png[alt="Java Class option selected with filter class name entered"]

+
Alternatively, select ‘Inline Groovy’ and enter the class using the skeleton provided in the edit box.
+
image::administration:SSO5.png[alt="Inline Groovy option selected with skeleton class shown in edit box"]
