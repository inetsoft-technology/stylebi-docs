...
// metadata for AI
ifdef::ai[]
Summary: How to design a multi-tenant data environment in StyleBI Enterprise by defining independent database connections per tenant, extending physical views and logical models for tenant-specific data, assigning connection permissions, and using aliases to provide tenant-appropriate names in the repository.
endif::ai[]
