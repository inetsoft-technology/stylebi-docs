...
// metadata for AI
ifdef::ai[]
How to design a multi-tenant data environment in StyleBI Enterprise. Covers a sequential six-step workflow: (1) enabling multi-tenancy on the server, (2) adding tenant-specific database connections to a Data Source, (3) extending a common Physical View with tenant-specific tables/columns, (4) extending a common Logical Model with tenant-specific entities/attributes, (5) assigning connection and action permissions to users and groups, and (6) setting aliases on folders, Data Worksheets, and Dashboards so tenants see appropriate names while admins maintain distinct names. Also notes that a common set of data accessible to all tenants can be defined alongside tenant-specific connections.
endif::ai[]
