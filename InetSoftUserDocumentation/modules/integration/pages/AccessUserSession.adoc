...
// metadata for AI
ifdef::ai[]
Summary: How to access and modify the SRPrincipal user session object in StyleBI, including accessing it from JSP/Servlet, Login Listeners, and Dashboard scripts. Covers the setProperty(), setLocale(), and setParameter() methods, plus a use case for simulating user sessions with VPM filtering.
endif::ai[]
