...
// metadata for AI
ifdef::ai[]
How to access and modify the SRPrincipal user session object in StyleBI, including accessing it from JSP/Servlet, Login Listeners, VPM trigger scripts, and Dashboard scripts. Covers the setProperty() and getProperty() methods (including the __internal__ property for non-SSO use), setLocale() with language/country/variant support that loads SreeBundle and srinter localization files, and setParameter() which injects named parameters accessible in Dashboard scripts, parameterized conditions on Data Worksheet data blocks, and parameterized conditions on Dashboard components. Login Listener setup requires implementing the LoginListener interface and configuring the sree.security.listeners property; optionally setAlias() customizes the portal welcome text. Also covers a use case for simulating user sessions using custom SRPrincipal properties read by a VPM trigger script for row-level filtering.
endif::ai[]
...
You can access the `SRPrincipal` object within VPM trigger scripts and Dashboard scripts (in the  *onRefresh* handler or in component-level script).  See xref:user:CreateNewVPM.adoc[] and xref:dashboardscript:AddScriptToDashboard.adoc[] for more information about using script.
...
* In Dashboards via the xref:dashboardscript:parameter.adoc[] script keyword.
...
* In a parameterized condition on a Dashboard component. (See xref:dashboard:AddConditions.adoc[] in xref:dashboard:VisualizeYourData.adoc[].)
