image::dataworksheet:newWorksheet3.png[alt="new worksheet dialog showing upload file, mashup data, and query options"]
...
+
image::dataworksheet:embeddedTable4.png[alt="import data file dialog opened after selecting upload file option"]
+
See xref:dataworksheet:UploadData.adoc[] for information about how to use the upload options.
+
Mashup Data:: To create your Data Worksheet by *mashing up data*, select the {buttonMashupData} option. This is especially useful if you are using a data model, or if you need to combine data from multiple sources.  *Mashup Data* creates a blank Data Worksheet named ‘Untitled-X’. Proceed to xref:#CreateDataBlocks[] below.
+
Data Source:: To create your Data Worksheet by *defining a query*, select a data source query option (e.g., ‘Orders Query’). This is primarily useful when you are *not* using a data model or multiple datasources, or if you want to copy over existing SQL strings. *Data Source*  creates a blank Data Worksheet named ‘Untitled’ and opens the ‘Database Query’ dialog box.
+
image::dataworksheet:newWorksheet4.png[alt="database query dialog opened after selecting a data source query option"]
+
...
image::dataworksheet:transform1.png[alt="data block created by dragging fields from the left panel"]

NOTE: You can drag fields from the _same_ data model or _same_ table directly into a single data block. To combine fields from _different_ sources (different models, different queries, etc.), define a join between the data blocks. See xref:dataworksheet:JoinData.adoc[] for more details.
...
image::dataworksheet:auto-update1.png[alt="data block linked to source worksheet with auto-update button in toolbar"]
...
You can now do data pipeline operations on this data, as described in the following sections (xref:dataworksheet:FilterData.adoc[], xref:dataworksheet:JoinData.adoc[], etc.), or you can proceed to create Dashboards based on this Data Worksheet. See xref:dashboard:VisualizeYourData.adoc[] for information about constructing a Dashboard.