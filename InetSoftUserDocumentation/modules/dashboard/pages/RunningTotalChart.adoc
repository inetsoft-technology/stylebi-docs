...
// metadata for AI
ifdef::ai[]
Summary: How to create a Running Total chart in Visual Composer (also called a cumulative total or running sum chart), which displays an aggregated measure that accumulates across dimension groups. Covers selecting Running Total from the Trend and Comparison menu, using the Custom Running calculation with aggregate method and optional Reset at date interval, and includes a step-by-step running average example. See CompareDataByDate for related date-based comparisons.
endif::ai[]
...
image::commonscript:runningTotalChartExp2.png[alt="running total chart accumulating values across groups"]
...
+
image::commonscript:barChartExp2.png[alt="drag dimension to Color Shape Size or Text region"]
...
+
image::commonscript:runningTotalChartExp1.png[alt="select Running Total from Trend and Comparison menu"]
...
image::CalculateRunningTotal_1.png[alt="Running calculation dialog with Aggregate and Reset at options"]
...
image::commonscript:runningTotalChartExp2.png[alt="completed running total chart"]
...
+
image::RunningTotalExample1.png[alt="Date dimension grouped by Month in Chart Editor"]
...
+
image::RunningTotalExample2.png[alt="Calculation dialog set to Running Sum with Year reset"]
...
. {finishEditing}
+
image::RunningTotalExample4.png[alt="completed running average chart reset by year"]
