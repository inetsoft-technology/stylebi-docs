...
// metadata for AI
ifdef::ai[]
How to create a Dot Plot chart in Visual Composer, which displays individual (un-aggregated) data points as stacked dots. Best for small datasets with fewer than ~20 points per dimension. Steps: select Point style with Stack enabled; drag a dimension to the X region; drag a measure to the 'Break By' region (not Y axis); set the measure's Aggregate property to None. Optionally, drag an additional dimension to the Break By region alongside the measure to display both values in chart tooltips. Optionally adjust point size via the Size field. Warning: too many records will prevent all data points from displaying.
endif::ai[]

A dot plot represents a measure by its individual data points (_without aggregation_).

image::commonscript:Ad Hoc Dot9.png[alt="dot plot chart example"]
...
+
image::commonscript:Ad Hoc Dot1.png[alt="select Point chart style and enable Stack"]
+
...
+
image::commonscript:Ad Hoc Dot2.png[alt="drag dimension to X region"]
+
...
+
image::commonscript:Ad Hoc Dot3.png[alt="drag measure to Break By region"]
+
...
+
image::commonscript:VS Dot4.png[alt="set aggregate to None for measure"]
...
+
image::commonscript:VS Dot5.png[alt="drag dimension to Break By region"]
+
...
+
image::commonscript:VS Dot6.png[alt="tooltip shows measure and dimension values"]
...
+
image::commonscript:Ad Hoc Dot8.png[alt="select larger size for data points"]
