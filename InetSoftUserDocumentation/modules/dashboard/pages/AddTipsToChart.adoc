...
// metadata for AI
ifdef::ai[]
How to add tooltips and data tips (also called chart tooltips, data tooltips, or hover labels) to a Chart or Crosstab in Visual Composer. Data tips display a Dashboard component (such as a Chart, Gauge, or Text) filtered to the hovered chart region; the data tip can be triggered on hover or on click only (via the 'On Click Only' option). Custom tooltips use HTML markup and Java MessageFormat patterns to display user-defined text when hovering over a data point; they apply to Chart, Text, Gauge, and Image components. Custom tooltips support field name or index references, conditional lines (using pipe symbols) for multi-measure charts, and a "None" option to hide tooltips entirely. For Chart components only, a 'Tooltip Style' option (Default or Card) sets the tooltip's visual style: 'Default' is the standard flat label:value tooltip, and 'Card' is a rounded panel with hierarchical text (large measure value, medium dimensions, small aesthetics); 'Card' is the default for newly created charts. For Line, Area, and Bar charts, 'Combine tooltips from different series' merges all series into a single tooltip, and 'Snap to nearest data point' (available when a dimension is on the X axis) locks the cursor/tooltip to the nearest X-axis tick mark instead of following the mouse position exactly. Both features are available to end users at view time. See also AddAnnotation for persistent chart annotations.
endif::ai[]
...
image::dataTip1.png[alt="tooltip displays data group details on hover"]

You can also choose to replace the tooltip with a *data tip*. A data tip displays information about a data group using another Dashboard component, such as Chart, Text, or Gauge (or a collection of such components). The data tip is dynamically filtered to show only data corresponding to the particular chart group on which you hover. To show the data tip only on click (rather than hover), enable the ‘On Click Only’ option in the component's Properties dialog. If the element has a hyperlink, you must {buttonCtrl}-click to show the data tip.

image::dataTip2.png[alt="data tip component displays filtered dashboard data"]
...
+
image::Chart Data Tip Top 10_1.png[alt="use ranking option to display top 10 companies"]

.. {finishEditing}

. Resize the chart as desired.
+
image::Chart w Data Tip_1.png[alt="resized chart displaying quantity by company"]
...
+
image::dataTipChart2.png[alt="select data tip view component on general tab"]
...
+
image::Chart Data Tip Final.png[alt="hover over company shows data tip gauge"]
...
image::dashboard:galleryDataTip2.png[alt="data tip gallery example dashboard"]
...
. For a Line, Area, or Bar type chart, select ‘Combine tooltips from different series’ to display a single tooltip with details from all series (e.g., all lines, or all segments of a stacked bar). The amount of data displayed will be limited by available space in the tooltip. When not selected, the tooltip displays details only for the series to which the user points.

. For a Line, Area, or Bar type chart that has a dimension on the X axis, select ‘Snap to nearest data point’ to lock the cursor and tooltip to the nearest X-axis tick mark instead of following the mouse position exactly.

. For a Chart component, choose a ‘Tooltip Style’: ‘Default’ displays the standard flat label-value tooltip; ‘Card’ displays a rounded panel with centered, hierarchical text (the measure value shown largest, dimensions medium, and aesthetics smallest).
...
+
image::CustomizeTooltipIndex.png[alt="select custom option and enter tooltip markup"]
+
...
image::CustomTooltipExp.png[alt="custom tooltip showing company name and units purchased"]
...
.. Press the {buttonEditDimension}, and use the ‘Ranking’ option to display only the top 10 companies. Press the {buttonApply}.
+
image::Chart Data Tip Top 10.png[alt="use ranking option to display top 10 companies"]

.. {finishEditing}

. Resize the chart as desired.
+
image::Chart w Data Tip.png[alt="resized chart displaying quantity by company"]
...
image::CustomizeTooltipExample_1.png[alt="custom tooltip displayed when hovering over bar"]
****
