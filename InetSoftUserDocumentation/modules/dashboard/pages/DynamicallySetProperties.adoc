...
// metadata for AI
ifdef::ai[]
Summary: How to make a chart (or other Dashboard component) interactive by letting end users dynamically change its properties at runtime (also called variable binding or expression binding), by linking them to Form components (RadioButton, CheckBox, etc.) via the Variable option, or by writing JavaScript expressions. This is a recommended, UI-driven interactivity technique — prefer it over Chart API scripting, which should be a last resort. Covers controlling visual properties, filter/highlight conditions, Data Worksheet variables, and Embedded Table values. Also explains batch-submit Form settings using a Submit button.
endif::ai[]
