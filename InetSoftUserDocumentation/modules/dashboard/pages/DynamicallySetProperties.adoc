...
// metadata for AI
ifdef::ai[]
Summary: How to make a chart (or other Dashboard component) interactive by letting end users dynamically change its properties at runtime (also called variable binding or expression binding), by linking them to Form components (RadioButton, CheckBox, ComboBox — also called a dropdown or dropdown menu — Slider, Spinner, etc.) via the Variable option, or by writing JavaScript expressions. This is a recommended, UI-driven interactivity technique — prefer it over Chart API scripting, which should be a last resort. Covers controlling visual properties, filter/highlight conditions, Data Worksheet variables, and Embedded Table values. Also explains batch-submit Form settings using a Submit button.
endif::ai[]
...
+
image::Variable Select Popup.png[alt="select variable option for property"]
+
...
+
image::Variable Selection.png[alt="select form component to control property"]
+
[IMPORTANT]
====
...
+
image::Ranking Variable.png[alt="ranking property requires integer variable"]
...
+
image::Crosstab Select Variable.png[alt="press value type choose variable option"]
...
+
image::Capture_006.png[alt="radiobutton controls chart y-axis field"]
****
...
+
image::Variable Select Popup Condition.png[alt="select variable option for condition value"]
From the adjacent pop-up menu, select the Form component that you want to control the value.
+
image::Variable Selection Condition.png[alt="select form component to control condition"]
...
+
image::ParameterHighlightListSelect.png[alt="select query option to set list values"]
...
+
image::ParameterHighlightListSelectDialog.png[alt="select state field for label and value"]
...
+
image::ParameterHighlightStretchInput.png[alt="expand checkbox to show all state options"]
...
+
image::ParameterHighlightTableAndList.png[alt="enlarge table so ten rows are visible"]
...
+
image::ParameterHighlightDefine.png[alt="press edit to open conditions dialog box"]
...
+
image::ParameterHighlightDefineVariable.png[alt="select stateselector checkbox as variable"]
...
image::ParameterHighlightFinal.png[alt="checkbox selections highlight cities in table"]
****
...
====
xref:dashboardscript:AdvancedDashboardScripting.adoc[], for more information about how to add scripted logic to a Dashboard.
====

You can enter a script in any property field that provides the {buttonValueType}  with an ‘Expression’ option.  This allows you to change the associated property in response to an event such as a user interaction.


image::Expression Select Popup.png[alt="select expression option for property"]
...
+
image::Right Click Properties.png[alt="right-click component to open properties panel"]
...
+
image::Expression Option.png[alt="press value type select expression from menu"]
. Press the {buttonEditFormula} to open the Formula Editor.
+
image::Edit Expression Button.png[alt="press edit formula to open formula editor"]
...
+
image::Formula Editor Function.png[alt="insert function from formula editor list"]
+
Many expressions are similar to formulas that you might use in Microsoft Excel^®^.   See xref:dashboardscript:DashboardObjectReference.adoc[] and xref:commonscript:UserFunctions.adoc[] for a full description of available properties and functions.
+
