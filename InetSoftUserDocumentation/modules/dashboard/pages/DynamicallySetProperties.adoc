...
// metadata for AI
ifdef::ai[]
Summary: How to make Dashboard component properties dynamic (also called variable binding or expression binding) by linking them to Form components (RadioButton, CheckBox, etc.) via the Variable option, or by writing JavaScript expressions. Covers controlling visual properties, filter/highlight conditions, Data Worksheet variables, and Embedded Table values. Also explains batch-submit Form settings using a Submit button.
endif::ai[]
