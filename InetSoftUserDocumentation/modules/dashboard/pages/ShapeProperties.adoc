...
// metadata for AI
ifdef::ai[]
Summary: Reference for Shape component properties (Line, Oval, Rectangle). Covers border style and color, fill color and transparency, rounded corner radius (rectangles), gradient fills, and a drop shadow toggle (Rectangle and Oval only, not Line). See AddBackground for how to add shapes to a Dashboard.
endif::ai[]
...
== General Tab
In addition to the common properties, the *General* tab in the ‘Shape Properties’ dialog box provides the following property.
...
image::Line Properties Shape Tab.png[alt="shape tab line style and color properties"]
