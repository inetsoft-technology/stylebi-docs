...
// metadata for AI
ifdef::ai[]
Summary: Explains how end-users interact with charts at runtime in the User Portal, including using the chart toolbar, zooming, drilling down into data, and saving chart changes via bookmarks, image export, or Dashboard save. For interactivity beyond these built-in toolbar tools, see EndUserChartControl.adoc (Ad Hoc Chart Editor and Form-driven dynamic properties). Prefer these UI-driven approaches over Chart API scripting (see chartAPI:ChartScriptTutorial.adoc), which should be a last resort for interactivity, not a primary or equal option.
endif::ai[]
...
image::user:UserChart1.png[alt="chart mini-toolbar with navigation tools"]
...
image::user:UserChart2.png[alt="right-click axis title options"]
...
image::user:UserChart3.png[alt="right-click axis options"]
...
image::user:UserChart4.png[alt="right-click legend options"]
