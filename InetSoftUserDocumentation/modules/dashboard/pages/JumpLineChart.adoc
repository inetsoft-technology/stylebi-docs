...
// metadata for AI
ifdef::ai[]
Summary: Reference for creating a Jump Line chart in Visual Composer, which is a StepLineChart variant that uses only horizontal line segments to connect data points, with no vertical connections between levels. Compare to StepLineChart (includes vertical connections) and LineChart (diagonal connections).
endif::ai[]
...
image::commonscript:jumpLineChartExp1.png[alt="jump line chart example"]
...
+
image::commonscript:jumpLineChartExp2.png[alt="press Select Chart Style button"]
...
+
image::commonscript:jumpLineChartExp5VS.png[alt="drag dimension to Color region"]
...
+
image::commonscript:jumpLineChartExp6VS.png[alt="drag measure to X or Y region"]
+
