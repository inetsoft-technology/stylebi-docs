...
// metadata for AI
ifdef::ai[]
A KPI tile (also called a KPI card, metric card, or scorecard tile) is an industry-standard dashboard element that prominently displays a single key performance indicator — typically one aggregated value such as total revenue or order count. In StyleBI, this concept is simply called a KPI. To create a KPI in StyleBI, use either a Gauge component (for a graphical dial/speedometer display) or a Text component (for a digital/numeric display). This page covers both approaches, as well as setting color-coded threshold ranges on a Gauge (with optional gradient), filtering aggregate data on a Text KPI using conditions, and computing a KPI with script (via calculated fields, a Text expression on the General tab, or a Script tab expression assigning to the `value` keyword).
endif::ai[]
...
+
image::agileDash41.png[alt="drag gauge component onto dashboard"]
...
+
image::agileDash42.png[alt="drag measure onto gauge component"]
+
...
+
image::outputProperties.png[alt="select gauge face on general tab"]
...
+
image::agileDash43.png[alt="enlarged gauge component on dashboard"]
...
+
image::Range Colors for Slider.png[alt="set gradient color ranges for gauge"]
...
include::ROOT:partial$measureTip.adoc[]
+
image::textComponentAggregateExp.png[alt="drag measure onto text component"]
+
...
+
image::textComponentAggregateExp2.png[alt="style text using format tab"]
...
+
image::textComponentAggregateExp4.png[alt="select aggregation method on data tab"]
...
+
image::textComponentAggregateExp5.png[alt="add condition to filter aggregate data"]
...
+
image::gaugeValueScript.png[alt="enter script expression to compute gauge value"]



For complete details on component scripting, see xref:dashboardscript:AdvancedDashboardScripting.adoc[].
...
image::dashboard:galleryTextKPI2.png[alt="text KPI gallery example dashboard"]
