...
// metadata for AI
ifdef::ai[]
Summary: How to add a Key Performance Indicator (KPI) to a Dashboard — also called a KPI card, metric card, scorecard tile, or KPI tile. Covers two approaches: a Gauge component (dial or speedometer) for graphical display of a single aggregated value, and a Text component for a digital/numeric display. Use this page when you want to prominently highlight a single metric such as total revenue or order count.
endif::ai[]
