...
// metadata for AI
ifdef::ai[]
How to add a Key Performance Indicator (KPI) to a Dashboard in Visual Composer — also called a KPI card, metric card, scorecard tile, or KPI tile. Covers two approaches: a Gauge component (dial or speedometer) for graphical display of a single aggregated value, and a Text component for a digital/numeric display. Also covers setting color-coded threshold ranges on a Gauge (with optional gradient), filtering aggregate data on a Text KPI using conditions, and computing a KPI with script (via calculated fields, a Text expression on the General tab, or a Script tab expression assigning to the `value` keyword). Use this page when you want to prominently highlight a single metric such as total revenue or order count.
endif::ai[]
