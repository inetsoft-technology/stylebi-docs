...
// metadata for AI
ifdef::ai[]
Summary: Explains how end-users can sort Table and Crosstab columns and chart dimension groups at runtime in the User Portal by clicking sort buttons. Covers ascending, descending, and original-order cycling. For design-time sorting in the Data Worksheet, see dataworksheet:SortData.
endif::ai[]
...
image::SortColumn.png[alt="hover over header and press Sort button"]
...
image::SortColumnByValue.png[alt="hover over crosstab cell to display sort order"]
...
image::sortButtonChart.png[alt="hover over measure axis to press Sort button"]
...
NOTE: Sorting a chart dimension also affects the order of items in the corresponding legend. See xref:LegendProperties.adoc#LegendOrder[Legend Order] for more details.