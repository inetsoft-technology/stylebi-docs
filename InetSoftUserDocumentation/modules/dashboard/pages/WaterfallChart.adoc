...
// metadata for AI
ifdef::ai[]
Summary: How to create a Waterfall Chart in Visual Composer. A waterfall chart displays how sequential positive and negative values contribute to a cumulative total — useful for financial analysis such as profit/loss breakdowns. See BasicChartingSteps for general chart creation fundamentals. There is no GUI option to connect adjacent bars with a line; to add connector ("bridge") lines, use xref:chartAPI:IntervalElementSetBridgeLine.adoc[] in script on a chart created with this Waterfall style, or, for a chart built from scratch in script, call xref:chartAPI:IntervalElementSetWaterfall.adoc[] with `true` first.
endif::ai[]
...
image::dashboard:galleryWaterfall2.png[alt="financial client demographics waterfall chart example"]
