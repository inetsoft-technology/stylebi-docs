...
// metadata for AI
ifdef::ai[]
Summary: How to create a Donut chart in Visual Composer, which is a PieChart with a hole in the center. The hole can be empty (simple donut) or display a KPI value such as a grand total (advanced donut). See PieChart for basic pie chart creation.
endif::ai[]
...
+
image::commonscript:DonutChartConfig.png[alt="drag measure to Y and dimension to Color"]
...
image::dashboard:galleryDataTip2.png[alt="executive sales summary dashboard donut chart example"]
