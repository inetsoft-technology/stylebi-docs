...
// metadata for AI
ifdef::ai[]
Summary: Reference for the Range Slider filter component properties in Visual Composer dashboards. Covers the Data tab (single-value vs. composite-value filtering, Compatible Tables for filtering multiple data blocks simultaneously) and the Advanced tab (Slider Size for string/numeric/date fields, Minimum Increment, Maximum Increment, Upper Bound Inclusive, Logarithmic Scale, and Label options for min/max/tick/current-value display — this Label option only controls tick/min/max/current-value markings on the slider track, not a descriptive caption). Range Slider has no built-in title/caption; to label what a Range Slider controls, add a separate Text component near it (see AddTextOrHTML). See AddFilterComponents for how to create a Range Slider.
endif::ai[]
...
* {commonProperties}
* xref:dashboardscript:RangeSlider.adoc[]  in xref:dashboardscript:DashboardObjectReference.adoc[] to set these properties dynamically using script.
...
NOTE: A Range Slider has no built-in title or caption. To label a Range Slider (for example, to show what metric it controls), add a separate Text component positioned near the Range Slider. See xref:AddTextOrHTML.adoc[].

== Data Tab
The *Data* tab in the ‘Range Slider Properties’ dialog box provides the following Range Slider-specific properties:

image::Range Slider Dialog Data Tab.png[alt="range slider data tab properties"]
...
+
image::Slider Composite Value.png[alt="filter data using composite values from two fields"]
+
****
...
Compatible Tables:: Lists data blocks with columns compatible for multi-block filtering. To configure this component to filter multiple data blocks simultaneously, see xref:AddFilterComponents.adoc#FilterMultipleDataBlocks[Filter Multiple Data Blocks].

== Advanced Tab
The *Advanced* tab in the ‘Range Slider Properties’ panel provides the following Range Slider-specific properties.

image::Capture_046.png[alt="range slider advanced tab properties"]
