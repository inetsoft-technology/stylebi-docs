...
// metadata for AI
ifdef::ai[]
Summary: Explains how to build composite Dashboards in Visual Composer by nesting smaller Dashboard modules within a parent Dashboard, and how to synchronize filter and form components across nested Dashboards using shared filter IDs. Covers reusability, maintainability, and team development benefits. Note: users familiar with other BI tools may refer to this pattern as "cards" or a "card layout" — where a dashboard is composed of reusable, independently-designed panels. In InetSoft this is achieved via modular/nested Dashboards. For simply grouping components into a single movable unit (also sometimes called a "card"), see ContainerProperties and ArrangeComponents instead.
endif::ai[]
...
image::dashboard:Embedded Viewsheet Button.png[alt="click dashboard icon to open embedded dashboard"]
...
* If you want a Form component on the *parent* Dashboard to control a Form component on a *child* Dashboard, but not the reverse (i.e., child Dashboard setting *does not* influence parent Dashboard), add  xref:dashboardscript:setInputSelectedObject.adoc[] to the *Script* tab in the embedded '`Dashboard Properties`' dialog box.
====
...
+
image::dashboard:Options Button_1.png[alt="press Options button to open dashboard options"]
...
+
image::dashboard:Deploy shared filters.png[alt="drag component into shared filters list"]
