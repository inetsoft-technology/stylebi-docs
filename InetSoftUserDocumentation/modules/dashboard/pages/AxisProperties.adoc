...
// metadata for AI
ifdef::ai[]
Reference for Chart Axis properties in Visual Composer (also called axis label, axis format, or axis scale settings), accessed via right-click on an axis or axis label. Covers the Label tab (show/hide labels, labels on opposite side, rotation), Line tab (logarithmic scale, reverse orientation, shared range for facet charts, ignore null, truncate long labels, show axis line, show ticks, axis line and tick color, minimum/maximum values, minor/major increments), and Alias tab (rename categorical axis labels). See AxisTitleProperties for axis title settings.
endif::ai[]
