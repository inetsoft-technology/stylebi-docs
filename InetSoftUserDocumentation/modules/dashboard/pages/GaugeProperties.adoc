...
// metadata for AI
ifdef::ai[]
Reference for Gauge component properties in Visual Composer (General, Advanced, Data, Script tabs). A Gauge is also known as a dial, speedometer, KPI dial, or metric dial. Covers gauge face (visual skin), number range (minimum, maximum, minor/major increments), tooltip, shadow, colored range zones with optional gradient blending, and Show Value (digital display). See AddAKPI for how to use a Gauge to display a KPI.
endif::ai[]
