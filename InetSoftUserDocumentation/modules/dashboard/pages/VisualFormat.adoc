...
// metadata for AI
ifdef::ai[]
Summary: Add Visual Format explains how to use the Format panel in Visual Composer to control the visual appearance of Dashboard components without scripting. Topics covered include setting font (face, style, size), foreground and background color, alignment, borders, wrap text, graphical presenters, and CSS classes/IDs. For Charts, color and font can be applied chart-wide (affecting all text including axis labels, axis titles, legends, and plot labels) or to individual regions independently by clicking a specific region before formatting.
endif::ai[]
