...
// metadata for AI
ifdef::ai[]
Summary: Add Visual Format explains how to use the Format panel in Visual Composer to control the visual appearance of Dashboard components without scripting. Topics covered include setting font (face, style, size), foreground and background color, alignment, borders, wrap text, graphical presenters (also called cell renderers, in-cell bars, or data bars), and CSS classes/IDs. For Charts, color and font can be applied chart-wide (affecting all text including axis labels, axis titles, legends, and plot labels) or to individual regions independently by clicking a specific region before formatting. DataFormat.adoc (number/date/percent display patterns such as currency symbols, date strings, and the "K"/"M"/"B" thousands/millions/billions suffix) is reached via this *same* Format panel, not a separate panel or location — it covers the value-display-pattern settings rather than the visual-appearance settings covered here. Not to be confused with: ConditionalFormat.adoc (rule-based highlighting that changes appearance based on data values).
endif::ai[]
...
image::textComponentAggregateExp2.png[alt="select component and open format tab"]



Certain components have multiple regions that can be selected and formatted independently. See xref:administration:CSSDashboardRegions.adoc[] for a list of regions that can be independently formatted.
...
image::fontTab.png[alt="set font face style and size in format panel"]
...
image::Format Color Tab Basic.png[alt="color options in format panel"]



=== Component Color Options

The text and regions affected by the color setting vary by component, as shown below:

[cols="1,2"]
|===
|Component |Color Target

|Chart
a|Color can be applied globally to entire Chart or to individual regions independently.
[square]
* *Global*: With no Chart region selected, foreground color applies to all Chart text: Axis labels, axis titles, legend content, legend titles, and plot labels.

* *Independent Region*: To target a specific region (e.g., axis label, axis title, legend), click  first to select it, and then set the color.

This relates to Chart *text* color (axis labels, legend, etc.).  To set Chart *data color*, see xref:GroupDataByDimension.adoc#GroupbyColor[Group by Color]. To synchronize data colors across two or  more Charts, use the '`Share Colors`' option in the Chart Editor.

|Crosstab|Same as Table.
|Table|Color can be set independently for title bar, column headers, and data cells. Click the desired region, then set the color.
|Freehand Table|Same as Table.
|Embedded Table|Same as Table.
|Selection List|Color can be set independently for title text and choice text. Click the title bar or a choice cell, then set the color.
|Selection Tree|Same as Selection List. Color can be set independently for each level of the hierarchy.
|Selection Container|Same as Selection List and Range Slider.
|Range Slider|Color setting affects all text displayed on the slider.
|Calendar|Color setting affects the title text.
|Text|Color setting affects the text.
|Gauge|Color setting affects labels and value (if present).
|Slider|Color setting affects the labels.
|Spinner|Color setting affects the spinner text.
|Checkbox|Color can be set independently for title text and choice labels. Click the title bar or a choice cell, then set the color.
|Radio Button|Same as Checkbox.
|Combo Box|Color setting affects the choice text.
|===
...
+
image::Format Color Tab.png[alt="select value type for color field"]

. Click on the color chip to select the desired foreground (text) color, or enter an RGB color code (hex).
+
image::Pick Static Color.png[alt="pick foreground color from color picker"]
...
+
image::Pick Transparency.png[alt="set fill transparency with alpha field"]
...
image::Format Alignment Tab_1.png[alt="alignment settings in format panel"]
...
image::Format Border Tab.png[alt="border settings in format panel"]
...
+
In cases where cells *share a border*, the following rules determine the border style and color:
+
[square]
* If either cell is a detail cell, prioritize the other cell's style
* If the top cell is a header or a trailer, prioritize its style
* If the bottom cell is a header or trailer, prioritize its style
* If both cells are the same type, prioritize the color with the higher RGB value
...
[TIP]
====
For the *Tabbed Interface* component, use the ‘Round Top Corners Only’ option to restrict the ‘Round Corner’ setting to the *top of the tab*, leaving the bottom corners with no rounding (useful for top tabs).  Use the ‘Round Bottom Corners Only’ option to restrict the ‘Round Corner’ setting to the *bottom of the tab*, leaving the top corners with no rounding (useful for bottom tabs). See xref:TabbedInterfaceProperties.adoc[] for  information about tab location.

To apply a unique style to the active (currently selected) tab, click on a tab to make it active, and then apply the desired styling from the *Format* panel.
====
...
image::PresenterMenu.png[alt="select graphical presenter from presenter menu"]
...
|Multi-Header|Displays multiple headers in a single cell for a  xref:AddFreehandTable.adoc[Freehand Table] component. When a cell contains a formula result in  array format, separated by commas (for example: `['a','b']`), selecting the Multi-Header presenter will display the cell`'s value as multiple crosstab headers within a single cell.
...
image::Format CSS Tab.png[alt="CSS class and ID settings in format panel"]
...
=== Apply CSS Styles to a Component
...
+
image::CSS Type text.png[alt="select component region and open format tab"]
...
+
image::CSS Type text2.png[alt="expand CSS menu showing component type"]
