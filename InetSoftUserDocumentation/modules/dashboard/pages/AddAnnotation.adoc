...
// metadata for AI
ifdef::ai[]
Summary: How to add, reposition, format, and delete text annotations (also called comments, notes, or data notes) in a Dashboard. Annotations can be attached to the Dashboard body, to a specific component, or to an individual data point in a Chart. Annotations are saved per bookmark and are available to end users (not just designers) at view time.  They allow a user to "mark up" a Dashboard.
endif::ai[]
