...
// metadata for AI
ifdef::ai[]
Summary: Reference for creating a Word Cloud chart in Visual Composer, which displays dimension labels at sizes and/or colors proportional to an associated measure value (e.g., frequency of occurrence). Place the dimension in the T region and the measure in the Size or Color region.
endif::ai[]
...
image::commonscript:WordCloudExample11.png[alt="word cloud chart example"]
...
+
image::commonscript:WordCloudConfigXY.png[alt="drag dimension to X or Y region"]
+
...
+
image::commonscript:WordCloudConfigText.png[alt="drag dimension to Text region"]
...
+
image::commonscript:WordCloudConfigSizeColor.png[alt="drag measure to Size or Color region"]
+
NOTE: When you use the ‘Count’ method, it does not matter which measure you choose to aggregate.

. *_Optional:_* Press the '`Edit Size`' button to adjust the minimum and maximum display size. To further increase the size range of displayed words, use the ‘Font Scale’ property under the *Advanced* tab of the ‘Chart Properties’ dialog box. (See xref:ChartProperties.adoc[] for more information.)

. *_Optional:_* Apply any desired filtering to restrict the size and number of clouds.  See xref:#WalkthroughWordCloud[] below for an example.

. {finishEditing}
+
image::commonscript:WordCloudExample11.png[alt="completed word cloud chart"]

[#WalkthroughWordCloud]
...
+
image::commonscript:WordCloudExample12.png[alt="drag date dimension to Y region"]

. Drag the ‘Name’ dimension  to the ‘Text’ region.
+
image::commonscript:WordCloudExample13.png[alt="drag name dimension to text region"]

. Drag the ‘Quantity Purchased’ measure (or any other measure)  to the ‘Color’ region.
+
image::commonscript:WordCloudExample2.png[alt="drag measure to color region"]

. Press the {buttonEditMeasure} next to the ‘Color’ field. Change the ‘Aggregate’ property to ‘Count’, and press the {buttonApply}.
+
image::commonscript:WordCloudExample3.png[alt="set aggregate to count for color measure"]
+
...
+
image::commonscript:WordCloudExample4.png[alt="drag measure to size region"]


. Press the {buttonEditMeasure} next to the ‘Size’ field, and verify that the ‘Aggregate’ property is set to ‘Count’.
+
image::commonscript:WordCloudExample5.png[alt="verify count aggregate for size measure"]
+
...
+
image::commonscript:WordCloudExample6.png[alt="choose color spectrum for word cloud"]
...
+
image::commonscript:WordCloudExample7.png[alt="set top 2 ranking for date dimension"]
...
+
image::commonscript:WordCloudExample8.png[alt="right-click legend to hide it"]
...
+
image::commonscript:WordCloudExample9.png[alt="set font face and size for word cloud labels"]
====
...
+
image::commonscript:WordCloudExample10.png[alt="adjust label size range with slider"]
+
...
image::commonscript:WordCloudExample11.png[alt="completed word cloud comparing top two years"]
