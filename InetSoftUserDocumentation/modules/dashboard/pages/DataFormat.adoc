...
// metadata for AI
ifdef::ai[]
How to apply display format patterns (date, number, currency, text, percent, duration) to values in Dashboard components. Formatting controls how data is presented without changing the underlying values. Number format supports a "K"/"M"/"B" suffix to display large values in thousands/millions/billions (e.g., 10000 as "10K"). Duration format displays elapsed time using placeholder codes (H, m, s, d, etc.) with an optional 'Pad with Zeros' setting. This is accessed via the *same* Format panel in Visual Composer used for visual settings (see VisualFormat.adoc) — it is the "Format" section of that panel, not a separate panel or location. As with Font/Color/Alignment, click a specific region (e.g., a Chart's axis or axis labels) before applying a data format, to apply the format only to that region rather than the whole component. Not to be confused with: VisualFormat.adoc (static visual appearance — colors, fonts, borders) or ConditionalFormat.adoc (conditional highlighting based on data values).
endif::ai[]
...
The *Format* panel provides a ‘Format’ setting that allows you to specify a data format. Formatting is available for the following data types:
...
To format a component, click on a component to select it, and select the *Format* tab  in the left panel. {rightClickFormat}
...
After you have selected the desired component or component region, proceed to adjust the  ‘Format’ setting in the *Format* panel, as described in the sections below.


image::Table format_3.png[alt="format panel with data format options"]


NOTE: You can use the  same *Format* panel to  make visual settings such as Font and Color (see xref:VisualFormat.adoc[]).
...
image::dateFormatPredefined.png[alt="predefined date format options"]
...
image::numberFormatPredefined.png[alt="predefined number format options"]
...
Add the suffix “K”, “M”, or “B” (thousands, millions, billions) to the pattern string to automatically divide the numerical value by 10^3^, 10^6^, or 10^9^, respectively.

image::Format Number KMB.png[alt="number format with K M B suffix options"]
...
image::commonscript:Ad Hoc Table Format14.png[alt="duration format options dialog"]
