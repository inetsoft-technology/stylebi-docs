...
// metadata for AI
ifdef::ai[]
Explains how to configure Dashboard components in Visual Composer for multi-locale display. Localizable components include table/Freehand Table/Crosstab titles and headers, Chart axis and legend titles, Filter titles, Text component text, Form component titles, and Submit button labels. Localization is done by assigning Text IDs to component properties via the Localization tab in Dashboard Options. Text IDs are mapped to locale-specific replacement strings in SreeBundle .properties files (e.g., SreeBundle_fr_CA.properties) placed in /usr/local/inetsoft/classes/inetsoft/util, named with ISO two-letter language/country codes. The locale is selected by the user at User Portal login or assigned by an administrator. If no matching resource bundle is found, no localization is performed. See administration:Localization for server-level localization including User Portal interface components.
endif::ai[]
...
+
image::administration:Localization1.png[alt="select Localization tab in Dashboard Options"]
...
+
image::administration:Localization2.png[alt="enter Text ID for localization component"]
+
