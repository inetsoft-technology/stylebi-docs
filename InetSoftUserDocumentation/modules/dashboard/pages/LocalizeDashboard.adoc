...
// metadata for AI
ifdef::ai[]
Summary: Explains how to configure Dashboard components in Visual Composer for multi-locale display by assigning Text IDs to component text properties, and how to create SreeBundle mapping files that translate those Text IDs into locale-specific strings. See administration:Localization for server-level localization.
endif::ai[]
