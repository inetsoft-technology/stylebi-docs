...
// metadata for AI
ifdef::ai[]
How to add interactive filter components to a Dashboard in Visual Composer, including: Selection List (for filtering dimensions or discrete measures), Selection Tree (for filtering hierarchical or recursive parent-child data), Range Slider (for date or numerical ranges), Calendar (for date range selection including non-continuous ranges), and Selection Container (to combine multiple filter components in one region). Also covers displaying aggregates next to filter values, configuring a filter to operate on multiple data blocks simultaneously, and batch-submitting filter selections using a Submit button. These components filter data at runtime, affecting all components bound to the same data block. For fixed (non-user-adjustable) filters on a specific component, see AddConditions instead.
endif::ai[]
...
You can add a variety of different Filter components to a Dashboard. These user-adjustable components filter the data received from the underlying data source, and so affect any components in the Dashboard that display data from the data source. (To attach a fixed filter to a *specific* component, see xref:AddConditions.adoc[].)
...
+
image::viewsheetExp17_1.png[alt="drag dimension to create selection list"]
+
...
+
image::viewsheetExp19_1.png[alt="resize selection list"]
...
+
image::viewsheetExp20_1a.png[alt="drag handle to resize cell"]
...
+
image::Selection Tree Exp 1.png[alt="drag dimension to selection tree"]
...
+
image::Selection Tree Exp 2.png[alt="drag second dimension to selection tree"]
...
image::Selection Tree Expanded.png[alt="selection tree with expanded hierarchy"]
====
...
+
image::Selection Tree Properties Data.png[alt="select Columns option in properties"]

. In the ‘Table’ menu, expand the desired data block to reveal its columns, or expand a cube dimension to reveal its levels.
+
image::Selection Tree Columns.png[alt="expand data block to reveal columns"]
...
+
image::Selection Tree Dialog.png[alt="selection tree hierarchy levels dialog"]
...
image::SelectionTreeRecursive3.png[alt="recursive parent-child tree structure"]
...
+
image::SelectionTreeRecursive1.png[alt="select Parent/Child IDs option"]
...
image::SelectionTreeRecursive2.png[alt="selection tree populated with hierarchy"]
...
image::RangeSlider_1.png[alt="range slider with date range"]
...
+
image::RangeSliderExp1.png[alt="drag measure onto range slider"]
+
...
+
image::Range Slider Edit1_2.png[alt="enter range slider values and press OK"]
...
+
image::Capture_051.png[alt="drag calendar element to dashboard"]
...
+
image::AddCalendarExp1.png[alt="drag date field onto calendar"]
...
include::dashboard:partial$selectionContainer.adoc[]
...
+
image::Selection List properties_2.png[alt="open selection list properties dialog"]
...
+
image::SelectionListAggResize1.png[alt="drag handle to adjust column layout"]

.. Adjust the width of the aggregate text region by dragging the handle at the left side of the text.
+
image::SelectionListAggResize2.png[alt="drag handle to resize aggregate text"]

.. Adjust the width of the aggregate bar region by dragging the handle at the left side of the bar.
+
image::SelectionListAggResize3.png[alt="drag handle to resize aggregate bar"]
...
+
image::multipleFilter1.png[alt="compatible tables listed in data tab"]
+
...
+
image::multipleFilter2.png[alt="press Add to add data block"]
...
image::multipleFilter3.png[alt="dashboard with three regional data blocks"]
...
+
image::multipleFilter4.png[alt="drag Company field to dashboard canvas"]
+
This creates a Selection List that filters the ‘Eastern Customers’ data block based on ‘Company’.
+
image::multipleFilter5.png[alt="selection list filtering eastern customers"]
...
+
image::multipleFilter6.png[alt="add compatible tables to additional tables"]
+
...
image::multipleFilter7.png[alt="selection list with merged company data"]
...
image::multipleFilter8.png[alt="selection list filtering three data tables"]
****
