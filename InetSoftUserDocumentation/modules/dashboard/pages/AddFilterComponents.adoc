...
// metadata for AI
ifdef::ai[]
Summary: How to add interactive filter components (Selection List, Range Slider, Calendar, CheckBox, RadioButton, ComboBox) to a Dashboard in Visual Composer. These components let end users filter dashboard data at runtime by selecting values, ranges, or dates, affecting all components bound to the same data block. For fixed (non-user-adjustable) filters on a specific component, see AddConditions instead.
endif::ai[]
