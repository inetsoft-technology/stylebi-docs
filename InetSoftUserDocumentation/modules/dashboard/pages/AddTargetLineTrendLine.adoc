...
// metadata for AI
ifdef::ai[]
How to add target lines, target bands, statistical measures, and trend lines (curve fits) to a Chart in Visual Composer. A target line marks a goal or threshold value on a chart measure (fixed value or computed as average, minimum, maximum, median, or sum); regions above and below can be filled with independent colors. A target band marks a range between two such values with configurable fill colors for the band interior and the regions above and below. A statistical measure draws lines representing confidence intervals, percentages, percentiles, quantiles, or standard deviation computed from the data. A trend line fits a curve (linear, quadratic, etc.) to the data; it can optionally be projected forward (extrapolated beyond the last data point) for simple forecasting. All four features are accessible to end users at view time as well as to designers.
endif::ai[]
...
image::commonscript:TargetLine5_3.png[alt="target line drawn on chart"]
...
+
image::TargetLine2_1.png[alt="set target line value"]
...
image::commonscript:TargetLine3_1.png[alt="entire chart option effect on target line"]
====

. From the ‘Label’ menu, select one of the following label options:
+
image::commonscript:Add Target_2.png[alt="select target line label option"]
+
[square]
...
+
image::TargetLine1_2.png[alt="customize target line label"]
...
+
image::TargetLine4_1.png[alt="set fill above and fill below colors"]
...
image::commonscript:TargetStats8_1.png[alt="target band drawn on chart"]
...
+
image::TargetBand1_1.png[alt="set target band from value"]

. In the ‘From Label’ menu, select one of the following label options:
+
image::TargetBand2_1.png[alt="select target band from label option"]
+
[square]
...
+
image::TargetBand3_1.png[alt="customize target band from label"]
+
...
image::commonscript:TargetBand4_1.png[alt="entire chart option effect on target band"]
====
...
+
image::TargetBand5_1.png[alt="set fill band and fill above below colors"]
...
image::dashboard:galleryTargetBand2.png[alt="generation and distribution dashboard target band"]
...
image::commonscript:TargetStats1_1.png[alt="statistical measure lines drawn on chart"]
...
+
image::TargetStats2_1.png[alt="select statistical measure computation"]
...
+
image::TargetStats4_1.png[alt="set confidence interval percentage"]
+
...
+
image::commonscript:TargetStats3_1.png[alt="confidence interval bounds on chart"]
...
+
image::TargetStats5_1.png[alt="set percentages computation options"]
+
...
+
image::commonscript:TargetStats6_1.png[alt="percentage lines on chart"]

Percentiles:: For the ‘Percentiles’ option, enter a value or comma-separated list of values as percentages.
+
image::TargetStats7_1.png[alt="set percentiles computation options"]
+
...
+
image::commonscript:TargetStats8_1.png[alt="percentile lines on chart"]

Quantiles:: For the ‘Quantiles’ option, enter the number of quantiles to display.
+
image::TargetStats9_1.png[alt="set quantiles computation options"]
+
...
+
image::commonscript:TargetStats10_1.png[alt="quantile lines on chart"]
...
+
image::TargetStats11_1.png[alt="set standard deviation factors"]
+
...
+
image::commonscript:TargetStats12_1.png[alt="standard deviation lines on chart"]
+
...
image::commonscript:TargetStats13_1.png[alt="entire chart option effect on standard deviation"]
====

. From the ‘Label’ menu, select one of the following label options:
+
image::Add Target_3.png[alt="select statistical measure label option"]
+
[square]
...
+
image::commonscript:TargetStats14_1.png[alt="enter custom labels for quantile lines"]
+
...
+
image::TargetLine1_3.png[alt="customize statistical measure labels"]
+
...
+
image::TargetStats15_1.png[alt="select fill band colors"]
...
+
image::TargetStats16_1.png[alt="set fill above and fill below colors"]
...
+
image::Chart Plot Properties.png[alt="open chart properties dialog box"]
...
+
NOTE: Projection requires that axis values be a linear and complete series of either numbers or dates. Projection is not available for the following chart types: Waterfall, Pie, Radar, Filled Radar, Map. It is also not available for the following data types: String, Boolean.
...
image::dashboard:galleryMap2.png[alt="marketing lead trends dashboard trend lines"]
