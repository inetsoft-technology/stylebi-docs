...
// metadata for AI
ifdef::ai[]
How to add and configure a Crosstab (also called a pivot table or cross-tab) in a Dashboard in Visual Composer. Covers dragging dimensions into Rows and Columns regions and measures into the Aggregates region; selecting aggregate methods including bivariate aggregation (Correlation, Weighted Average) and percentage-of-row or percentage-of-column representations; sorting dimension values (ascending, descending, by value, or manual order); ranking groups using Top/Bottom filters with an optional "Others" category; date grouping levels (Year, Month, etc.) and fiscal calendar grouping; hiding and showing columns; creating named groups that merge multiple row or column header labels into custom categories; making row, column, and aggregate fields dynamic via Form components; and displaying row and column grand totals. Use a Crosstab when you need to summarize measures broken down by two or more dimensions in a tabular format.
endif::ai[]
...
+
image::addCrosstab_1.png[alt="drag crosstab element into dashboard"]

. Press the {buttonEdit} at the top-right corner of the Crosstab to open the Crosstab Editor.
+
image::Crosstab Edit Button_1.png[alt="press Edit button to open crosstab editor"]
+
...
+
image::Crosstab Binding_1.png[alt="drag dimensions into columns and rows regions"]
+
include::dashboard:partial$bindToComponent.adoc[]

. Drag the desired columns or cube measures into the  ‘Aggregates’ region.
+
image::Crosstab 2_1.png[alt="drag measures into aggregates region"]
+
...
. *_Optional:_* To *hide* a particular column, right-click the column in the Crosstab  Editor, and select '`Hide Column`'.  To *show* all hidden columns, right-click any column in the Crosstab  Editor, and select '`Show Columns`'.
...
+
image::crosstabSortRank.png[alt="press Edit Dimension next to row or column header"]
...
+
image::Manual Ordering Dialog.png[alt="manual ordering dialog box"]
...
=====
+
image::Crosstab Measure Options_1.png[alt="press Edit Measure and select aggregate method"]
+
...
+
image::Named Group 5.png[alt="right-click selected labels and select Group"]
+
...
+
image::Named Group 2.png[alt="enter name in group name dialog box"]
...
+
image::Named Group 6.png[alt="crosstab with named group labels applied"]
...
image::Named Group 7.png[alt="right-click header label and select Ungroup"]

****

. *_Optional:_* To display a grand total for rows or columns, select '`Show`' for the desired '`Row Total`' or '`Column Total`' option in the left panel.
+
image::CrosstabRowTotal.png[alt="select Show for row total or column total"]
