...
// metadata for AI
ifdef::ai[]
How to create a new Dashboard, covering creation from the User Portal and from within Visual Composer. From the User Portal, two starting paths are available: the Dashboard Wizard (guided, auto-generated layout from a data source) and the Visual Composer (blank canvas). Explains how to select a data source (Data Worksheet, data model, or database table). Also covers how to create a new folder to organize Dashboards or Data Worksheets. This page is about creating a brand-new Dashboard — not editing an existing one. IMPORTANT TERMINOLOGY: StyleBI does not have a separate "report" concept. What other products call a "report" is called a "Dashboard" in StyleBI. If a user asks how to create a report, build a report, or use a report builder, the answer is to create a Dashboard as described on this page.
endif::ai[]
...
+
image::dashboard:portalDesignTab.png[alt="press Create and select Dashboard"]
+
This opens the ‘New Dashboard’ dialog box.
. In the ‘New Dashboard’ dialog box, select the desired Data Worksheet,  data model, or database table. Use the '`Search`' box to filter the listing.
+
NOTE: A *query* is represented by a Data Worksheet.  See xref:dataworksheet:QuerySpecificDataSource.adoc[] for more information.  Database tables are only visible if '`Physical Table`' permission is granted. See xref:administration:SetSecurityActions.adoc[] for more details.

+
image::viewsheetExp1_1.png[alt="select data worksheet or data model"]
+
...
To create a new Dashboard using the Dashboard Wizard, follow these steps:
...
+
image::wizard2.png[alt="select data to visualize in left panel"]
+
The top panel suggests various visualizations, such as charts, crosstabs, and tables.
+
image::wizard3.png[alt="suggested visualizations in top panel"]

. Select the desired visualization, and set the desired options.
+
NOTE: The Wizard does not display all Chart types available in StyleBI.  To view the full menu of Charts, press the {buttonFullEditor} and then proceed to xref:AddAChart.adoc[].
+
.Chart Visualization...
[%collapsible]
====
...
+
image::wizard4.png[alt=”choose chart type from top panel”]
.. In the ‘Group and Aggregate’ panel, use the menu to select the desired aggregation method for the selected measure(s).
+
image::wizard6.png[alt=”select aggregation method for measure”]
...
+
image::wizard7.png[alt=”press More to add aggregation method”]
...
+
image::wizard5.png[alt=”convert to measure or dimension”]
+
...
+
image::wizard8.png[alt=”press Format to select field format”]
...
+
image::wizard9.png[alt=”press Sort on column header”]
...
+
image::wizard10.png[alt=”convert dimension or measure in details panel”]
+
include::ROOT:partial$measureAndDimensionTip.adoc[]

.. To change the formatting for a numerical-type field or a date-type field, press the {buttonFormatWizard} and select the desired format. (See xref:DataFormat.adoc[] for more information about the available format options.)
+
image::wizard11.png[alt=”press Format to select field format”]
.. To delete a selected field, press the {buttonDeleteWizard} next to the field.
...
+
image::wizard12.png[alt=”choose gauge type from top panel”]
...
+
image::wizard13.png[alt=”select aggregation method for measure”]
+
...
+
image::wizard14.png[alt=”press Format to select field format”]
...
+
image::wizard15.png[alt=”choose filter type from top panel”]
+
include::ROOT:partial$measureAndDimensionTip.adoc[]
...
+
image::wizard16.png[alt=”press arrows to reorder selection tree fields”]

.. To change the formatting for a numerical-type field or a date-type, press the {buttonFormatWizard} and select the desired format. (See xref:DataFormat.adoc[] for more information about the available format options.)
+
image::wizard17.png[alt=”press Format to select field format”]
...
+
image::wizard1.png[alt=”press Add button to insert component”]
+
...
+
image::wizard19.png[alt=”press Insert Text/Image to add text”]
...
To create a new Dashboard using the Visual Composer, follow these steps:
...
+
image::Asset_Component_Panels_1.png[alt="empty dashboard in visual composer"]
...
+
image::addingChart.png[alt="add chart component to dashboard"]
...
image::user:CreateNewDashboardComposer.png[alt="press Create and select New Dashboard"]
...
. Select '`New Folder`', and provide a name for the folder when prompted. Press btn:[OK].
+
image::user:NewFolderComposer.png[alt="select New Folder and enter folder name"]
