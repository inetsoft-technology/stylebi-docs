...
:page-aliases: viewsheet:CreateNewDashboard.adoc
%experimental%
...
. In the ‘New Dashboard’ dialog box, select the desired Data Worksheet,  data model, or database table. (A *query* is represented by a Data Worksheet.  See xref:dataworksheet:QuerySpecificDataSource.adoc[] for more information.)  Note that database tables are only visible if '`Physical Table`' permission is granted. See xref:administration:SetSecurityActions.adoc[] for more details.
