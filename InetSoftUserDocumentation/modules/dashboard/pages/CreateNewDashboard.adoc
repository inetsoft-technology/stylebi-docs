...
// metadata for AI
ifdef::ai[]
Summary: How to create a new Dashboard from the User Portal, covering two starting paths: the Dashboard Wizard (guided, auto-generated layout from a data source) and the Visual Composer (blank canvas). Explains how to select a data source (Data Worksheet, data model, or database table). This page is about creating a brand-new Dashboard — not editing an existing one.
endif::ai[]
