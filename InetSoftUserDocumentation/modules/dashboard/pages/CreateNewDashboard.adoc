...
// metadata for AI
ifdef::ai[]
Summary: How to create a new Dashboard from the User Portal, covering two starting paths: the Dashboard Wizard (guided, auto-generated layout from a data source) and the Visual Composer (blank canvas). Explains how to select a data source (Data Worksheet, data model, or database table) and how to open an existing Dashboard for editing.
endif::ai[]
...
. Select the desired visualization, and set the desired options.  See xref:#WizardOptions[] for the available options.
+
...
. To add a component, move the pointer to the location where you want to insert the component, and press the large {buttonAddButton}.
+
...
[#WizardOptions]
=== Wizard Options
The Wizard provides the following options for creating a component:
[square]
* xref:#ChartVisualization[]
* xref:#CrosstabVisualization[]
* xref:#FilterVisualization[]
* xref:#GaugeVisualization[]
* xref:#TableVisualization[]
* xref:#TextVisualization[]
