...
// metadata for AI
ifdef::ai[]
Summary: Reference for Table component properties in Visual Composer dashboards, accessible via right-click > Properties. Covers the General tab (table styles from the Style Library) and Advanced tab (Shrink to Fit, Enable Table Editing with add/delete/edit rows, Write Back to Storage for persisting user edits, Data Tip View, and Flyover Views). See AddTable for how to create and bind a Table.
endif::ai[]
...
* {commonProperties}
* xref:dashboardscript:Table.adoc[]  in xref:dashboardscript:DashboardObjectReference.adoc[] to set these properties dynamically using script.
...
image::TableProperties.png[alt="table properties general tab with table styles"]
...
image::Table Dialog Advanced Tab.png[alt="table advanced tab properties"]
...
Data Tip View:: A Dashboard component is displayed when mouse hovers over table area. The component is filtered based on the hover region. Select ‘On Click Only’ if you want the data tip to appear only when the user clicks (rather than hovers). If the clicked cell has a hyperlink, you must use {buttonCtrl}-click  to show the data tip. See xref:AddTipsToChart.adoc[] for more details.
