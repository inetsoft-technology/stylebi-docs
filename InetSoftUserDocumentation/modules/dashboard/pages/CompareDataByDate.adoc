...
// metadata for AI
ifdef::ai[]
How to compare data across multiple date ranges in Visual Composer using Charts and Crosstabs — sometimes called year-over-year (YoY), period comparison, or date comparison — supporting standard periods (years, quarters, months, weeks) or custom date ranges, with three comparison modes (all periods, same period, period-to-date). Results can be displayed as values, changes, percentage changes, or combined change-and-value views; additional display options include "In Separate Sub-Graphs" and "Only Show Most Recent Date On Axis Label." Comparison parameters can be driven dynamically by Form components via variables or expressions. A Date-type field is required; color or axis will be used to distinguish date groups depending on existing chart settings. Multiple Charts and Crosstabs in the same Dashboard can share a date comparison configuration using the "Share Date Comparison With" option.
endif::ai[]
...
+
image::commonscript:dateComparison1.png[alt="press More button and select Date Comparison"]
+
This opens the ‘Date Comparison’ dialog box.
+
image::commonscript:dateComparison2.png[alt="date comparison dialog box"]
...
+
image::commonscript:dateComparison3.png[alt="enable share date comparison with option"]
...
+
image::commonscript:dateComparison12.png[alt="select Standard Periods tab"]
...
+
image::commonscript:dateComparison4.png[alt="enter number of past periods and type to compare"]
+
...
+
image::commonscript:dateComparison10.png[alt="select Custom Periods tab"]

.. Enter the desired period ranges by typing the dates in the indicated format or by pressing the {buttonSelectDate}.
+
image::commonscript:dateComparison5.png[alt="enter desired period ranges by typing dates"]
+
...
+
image::commonscript:dateComparison8.png[alt="select All from left menu and set Granularity"]
+
...
image::commonscript:dateComparison21.png[alt="configure month to date comparison options"]

The resulting date comparison is shown below. {dateCompareDetails}

image::commonscript:dateComparison22.png[alt="chart and crosstab showing month-to-date comparison"]
...
image::commonscript:dateComparison15.png[alt="configure options for six quarters by month"]

The resulting date comparison is shown below. {dateCompareDetails}

image::commonscript:dateComparison16.png[alt="chart and crosstab showing six quarters by month"]
...
image::commonscript:dateComparison13.png[alt="select Same Month from left menu and set Granularity"]
...
image::commonscript:dateComparison23.png[alt="configure same month year-over-year comparison options"]

The resulting date comparison is shown below. {dateCompareDetails}

image::commonscript:dateComparison24.png[alt="chart and crosstab comparing current month across years"]
...
image::commonscript:dateComparison17.png[alt="configure second month of each quarter comparison options"]

The resulting date comparison is shown below. {dateCompareDetails}

image::commonscript:dateComparison18.png[alt="chart and crosstab comparing second month of each quarter"]
...
image::commonscript:dateComparison14.png[alt="select Quarter To Date and set period to date options"]
...
image::commonscript:dateComparison19.png[alt="configure month-to-date for each quarter options"]

The resulting date comparison is shown below. {dateCompareDetails}

image::commonscript:dateComparison20.png[alt="chart and crosstab showing month-to-date by quarter"]
