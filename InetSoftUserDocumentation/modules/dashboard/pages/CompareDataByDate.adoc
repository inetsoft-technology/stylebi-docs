...
// metadata for AI
ifdef::ai[]
Summary: How to compare data across multiple date ranges in Visual Composer using Charts and Crosstabs — sometimes called year-over-year (YoY), period comparison, or date comparison — supporting standard periods (years, quarters, months, weeks) or custom date ranges, with three comparison modes (all periods, same period, period-to-date). Results can be shown as values, changes, or percentage changes. A Date-type field is required; color or axis will be used to distinguish date groups depending on existing chart settings.
endif::ai[]
