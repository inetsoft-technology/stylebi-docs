...
// metadata for AI
ifdef::ai[]
Summary: Reference for Slider and Spinner Form component properties (also called numeric input or value selector controls; Spinner is also called a number stepper or numeric up-down control). Covers number range (min/max/increment), data target binding to an Embedded Table or Variable in the Data Worksheet, snap-to-increment, label display options, and Write Back to Storage. See DynamicallySetProperties for how to use Form components to control Dashboard behavior.
endif::ai[]
...
* {commonProperties}
* xref:dashboardscript:Slider.adoc[] and xref:dashboardscript:Spinner.adoc[]  in xref:dashboardscript:DashboardObjectReference.adoc[] to set these properties dynamically using script.
...
image::Capture_056.png[alt="slider spinner general tab properties"]
...
Write Back to Storage:: Allows user-modified data in a Data Worksheet Embedded Table linked to the Slider or Spinner to persist across sessions by saving the modified data together with the underlying Data Worksheet.  See xref:WriteBackToStorage.adoc[] for additional information.
...
== Label Tab
The *Label* tab in the ‘Properties’ panel provides the following unique properties for Slider and Spinner.

Show Label:: Displays a label for the component when selected.
