...
// metadata for AI
ifdef::ai[]
How to create a Candle (candlestick) chart in Visual Composer, which plots four measures (high, low, open, close) against a date dimension for financial or trading data. Functionally similar to StockChart. Each candle has a wick representing the high and low extremes, and a body representing open and close values; candles are filled when open exceeds close, and empty when close exceeds open. Recommended aggregate functions are Max for High, Min for Low, Last (by date) for Close, and First (by date) for Open.
endif::ai[]
...
image::dashboard:galleryDataTip2.png[alt="candle chart dashboard example"]
