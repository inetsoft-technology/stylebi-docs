...
// metadata for AI
ifdef::ai[]
How to create a geographic Map Chart in Visual Composer. Covers two approaches: (1) binding geographic name fields (country, state, city, zip code, etc.) to display data by region, and (2) binding latitude and longitude coordinate fields to display data by exact location. Explains how to mark fields as geographic using the Edit Geographic dialog, including selecting a map region (Asia, Canada, Europe, Mexico, U.S., or World), selecting a geographic layer, and resolving unmatched location names via the Geographic Mapping dialog. Also covers adding color, shape, size, or text encodings for measures; using an optional web map background (Mapbox or Google Map); adding a path field for trajectory lines; setting zoom and pan for the initial map view; and coordinate transformations for Alaska and Hawaii. For creating maps with user-defined regions, see the administration section. Use when you need to visualize data by geographic region or by latitude/longitude coordinates.
endif::ai[]
...
+
image::commonscript:MapChartType_1.png[alt="choose Map chart style"]
+
...
+
image::webMap1.png[alt="right-click chart to open Chart Properties"]
...
+
image::Report_Ad Hoc Chart Map4.png[alt="enable Use Web Map as Background on Advanced tab"]
...
+
image::commonscript:bindGeographicField.png[alt="drag geographic dimension to Geographic field"]
+
...
+
image::commonscript:bindGeographicFieldStateCity_1.png[alt="bind City and State geographic fields to map"]
...
+
image::commonscript:bindGeographicFieldVisual_1.png[alt="drag measure to Color Shape Size or Text region"]
+
...
+
image::commonscript:bindGeographicFieldSetAgg.png[alt="select aggregation method for geographic measure"]
...
+
image::commonscript:bindGeographicFieldRanking_1.png[alt="set ranking for geographic dimension data"]

. *_Optional:_*  To create a particular trajectory through the points on the chart, drag a path field to the ‘Path’ region. The values of this field are used to assign the sort order for the plotted data so that connecting lines are drawn between points that are adjacent in the path sort order.  See the "`Hurricane`" example in xref:#ExploreMore[] for an illustration of using ‘Path’.
+
...
+
image::bindGeographicZoom_1.png[alt="select region and press Zoom to set initial map view"]
+
...
+
image::commonscript:MapChartType_4.png[alt="zoomed map chart with data displayed"]
...
To display data on a map by latitude and longitude, follow the steps below:
...
.. Right-click the Chart and select ‘Properties’. This opens the ‘Chart Properties’ dialog box.
+
image::commonscript:MapChartType_2.png[alt="right-click chart to open Chart Properties dialog"]

.. Under the *Advanced* tab, enable ‘Use Web Map as Background’ and press btn:[OK]. (If this option is not available, ask your administrator  to enable the Web Map feature.)
+
image::commonscript:MapChartType_3.png[alt="enable Use Web Map as Background on Advanced tab"]
...
. In the *Data Source* panel, right-click the field containing latitude values, or press the {buttonActions}, and select ‘Set Geographic’ from the context menu.
+
image::commonscript:Map Chart LongLat SetGeo.png[alt="right-click latitude field and select Set Geographic"]
+
This opens the ‘Edit Geographic’ dialog box. If you need to open the ‘Edit Geographic’ dialog box later, right-click the field or press the {buttonActions}, and select ‘Edit Geographic’ from the context menu.

. From the ‘Map’ menu, select the desired region, and press btn:[OK].
+
image::commonscript:Map Chart LongLat US.png[alt="select a region from the Map menu in Edit Geographic"]
+
...
. Repeat the above steps to set the field containing longitude values as a geographic field.

. Drag the longitude field to the ‘X’ region.

. Drag the latitude field to the ‘Y’ region.
+
image::commonscript:Map Chart LongLat BindGeo.png[alt="drag latitude and longitude fields to Y and X regions"]
+
Data points now appear on the map to mark the locations specified by the latitude and longitude data.
...
. *_Optional:_* To add labels to the points on the map, drag a label field from the *Data Source* panel to the ‘Text’ field.
+
image::commonscript:Map Chart LongLat NameLabel.png[alt="drag label field to Text region to add map labels"]
+
NOTE: By default, the label values are displayed as tooltips. If the labels appear crowded, you can zoom the map to see them more clearly. (See below.)

. *_Optional:_*  Set the initial map view by selecting a region, and pressing the {buttonZoom}.
+
image::commonscript:MapChartType_6.png[alt="select region and press Zoom to set initial map view"]
+
...
+
image::commonscript:MapChartType_5.png[alt="map chart with labeled latitude and longitude points"]
...
+
image::commonscript:Map Chart LongLat SetGeo.png[alt="right-click Lat measure and select Set Geographic"]
+
...
+
image::commonscript:Map Chart LongLat US.png[alt="select U.S. from the Map menu in Edit Geographic"]
+
IMPORTANT: If you are using a Web Map background with latitude and longitude data, you should always use the ‘World’ map option.
...
+
image::commonscript:Map Chart LongLat BindGeo.png[alt="drag Lat and Long fields to X and Y regions"]
+
...
+
image::commonscript:Map Chart LongLat NameLabel.png[alt="drag Name field to Text region to add map labels"]
+
...
You can also press the {buttonPanMap} and drag on the map to reposition, or  press the {buttonZoomIn} or {buttonZoomOut} to zoom. Press the {buttonClearMapZoom} to reset the view.
+
image::commonscript:MapChartType_5.png[alt="map chart with labeled Route 66 landmark points"]

. *_Optional:_* Create a path through the points by dragging the '`Long`' field to the '`Path`' area. See the "`Hurricane`" example in xref:#ExploreMore[] for another illustration of using ‘Path’.
+
image::commonscript:MapChartType_7.png[alt="map chart with labeled Route 66 landmark points"]
...
+
image::setGeographic.png[alt="right-click field and select Set Geographic"]
+
...
+
image::editGeographic1.png[alt="Edit Geographic dialog box with Map and Layer settings"]
...
+
image::editGeographic2.png[alt="match unmatched locations in Geographic Mapping dialog"]
...
image::dashboard:galleryBubble2.png[alt="Global Visual Analysis world map dashboard sample"]
...
image::dashboard:galleryMap2.png[alt="Marketing Lead Trends state map dashboard sample"]
...
image::dashboard:galleryHurricane2.png[alt="Hurricane map chart dashboard sample"]
