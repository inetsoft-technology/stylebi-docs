...
// metadata for AI
ifdef::ai[]
Summary: Explains how to apply trend and comparison calculation methods in Visual Composer to Chart or Crosstab measures. Covers six calculation types: Percent (percent of grand total, subtotal, or a dimension), Change (difference from first, previous, next, or last group), Running (cumulative aggregate with optional date-based reset), Sliding (sliding window aggregate for smoothing), Value Of (reference a neighboring group value without calculation), and Compound Growth (compounding accumulation for percentage values). Methods are configured via the Trend and Comparison option in the Edit Measure panel. For a running total, period-over-period difference, or percent change that will be displayed in a Chart or Crosstab, prefer these built-in calculations over a hand-built Data Worksheet self-reference formula (see AccessDataInExpression). These calculations reset correctly at each group boundary; a Worksheet field[-1]/field[n] self-reference does not.
endif::ai[]

There are many different ways to represent an aggregated *measure* in a Chart or Crosstab. For example, you can represent the aggregated measure  in terms of its _percentage_ of a total, or by its _difference_ from a preceding group, or by a _moving average_ based on adjacent groups, etc. The following sections explain the different options, and how to implement them in a Chart or Crosstab.
...
+
image::Chart Edit Y Axis_1.png[alt="press edit measure button next to measure"]

. Select one of the calculation methods from the ‘Trend and Comparison’ menu in the pop-up panel.
+
image::CalculateMenu_1.png[alt="select calculation method from trend and comparison menu"]
+
...
+
image::CalculationDialogBox_1.png[alt="select calculation type from calculate menu"]
...
+
image::CalculateMenuCustom_1.png[alt="new custom method added to trend and comparison menu"]
...
image::CalculatePercent_1.png[alt="percent calculation dialog box options"]
...
+
image::ChartCalculationPercentExample1.png[alt="chart editor with dimensions and measure configured"]
...
+
image::ChartCalculationPercentExample2.png[alt="enable show values option in chart properties"]
+
This displays the aggregated measure value for each group on the chart.
+
image::ChartCalculationPercentExample3.png[alt="chart displaying aggregate values for each group"]
...
+
image::ChartCalculationPercentExample4.png[alt="select custom option in trend and comparison menu"]
...
+
image::ChartCalculationPercentExample5.png[alt="select percent calculation with category dimension"]

. {finishEditing}
+
image::ChartCalculationPercentExample6.png[alt="chart showing percentages of category totals"]
+
...
image::CalculateChange_1.png[alt="change calculation dialog box options"]
...
image::CalculateRunningTotal_1.png[alt="running calculation dialog box options"]
...
+
image::RunningTotalExample1.png[alt="set date dimension level to month"]

. Press the {buttonEditMeasure} next to the ‘Quantity Purchased’ measure.

. In the ‘Trend and Comparison’ menu of the pop-up panel, select the ‘Custom’ option.
+
image::RunningTotalExample3.png[alt="select custom option in trend and comparison menu"]
+
...
+
image::RunningTotalExample2.png[alt="set running sum reset at year in calculation dialog"]
...
. {finishEditing}
+
image::RunningTotalExample4.png[alt="time series chart showing running total by month"]
+
...
image::MovingCalculationDialog_1.png[alt="sliding calculation dialog box with window settings"]
...
image::CalculateValue_1.png[alt="value of calculation dialog box options"]
...
image::CalculateCompoundGrowth.png[alt="compound growth calculation dialog box options"]
...
image::dashboard:galleryMultipleMeasure3.png[alt="sales revenue profit analysis dashboard example"]
