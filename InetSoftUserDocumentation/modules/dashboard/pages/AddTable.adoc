...
// metadata for AI
ifdef::ai[]
How to add and configure a Table in Visual Composer, including binding data columns, hiding or sorting columns, applying Table Styles, resizing the table and its columns, aligning header and data cells, and formatting columns as currency, date, or number. Also covers reordering columns. There is no dedicated toggle to hide a Table's header row — the way to hide it is to set its height to 0 via 'Set Cell Size' (or `table.setRowHeight(0, 0)` from script).
endif::ai[]
...
+
image::dashboard1.png[alt="drag table component from toolbox into dashboard"]
...
image::viewsheetExp2.png[alt="drag entire data block into dashboard"]
====
+
...
+
image::dashboard2.png[alt="drag columns into details region of table editor"]
+
include::dashboard:partial$bindToComponent.adoc[]

. *_Optional:_* To hide a column, press the {buttonShowHide}.  To remove the column entirely, drag the column back to the left *Data Source* panel.
+
image::dashboard3.png[alt="press Show/Hide button to hide a column"]
...
+
image::dashboard4.png[alt="press Sort button to sort a column"]
...
+
image::formatTab.png[alt="select Format tab at bottom of left panel"]
...
image::viewsheetExp4.png[alt="select a table style in table properties panel"]
...
image::viewsheetExp3.png[alt="drag bottom or right border to resize table"]
...
image::viewsheetExp5.png[alt="drag column borders to resize a column"]
...
image::viewsheetExp67.png[alt="right-click cell and select Set Cell Size"]

[TIP]
====
To hide the Table's *header row* entirely, right-click a header cell, select ‘Set Cell Size’, and set the height to `0`. The same result can be achieved from script — see xref:dashboardscript:TableSetRowHeight.adoc[].
...
image::viewsheetExp8.png[alt="press alignment button to align column header"]
...
image::viewsheetExp9.png[alt="choose Currency option from Format menu"]
...
image::viewsheetExp11.png[alt="choose Date option from Format menu"]
...
image::viewsheetExp14.png[alt="drag column header to change column order"]
