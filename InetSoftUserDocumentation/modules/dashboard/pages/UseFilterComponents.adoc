...
// metadata for AI
ifdef::ai[]
Summary: How end users interact with filter components in the User Portal at Dashboard view time — covering Selection List (single/multi-select, mini-toolbar, search, sort), Selection Tree (hierarchical selection, subtree select/clear), Range Slider (date/numeric range selection), Calendar (day/week/month/year range selection), and Selection Container (adding filters by dragging columns, using the Add Filter button, adding post-aggregate filters, hiding/showing/removing filters). This is the end-user companion to AddFilterComponents, which covers how designers configure these components.
endif::ai[]
...
image::SelectionList.png[alt="selection list with selectable items"]

[TIP]
====
If you want to make only a single selection (all selections being mutually exclusive), hold down the {buttonAlt} key on the keyboard and make your selection. This changes the Selection List to “single-selection” mode. {buttonAlt}-click again to return to multiple-selection mode.

Another way to switch between single- and multiple-selection mode is to press the {buttonSelectionSingle} or {buttonSelectionMulti} next to the selection that you want to make.
====
...
include::dashboard:partial$selectionIcons.adoc[]
...
image::selectSubtreeMenu.png[alt="right-click menu with select subtree option"]
...
image::RangeSlider_1.png[alt="range slider with adjustable end handles"]
...
image::Range Slider Edit1_2.png[alt="range slider edit dialog for precise values"]
...
image::CalendarRangeTypes_1.png[alt="calendar component showing date range selection"]
...
+
image::LastNextMonth.png[alt="calendar navigation buttons for previous and next month"]
...
image::CalendarDaySelection.png[alt="click day name to select column of days"]
====
+
...
+
image::CalendarYearSelection.png[alt="calendar in year view with month selection"]
...
image::SelectionContainer4.png[alt="drag column from table into selection container"]
...
image::SelectionContainer6.png[alt="press add filter button in selection container toolbar"]

This opens the ‘Add Filter’ dialog box, where you can select the field to add as a filter to the Selection Container.   Use the '`Search`' box at top to narrow the list of  fields shown in the dialog box.

image::SelectionContainer7.png[alt="add filter dialog box to select field"]
...
image::Chart Filter1.png[alt="right-click chart field to add post-aggregate filter"]
...
image::SelectionContainer5.png[alt="right-click filter to remove from selection container"]
...
image::SelectionContainer1.png[alt="collapse or expand filter in selection container"]
...
image::SelectionContainer2.png[alt="clear all filters in selection container"]
...
+
image::censusExp1.png[alt="open census dashboard with selection lists"]
...
+
image::Visualization9.png[alt="state selections updating region selection list"]
+
...
+
image::Visualization10.png[alt="filter by region updating state selection list"]
+
...
+
image::Visualization11.png[alt="selection list updated after selecting incompatible item"]
****