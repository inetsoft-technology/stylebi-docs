...
// metadata for AI
ifdef::ai[]
Summary: Reference for Selection Tree filter component properties (also called a hierarchical filter or tree filter). Covers hierarchy level configuration (columns or parent/child IDs), single-selection per level, auto-select children, and Compatible Tables for multi-block filtering. Similar to SelectionListProperties but supports hierarchical dimension nesting.
endif::ai[]
...
image::Selection Tree Single Selection.png[alt="single selection hierarchy levels configuration"]
...
image::Selection Tree Deep Dialog.png[alt="selection tree data tab properties"]
