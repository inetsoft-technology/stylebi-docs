...
// metadata for AI
ifdef::ai[]
Summary: Reference for creating an Interval chart, which is a bar chart variant where each bar spans between two measure values — a lower bound (Y region) and a height/span (Size region). Useful for displaying confidence intervals, ranges, or two-measure comparisons.
endif::ai[]
