...
// metadata for AI
ifdef::ai[]
How to add a background to a Dashboard in Visual Composer using two approaches: (1) assigning a solid background color to the entire Dashboard via the Format tab, or (2) using an Image or Shape component as a visual background behind other components. For Image components, covers uploading image files (JPEG, GIF, SVG, PNG, TIFF), selecting skins, and scaling options (Maintain Aspect Ratio or 9-Cell Grid). For Shape components, covers line and fill settings including gradient fills (linear or radial with configurable color stops), and round corners for rectangles. Also covers using 'Send to Back', locking background components, and layering other components on top. Use this page to improve Dashboard visual design by adding branding, logos, colored panels, or a consistent color scheme.
endif::ai[]
...
+
image::formatTab1.png[alt="set background color using format tab"]
...
+
image::viewsheetExp47.png[alt="select skin or upload image file"]
...
+
image::viewsheetExp48.png[alt="select scale image and aspect ratio option"]
...
image::gradientFill1.png[alt="select gradient option and press gradient chip"]
...
image::gradientFill2.png[alt="select gradient type and press Add button"]

Click the color chip and select the color for that region of the gradient.

image::gradientFill3.png[alt="click color chip to select gradient color"]
...
image::gradientFill4.png[alt="drag color chips to adjust gradient distribution"]
...
+
image::viewsheetExp58.png[alt="set round corner property for rectangle"]
...
+
image::Background Overlap Prompt.png[alt="drag image or shape into desired position"]

. Resize the Image or Shape as desired.
+
image::Resize Background Image.png[alt="resize image or shape as desired"]
...
+
image::lockComponent.png[alt="right-click image or shape and select Lock"]
+
...
image::Image Background.png[alt="components placed on top of background image"]