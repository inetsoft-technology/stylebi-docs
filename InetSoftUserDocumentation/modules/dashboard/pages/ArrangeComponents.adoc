...
// metadata for AI
ifdef::ai[]
How to position and organize Dashboard components in Visual Composer using snapping, alignment tools, grouping (Container), tabbed layouts, and Selection Containers (for grouping filters). Also covers selecting multiple components, resizing, scaling a Dashboard to the screen, adapting component visibility (hiding/showing components with Form controls or Pop/Data Tip features), adapting component data display (using Form controls to dynamically change chart fields), and enabling end-user editing of Charts and Crosstabs. Use this page when you need to arrange, combine, show/hide, or give users control over Dashboard components to make efficient use of screen space. For more complete responsive design, see "Create a Device Layout". Note: what other BI tools (such as Power BI or Tableau) call "cards" — grouping multiple visualizations into a single movable unit — is done in InetSoft by grouping components into a Container; see the grouping section of this page.
endif::ai[]
...
[#SelectComponents]
== Select Components
include::dashboard:partial$selectComponents.adoc[]
...
When moving components on the Dashboard, you can choose to snap components to the *grid* or to other *components*.  Press the {buttonSnapping} in the toolbar and select '`Snap to Grid`' or '`Snap to Objects`', or both.  You can adjust the grid snapping interval using  the '`Grid Height`' property in the '`Options`' dialog box.  See xref:DashboardOptions.adoc[] for more information.
...
+
TIP: You can also select _existing groups_ to combine into a new Container. However, a Container component contains only *individual* components, not other Container components. Therefore, if you group together two existing groups, the existing Containers are deleted, and a single new Container is created to contain all the individual components from the two preexisting groups.

. Right-click any selected component, and choose ‘Group’ from the context menu.
+
image::groupComponents.png[alt="right-click selected components and choose Group"]
+
...
image::Align1.png[alt="press Arrange button and choose alignment option"]
...
image::Align2.png[alt="press Arrange and choose distribute horizontal or vertical"]
...
image::Align3.png[alt="press Arrange and choose resize option"]
...
+
image::Capture_012_1.png[alt="drag component onto another opens layout option dialog"]

. Choose the ‘Place component into tabbed interface’ option. This places the two components (or groups) into a tabbed interface.
+
image::Capture_014_1.png[alt="components placed into tabbed interface"]
...
+
image::Viewsheet Options Button.png[alt="press Options button in dashboard toolbar"]
...
+
image::Viewsheet Options ScaleToScreen.png[alt="enable scale to screen option in layout tab"]
