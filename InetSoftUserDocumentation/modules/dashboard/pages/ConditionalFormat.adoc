...
// metadata for AI
ifdef::ai[]
Summary: How to apply conditional formatting in Visual Composer (highlights, also called color rules) to Table columns, Crosstabs, Charts, Images, and Text components. Highlights change visual appearance (foreground color, background color, font) based on data conditions, making it easy to draw attention to exceptions or thresholds. Multiple highlights on the same element combine additively. Not to be confused with: VisualFormat.adoc (static visual appearance — colors, fonts, borders) or DataFormat.adoc (number/date/percent display patterns).
endif::ai[]
