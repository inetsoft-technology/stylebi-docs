...
// metadata for AI
ifdef::ai[]
How to apply conditional formatting in Visual Composer (highlights, also called color rules) to Table columns, Crosstabs, Charts, Images, and Text components. Highlights change visual appearance (foreground color, background color, font) based on data conditions, making it easy to draw attention to exceptions or thresholds. Multiple highlights on the same element combine additively. For Table highlights, the "Apply to Row" option extends formatting to the entire row (cell-based highlights take precedence over row-based). Highlight conditions support multiple operand types: Value, Field (compare two columns row by row), Expression (JavaScript or SQL), Session Data, or Variable (runtime value from a Form component). Not to be confused with: VisualFormat.adoc (static visual appearance — colors, fonts, borders) or DataFormat.adoc (number/date/percent display patterns).
endif::ai[]
...
+
image::Highlight Dialog Box.png[alt="highlight panel with add button and attributes options"]
...
+
image::dataworksheet:ConditionDialogBox.png[alt="conditions dialog box with operand and comparison menus"]
