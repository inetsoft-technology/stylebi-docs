...
// metadata for AI
ifdef::ai[]
Explains how to open an existing Dashboard for editing in Visual Composer, either from the Portal toolbar (using the Edit button) or from Visual Composer's Assets panel by double-clicking, dragging, or right-clicking and selecting 'Open Sheet'. Also covers opening a Dashboard in Metadata mode (without data) via right-click 'Open in Metadata mode' or by pressing Cancel in the loading dialog. Recommends using a development or test environment rather than production, notes that the Modification History audit report tracks who makes edits (but not the specifics), and advises maintaining backups.
endif::ai[]
...
+
image::Open Sheet.png[alt="right-click dashboard and select Open Sheet"]
+
