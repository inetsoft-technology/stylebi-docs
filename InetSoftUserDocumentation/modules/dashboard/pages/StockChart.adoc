...
// metadata for AI
ifdef::ai[]
Summary: Reference for creating a Stock (OHLC) chart in Visual Composer, which displays four measures (low, high, open, close) against a date dimension for financial trading data. Functionally the same as CandleChart but with a different visual style (tick marks vs. filled body).
endif::ai[]
...
image::commonscript:stockChartExp4.png[alt="stock chart example"]
...
+
image::commonscript:stockChartExp1.png[alt="press Select Chart Style, choose Stock"]
+
Note that the Chart Editor now provides binding regions for ‘High’, ‘Close’, ‘Open’, and ‘Low’.
+
image::commonscript:CandleChartType2.png[alt="chart editor with High Close Open Low regions"]
...
+
image::commonscript:stockChartExp3.png[alt="drag date dimension to X region"]
+
...
+
image::commonscript:CandleChartType3.png[alt="set Level to desired date grouping"]
...
+
image::commonscript:stockChartExp2.png[alt="drag high close open low measures to regions"]
...
+
image::commonscript:Report_CandleChartType7.png[alt="set High to Max and Low to Min aggregate"]
...
+
image::commonscript:Report_CandleChartType7b.png[alt="set Close to Last and Open to First aggregate"]

. {finishEditing}
+
image::commonscript:CandleChartType5.png[alt="completed stock chart"]
...
+
image::commonscript:stockChartExp4.png[alt="use Zoom Chart to view limited date range"]
+
