...
[square]
* To control how the Dashboard displays on devices of different dimensions, see xref:#DeviceLayouts[].

* If you want paginated, *report-style* output with page breaks, page numbering, and repeating headers/footers, see xref:#PrintLayouts[].

TIP: StyleBI does not have a separate "`report`" asset. Use xref:#PrintLayouts[].
...
A _print layout_ allows your Dashboard to print like a traditional report, including page breaks, page numbering, and headers/footers that repeat identically on every printed page.  The print layout  automatically modifies certain Dashboard components when the Dashboard is printed: *Tables* are fully expanded to show all rows, and *Filter* components such as Selection Lists and Range Sliders are presented with their current selections only.
...
. Edit the layout as desired. See xref:#DeviceLayouts[] and xref:#PrintLayouts[] for more details.
...
. Define one or more device layouts. See xref:#DeviceLayouts[] and xref:#PrintLayouts[] for more details.
...
. *_Optional:_* For a print layout, add additional Text components into the page header or footer. Any text you place in the header or footer -- title, page number, generation date, etc. -- repeats identically on every page of the printed output. See below:
+
...
A common *report-style footer* combines these codes in one Text component, for example, use `Page pass:[{P}] of pass:[{N}]` to number each page against the total, or `Generated pass:[{D}] pass:[{T}]` to stamp the printed date and time. To reproduce a traditional report`'s *title-plus-pagination* layout, simply place a Text component with the Dashboard title in the header, and one of these page/date patterns in the footer.
====
