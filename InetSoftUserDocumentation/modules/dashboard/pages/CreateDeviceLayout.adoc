...
// metadata for AI
ifdef::ai[]
How to create and configure layouts that control how a Dashboard appears on different devices and on the printed page. Covers device layouts for device classes (Extra small, Small, Medium, Large) and creating custom device classes based on min/max CSS pixel widths, with the 'Mobile Only' option to prevent layouts from triggering on narrow desktop browser windows. StyleBI does not have a separate "report" asset -- what other products call a report is a Dashboard -- so print layouts are how a Dashboard produces paginated, report-style output: multiple pages, page breaks, page numbering, font scaling, and headers/footers that repeat on every page, the same capability report pagination provided. Covers assigning Dashboard components to a layout by dragging from the Components folder, repositioning and resizing components, using guidelines, adding PageBreak components, and applying Table Flow Control (Fit Page Width or Fit Contents) for tables in print layout. Also covers adding Text and Image components to print layout headers and footers, including special codes for inserting page numbers ({P}), total pages ({N}), date ({D}), and time ({T}) -- for example, a "Page {P} of {N}" footer, a common report pagination pattern. Explains how to edit and delete existing device and print layouts.
endif::ai[]
...
You can set the layout that determines how a Dashboard is displayed on an electronic device and on the printed page. StyleBI does not have a separate "`report`" asset -- what other products call a report is simply a Dashboard (see xref:ROOT:Glossary.adoc[]) -- so if you need paginated, report-style output with page breaks, page numbering, and repeating headers/footers, that is what a *print layout* provides. See xref:#PrintLayouts[] below.

[#DeviceLayouts]
== Device Layouts
A Dashboard _layout_ for an electronic device identifies a class of devices (e.g., phone, tablet) based on screen dimensions, and lets you tailor the display of the Dashboard to that class of devices.

[#CreateLayoutForMobileDevice]
=== Create a Layout for Mobile Device
To create a Dashboard layout for a device class, follow the steps below:

. Press the {buttonOptions} in the toolbar to open the ‘Dashboard Options’ dialog box.

. Select the *Layout* tab.
+
image::Viewsheet Properties Dialog Screens.png[alt="dashboard options dialog with Layout tab selected"]

. Press the btn:[New] button in the ‘Device Layouts’ panel. This opens the ‘Dashboard Device Layout’ dialog box.
+
image::Viewsheet Layout Dialog.png[alt="dashboard device layout dialog box with device class selection"]
...
+
image::Viewsheet Layout Dialog New Dev.png[alt="press New Device Class button to add custom device"]
+
This opens the ‘Screen Size’ dialog box.
+
image::Screen Size Dialog.png[alt="screen size dialog with label and description fields"]
...
+
image::Screen Size Dialog 2.png[alt="screen size dialog with minimum and maximum width fields entered"]

.. Press btn:[OK] to close the dialog box and add the new device class to the list of existing device classes.
+
image::Viewsheet Layout Dialog New Dev 2.png[alt="new custom device class added to device class list"]
...
image::delete Custom Device.png[alt="edit and delete buttons next to custom device class label"]
...
+
image::Viewsheet Layout Panel.png[alt="select device classes and enter layout name in panel"]
+
...
[TIP]
.How to make your Dashboard print like a report
====
StyleBI does not have a separate "`report`" asset -- what other products call a report is simply a Dashboard (see xref:ROOT:Glossary.adoc[]). A *print layout* is how you get the same paginated, multi-page output a report used to provide: page breaks, page numbering, and headers/footers that repeat identically on every printed page.
====

[#CreateLayoutForPrint]
=== Create a Layout for Print
A _print layout_ specifies the page settings for printing the Dashboard. This allows you to tailor the arrangement of the Dashboard to these page settings, producing paginated, report-like PDF output with multiple pages, page breaks, and headers/footers. The print layout  automatically modifies certain Dashboard components when the Dashboard is printed: *Tables* are fully expanded to show all rows, and *Filter* components such as Selection Lists and Range Sliders are presented with their current selections only.
...
+
image::Viewsheet Layout Dialog Print.png[alt="dashboard print layout dialog box with paper and margin settings"]
...
== Edit or Delete an Existing Layout
...
+
image::Viewsheet Layout Edit.png[alt="select layout and press Edit button in device layouts panel"]
...
+
image::Viewsheet Layout Assignment 1.png[alt="select layout from Layout menu in dashboard status bar"]
+
...
+
image::Viewsheet Layout Content.png[alt="press Edit Content button in toolbar for print layout"]
...
+
image::Viewsheet Layout Guides.png[alt="press Guides button to choose guideline proportions for layout view"]
+
...
+
image::Viewsheet Layout Assignment 2.png[alt="drag components from Components folder into layout area"]
...
+
image::Viewsheet Layout Assignment 3.png[alt="right-click table and select Table Flow Control in print layout"]
+
...
+
image::Viewsheet Layout Assignment 4.png[alt="table with Fit Page Width or Fit Contents option applied"]

. *_Optional:_* For a print layout, add additional Text components into the page header or footer. A header or footer is exactly the kind of element that made a printed report a report: whatever you place there -- a title, a page number, a generation date -- repeats identically on every page of the printed output. See below:
+
...
+
image::Viewsheet Layout Header Footer.png[alt="select Edit Header or Edit Footer from Edit Content menu"]

.. Drag a Text component from the *Toolbox* folder into the header or footer region.
+
image::Viewsheet Layout HeaderText.png[alt="drag text component into header or footer region"]
...
+
image::Viewsheet Layout HeaderText2.png[alt="text properties dialog with text entered for header"]
+
NOTE: You can use the following special codes within the text that you enter: `pass:[{P}]` inserts the current page number, `pass:[{N}]` inserts the total number of pages, and `pass:[{D}]` and `pass:[{T}]` insert the current date and time. To control the formatting of the inserted values, use the optional `format` parameter, for example, `pass:[{P,format}]`. The format for page numbering should follow the syntax described in xref:dashboard:DataFormat.adoc#NumberFormat[Number Format]. The format for date and time should follow the syntax described in xref:dashboard:DataFormat.adoc#DateFormat[Date Format].
+
TIP: A common report-style footer combines these codes in one Text component, for example `Page pass:[{P}] of pass:[{N}]` to number each page against the total, or `Generated pass:[{D}] pass:[{T}]` to stamp the printed date and time. Placing a Text component with the Dashboard's title in the header, and one of these page/date patterns in the footer, is the most common way to reproduce a traditional report's title-plus-pagination layout.
...
+
image::Viewsheet Layout HeaderText3.png[alt="format tab applied to style text component in header"]
+
...
. *_Optional:_*  For a print layout, add additional Image components into the page header or footer. Like a Text component in the header or footer, an Image component placed here (for example, a company logo) repeats identically on every printed page.
+
...
+
image::Viewsheet Layout HeaderImage.png[alt="drag image component into header or footer region"]
...
+
image::Viewsheet Layout HeaderImage2.png[alt="image properties general tab to select or upload image for header"]
...
+
image::Viewsheet Layout HeaderImage3.png[alt="format tab applied to style image component in header"]
