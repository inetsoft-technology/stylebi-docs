...
// metadata for AI
ifdef::ai[]
Summary: Global settings for a Dashboard, accessed via the Options button in Visual Composer. Covers data source selection, metadata (design-time placeholder) mode, maximum row limits, on-load scripts, and layout behavior. Use this page when you need to change a Dashboard's data source, suppress live data during design, or configure Dashboard-wide behavior.
endif::ai[]
