...
// metadata for AI
ifdef::ai[]
Global settings for a Dashboard, accessed via the Options button (or right-click context menu) in Visual Composer. The dialog has five tabs: Options, Filters, Layout, Localization, and Script. The Options tab covers: data source selection (and converting it to a Data Worksheet), Use Metadata mode (suppresses live data during design), Hide INFO Notifications in Composer, Prompt for Parameters (with selective customization), Selection Association (cross-filter interaction), Max Rows Warning, Create MV On-demand (materialized view auto-creation), Design mode sample data size, Snap Grid, Alias, Description, Server-Side Update (auto-refresh interval), and List on Portal Tree visibility. The Filters tab configures how selection elements interact across Dashboards in the Portal. The Layout tab provides Target Screen borders, Scale to Screen, Fit to Width, Balance Padding, and device layout selection. The Localization tab identifies elements for translation. The Script tab adds Dashboard-level initialization scripts. Use this page when you need to change a Dashboard's data source, suppress live data during design, configure parameter prompting, set up server-side refresh, or adjust any other Dashboard-wide behavior.
endif::ai[]
...
image::dataworksheet:VSOptions Button Toolbar.png[alt="press Options button to open Dashboard Options dialog"]
...
+
image::UseMetadataView.png[alt="use metadata displays meta-data instead of live data"]
+
...
Hide INFO Notifications in Composer:: When enabled, this suppresses the popup toast alerts for messages that have *Info* level and below. These alerts are instead sent directly to the Console.  (See xref:VisualComposerConsole.adoc[] for more information.) *Warning* and *Error* alerts continue to generate toast messages as normal.
...
Grid Height::  Number of pixels defining the snap increment for the '`Snap to Grid`' feature.  See xref:ArrangeComponents.adoc#SnapComponents[Snap Components] for more details.
...
+
image::Viewsheet Options Target Screen2.png[alt="target screen dashed border shows specified pixel dimensions"]
+
By laying out the Dashboard components within these border lines, you can ensure that the Dashboard will fit within a screen of the specified dimensions. See xref:dashboard:CreateDeviceLayout.adoc[] for a more flexible method of adapting the Dashboard to specific devices.
...
== Script Tab
The *Script* tab allows you to create scripts that will execute whenever the Dashboard initializes or reloads. See xref:dashboardscript:AddScriptToDashboard.adoc#AddDashboardScript[Add Dashboard-Level Script] for more information.
