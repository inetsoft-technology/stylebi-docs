...
// metadata for AI
ifdef::ai[]
Summary: Reference for Text component properties. The Text component is commonly used as a KPI card or metric card to display a single aggregated value (scorecard tile). Covers the Text field (static label or dynamic expression), data binding for displaying aggregate values, and formatting. The Text component can also embed external content via URL. See AddTextOrHTML for how to add a Text component.
endif::ai[]
...
* {commonProperties}
* xref:dashboardscript:Text.adoc[]  in xref:dashboardscript:DashboardObjectReference.adoc[] to set these properties dynamically using script.
...
image::Text Properties_1.png[alt="text properties general tab settings"]

Text:: Display a label, or specify a single calculated value. To use an Excel-style aggregation function, press the  and select the ‘Expression’. See xref:dashboardscript:AddScriptToDashboard.adoc#AddPropertyScript[Add Property Script] for more information on using the ‘Expression’ option.
+
image::textDynamic.png[alt="text display label or calculated value"]
+
