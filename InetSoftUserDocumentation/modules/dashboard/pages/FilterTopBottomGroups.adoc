...
// metadata for AI
ifdef::ai[]
How to apply a ranking filter (top-N or bottom-N) in Visual Composer to Charts and Crosstabs to show only the highest or lowest dimension groups by a selected measure — for example, "top 5 products by revenue." Covers ranking in both the Chart Editor and Crosstab Editor, optional sorting of ranked groups by value (ascending, descending, or manual order), and the option to group excluded items into an "Others" category. Notes that chart ranking is relative to the parent group, so "top N" applies within each parent group, potentially showing more than N items total across the chart.
endif::ai[]
...
+
image::topNFilter2.png[alt="select top or bottom from rank menu"]

. Enter the number of top or bottom groups to select, and choose the measure on which to rank the groups.
+
image::topNFilter3.png[alt="enter number of groups and select ranking measure"]
+
...
+
image::topNFilter4.png[alt="select sort order for top or bottom groups"]
+
...
+
image::topNFilter5.png[alt="chart showing top N filtered results"]
...
+
image::crosstabSortRank.png[alt="press edit dimension button on crosstab row or column header"]
