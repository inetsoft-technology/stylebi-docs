...
// metadata for AI
ifdef::ai[]
Summary: Reference for Freehand Table component properties in Visual Composer (also called a free-form table or custom layout table), which differ from regular Table properties. Covers header/trailer rows and columns, Shrink to Fit, Fill Blank Cell With Zero, Data Tip, and Flyover Views (hover tooltips). See TableProperties for regular Table settings.
endif::ai[]
...
* {commonProperties}
* xref:dashboardscript:FreehandTable.adoc[]  in xref:dashboardscript:DashboardObjectReference.adoc[] to set these properties dynamically using script.
====
...
image::Freehand Properties Advanced.png[alt="freehand table advanced properties tab"]
