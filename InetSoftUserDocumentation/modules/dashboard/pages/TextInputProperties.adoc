...
// metadata for AI
ifdef::ai[]
Summary: Reference for Text Input form component properties in Visual Composer dashboards. Covers the General tab (submit behavior, placeholder text, default text, multi-line mode, inset border style), the Option tab for input editor/validation, and the Label tab (show label, label text, label position, gap). See WriteBackToStorage for persisting user input to the database.
endif::ai[]
...
* {commonProperties}
* xref:dashboardscript:TextInput.adoc[]  in xref:dashboardscript:DashboardObjectReference.adoc[] to set these properties dynamically using script.
...
image::TextInput Properties Dialog.png[alt="text input general tab properties"]
...
Show Label:: Displays a label for the TextInput component when selected.
