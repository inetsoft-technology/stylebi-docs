...
// metadata for AI
ifdef::ai[]
Summary: How to create a Scatter Contour chart in a Dashboard, which shows density contours for scatter plot data without displaying individual points. Covers binding two measures to X and Y axes, optional weighting via a Size measure, color scheme selection, and adjusting contour characteristics (bandwidth, resolution, levels) in Chart Properties. See ContourMapChart to overlay contours on a geographic map.
endif::ai[]
