...
// metadata for AI
ifdef::ai[]
Summary: How to create a Scatter Contour chart in Visual Composer, which shows density contours for scatter plot data without displaying individual points. Covers binding two measures to X and Y axes, optional weighting via a Size measure, color scheme selection, and adjusting contour characteristics (bandwidth, resolution, levels) in Chart Properties. See ContourMapChart to overlay contours on a geographic map.
endif::ai[]
...
image::commonscript:ScatterContour1.png[alt="scatter contour chart example"]
...
+
image::commonscript:scatterContourChartExp1.png[alt="select scatter contour chart style"]
...
+
image::commonscript:scatterContourChartExp2.png[alt="drag measure to X or Y region"]
+
...
+
image::commonscript:scatterContourChartExp3.png[alt="select aggregation method for measure"]
...
+
image::commonscript:scatterContourChartExp4.png[alt="drag second measure to remaining axis region"]
...
+
image::commonscript:scatterContourChartExp5.png[alt="set aggregation for second measure"]
+
This creates the desired scatter contour chart.
+
image::commonscript:scatterContourChartExp6.png[alt="completed scatter contour chart"]

. *_Optional:_* Drag a *dimension* to the ‘Text’ area to display labels on the Chart.
+
include::ROOT:partial$dimensionTip.adoc[]
+
image::commonscript:scatterContourChartExp7.png[alt="drag dimension to text region for labels"]
+
...
+
image::commonscript:scatterContourChartExp8.png[alt="select alternate color scheme"]
+
...
+
image::commonscript:contourMapExp7.png[alt="adjust contour size focus setting"]
+
