...
// metadata for AI
ifdef::ai[]
How to improve Dashboard performance using materialized views, which pre-aggregate and cache data so the Dashboard avoids querying the database on every load. Covers creating, deleting, and scheduling materialized views from the User Portal. When a VPM is in use, materialized views must be managed from Enterprise Manager instead.
endif::ai[]
