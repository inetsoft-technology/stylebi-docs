...
// metadata for AI
ifdef::ai[]
Summary: How to create a Network chart in Visual Composer, which displays pairwise peer relationships with nodes positioned freely. Requires Source and Target dimension fields; optionally supports a weight measure, node coloring by dimension, and node size by measure. An alternative to CircularNetworkChart, which arranges nodes along a circle.
endif::ai[]
