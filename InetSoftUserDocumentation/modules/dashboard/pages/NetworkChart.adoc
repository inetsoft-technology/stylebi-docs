...
// metadata for AI
ifdef::ai[]
How to create a Network chart in Visual Composer, which displays pairwise peer relationships with nodes positioned freely. Requires Source and Target dimension fields. Optional features include: breaking out the chart along an X or Y dimension; coloring or sizing nodes by a dimension or measure; coloring or sizing connection lines by a weight measure; adding node labels via the Text region; and setting fixed size or color for nodes and lines. Node color is based on Target field values — source-only nodes inherit edge color by default, but the 'Apply Aesthetics to Source Nodes' option in Chart Properties can change this. An alternative to CircularNetworkChart, which arranges nodes along a circle.
endif::ai[]

A network chart displays a network structure defined by pairwise peer relationships.   It is an alternative representation of a xref:CircularNetworkChart.adoc[].

image::commonscript:networkChart1.png[alt="network chart showing peer relationship connections"]

To create a  network chart, follow the basic steps below:
...
image::commonscript:networkChart2.png[alt="sample data set with peer relationships and weighting"]
...
. Press the {buttonSelectChartStyle}. Choose the ‘Network’  style.  Press the {buttonApply}.
+
image::commonscript:networkChart1VS.png[alt="choose Network chart style and press Apply"]
...
+
image::commonscript:networkChart2VS.png[alt="drag first peer dimension to Source region"]
+
...
+
image::commonscript:networkChart3VS.png[alt="drag second peer dimension to Target region"]
...
+
image::commonscript:circularNetwork5VS.png[alt="drag dimension to Color region to color network nodes"]
+
...
+
image::commonscript:networkChart6VS.png[alt="drag weight field to Color or Size region for connections"]

. Press the {buttonEditMeasure} next to the measure, and select the desired aggregation method for the measure.
+
image::commonscript:networkChart6bVS.png[alt="select aggregation method for network chart measure"]
...
+
image::commonscript:networkChart8VS.png[alt="set fixed size or color for network chart nodes"]
...
+
image::commonscript:networkChart9VS.png[alt="set fixed size or color for network chart lines"]

. To add labels to the nodes, drag a dimension to the ‘Text’ region.
+
image::commonscript:networkChart10VS.png[alt="drag dimension to Text region to label network nodes"]

.  To enlarge or reduce the network chart spacing to fit the plot area, see xref:BasicChartingSteps.adoc#AdjustChartSpacing[Adjust Chart Spacing] in xref:BasicChartingSteps.adoc[].
