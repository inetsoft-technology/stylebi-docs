...
// metadata for AI
ifdef::ai[]
Summary: Reference for the Visual Composer toolbar buttons, including Create (new Data Worksheet or Dashboard), Save, Save As, Preview, Options, and other editing controls. See CreateNewDashboard and DashboardOptions for details on specific toolbar actions. Note: charts automatically animate on entrance and dim non-hovered elements on hover when viewed via Preview, in the Portal, or embedded — this is not configurable (no script property, GUI option, or admin setting), and does not occur while editing in the Composer canvas, Wizard, or chart binding editor.
endif::ai[]
...
{buttonPreview}:: View the Dashboard as the user does. Charts display automatic entrance and hover animations in Preview and in the Portal. (Animations are not configurable.)
...
{buttonSnapping}:: Set snap positioning for Dashboard components. (See xref:dashboard:ArrangeComponents.adoc[].)

{buttonArrange}:: Set alignment and spacing for Dashboard components. (See xref:dashboard:ArrangeComponents.adoc[].)
