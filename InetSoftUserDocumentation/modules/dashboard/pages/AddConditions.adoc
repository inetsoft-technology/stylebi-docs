...
// metadata for AI
ifdef::ai[]
How to apply a condition filter to a specific Dashboard component (Chart, Table, Crosstab, Gauge, or Text) in Visual Composer. A Condition affects only the single component it is applied to, unlike Filter components (e.g., Selection Lists) which affect all components sharing the same data block. The comparison value can be a literal Value, another Field, a JavaScript/SQL Expression, a Variable (allowing dynamic user input at runtime via Form components or prompted parameters), or Session Data (current user's username, roles, or groups). When a Variable is used and no value is provided, the condition is ignored and all records are returned. The page also covers how to suppress parameter prompting for all or specific parameters via the Dashboard Options dialog.
endif::ai[]
...
+
image::dataworksheet:ConditionDialogBox.png[alt="right-click element and select Conditions"]
...
. Press the {buttonChangeValueType}, and select ‘Value’, ‘Field’, ‘Expression’, ‘Variable’, or ‘Session Data’.  See xref:#ValueTypeOptions[] for more information about these options.
+
...
[#ValueTypeOptions]
== Value Type Options
The {buttonValueType} provides the following options.  See xref:#ApplyaCondition[] for information on how to access the button.
...
image::dataworksheet:nullVariable.png[alt="add is not equal to null condition clause"]
...
* If you select the ‘Session Data’ option, choose one of ‘User’, ‘Roles’, or ‘Groups’. These parameters return information about the user who is currently viewing the Dashboard; respectively, the username, the array of roles to which the user belongs, and the array of groups to which the user belongs. The adjacent menu automatically chooses the `[equal to]` or `[one of]` operator to match your selection.
+
...
+
image::OptionsPromptForParameters.png[alt="disable prompt for parameters option"]
...
+
image::Viewsheet Options Customize.png[alt="press Customize button next to prompt for parameters"]
+
...
+
image::CustomizeParametersDialog.png[alt="select parameters to disable and press Add"]
+
