...
// metadata for AI
ifdef::ai[]
Overview of all drill-down and data-exploration techniques available in a Dashboard in Visual Composer. All of the following are considered drill-down techniques for charts: chart brushing (cross-chart filter highlighting related data); chart zoom (focus on selected groups by include or exclude); drilling through chart or crosstab date/dimension hierarchies (drill down/up buttons on axes and headers, facet chart behavior, expand/collapse hierarchy in crosstabs — StyleBI calls this "drill through," which is DIFFERENT from the Power BI meaning of that term; Power BI's "drill through" — navigating to a separate report page filtered to a selected value — corresponds to StyleBI's drill-down-with-filter or hyperlink navigation, described below); drilling to chart or crosstab detail data (Show Details button opens a Data panel with underlying records, supports formatting, table styles, and export); hyperlinks on chart elements, labels, tables, and crosstabs (link to another dashboard with parameter passing, or to a web page); data tips (hover over a chart element to display a popup component — such as a Chart or Gauge — that follows the mouse and is filtered to the hovered data point; configured in AddTipsToChart); flyover views (hover or click on a chart/table/crosstab to filter other Dashboard components that remain fixed in their positions on the dashboard — unlike data tips, flyover components do not pop up or follow the mouse); and drill-down-with-filter (simultaneously drill to next hierarchy level and filter to selected group, affecting all components showing that dataset). Most techniques are available to end users at view time. Use this page to choose the right approach for interactive data exploration and navigation between detail levels.
endif::ai[]
...
+
include::dashboard:partial$selectChartData.adoc[]
...
NOTE: If you are coming from Power BI, StyleBI's "drill through" means something different — it refers to navigating hierarchy levels *within the same chart*, not jumping to a separate report page. For that behavior, see the drill-down-with-filter and hyperlink techniques described later on this page.
...
. Right-click on a desired Chart region, as described below. {rightClickOptions}
+
[square]
* Right-click a *Chart element* and select the '`Hyperlink`' option to add hyperlinks to the Chart elements (e.g., bars or points).
* Right-click on a *Chart dimension label* and select the '`Hyperlink`' option to add hyperlinks to the labels.
* Right-click the *Chart title* and select the '`Hyperlink Title`' option to add a hyperlink to the title.
* Right-click an *empty plot region* and select '`Hyperlink Empty Plot Area`' to add a hyperlink to the entire plot region.
...
. *_Optional:_*  Enable ‘Send Dashboard parameters’ to pass all existing Dashboard parameters to the target  Dashboard, including Form component values. The Dashboard parameter names must exactly match parameter names  defined in the target  Dashboard. See xref:dashboardscript:parameter.adoc[] in xref:dashboardscript:AdvancedDashboardScripting.adoc[] for more information about Dashboard parameters.
+
[.videotitle]
...
=====
+
If the parameter name matches the name of a *Form component* in the target Dashboard, the Form component will be set to that parameter value when the hyperlink is  used.
...
TIP: To access the current flyover filter condition in script, use xref:dashboardscript:CommonDataConditions.adoc[].
...
image::dashboard:galleryDataTip2.png[alt="executive sales summary dashboard flyover example"]
