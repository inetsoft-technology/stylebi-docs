...
:page-aliases: viewsheet:DrillDownIntoData.adoc
%experimental%
...
. *_Optional:_*  Enable ‘Send Dashboard parameters’ to pass all existing Dashboard parameters to the target  Dashboard, including Form component values. The Dashboard parameter names must exactly match those defined in the target  Dashboard. See xref:dashboardscript:parameter.adoc[] in xref:dashboardscript:AdvancedDashboardScripting.adoc[] for more information about Dashboard parameters.
+
[.videotitle]
