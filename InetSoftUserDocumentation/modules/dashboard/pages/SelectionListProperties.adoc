...
// metadata for AI
ifdef::ai[]
Summary: Reference for Selection List filter component properties (also called a checkbox filter, multi-select filter, filter list, or — the common Excel/Power BI term for this same widget — a slicer). Covers display style (list vs. dropdown), sort order, submit behavior, single-selection mode, and data binding including an optional measure bar display. Also covers Compatible Tables for filtering multiple data blocks simultaneously.
endif::ai[]
...
* {commonProperties}
* xref:dashboardscript:SelectionListTree.adoc[]  in xref:dashboardscript:DashboardObjectReference.adoc[] to set these properties dynamically using script.
...
image::Capture_038.png[alt="selection list general tab properties"]
...
* *Allow toggle between single and multi-selection*: This allows the user to switch between single-selection and multi-selection by using the {buttonSelectionSingle} or {buttonSelectionMulti}, respectively.
...
image::Selection List properties_1.png[alt="selection list data tab properties"]
