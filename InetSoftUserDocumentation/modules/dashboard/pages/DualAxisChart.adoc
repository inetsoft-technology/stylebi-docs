...
// metadata for AI
ifdef::ai[]
Summary: How to create a Dual Axis chart in Visual Composer (also called a secondary axis chart, double axis chart, or two-Y-axis chart), which displays two measures on separate left and right axes. Enable 'Secondary Axis' in the Edit Measure panel after switching to single-graph mode. See MultipleMeasureChart for general multi-measure techniques.
endif::ai[]
...
image::commonscript:Chart Secondary Axis1_1.png[alt="two measures plotted on adjacent axes"]

In some cases, you may want to use *dual Y-axes* to represent these measures, as shown below:

image::commonscript:Chart Secondary Axis2.png[alt="dual Y-axes chart with two measures"]
...
+
image::commonscript:Chart Switch to Single.png[alt="press Switch to Single Graph button"]
+
...
image::commonscript:Chart Switch to Separate.png[alt="press Switch to Separate Graph button"]
====
...
+
image::commonscript:SecondaryYAxis.png[alt="enable Secondary Axis option"]

. Press the {buttonApply} to update the chart axes.
+
image::commonscript:SecondaryYAxis2.png[alt="chart updated with secondary Y-axis"]
...
image::dashboard:galleryDualAxis2.png[alt="accounts receivable dual axis chart example"]
