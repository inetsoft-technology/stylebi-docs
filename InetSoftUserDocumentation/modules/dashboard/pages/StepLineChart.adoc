...
// metadata for AI
ifdef::ai[]
Summary: Reference for creating a Step Line chart in Visual Composer, which is a LineChart variant that connects data points using only horizontal and vertical (staircase) directions. See JumpLineChart for a variant using only horizontal segments, and StepAreaChart for the filled staircase variant.
endif::ai[]
...
image::commonscript:stepChartExp1.png[alt="step line chart example"]
...
+
image::commonscript:stepChartExp2.png[alt="select step line chart style"]
...
+
image::commonscript:stepChartExpVS1.png[alt="drag dimension to X or Y region"]
+
...
+
image::commonscript:stepChartExpVS2.png[alt="drag dimension to color or shape region"]
...
+
image::commonscript:stepChartExpVS3.png[alt="drag measure to chart region"]
+
