...
The following plot options are available for xref:dashboard:BarChart.adoc[] (stacked and non-stacked) and xref:IntervalChart.adoc[] types.
...
Round All Corners:: When enabled, rounds all four corners of each bar equally using the ‘Bar Corner Radius’ value. When disabled (default), only the corners furthest from the axis are rounded. (This option is always *enabled* for xref:IntervalChart.adoc[], and is not shown for this Chart type. To control it, see xref:dashboardscript:PlotBarRoundAllCorners.adoc[] or xref:chartAPI:IntervalElementSetRoundAllCorners.adoc[].)

+
NOTE: The ‘Round All Corners’ option only appears once a value has been entered for ‘Bar Corner Radius’ (above).
