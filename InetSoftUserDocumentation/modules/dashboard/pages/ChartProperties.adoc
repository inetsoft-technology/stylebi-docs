...
// metadata for AI
ifdef::ai[]
Summary: Reference for Chart component properties (also called chart settings or chart configuration), accessed via right-click > Properties on the chart body. Covers the General tab (title, padding, alpha), Advanced tab (drill-down, tooltip, chart-specific settings), Line tab (line styles), Hierarchy tab (drill paths), and Script tab (Chart scripting). See BasicChartingSteps and ChartTypes for how to create and configure a Chart.
endif::ai[]
