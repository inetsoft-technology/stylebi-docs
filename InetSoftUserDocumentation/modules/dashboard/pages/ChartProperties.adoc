= Chart Properties
:page-aliases: viewsheet:ChartProperties.adoc
%experimental%
...
The following plot options are  available for xref:dashboard:LineChart.adoc[] and xref:RadarChart.adoc[] types.
...
The following plot options are  available for xref:dashboard:BarChart.adoc[]  types.
...
Round All Corners:: Specifies (when enabled) that the '`Bar Corner Radius`' should be applied to all four corners of the bar.  When disabled (default), only the corners furthest from the axis are rounded.
