...
// metadata for AI
ifdef::ai[]
Reference for Chart component properties in Visual Composer (also called chart settings or chart configuration), accessed via right-click > Properties on the chart body. Covers the General tab (tooltip, data tip view, flyover views), Advanced tab (chart options such as glossy effect, sparkline, ad hoc editing, date comparison, drilling, top-N; plot settings such as background, alpha, banding; and chart-type-specific plot options for Pie, Bar, Network/Tree, Circle Packing/Treemap, Word Cloud, Line/Area/Radar, Point, Map, Contour, and Interval charts; and target lines), Line tab (X/Y grid lines, quadrant grid, diagonal line, table grid, trend lines, and projection), Hierarchy tab (custom drill-down hierarchy configuration), and Script tab (Chart scripting). See AddAChart and BasicChartingSteps for how to create and configure a Chart.
endif::ai[]
...
* {commonProperties}
* xref:dashboardscript:Chart.adoc[]  in xref:dashboardscript:DashboardObjectReference.adoc[] to set these properties dynamically using script.
====
...
The following plot options are  available for xref:dashboard:PieChart.adoc[] types.
...
The following plot options are available for xref:dashboard:BarChart.adoc[] types (non-stacked).

Corner Radius:: A value between 0 and 0.5 that controls the rounding of bar corners. A value of `0` produces square corners; `0.5` produces the maximum rounding. Only the open (value) end of each bar is rounded unless '`Round All Corners`' is also enabled. Default is 0.3.
...
The following plot options are  available for xref:dashboard:NetworkChart.adoc[], xref:CircularNetworkChart.adoc[] and xref:TreeChart.adoc[] types.
...
Layout Direction:: For xref:TreeChart.adoc[] only, sets the direction in which the tree layout grows: ‘Top to Bottom’ (default), ‘Bottom to Top’, ‘Left to Right’, or ‘Right to Left’.

Node Corner Radius:: For xref:TreeChart.adoc[] only, a value between 0 and 0.5 that controls the rounding of node corners. A value of `0` produces square corners; `0.5` produces the maximum rounding. Default is 0.3.


=== Plot Options (Circle Packing/ Treemap)
The following plot option is  available for xref:dashboard:CirclePackingChart.adoc[] and xref:TreeChart.adoc[] types.


Include Parent Labels:: This includes the parent category names in the circle labels.

=== Plot Options (Word Cloud Chart)
The following plot option is available for xref:dashboard:WordcloudChart.adoc[] types.
...
The following plot options are  available for xref:dashboard:LineChart.adoc[], xref:dashboard:AreaChart.adoc[], and xref:RadarChart.adoc[] types.
...
The following plot options are  available for xref:dashboard:PointChart.adoc[] types.
...
The following plot options are  available for xref:dashboard:MapChart.adoc[]  types.
...
The following plot options are  available for xref:dashboard:ContourMapChart.adoc[] and xref:ScatterContourChart.adoc[] types.
...
The following plot options are  available for xref:dashboard:BarChart.adoc[] and xref:IntervalChart.adoc[] types.
