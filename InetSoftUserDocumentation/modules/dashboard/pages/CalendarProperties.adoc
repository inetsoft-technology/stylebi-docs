...
// metadata for AI
ifdef::ai[]
Reference for Calendar filter component properties in Visual Composer (also called a date picker or date filter). Covers date field selection (Data tab), display style (Calendar wall or Dropdown), View Mode (Simple View for a single calendar unit, or Range for two side-by-side calendars), Calendar Type (Yearly, Monthly, Day Selection, Single Selection), Min/Max date constraints, Submit on Change behavior, and the Compatible Tables setting for filtering multiple data blocks simultaneously. See AddFilterComponents for how to create a Calendar.
endif::ai[]
...
* {commonProperties}
* xref:dashboardscript:Calendar.adoc[]  in xref:dashboardscript:DashboardObjectReference.adoc[] to set these properties dynamically using script.
====
...
image::Calendar Properties Data Tab.png[alt="calendar properties data tab with date field selection"]
...
image::Capture_054.png[alt="calendar advanced tab with show as and view mode options"]
