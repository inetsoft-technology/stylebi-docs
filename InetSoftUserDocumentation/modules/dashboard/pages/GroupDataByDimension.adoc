...
// metadata for AI
ifdef::ai[]
Summary: Explains how to group chart data in Visual Composer by color, shape, size, or text labels, how to group dates by level (week/month/year/quarter), how to create named groups (also called custom groups or groups by category) by merging axis labels, and how to apply top-N/bottom-N ranking filters to chart dimensions. Also covers the 'Share Colors' option (in the Group by Color section) for keeping chart colors consistent/synchronized across multiple charts in a Dashboard, so that values from columns or data model attributes with the same name always get the same color, even across different data sources — this is the mechanism to use when charts show mismatched colors for what should be the same category, as opposed to xref:VisualFormat.adoc[] (Format panel), which sets a component's own visual appearance rather than synchronizing colors between charts. Also covers Size legend color: when no field is bound to Color, the Size legend's symbols simply use whatever static color is set for the chart (see xref:AddUniformDataStyle.adoc[]); when Color and Size are bound to different fields, the Size legend symbols are always a fixed gray and cannot be recolored at all. To instead apply one uniform color, shape, or size to all of a chart's marks (no grouping), see xref:AddUniformDataStyle.adoc[] (Add Uniform Data Style).
endif::ai[]
...
xref:AddUniformDataStyle.adoc#SetAStaticColor[Set a Static Color], to apply one uniform color to every mark on the chart (no grouping) instead.
...
+
image::VS Group Visual2.png[alt="drag dimension to color field in chart editor"]
+
...
+
image::VS Group Visual3.png[alt="press edit button next to color field"]

. *_Optional:_* Press the {buttonSelectPalette} to open the ‘Select Palette’ dialog box.
+
image::VS Group Visual4.png[alt="press select palette to open palette dialog"]
...
+
image::SelectPalette.png[alt="choose palette from select palette dialog"]
+
...
. Click on the color chips to create the desired color order. (Colors are assigned to values from left to right.) To reset the color order, press the {buttonResetToDefault} on the color menu.
+
image::VS Group Visual6.png[alt="click color chips to set color order"]
...
+
image::fixedColorMapping3.png[alt="enable share colors option to synchronize chart colors"]
+
--
...
+
image::fixedColorMapping1.png[alt="press assign fixed mapping button to open color mapping dialog"]

.. In the ‘Color Mapping’ dialog box, press the ‘Add Row’ btn:[+] button to add a new value, and click the color swatch to set a fixed color for that value. Press the ‘Delete Row’ btn:[-] button to remove a mapping.
+
image::fixedColorMapping2.png[alt="add rows and assign fixed colors in color mapping dialog"]
+
NOTE: Clicking the color swatch opens the same '`Select Color`' dialog box used in xref:AddUniformDataStyle.adoc#SetAStaticColor[Set a Static Color] — see that section for details on using the dialog's hue slider, saturation/brightness square, and HSV, RGB, and hex fields.
+
To assign a color to a dimension value that does not appear in the current data, enable the ‘Switch To Manual Input’ option, and type the value directly.
+
image::fixedColorMapping4.png[alt="manually enter a dimension value not included in the data"]

. Press btn:[OK].

. Press the {buttonApply} to apply the color setting.

To  apply one uniform color to every mark on the chart (no grouping), see xref:AddUniformDataStyle.adoc#SetAStaticColor[Set a Static Color].
...
====
[square]
* xref:MultipleMeasureChart.adoc#ColorShapeSize[Color, Shape, Size] in xref:MultipleMeasureChart.adoc[] to edit the shapes used for measures.
* xref:AddUniformDataStyle.adoc#SetAStaticShape[Set a Static Shape], to apply one uniform shape to every mark on the chart (no grouping) instead.
====
...
+
image::VS Group Visual7.png[alt="drag dimension to shape field in chart editor"]
+
...
+
image::VS Group Visual8.png[alt="shape panel opened for editing shape options"]
+
NOTE: The available shape options vary depending on the Chart type.

. Click on the shape chips to create the desired shape order. (Shapes are assigned to values from left to right.) To reset the shape order, press the {buttonResetToDefault} on the shape menu.
+
image::VS Group Visual9.png[alt="click shape chips to set shape order"]
. *_Optional:_* To upload a custom image to use as a shape, press the {buttonAddShape}, and select the desired file.
+
image::addShapeButton.png[alt="press add shape button to upload custom image"]
+
NOTE: An administrator can make custom shapes available by placing the image files (png, gif, jpg, svg) in the _portal/shapes_ directory. (See xref:administration:Presentation.adoc#LookandFeel[Look and Feel] and xref:administration:Storage.adoc[] for  information on how to upload a file to this location.) The icons are sorted alphabetically by file name. Custom shape images can be any size, and are scaled according to the ‘Size’ setting in the Chart Editor.


. Press the {buttonApply} to apply the shape setting.

To apply one uniform shape to every mark on the chart (no grouping), see xref:AddUniformDataStyle.adoc#SetAStaticShape[Set a Static Shape].

[#GroupbySize]
== Group by Size

[WARNING,caption=see also]
xref:AddUniformDataStyle.adoc#SetAStaticSize[Set a Static Size], to apply one uniform size to every mark on the chart (no grouping) instead.
...
+
image::VS Group Visual10.png[alt="drag dimension to size field in chart editor"]
+
...
+
image::Ad Hoc Group Visual11.png[alt="size panel with slider to set size range"]

. Press the {buttonApply} to apply the size setting.

To apply one uniform size to every mark on the chart (no grouping), see xref:AddUniformDataStyle.adoc#SetAStaticSize[Set a Static Size].

[#SizeLegendColor]
=== Size Legend Color

The color of the symbols in a Size legend depends on the  *Color* binding in the Chart Editor:

[square]
* If no field is bound in  the '`Color`' region, the Size legend symbols use whatever single static color is set for the Chart's data marks — see xref:AddUniformDataStyle.adoc#SetAStaticColor[Set a Static Color]. This is not a separate, legend-specific setting; the legend symbols simply match the same color as the marks.

* If the *same field* is bound in both the '`Color`' and '`Size`' regions, the Size legend symbols use the color mapping and palette assigned to that field. See xref:#GroupbyColor[] above.

* If  *different fields* are bound in  the '`Color`' and '`Size`' regions, the color of the Size legend symbols *cannot be changed* — the symbols always display in  the default gray color.
...
+
image::VS Group Visual12.png[alt="drag dimension to text field in chart editor"]
+
...
+
image::VS Group Visual13.png[alt="press edit text button to open format panel"]
...
+
image::viewsheetExp34.png[alt="set date grouping level for dimension in chart editor"]
+
include::ROOT:partial$dimensionTip.adoc[]
...
+
image::Named Group 1.png[alt="right-click selected labels and choose group items"]
+
...
+
image::Named Group 2_1.png[alt="enter group name in group name dialog box"]
+
...
+
image::Named Group 3.png[alt="chart updated with named group replacing original labels"]
...
+
image::Named Group Exp 1.png[alt="chart with state dimension on x-axis and total on y-axis"]
...
+
image::Named Group Exp 2.png[alt="right-click selected axis labels and choose group items"]
+
This opens the ‘Group Name’ dialog box.

. In the ‘Group Name’ dialog box, enter the label for the new composite group: “New England”.
+
image::Named Group Exp 3.png[alt="enter new england as group name in dialog"]
+
This adds ‘New England’ as a new X-axis label, and agglomerates the data from ‘MA’ and ‘CT’ into this new composite group.
+
image::Named Group Exp 4.png[alt="chart showing new england named group on x-axis"]
...
+
image::Named Group Exp 5.png[alt="chart showing multiple regional named groups on x-axis"]
...
+
image::Named Group Exp 6.png[alt="set alias for IL label in axis properties dialog"]
.. Press btn:[OK] to close the dialog box. The Chart now displays all states grouped by region.
+
image::Named Group Exp 7.png[alt="chart showing all states grouped by region"]
...
+
image::Named Group 4.png[alt="right-click group label and select ungroup items"]

[#SortRankChartGroups]
== Sort and Rank Chart Groups
[.videotitle]
...
+
image::topNFilter1.png[alt="press edit dimension button in chart editor"]
...
+
image::topNFilter2.png[alt="select top or bottom from ranking menu"]
. Enter the number of top or bottom groups to select, and choose the measure on which to rank the groups.
+
image::topNFilter3.png[alt="enter count and select ranking measure"]
...
+
image::topNFilter4.png[alt="select sort order for ranked groups"]
+
NOTE: The sort order set here also controls the order of items in the chart legend. See xref:LegendProperties.adoc#LegendOrder[Legend Order] for more details.
...
+
image::Manual Ordering Dialog.png[alt="reorder items in manual ordering dialog box"]
...
+
image::topNFilter5.png[alt="chart showing ranked top or bottom groups"]
...
image::dashboard:galleryDataTip2.png[alt="executive sales summary dashboard example"]
****
