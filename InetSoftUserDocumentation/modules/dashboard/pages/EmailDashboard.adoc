...
// metadata for AI
ifdef::ai[]
How to immediately email a Dashboard from the User Portal. Covers the Email tab (Mail To recipients, CC, From address, Subject, and Message body) and the Content tab (export format: Excel, PDF, PowerPoint, HTML, Embedded PNG, CSV; Data Size options: Match Layout, Expand Components, Only Data Elements; Current View toggle; Include Link option to embed a URL; and bookmarks to include as separate pages/sheets). For scheduling recurring emails, see user:ScheduleDashboard.
endif::ai[]
...
+
image::emailButton.png[alt="press Email button on toolbar"]
+
This opens the ‘Email’ dialog box.
+
image::emailDialogBox.png[alt="email dialog box opens"]
...
+
image::SelectEmailsDialogBox.png[alt="select emails dialog to choose recipients"]
+
...
Excel:: {ExportExcel}
+
image::Viewsheet Export Content Tab.png[alt="choose export type in content tab"]
