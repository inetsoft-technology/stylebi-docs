...
+
NOTE: If you want to assemble data in a manner typical of a *join* (combining different tables by relating rows), you should do this in a Data Worksheet (see xref:dataworksheet:JoinData.adoc[]) rather than a Freehand Table. After you join the desired datasets, you can  add custom calculations as Expression Columns in the Data Worksheet or  Calculated Fields in the Dashboard (see xref:dataworksheet:CreateNewDataField.adoc[]) and display the result in a regular Table or Crosstab.
