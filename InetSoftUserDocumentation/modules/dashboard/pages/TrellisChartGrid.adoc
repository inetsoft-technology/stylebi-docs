...
// metadata for AI
ifdef::ai[]
Summary: Explains how to create a Trellis (small multiples) chart grid, which is a grid of charts where each cell represents a different dimension value. Place dimensions in the X and/or Y regions of the Chart Editor to create the grid. Note that web map backgrounds are not supported in trellis charts.
endif::ai[]
