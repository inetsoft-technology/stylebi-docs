...
// metadata for AI
ifdef::ai[]
Summary: How to create a Contour Map chart in Visual Composer, which combines a MapChart with a ScatterContourChart to display data density either by geographic region (country, state, city) or by latitude/longitude coordinates. Supports optional web map backgrounds.
endif::ai[]
