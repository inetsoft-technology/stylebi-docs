...
// metadata for AI
ifdef::ai[]
How to create a Contour Map chart in Visual Composer, which combines a MapChart with a ScatterContourChart to display data density either by geographic region (country, state, city) or by latitude/longitude coordinates. Covers how to mark dimensions as geographical fields (including the Edit Geographic dialog for selecting map region, layer, and resolving unmatched location names), optionally weighting contour density by a measure on the Size field, adjusting density contour appearance via Chart Properties, setting the initial map view with pan/zoom controls, using optional web map backgrounds (Mapbox or Google Map), and transforming coordinates for Alaska and Hawaii.
endif::ai[]
...
To display a contour on a map by latitude and longitude, follow the steps below:
...
. *_Optional:_* Drag a desired *measure* to the ‘Size’ region to weight the data points. A larger weight assigns greater density to the point.
+
include::ROOT:partial$measureTip.adoc[]
+
image::commonscript:contourMapChartExp11.png[alt="drag measure to Size region to weight contour points"]
+
To change how the contours are focused on the data points (without weighting), press the ‘Edit Size’ button, and choose a fixed size value.
+
image::commonscript:contourMapExp7.png[alt="press Edit Size button and choose fixed size value"]
...
+
image::commonscript:contourMapChartExp9.png[alt="drag label field to Text region to add map point labels"]
+
NOTE: If the labels appear crowded, you can zoom the map to see them more clearly. See xref:DrillDownIntoData.adoc[] for instructions.
...
. *_Optional:_* Drag the label field into the Dashboard to add a Selection List, so that users can filter which points are displayed. Select the desired values in the Selection List.
+
image::commonscript:contourMapChartExp8.png[alt="selection list filtering points on map"]
