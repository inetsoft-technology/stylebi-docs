...
// metadata for AI
ifdef::ai[]
Summary: How to add text labels, titles, descriptions, and HTML content to a Dashboard in Visual Composer. Text components support UTF-8 characters and HTML markup, and can be populated statically or dynamically via script or expressions. Also covers displaying HTML using the Basic HTML presenter or by wrapping content in html tags, embedding multimedia such as iframes and videos, and displaying aggregated measures as text. A Text component is also the way to add a descriptive label/title next to components with no built-in title, such as a Range Slider or Gauge.
endif::ai[]
...
You can add text, such as titles or descriptions, to any region of a Dashboard.   Text is also commonly used to add  descriptive labels or titles for components that have no built-in title, such as Range Slider (see xref:RangeSliderProperties.adoc[]) and  Gauge (see xref:GaugeProperties.adoc[]). Text can contain UTF-8 (unicode) characters as well as HTML markup.

TIP: To assemble the text string using script in the *Script* tab, see the xref:dashboardscript:TextText.adoc[] property. To assemble the text string using Expression script on the '`Text`' property in the '`Text Properties`' dialog box, see xref:DynamicallySetProperties.adoc#ControlPropertieswithExpression[Control Properties with Expression].
