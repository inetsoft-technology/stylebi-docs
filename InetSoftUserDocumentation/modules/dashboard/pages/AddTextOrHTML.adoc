...
TIP: A Text component is also commonly used to add a descriptive label or title next to a component that has no built-in title of its own, such as a Range Slider (see xref:RangeSliderProperties.adoc[]) or a Gauge (see xref:GaugeProperties.adoc[]). Simply position a Text component near the component to serve as its caption.
