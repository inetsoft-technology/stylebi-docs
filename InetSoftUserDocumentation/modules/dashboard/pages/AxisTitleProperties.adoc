...
// metadata for AI
ifdef::ai[]
Reference for editing and formatting chart axis titles in Visual Composer, including custom title text, placeholders for dynamic field names (e.g., {0} for the default axis name), font/color/alignment formatting, and how to hide or show axis titles. Note that custom title text persists even if the field displayed on the axis changes. See AxisProperties for axis label properties.
endif::ai[]
...
* xref:dashboardscript:AxisTitleProperties.adoc[]  in xref:dashboardscript:DashboardObjectReference.adoc[] to set these properties dynamically using script.
====
...
image::Chart Title Format Dialog.png[alt="title properties panel with title field"]
...
image::Format Title.png[alt="format tab selected to style axis title"]
