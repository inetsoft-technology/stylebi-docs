...
// metadata for AI
ifdef::ai[]
Summary: Explains how to configure Embedded Tables in Visual Composer dashboards to be editable by users, allowing them to add, delete, or modify rows. Covers setting column-level edit options (Boolean, ComboBox, Date, Float, Integer, Password, and Text editors with validation), hiding columns, and accessing user-modified data via the getFormRows and getFormRow script functions (typically called from a Submit Button's onClick handler). Also covers using the onRefresh Dashboard script handler and the event object to detect and respond to user edits dynamically, including a worked example that auto-calculates a Total column based on user-edited Price and Quantity values.
endif::ai[]
...
+
image::dashboard:Column Option Menu.png[alt="right-click column header to open column options"]
...
+
image::dashboard:Column Editor Options.png[alt="select column editor type in column option dialog"]
...
To retrieve the value of a hidden column, use the script function xref:dashboardscript:TableGetHiddenColumnValue.adoc[]. See xref:#AccessUserModifiedData[] for more details.
...
image::dashboard:Column Boolean Editor.png[alt="boolean editor checkbox for true or false input"]
...
image::dashboard:Column ComboBox Editor.png[alt="combobox editor dropdown for selecting values"]
...
+
image::dashboardscript:Radio embedded.png[alt="add embedded list values with label and value fields"]
...
+
image::dashboardscript:Radio query.png[alt="select query data block and label value fields"]
...
image::dashboard:Column Date Editor.png[alt="date editor with minimum maximum and error message"]
...
image::dashboard:Column Float Editor.png[alt="float editor with minimum maximum and error message"]
...
image::dashboardscript:Column Number Editor.png[alt="integer editor with minimum maximum and error message"]
...
image::dashboardscript:Column Text Editor.png[alt="text editor with pattern check and error message"]

To mask the entered text, use the '`Password Editor`' option instead of the ‘Text Editor’ option.

image::dashboardscript:Column Password Editor.png[alt="password editor masks entered text with asterisks"]
...
These functions are typically called within the onClick handler of a Submit Button (see xref:SubmitButtonProperties.adoc[]).  See xref:dashboardscript:AddScriptToDashboard.adoc#AddComponentScript[Add Component Script] for information about the onClick handler.


When a user adds or changes a row in an editable table, the new or modified row is highlighted in color. To clear the highlight on these rows and remove them from the corresponding array returned by `getFormRows`, use the table’s xref:dashboardscript:TableCommit.adoc[] function.
...
When a user makes a modification to an Editable Table or Form component, the change triggers the Dashboard’s onRefresh Handler script to re-execute. (See xref:dashboardscript:AddScriptToDashboard.adoc#AddDashboardScript[Add Dashboard Script] for more information about this handler.) The onRefresh handler offers an xref:dashboardscript:SpecialFunctionsEvent.adoc[] object that  you can use to detect the user’s action and make an appropriate response. The event object provides access to the following Dashboard event:
...
+
image::dashboardscript:UpdateEditableTable1.png[alt="editable table with name price quantity and total columns"]
