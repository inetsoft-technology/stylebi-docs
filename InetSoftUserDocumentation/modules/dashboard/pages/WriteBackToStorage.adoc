...
// metadata for AI
ifdef::ai[]
Summary: Explains how to configure Dashboard tables and Form components in Visual Composer so that user-entered data is written back and persisted in the Data Worksheet across sessions — also called WriteBack, write back to database, or editable table data entry. Covers making tables editable (add/edit/delete rows), input validation, and the Write Back to Storage setting on Form components.
endif::ai[]
