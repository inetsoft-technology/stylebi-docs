...
// metadata for AI
ifdef::ai[]
Step-by-step instructions for adding a Chart to a Dashboard in Visual Composer, including understanding dimensions vs. measures, binding data to chart axes (X/Y regions), selecting a chart type, using color/shape/size/text regions for visual encoding, setting aggregation methods, and date grouping. The 'X' and 'Y' regions accept multiple fields, but each of the 'Color', 'Shape', and 'Size' regions holds only a single field at a time — dragging a new field into one of these regions replaces the field already there rather than combining with it. The 'Text' region also normally holds a single field, but can instead be composed of multiple fields, literal text, and spacing using the Text Layout Designer — see TextLayoutDesigner. Also covers post-creation adjustments: hiding or formatting chart axes (Axis Properties), adding a chart title, modifying chart legends (Legend Properties dialog — General, Scale, and Alias tabs, including renaming legend labels), and adjusting chart spacing (Resize Plot). This is the foundational chart creation page — also called chart setup or chart binding — read this before consulting chart-type-specific pages such as BarChart or LineChart.
endif::ai[]
...
image::addingChart1.png[alt="chart dataset columns classified as dimensions or measures"]
...
include::partial$availableToEndUsers.adoc[]
See xref:EndUserChartControl.adoc[] for information about how to make the Ad Hoc Chart Editor available to end users. In the Ad Hoc Chart Editor, press the ‘`Enable Manual Refresh`’ button in the mini-toolbar to update the chart only when the {buttonRefresh} button is pressed.  Press the ‘`Enable Auto Refresh`’ button to update the Chart when any changes are made in the Editor.
...
+
image::viewsheetExp37.png[alt="press Select Chart Style and choose a style"]
+
...
+
image::addingChart2a.png[alt="drag dimension from data source to X region"]
+
...
+
*Optional:* You can drag  multiple  dimensions to the ‘X’ region. Dimensions will be represented automatically  against  the *top* or *bottom* X-axis depending on the order that  you place them in the ‘X’ region.  You can change that order by dragging the fields within the '`X`' region.
+
TIP: If the Chart appears to be missing rows or values you expect,  check whether the underlying data block has a '`Max Rows`' limit set (see xref:dataworksheet:DataBlockProperties.adoc[]).


. To rotate the Chart (interchange the X and Y axes), press the {buttonSwapXY}.  You can also do this by interchanging the fields in the '`X`' region and  '`Y`' region of the Editor.
...
=====
+
image::addingChart3.png[alt="drag dimension to Color, Shape, Size, or Text region"]
+
NOTE: Unlike the ‘X’ and ‘Y’ regions (which accept multiple fields), each of the ‘Color’, ‘Shape’, and ‘Size’ regions holds only a single field at a time. Dragging a new field into one of these regions replaces the current field. The ‘Text’ region also normally holds a single field, but you can instead compose it from multiple fields, literal text, and spacing using the Text Layout Designer. See xref:TextLayoutDesigner.adoc[].
+
...
=====
+
image::EditColor.png[alt="press Edit Color to change dimension color"]


. *_Optional:_* To upload a custom image to use as a shape, press the {buttonAddShape}, and select the desired file (jpg, png, or svg).
+
image::addShapeButton.png[alt="press Add Shape and select desired file"]
+
...
=====
+
image::viewsheetExp34.png[alt="press Edit Dimension and set date grouping level"]
+
...
+
image::addingChart4.png[alt="drag measure from data source to Y region"]
+
...
+
image::addingChart5.png[alt="drag measure to Color, Shape, Size, or Text region"]
+
...
=====
+
image::EditColor2.png[alt="press Edit Color to change measure color"]
...
====
+
image::Chart Edit Y Axis_1.png[alt="press Edit Measure and select aggregation method"]
+
...
+
image::Chart Add Title2.png[alt="right-click chart and select Properties"]

. Under the *General* tab, enter the desired text for the 'Title' property.
. Select the ‘Visible’ option next to the 'Title' property.
+
image::Chart Add Title.png[alt="enter title text and select Visible option"]
...
+
image::Chart Add Title3.png[alt="click title area and select Format tab"]

. To increase the vertical space available for the title, drag down the handle under the title text.
+
image::Chart Add Title4.png[alt="drag handle to increase vertical space for title"]
...
image::Chart Legend Format Dialog.png[alt="right-click legend and select Legend Properties"]
...
+
image::Chart Legend Move.png[alt="drag legend to change legend position"]
...
+
image::legendFormatAlias.png[alt="double-click alias column cells to rename legend labels"]

. Press btn:[OK] to close the panel. This updates the legend labels with the specified replacement text.
+
image::legendFormatAliasUpdated.png[alt="legend updated with new alias labels"]
...
+
image::Change Chart Spacing.png[alt="right-click chart and select Resize Plot"]
...
+
image::Change Chart Spacing2.png[alt="drag slider control to change element spacing"]
...
image::dashboard:galleryMultipleMeasure3.png[alt="census dashboard with color and fill chart"]
