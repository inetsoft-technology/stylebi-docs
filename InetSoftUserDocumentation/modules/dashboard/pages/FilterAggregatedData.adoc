...
// metadata for AI
ifdef::ai[]
How to filter data after it has been grouped and aggregated in Visual Composer, applied to a Chart, Crosstab, or Freehand Table. Unlike Selection Lists and Conditions which filter pre-aggregate detail data, this page covers two post-aggregate filtering methods: (1) configuring a Range Slider to target an aggregated field via its Data tab 'Components' node, and (2) using ad hoc filtering by right-clicking a column (in a Crosstab or Freehand Table) or a dimension/measure label (in a Chart) and selecting 'Filter'. Note: the right-click 'Filter' option is only available if the Dashboard contains a Selection Container component; filter selections appear in that container and can be cleared individually or all at once.

Important behavioral distinction for ad hoc chart filters: filtering by a *dimension* (categorical/string field) targets the underlying data source and filters ALL Dashboard components bound to that data source — same as a regular Selection List. Filtering by a *measure* (aggregated value such as Sum or Count) targets only that chart's post-aggregate output; other Dashboard components are NOT affected.
endif::ai[]
...
+
image::Range Slider_1.png[alt="drag range slider element onto dashboard"]

. Right-click on the Range Slider and select ‘Properties’ from the context menu. This opens the ‘Range Slider Properties’ panel.

. Select the *Data* tab. Expand the ‘Components’ node, and select the component and aggregated field that you want to filter.
+
image::Range Slider Assemblies.png[alt="select assemblies node in range slider data tab"]
...
[NOTE]
====
The scope of an ad hoc filter depends on whether you filter  a *dimension* or a *measure*:

[square]
* *Filter a dimension* (categorical field) —  filter is applied to the underlying data source. All Dashboard components bound to the same data source are filtered.
* *Filter a measure* (aggregated  value) —  filter is applied only to post-aggregate data in the Chart, Crosstab, or Freehand Table where the filter is applied. Other components on the Dashboard are not affected.
====
...
+
image::filtering1.png[alt="right-click column and select filter from context menu"]
+
...
+
image::filtering2_1.png[alt="filter control displayed for selected column"]
...
+
image::filtering3_1.png[alt="filter selections shown in selection container"]

. To remove the filter from a particular column, follow the steps above, and press the {buttonUnselect} on the Filter control.
+
image::filtering5_1.png[alt="press unselect button to remove column filter"]

. To remove all filters on the table, press the {buttonUnselectAll} in the Selection Container.
+
image::filtering4_1.png[alt="press unselect all in selection container"]
...
+
image::Chart Filter1.png[alt="right-click chart dimension and select filter"]
+
...
+
image::Chart Filter2.png[alt="filter control for selected chart dimension"]
...
+
image::Chart Filter3.png[alt="chart filter selection shown in selection container"]

. To remove the filter from a particular field, follow the steps above, and press the {buttonUnselect} on the Filter control.
+
image::Chart Filter4.png[alt="press unselect button to remove chart filter"]

. To remove all filters on the chart, press the {buttonUnselectAll}.
+
image::Chart Filter5.png[alt="press unselect all to remove all chart filters"]