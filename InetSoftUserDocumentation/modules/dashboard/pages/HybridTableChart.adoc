...
// metadata for AI
ifdef::ai[]
Summary: Reference for creating a Hybrid Table chart in Visual Composer, which combines a graphical chart (e.g., bar chart) with tabular text columns in the same component. Uses the Discrete option on measures to render them as text columns rather than chart elements.
endif::ai[]
