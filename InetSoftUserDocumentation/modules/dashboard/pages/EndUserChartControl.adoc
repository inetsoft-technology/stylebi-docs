...
// metadata for AI
ifdef::ai[]
How to make a chart interactive for end users, beyond the built-in toolbar tools (see UseCharts.adoc, DrillDownIntoData.adoc), by allowing them to modify charts at runtime using two approaches: the Ad Hoc Chart Editor (enabled via 'Enable Ad Hoc Editing' on the Advanced tab of Chart Properties, gives users full access to the Chart Editor) or dynamic properties (linking Form components such as RadioButton or ComboBox — also called a dropdown or dropdown menu — to data-binding properties — axis fields, aggregation methods, etc. — using the 'Variable' or 'Expression' option in Change Value Type). The dynamic properties walkthrough demonstrates letting a user switch the Y-axis field (i.e., which measure is displayed) via a RadioButton; a ComboBox/dropdown works the same way — NO script required for this. IMPORTANT: The Change Value Type / Variable binding approach works for data-binding properties such as axis fields and aggregation, but NOT for chart style (chart type). To let users switch chart type at runtime, use a Form control (e.g., RadioButton or ComboBox) whose selected value is read in the chart's Script tab to set Chart1.chartStyle. See DynamicallySetProperties for full details on dynamic property configuration.
endif::ai[]
...
+
image::enableAdHocEditing.png[alt="select enable ad hoc editing option"]
...
+
image::Crosstab Select Variable.png[alt="press Change Value Type and choose Variable option"]
...
+
image::Capture_006.png[alt="chart displays y-axis field from radio button selection"]
****
