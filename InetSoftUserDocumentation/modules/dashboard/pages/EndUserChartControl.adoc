...
// metadata for AI
ifdef::ai[]
How to allow end users to modify charts at runtime using two approaches: the Ad Hoc Chart Editor (enabled via 'Enable Ad Hoc Editing' on the Advanced tab of Chart Properties, gives users full access to the Chart Editor) or dynamic properties (linking Form components such as RadioButton to chart fields using the 'Variable' or 'Expression' option in Change Value Type). The dynamic properties walkthrough demonstrates letting a user switch the Y-axis field via a RadioButton. See DynamicallySetProperties for full details on dynamic property configuration.
endif::ai[]
