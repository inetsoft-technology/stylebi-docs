...
// metadata for AI
ifdef::ai[]
Summary: How to create a Radar (spider/web) chart in Visual Composer, which visualizes multiple measures as axes or a single measure broken out by dimension values. Covers both the Multiple Measures and Single Measure binding approaches, setting aggregate and Trend and Comparison methods (e.g., Percent of Grand Total), and renaming axis labels via Axis Properties aliases. See ChartProperties for the Show Points option. By default, a Radar chart displays circular concentric grid rings; there is no GUI option to change this. To display a polygon ("spider-web") grid instead, use xref:chartAPI:PolarCoordSetWebGrid.adoc[] in script.
endif::ai[]
...
image::dashboard:galleryBar2.png[alt="Auto Insurance Overview radar chart dashboard sample"]
