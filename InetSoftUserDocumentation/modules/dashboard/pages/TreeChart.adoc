...
// metadata for AI
ifdef::ai[]
Summary: Reference for creating a Tree chart in Visual Composer, which visualizes organizational hierarchies or parent-child relationships as a tree-shaped diagram with nodes and branches. Requires Source and Target dimension fields defining parent-child pairs.
endif::ai[]

A tree chart displays an organizational hierarchy defined by parent-child relationships.

image::commonscript:treeChart1.png[alt="tree chart hierarchy example"]
...
image::commonscript:treeChartVS2.png[alt="parent-child data set for tree chart"]
...
+
image::commonscript:treeChartVS1.png[alt="select tree chart style"]
+
By default, the tree grows from top to bottom. To change this, set ‘Layout Direction’ (Top to Bottom, Bottom to Top, Left to Right, or Right to Left) in the ‘Chart Properties’ dialog box. See xref:ChartProperties.adoc[] for more information.
...
+
image::commonscript:treeChartVS3.png[alt="drag parent dimension to source region"]
+
...
+
image::commonscript:treeChartVS4.png[alt="drag child dimension to target region"]
...
+
image::commonscript:treeChartVS5.png[alt="drag dimension to X or Y to break out diagram"]
...
+
image::commonscript:treeChartVS6.png[alt="drag dimension to color or size region"]
+
...
+
image::commonscript:treeChartVS7.png[alt="drag measure to node color size or text region"]


. Press the {buttonEditMeasure} next to the measure, and select the desired aggregation method for the measure.
+
image::Chart Edit Y Axis_1.png[alt="select aggregation method for measure"]
...
+
image::commonscript:treeChartVS8.png[alt="set fixed size or color for nodes"]
...
+
image::commonscript:treeChartVS9.png[alt="set fixed size or color for lines"]
