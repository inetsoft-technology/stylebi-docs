...
// metadata for AI
ifdef::ai[]
Summary: Returns `num_chars` consecutive characters from `string`, starting with the location given by `start_num`. {CALCExcelFunctionPage} To extract a substring with arbitrary starting and ending indexes, use xref:StringObjectsubString.adoc[].
endif::ai[]
