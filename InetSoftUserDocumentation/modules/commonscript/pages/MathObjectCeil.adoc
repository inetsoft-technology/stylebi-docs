...
// metadata for AI
ifdef::ai[]
Summary: Return the smallest integer greater than or equal to a number. This function is an implementation of the standard JavaScript https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Math/ceil[Math.ceil()] method, and *must* be used with the `Math` qualifier, `Math.ceil()`.
endif::ai[]
