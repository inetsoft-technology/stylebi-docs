...
// metadata for AI
ifdef::ai[]
Summary: Returns the depreciation of an asset for accounting year `period` on a prorated basis according to the *linear* depreciation method of the French accounting system. {CALCExcelFunctionPage} This function is the same as xref:CALC_amordegrc.adoc[] except in calculation of the depreciation coefficients.
endif::ai[]
