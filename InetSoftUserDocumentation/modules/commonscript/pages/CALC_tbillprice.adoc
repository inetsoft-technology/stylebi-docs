...
// metadata for AI
ifdef::ai[]
Summary: Returns the issue *price* per $100 face value for a Treasury bill using the 360-day year convention. The `settlement` and `maturity` dates must be within one calendar year of each other. {CALCExcelFunctionPage}
endif::ai[]
