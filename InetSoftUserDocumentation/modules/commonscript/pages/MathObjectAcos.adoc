...
// metadata for AI
ifdef::ai[]
Summary: Return the inverse cosine (in radians) of a number between 0 and 1. This function is an implementation of the standard JavaScript https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Math/acos[Math.acos()] method, and can be used with or without the `Math` qualifier, `acos()` or `Math.acos()`.
endif::ai[]
