...
// metadata for AI
ifdef::ai[]
Summary: Remove duplicate entries from input array to yield a list of unique elements with optional sorting and grouping. See xref:viewsheetscript:SpecialFreehandFunctions.adoc#toList[toList] in xref:viewsheetscript:SpecialFreehandFunctions.adoc[] for application to Freehand Table.
endif::ai[]
