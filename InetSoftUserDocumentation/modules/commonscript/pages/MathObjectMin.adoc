...
// metadata for AI
ifdef::ai[]
Summary: Return the minimum of the specified values. This function is an implementation of the standard JavaScript https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Math/min[Math.min()] method, and *must* be used with the `Math` qualifier, `Math.min()`.
endif::ai[]
