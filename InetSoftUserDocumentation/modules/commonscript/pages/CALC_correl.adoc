...
// metadata for AI
ifdef::ai[]
Summary: Documents the `CALC.correl(array1, array2)` function, which returns the correlation coefficient (a number from -1 to +1) of the values in two arrays of real numbers. A value of +1 indicates a perfect positive linear relationship, -1 a perfect negative linear relationship, and 0 no linear relationship. The result is comparable to that of `CALC.pearson()`. Available in both Dashboard script and Data Worksheet script.
endif::ai[]
