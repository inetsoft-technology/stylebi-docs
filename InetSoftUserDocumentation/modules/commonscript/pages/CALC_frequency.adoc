...
// metadata for AI
ifdef::ai[]
Summary: Returns an array representing the frequencies with which values in `data_array` occur within the ranges specified by `bins_array`. For example, if `bins_array` specifies four values, `[value1, value2, value3, value4]`, this groups the values in `data_array` into the following five ranges:
endif::ai[]
