...
// metadata for AI
ifdef::ai[]
Summary: Return the natural logarithm (base stem:[e]) of a number. This function is an implementation of the standard JavaScript https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Math/log[Math.log()] method, and *must* be used with the `Math` qualifier, `Math.log()`.
endif::ai[]
