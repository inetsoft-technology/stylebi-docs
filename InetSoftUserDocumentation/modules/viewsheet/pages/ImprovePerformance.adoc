...
// metadata for AI
ifdef::ai[]
Summary: How to improve Dashboard performance using materialized views, which pre-aggregate and cache data so the Dashboard avoids querying the database on every load. Covers creating, scheduling, and managing materialized views from Visual Composer. When a VPM is in use, materialized views must be managed from Enterprise Manager instead.
endif::ai[]
...
IMPORTANT: If a VPM is in use, materialized views can only be created in Enterprise Manager.  See xref:administration:MaterializedView.adoc[] in xref:administration:ManageServer.adoc[] for more details.
...
+
image::Materialized View Portal1.png[alt="right-click dashboard and select Materialize"]

. Press the btn:[Analyze] button in the ‘Materialization’ dialog box.
+
image::Materialized View Portal2.png[alt="press Analyze button in materialization dialog"]
+
This lists the assets that can be materialized.
+
image::Materialized View Portal3.png[alt="assets listed for materialized view creation"]
