...
// metadata for AI
ifdef::ai[]
Summary: Reference for creating a Point chart, which displays a measure as a collection of points and is the default chart style for numerical dimensions. See BubbleChart to encode a second measure as point size, ScatterChart to place measures on both axes, or DotPlotChart to show individual un-aggregated values.
endif::ai[]
