...
// metadata for AI
ifdef::ai[]
Summary: Reference for creating a Radar (spider/web) chart, which visualizes a single measure across multiple dimensions or multiple measures along a single dimension. See ChartProperties for the Show Points option to display data point markers on radar chart lines.
endif::ai[]
