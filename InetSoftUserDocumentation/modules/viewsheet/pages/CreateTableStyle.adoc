...
// metadata for AI
ifdef::ai[]
Summary: How to create custom reusable Table Styles that can be applied to Tables and Crosstabs across multiple Dashboards. Covers defining cell formatting for each region of the table (header, body, total rows, etc.) in the Table Style Editor. Use when the built-in predefined styles are insufficient.
endif::ai[]
