...
ifdef::ai[]
Summary:
Visual Format defines presentation-level formatting rules for controlling the visual appearance of displayed values and elements in dashboad.
It supports consistent styling, number formats, and conditional visual rules across different visual levels, including detailed and aggregated results, without affecting data access or permissions.
endif::ai[]
