...
// metadata for AI
ifdef::ai[]
Summary: Explains how to build composite Dashboards by nesting smaller Dashboard modules within a parent Dashboard, and how to synchronize filter and form components across nested Dashboards using shared filter IDs. Covers reusability, maintainability, and team development benefits.
endif::ai[]
...
image::viewsheet:Embedded Viewsheet Button.png[alt="click dashboard icon to open embedded dashboard"]
...
* If you want a Form component on the *parent* Dashboard to control a Form component on a *child* Dashboard, but not the reverse (i.e., child Dashboard setting *does not* influence parent Dashboard), add  xref:viewsheetscript:setInputSelectedObject.adoc[] to the *Script* tab in the embedded '`Dashboard Properties`' dialog box.
====
...
+
image::viewsheet:Options Button_1.png[alt="press Options button to open dashboard options"]
...
+
image::viewsheet:Deploy shared filters.png[alt="drag component into shared filters list"]
