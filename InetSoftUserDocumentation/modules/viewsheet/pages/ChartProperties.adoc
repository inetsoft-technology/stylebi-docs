...
Smooth Lines:: _(xref:CircularNetworkChart.adoc[] only)_ When enabled, draws chart edges as smooth Bezier curves instead of straight lines. New Circular Network charts have this option enabled by default.
...
The following plot options are  available for xref:viewsheet:LineChart.adoc[] and xref:RadarChart.adoc[] types.

Show Points:: Adds markers at the locations of the individual data points.

Smooth Lines:: _(xref:LineChart.adoc[], xref:AreaChart.adoc[], Stacked Line, and Stacked Area charts only)_ When enabled, renders chart lines as smooth curves (Catmull-Rom interpolation) instead of straight segments. Enabled by default for Area charts; disabled by default for Line charts.
