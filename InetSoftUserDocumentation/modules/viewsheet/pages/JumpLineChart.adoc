...
// metadata for AI
ifdef::ai[]
Summary: Reference for creating a Jump Line chart, which is a StepLineChart variant that uses only horizontal line segments to connect data points, with no vertical connections between levels. Compare to StepLineChart (includes vertical connections) and LineChart (diagonal connections).
endif::ai[]
