...
[%collapsible]
=====
.Drill down with hyperlinks...
video::306732112[vimeo,640,400]
...
////
ifdef::testdrive[]
NOTE: This data source is not available in Test Drive.
endif::[]
////
...
. *_Optional:_*  Enable ‘Send Dashboard parameters’ to pass all existing Dashboard parameters to the target  Dashboard. The Dashboard parameter names must exactly match those defined in the target  Dashboard. See xref:viewsheetscript:parameter.adoc[] in xref:viewsheetscript:AdvancedDashboardScripting.adoc[] for more information about Dashboard parameters.
...
NOTE: This is different than the operations described in xref:#DrillThroughChart[] and xref:#DrillThroughCrosstab[] above, which drill to the next level of hierarchy but *do not* filter.
