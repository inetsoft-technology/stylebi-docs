...
// metadata for AI
ifdef::ai[]
Summary: Reference for creating a Stock (OHLC) chart, which displays four measures (low, high, open, close) against a date dimension for financial trading data. Functionally the same as CandleChart but with a different visual style (tick marks vs. filled body).
endif::ai[]
