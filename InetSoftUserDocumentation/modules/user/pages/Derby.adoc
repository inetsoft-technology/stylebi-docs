...
// metadata for AI
ifdef::ai[]
Summary: Instructions for connecting to Apache Derby as a JDBC data source in InetSoft. Requires a third-party JDBC driver (plugin driver required). Covers specifying the driver class name, the JDBC URL in the form jdbc:derby://{host}[:{port}]/databaseName, and entering login credentials. See ConnectToDataSource for the full list of available connectors.
endif::ai[]
...
+
image::dataSourceNewDerby2.png[alt="Derby data source definition screen"]
...
+
image::dataSourceDerby2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceDerby3.png[alt="connection error icon"]
+
