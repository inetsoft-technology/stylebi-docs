...
// metadata for AI
ifdef::ai[]
Summary: How to connect to Ingres as a data source in InetSoft Style Intelligence using a JDBC driver (plugin driver required). You must supply a Driver Class name and a JDBC URL — either jdbc:ingres://{host}:{port}/{databaseName} or jdbc:edbc://{host}[:{port}]/{databaseName} depending on the Ingres version — plus login credentials. After connecting, proceed to create a data set.
endif::ai[]
...
+
image::dataSourceNewIngres2.png[alt="Ingres data source definition screen"]
...
+
image::dataSourceIngres2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceIngres3.png[alt="connection error icon"]
+
