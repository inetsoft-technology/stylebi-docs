...
// metadata for AI
ifdef::ai[]
Summary: How to connect to Google Spreadsheets (Google Sheets) as a data source in InetSoft Style Intelligence. Authentication uses the Sign in with Google OAuth flow. Optional Connect Timeout and Read Timeout settings are available. An API key is required for open-source deployments. After connecting, proceed to define a data set using QueryGoogleSpreadsheets.
endif::ai[]
...
+
image::dataSourceGoogleSheets2.png[alt="Google Spreadsheets data source definition screen"]
...
+
image::dataSourceGoogleSheets3.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceGoogleSheets4.png[alt="connection error icon"]
+
