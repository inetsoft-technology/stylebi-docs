...
// metadata for AI
ifdef::ai[]
Summary: How to connect to MemSQL (SingleStore) as a data source in InetSoft using the MySQL database type. Supports Custom URL or host/port/database name entry, and login credentials. Requires uploading a JDBC driver plugin and supports HikariCP connection pool configuration.
endif::ai[]
...
+
image::dataSourceNewMemSQL2.png[alt="MemSQL data source definition screen"]
...
+
image::dataSourceMemSQL2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceMemSQL3.png[alt="connection error icon"]
+
