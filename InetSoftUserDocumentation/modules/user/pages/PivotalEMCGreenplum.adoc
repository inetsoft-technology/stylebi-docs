...
// metadata for AI
ifdef::ai[]
Summary: How to connect to Pivotal (EMC) Greenplum as a data source in InetSoft using a JDBC driver. Covers selecting the Custom database type, entering the Driver Class, and using the JDBC URL format `jdbc:pivotal:greenplum://{hostname}:{port};DatabaseName={databaseName}`.
endif::ai[]
include::partial$dataSourceUserIntro.adoc[]


. Select the ‘Pivotal Greenplum’ data source, and press btn:[Create]. This opens the data source definition screen.
+
image::dataSourceNewPivotalGreenplum2.png[alt="Pivotal (EMC) Greenplum data source definition screen"]
...
+
 jdbc:pivotal:greenplum://{hostname}:{port};DatabaseName={databaseName}
...
+
image::dataSourcePivotalGreenplum2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourcePivotalGreenplum3.png[alt="connection error icon"]
+
