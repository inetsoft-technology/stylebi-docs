...
// metadata for AI
ifdef::ai[]
Summary: Instructions for connecting to Exasol as a JDBC data source in InetSoft. Requires a plugin driver. Steps include selecting the Exasol data source, setting the Database Type to Custom, entering the driver class name (provided by the driver vendor), and specifying the JDBC URL in the form jdbc:exa:{hostname}:{port}. Also covers entering login credentials and verifying the connection. See ConnectToDataSource for the full list of available connectors.
endif::ai[]
include::partial$dataSourceUserIntro.adoc[]

. Select the ‘Exasol’ data source, and press btn:[Create]. This opens the data source definition screen.
+
image::dataSourceNewExasol2.png[alt="Exasol data source definition screen"]
...
+
image::dataSourceExasol2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceExasol3.png[alt="connection error icon"]
+
