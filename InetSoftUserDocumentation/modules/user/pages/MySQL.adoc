...
// metadata for AI
ifdef::ai[]
Summary: How to connect to MySQL as a data source in InetSoft. Requires uploading the MySQL JDBC driver plugin. Supports Custom URL or host name/port/database name entry with login credentials. Supports HikariCP connection pool configuration.
endif::ai[]
...
+
image::dataSourceNewMySQL2.png[alt="MySQL data source definition screen"]
...
+
image::dataSourceMySQL_2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceMySQL_3.png[alt="connection error icon"]
+
