...
// metadata for AI
ifdef::ai[]
Summary: Instructions for connecting to Presto as a data source in InetSoft. Covers connection parameters, authentication, and any driver or API key requirements. See ConnectToDataSource for the full list of available connectors.
endif::ai[]
include::partial$dataSourceUserIntro.adoc[]


. Select the ‘Presto’ data source, and press btn:[Create]. This opens the data source definition screen.
+
image::dataSourceNewPresto2.png[alt="Presto data source definition screen"]
...
+
image::dataSourcePresto2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourcePresto3.png[alt="connection error icon"]
+
