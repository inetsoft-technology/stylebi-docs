...
// metadata for AI
ifdef::ai[]
Summary: How to connect to Google Calendar as a data source in InetSoft Style Intelligence. Authentication uses the Sign in with Google OAuth flow to populate the token fields. An API key is required for open-source deployments. After connecting, proceed to define a data set using QueryGoogleCalendar.
endif::ai[]
...
+
image::datasourceGoogleCalendar1.png[alt="Google Calendar data source definition screen"]
...
+
image::dataSourceGoogleCalendar2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceGoogleCalendar3.png[alt="connection error icon"]
+
