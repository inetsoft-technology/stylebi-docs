...
// metadata for AI
ifdef::ai[]
Summary: How to connect to an Oracle database as a data source in InetSoft. The Oracle driver is bundled. Supports Custom URL or host name/port/SID entry with login credentials; use the Custom option to connect by service name instead of SID. Also covers a known limitation with CLOB columns and how to work around it using CAST to VARCHAR2.
endif::ai[]
...
+
image::dataSourceNewOracle2.png[alt="Oracle data source definition screen"]
...
+
TIP: To connect to a service name instead of an SID,  use the '`Custom`' option.
+
...
+
image::dataSourceOracle2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceOracle3.png[alt="connection error icon"]
+
