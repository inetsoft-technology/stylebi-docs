...
// metadata for AI
ifdef::ai[]
Summary: How to connect to AWS EMR (Elastic MapReduce) as a data source in InetSoft using a custom JDBC driver. Requires a plugin driver, the driver class name, and a JDBC URL of the form jdbc:hive2://{hostname}:{port}/{db}, plus login credentials.
endif::ai[]
include::partial$dataSourceUserIntro.adoc[]

. Select the ‘AWS EMR’ data source, and press btn:[Create]. This opens the data source definition screen.
+
image::dataSourceNewAWSEMR2.png[alt="AWS EMR data source definition screen"]
...
+
image::dataSourceAWSEMR3.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceAWSEMR4.png[alt="connection error icon"]
+
