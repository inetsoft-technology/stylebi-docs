...
// metadata for AI
ifdef::ai[]
Step-by-step instructions for creating a physical view (join diagram, table joins, database schema view) that defines the database table join structure underlying a data model, for relational database data sources. Covers: adding tables to the physical view (with optional search filtering and inline SQL custom tables), adding and configuring joins between tables (including join operator such as inner/outer/inequality, cardinality, merging rule, and weak join settings in the Edit Join dialog), removing and editing joins, and saving the physical view. Includes a caution about query traps that can cause unexpected query results.
endif::ai[]
...
+
image::dataModel2.png[alt="press New Physical View button"]


. Provide a name for the physical view and press ‘OK’.
+
image::dataModel3.png[alt="name physical view and press OK"]
...
+
image::dataModel4.png[alt="filter tables using Search box"]
...
+
image::dataModel5.png[alt="select tables to add"]
...
+
image::dataModel12.png[alt="drag join handle to open Join Editor"]
+
If a join between the tables already exists, click on the join link to open the ‘Edit Join’ dialog box.
+
image::dataModel13.png[alt="click join link to open Edit Join"]
+
...
+
image::dataModel15.png[alt="modify join settings in Edit Join dialog"]
...
+
image::dataModel14.png[alt="right-click join link and select Remove Join Condition"]
...
+
image::dataModel16.png[alt="drag join field to create new join"]
...
image::dataModel17.png[alt="select table and press Add a join"]
...
image::dataModel18.png[alt="choose join columns and press OK"]
====
...
+
image::dataModel19.png[alt="press Edit a join button"]
