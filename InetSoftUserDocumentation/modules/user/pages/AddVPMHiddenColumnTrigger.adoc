...
// metadata for AI
ifdef::ai[]
How to add a scripted trigger to a VPM hidden column, allowing the column restriction to be applied selectively. The trigger script uses standard JavaScript and returns an array of hidden columns. Available script parameters include: user (current user name), roles (user's roles), groups (user's groups), parameter (dashboard or worksheet parameters), tables (list of tables in the query), columns (columns in the query), and hiddenColumns (hidden columns currently defined in the VPM). Requires an existing VPM. See CreateNewVPM and AddDataSecurity.
endif::ai[]
...
+
image::dataModel79.png[alt="select Hidden Columns tab"]

. Select the *Trigger* sub-tab.
+
image::dataModel80.png[alt="select Trigger sub-tab"]
