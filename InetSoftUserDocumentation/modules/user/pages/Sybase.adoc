...
. Select the ‘Sybase’ data source, and press btn:[Create]. This opens the data source definition screen.
+
image:dataSourceNewSybase2.png[]

. In the ‘Name’ field, enter a label for this database connection.

. From the ‘Database Type’ menu, select ‘Sybase’ (if not already selected).
