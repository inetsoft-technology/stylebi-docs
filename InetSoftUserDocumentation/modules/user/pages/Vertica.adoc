...
// metadata for AI
ifdef::ai[]
Summary: How to connect to Vertica as a data source in InetSoft using a JDBC driver (plugin required). Requires setting Database Type to Custom, supplying the Driver Class name and a JDBC URL in the form jdbc:vertica://{host}:{port}/{databaseName}, plus login credentials.
endif::ai[]
include::partial$dataSourceUserIntro.adoc[]

. Select the ‘Vertica’ data source, and press btn:[Create]. This opens the data source definition screen.
+
image::dataSourceNewVertica2.png[alt="Vertica data source definition screen"]
...
+
image::dataSourceVertica2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceVertica3.png[alt="connection error icon"]
+
