...
// metadata for AI
ifdef::ai[]
Summary: How to connect to a GraphQL endpoint as a data source in InetSoft Style Intelligence. You supply the service URL and choose an authentication method: Basic (username/password), Two-step Token (with Authentication URI, HTTP parameters, and token regex), or OAuth 2.0 (Client ID/Secret, Authorization URI, Token URI, and Scope). An API key is required for open-source deployments. After connecting, proceed to define a data set using QueryGraphQL.
endif::ai[]
...
+
image::dataSourceGraphQL1.png[alt="GraphQL data source definition screen"]
...
+
image::dataSourceGraphQL2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceGraphQL3.png[alt="connection error icon"]
+
