...
// metadata for AI
ifdef::ai[]
Summary: How to connect to Google Cloud SQL as a data source in InetSoft Style Intelligence using a JDBC driver (plugin driver required). You can connect via a Custom URL (jdbc:mysql:///{DatabaseName}?cloudSqlInstance=...) or by entering the Host Name, Port, Database Name, and login credentials manually. After connecting, proceed to create a data set.
endif::ai[]
...
+
image::dataSourceNewGoogleCloudSQL2.png[alt="Google Cloud SQL data source definition screen"]
...
+
image::dataSourceGoogleCloudSQL2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceGoogleCloudSQL3.png[alt="connection error icon"]
+
