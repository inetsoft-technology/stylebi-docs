...
// metadata for AI
ifdef::ai[]
Summary: How to connect to QuickBooks Reports as a data source in InetSoft using OAuth authorization. Covers pressing the Authorize button to log in and populate the token and Company ID fields, and selecting Production Mode to target live rather than sandbox data.
endif::ai[]
...
+
image::dataSourceNewQuickBooksReports1.png[alt="Quickbooks Reports data source definition screen"]
...
+
image::dataSourceQuickbooks2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceQuickBooks3.png[alt="connection error icon"]
+
