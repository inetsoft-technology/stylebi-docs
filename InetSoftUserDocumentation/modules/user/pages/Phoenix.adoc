...
// metadata for AI
ifdef::ai[]
Summary: How to connect to Apache Phoenix as a data source in InetSoft using a JDBC driver. Covers selecting the Custom database type, specifying the Driver Class, entering the JDBC URL in the form `jdbc:phoenix:{host}:{port}`, and providing login credentials.
endif::ai[]
...
+
image::dataSourceNewPhoenix2.png[alt="Phoenix data source definition screen"]
...
+
image::dataSourcePhoenix2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourcePhoenix3.png[alt="connection error icon"]
+
