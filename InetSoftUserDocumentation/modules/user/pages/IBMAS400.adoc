...
// metadata for AI
ifdef::ai[]
Summary: How to connect to IBM AS400 (iSeries) as a data source in InetSoft Style Intelligence using a JDBC driver (plugin driver required). You must supply a Driver Class name and a JDBC URL of the form jdbc:as400://[{system-name}/{default-schema}], plus login credentials. After connecting, proceed to create a data set.
endif::ai[]
include::partial$dataSourceUserIntro.adoc[]

. Select the ‘IBM AS400’ data source, and press btn:[Create]. This opens the data source definition screen.
+
image::dataSourceNewIBMAS4002.png[alt="IBM AS400 data source definition screen"]
...
+
image::dataSourceIBMAS4002.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceIBMAS4003.png[alt="connection error icon"]
+
