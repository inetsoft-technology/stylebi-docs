...
// metadata for AI
ifdef::ai[]
Summary: Instructions for connecting to Amazon Aurora RDS MySQL as a data source in InetSoft. Covers connection parameters, authentication, and any driver or API key requirements. See ConnectToDataSource for the full list of available connectors.
endif::ai[]
...
+
image::dataSourceMySQL3.png[alt="Amazon Aurora RDS MySQL data source definition screen"]
+
...
+
image::dataSourceMySQL4.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceMySQL5.png[alt="connection error icon"]
+
