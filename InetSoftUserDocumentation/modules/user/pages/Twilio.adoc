...
// metadata for AI
ifdef::ai[]
Summary: Instructions for connecting to Twilio as a data source in InetSoft. Covers connection parameters, authentication, and any driver or API key requirements. See ConnectToDataSource for the full list of available connectors.
endif::ai[]
...
+
image::dataSourceNewTwilio2.png[alt="Twilio data source definition screen"]
...
+
image::dataSourceTwilio3.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceTwilio4.png[alt="connection error icon"]
+
