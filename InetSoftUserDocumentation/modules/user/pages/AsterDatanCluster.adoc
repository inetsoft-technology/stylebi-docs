...
// metadata for AI
ifdef::ai[]
Summary: Instructions for connecting to Aster Data nCluster as a data source in InetSoft. Covers connection parameters, authentication, and any driver or API key requirements. See ConnectToDataSource for the full list of available connectors.
endif::ai[]
include::partial$dataSourceUserIntro.adoc[]

. Select the ‘Aster Data nCluster’ data source, and press btn:[Create]. This opens the data source definition screen.
+
image::dataSourceNewAsterDatanCluster2.png[alt="Aster Data nCluster data source definition screen"]
...
. Enter the ‘JDBC URL’ that should be used to connect to the database, which has the following form. (You can obtain this from the database administrator.)
 ...
 jdbc:ncluster://<hostname>:<port>/<database>
...
+
image::dataSourceAsterDatanCluster2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceAsterDatanCluster3.png[alt="connection error icon"]
+
