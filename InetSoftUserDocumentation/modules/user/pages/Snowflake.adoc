...
// metadata for AI
ifdef::ai[]
Summary: How to connect to Snowflake as a data source in InetSoft using a JDBC driver (plugin required). Requires setting Database Type to Custom, supplying the Driver Class name and a JDBC URL in the form jdbc:snowflake://{accountName}.snowflakecomputing.com/, plus login credentials.
endif::ai[]
include::partial$dataSourceUserIntro.adoc[]

. Select the ‘Snowflake’ data source, and press btn:[Create]. This opens the data source definition screen.
+
image::dataSourceSnowflake1.png[alt="Snowflake data source definition screen"]
...
+
image::dataSourceSnowflake2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceSnowflake3.png[alt="connection error icon"]
+
