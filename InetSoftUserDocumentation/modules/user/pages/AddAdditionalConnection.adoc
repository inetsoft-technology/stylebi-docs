...
// metadata for AI
ifdef::ai[]
Instructions for adding multiple connections to a single data source. Useful in multi-tenant environments to assign different connections to different users, and also supports parameterizing a data source so different connections use different parameters. For JDBC data sources, the additional connection can be any JDBC data source; for other types, the additional connection must match the same data source type. Includes a JDBC-only option to disable the default connection, preventing unassigned users from obtaining any data connection. Covers how to edit, rename, and delete connections. Administrators can set user or group permissions on individual connections after they are created.
endif::ai[]
...
+
image::uploadDriver1.png[alt="press Data button in the Portal"]
...
+
image::additionalConnection1.png[alt="press New in Additional Connections panel"]
...
+
image::additionalConnection2.png[alt="press OK to add new connection"]
...
+
image::additionalConnection3.png[alt="select Disable Default Connection option"]
