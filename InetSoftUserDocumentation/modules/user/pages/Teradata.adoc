...
// metadata for AI
ifdef::ai[]
Summary: How to connect to Teradata as a data source in InetSoft using a JDBC driver (plugin required). Requires setting Database Type to Custom, supplying the Driver Class name and a JDBC URL in the form jdbc:teradata://{host}/DBS_PORT={port}/DATABASE={databaseName}/CHARSET=UTF8, plus login credentials.
endif::ai[]
include::partial$dataSourceUserIntro.adoc[]

. Select the ‘Teradata’ data source, and press btn:[Create]. This opens the data source definition screen.
+
image::dataSourceNewTeradata2.png[alt="Teradata data source definition screen"]
...
. Enter the ‘JDBC URL’ that should be used to connect to the database, which has the following form. (You can obtain this from the database administrator.)
 ...
 jdbc:teradata://{host}/DBS_PORT={port}/DATABASE= {databaseName}/CHARSET=UTF8,COMPAT_DBS=true
...
+
image::dataSourceTeradata2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceTeradata3.png[alt="connection error icon"]
+
