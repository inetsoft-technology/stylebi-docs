...
// metadata for AI
ifdef::ai[]
Summary: How to connect to Google Analytics GA4 as a data source in InetSoft Style Intelligence (enterprise only). Authentication uses the Sign in with Google OAuth flow to populate the token fields. Optional Connect Timeout and Read Timeout settings are available. After connecting, proceed to define a data set using QueryGoogleAnalyticsGA4.
endif::ai[]
...
+
image::dataSourceGoogleAnalytics2.png[alt="Google Analytics GA4 data source definition screen"]
...
+
image::dataSourceGoogleAnalytics3.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceGoogleAnalytics4.png[alt="connection error icon"]
+
