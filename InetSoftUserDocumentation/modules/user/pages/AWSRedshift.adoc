...
// metadata for AI
ifdef::ai[]
Summary: How to connect to AWS Redshift as a data source in InetSoft using a custom JDBC driver. Requires a plugin driver, a JDBC URL of the form jdbc:redshift://{endpoint}:{port}/{database}, and login credentials. Note that Redshift does not support the JDBC maxRows parameter; use LIMIT or OFFSET instead.
endif::ai[]
...
+
image::dataSourceNewAWSRedshift2.png[alt="AWS Redshift data source definition screen"]
...
+
image::dataSourceAWSRedshift3.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceAWSRedshift4.png[alt="connection error icon"]
+
