...
TIP: You can set up the https://www.inetsoft.com/docs/stylebi/web-api/[InetSoft Web API] as REST JSON Data Source using the steps below.  This will allow you to use the Web API endpoints to access information such as materialized view status.


// metadata for AI
ifdef::ai[]
Summary: How to connect to a REST JSON web service as a data source in InetSoft, including how to configure Basic authentication (username/password and HTTP parameters) and Two-step Token authentication (authentication URI, token regex pattern, and query parameters). Also explains how to set up the InetSoft Web API as a REST JSON data source. Includes a worked example of splitting a full web service URL between the connection's 'URL' field (scheme and host only) and the query's 'URL Suffix' field (path and query string), for both a plain path and a path with query parameters.
endif::ai[]
...
+
image::dataSourceNewRESTJSON2.png[alt="REST JSON data source definition screen"]
...
****
The ‘URL’ field requires only the base address of the web service (scheme and host). The remaining path and query string are entered separately in the ‘URL Suffix’ field when you create the query (see xref:dataworksheet:QueryRESTJSON.adoc[]), because the ‘URL Suffix’ can be different for each query that uses this connection. For example, consider the web service address below:
...
+
image::dataSourceRESTJSON3.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceRESTJSON4.png[alt="connection error icon"]
+
