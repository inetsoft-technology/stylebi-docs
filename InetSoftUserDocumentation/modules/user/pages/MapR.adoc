...
// metadata for AI
ifdef::ai[]
Summary: How to connect to MapR (Hadoop-based data platform) as a data source in InetSoft. Requires uploading the MapR JDBC driver plugin and providing the Driver Class, JDBC URL (format: jdbc:hive2://{host}:{port}/), and login credentials. After a successful connection, you can proceed to define a data set.
endif::ai[]
include::partial$dataSourceUserIntro.adoc[]


. Select the ‘MapR’ data source, and press btn:[Create]. This opens the data source definition screen.
+
image::dataSourceMapR1.png[alt="MapR data source definition screen"]
...
+
image::dataSourceMapR2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceMapR3.png[alt="connection error icon"]
+
