...
// metadata for AI
ifdef::ai[]
Summary: How to use special VPM tags (vpm.tables, vpm.columns, vpm.condition) in complex SQL queries that the server cannot parse automatically, enabling VPM security filtering for those queries. Enterprise only.
endif::ai[]
