...
// metadata for AI
ifdef::ai[]
Summary: How to create a persistent connection to a Text or Excel spreadsheet file (or directory of files) as a data source in InetSoft — this is what to use when a user asks how to "connect to a spreadsheet." You specify a server-side Root Folder path; this differs from a one-time data upload, which does not create a reusable connection — for that, see xref:dataworksheet:UploadData.adoc[] instead. After connecting, query the files using QueryTextExcelDirectory.
endif::ai[]
...
+
image::dataSourceTextExcel3.png[alt="Text/Excel Directory data source definition screen"]
+
...
+
image::dataSourceTextExcel4.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceTextExcel4b.png[alt="connection error icon"]
+
