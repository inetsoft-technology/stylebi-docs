...
// metadata for AI
ifdef::ai[]
How to open and edit an existing logical model (data model, semantic layer, data dictionary), including adding entities and attributes, renaming entities, modifying attribute properties (attribute name, data type, physical column mapping, format, reference type, browse data), and adding auto-drill hyperlinks that link to dashboards or web pages. Auto-drill configuration includes: passing the hyperlinked value or a constant as a parameter, using a Data Worksheet query lookup to retrieve a related value to pass as a parameter, embedding parameters in URLs, sending dashboard-level parameters via the 'Send Dashboard Parameters' option, controlling the target browser window/frame, and disabling parameter prompts on drill-down dashboards.
endif::ai[]
...
+
image::dataModel92.png[alt="click logical model to open for editing"]
...
image::dataModel93.png[alt="click entity and enter new name"]
...
+
image::dataModel46.png[alt="expand entity and select attribute"]
...
Data Type:: Assign a data type (‘Integer’, String’, etc.) from the ‘Data Type’ menu.  The following types are available:
+
...
[%collapsible]
====
For examples of how  to construct dates using these symbols, see xref:dashboard:DataFormat.adoc#DateFormat[Date Format] in xref:dashboard:VisualizeYourData.adoc[].
...
. Expand an entity in the model, and click to select the attribute that you want to modify.
+
image::dataModel47.png[alt="expand entity and select attribute to drill"]

. Press the ‘Edit Auto Drill’ btn:[...] button next to the ‘Auto Drill’ field. This opens the ‘Edit Auto Drill’ dialog box.
+
image::dataModel48.png[alt="press Edit Auto Drill button"]
...
+
image::dataModel49.png[alt="press Add Drill to create new auto-drill"]
...
+
image::dataModel50.png[alt="select Asset Link and choose dashboard"]
+
...
+
image::dataModel51.png[alt="select Web Link and enter URL"]
...
+
image::dataModel97.png[alt="press Query Selection button"]
...
image::dataModel95.png[alt="select Data Worksheet and field"]
...
+
image::dataModel53.png[alt="select Field as parameter source"]
+
If you are using  ‘Query’ look-up (see above), you can choose the *value returned by the  query* to pass in the hyperlink.
+
image::dataModel96.png[alt="choose query value as parameter"]
+
...
+
TIP: Dashboard parameters are parameters defined within the dashboard containing the hyperlinks.  See xref:dashboard:DynamicallySetProperties.adoc[] in xref:dashboard:VisualizeYourData.adoc[] and xref:dashboardscript:parameter.adoc[] in xref:dashboardscript:AdvancedDashboardScripting.adoc[] for more information about parameters.
+
...
[example]
====
Consider a dashboard that has the following dashboard parameters defined in the *onInit* script. (See xref:dashboardscript:AddScriptToDashboard.adoc[] for  information about how to add script.)
