...
// metadata for AI
ifdef::ai[]
Summary: How to connect to HubSpot as a data source in InetSoft Style Intelligence. Authentication uses an OAuth flow via the Authorize button to populate the token fields. An API key is required for open-source deployments. After connecting, proceed to define a data set using QueryHubSpot.
endif::ai[]
...
+
image::dataSourceNewHubSpot2.png[alt="HubSpot data source definition screen"]
...
+
image::dataSourceHubSpot2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceHubSpot3.png[alt="connection error icon"]
+
