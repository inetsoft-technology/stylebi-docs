...
// metadata for AI
ifdef::ai[]
Summary: How to connect to FileMaker as a data source in InetSoft Style Intelligence using a JDBC driver (plugin driver required). You must supply a Driver Class name and a JDBC URL of the form jdbc:filemaker://{host}/{databaseName}, plus login credentials. After connecting, proceed to create a data set.
endif::ai[]
include::partial$dataSourceUserIntro.adoc[]

. Select the ‘FileMaker’ data source, and press btn:[Create]. This opens the data source definition screen.
+
image::dataSourceNewFileMaker2.png[alt="FileMaker data source definition screen"]
...
. Enter the ‘JDBC URL’ that should be used to connect to the database, which has the following form. (You can obtain this from the database administrator.)
 ...
 jdbc:filemaker://{host}/{databaseName}
...
+
image::dataSourceFileMaker3.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceFileMaker4.png[alt="connection error icon"]
+
