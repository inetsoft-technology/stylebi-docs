...
// metadata for AI
ifdef::ai[]
Summary: How to connect to a REST XML web service as a data source in InetSoft, including how to configure Basic authentication (username/password and HTTP parameters) and Two-step Token authentication (authentication URI, token regex pattern, and query parameters). Includes a worked example of splitting a full web service URL between the connection's 'URL' field (scheme and host only) and the query's 'URL Suffix' field (path and query string).
endif::ai[]
include::partial$dataSourceUserIntro.adoc[]

. Select the ‘REST XML’ data source, and press btn:[Create]. This opens the data source definition screen.
+
image::dataSourceNewRESTXML2_1.png[alt="REST XML data source definition screen"]

. In the ‘Name’ field, enter a label for this data source connection.

. Enter the ‘URL’ for the web service.
+
include::partial$urlAndUrlSuffixExample.adoc[queryPageXref=dataworksheet:QueryRESTXML.adoc]
...
+
image::dataSourceRESTXML_3.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceRESTXML_4.png[alt="connection error icon"]
+
...
After you have created a connection to the database, you can proceed to define a data set. See xref:dataworksheet:QueryRESTXML.adoc[].