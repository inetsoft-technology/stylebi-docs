...
// metadata for AI
ifdef::ai[]
Summary: How to connect to Cisco Information Server (Composite Software) as a data source in InetSoft using a custom JDBC driver. Requires a plugin driver, the driver class name, and a JDBC URL of the form jdbc:compositesw:dbapi@{hostname}:{port}?domain={domain}&dataSource={datasource}, plus login credentials.
endif::ai[]
...
+
image::dataSourceNewCiscoInfoServer2.png[alt="Cisco Info Server data source definition screen"]
...
+
image::dataSourceCiscoInfoServer3.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceCiscoInfoServer4.png[alt="connection error icon"]
+
