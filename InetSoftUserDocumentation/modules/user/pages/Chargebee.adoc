...
// metadata for AI
ifdef::ai[]
Summary: Instructions for connecting to Chargebee as a data source in InetSoft. Covers connection parameters, authentication, and any driver or API key requirements. See ConnectToDataSource for the full list of available connectors.
endif::ai[]
...
+
image::dataSourceNewChargebee2.png[alt="Chargebee data source definition screen"]
...
+
image::dataSourceREST3.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceREST4.png[alt="connection error icon"]
+
