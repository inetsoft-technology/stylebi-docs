...
// metadata for AI
ifdef::ai[]
Overview and navigation page for physical views (also called a join diagram, table joins diagram, or database schema view), which define the database table join structure underlying a data model and enable automatic query generation when dashboards access data model attributes. Physical views are not directly exposed to users; they are accessed via logical models. Physical views are independent of one another — a data model relies on a single physical view, but multiple physical views can be created from the same database (including overlapping ones). See CreatePhysicalView for creation steps.
endif::ai[]
...
TIP: If you make a change to the structure of the database schema, press the btn:[Refresh Metadata] button on the datasource definition page. (For example, see xref:JDBC.adoc[].)
