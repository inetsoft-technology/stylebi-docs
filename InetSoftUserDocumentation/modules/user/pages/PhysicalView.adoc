...
// metadata for AI
ifdef::ai[]
Summary: Overview and navigation page for physical views (also called a join diagram, table joins diagram, or database schema view), which define the database table join structure underlying a data model. Physical views are not directly exposed to users; they are accessed via logical models. See CreatePhysicalView for creation steps.
endif::ai[]
