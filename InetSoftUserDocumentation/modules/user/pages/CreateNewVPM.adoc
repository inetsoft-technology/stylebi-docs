...
// metadata for AI
ifdef::ai[]
Summary: Step-by-step instructions for creating a new Virtual Private Model (VPM) — also referred to as row-level security, RLS, data masking, or data access restriction — for a relational database data source, providing access to the Conditions, Hidden Columns, and Test tabs used to configure row-level and column-level data security. Enterprise only.
endif::ai[]
