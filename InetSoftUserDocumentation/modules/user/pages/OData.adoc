...
// metadata for AI
ifdef::ai[]
Summary: How to connect to an OData service as a data source in InetSoft. Requires a web service root URL and supports three authentication methods: Basic Authentication (username/password), Password Credentials Grant (OAuth with Token URI), and OpenID Connect OAuth (Client ID, Client Secret, Scope, Authorization URI, Token URI).
endif::ai[]
...
+
image::dataSourceOData2.png[alt="OData data source definition screen"]
...
+
image::dataSourceOData3.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceOData4.png[alt="connection error icon"]
+
