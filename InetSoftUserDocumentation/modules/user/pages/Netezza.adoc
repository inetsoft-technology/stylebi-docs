...
// metadata for AI
ifdef::ai[]
Summary: How to connect to IBM Netezza as a data source in InetSoft. Requires uploading the Netezza JDBC driver plugin and providing the Driver Class, JDBC URL (format: jdbc:netezza://{host}:{port}/{databaseName}), and login credentials. Supports HikariCP connection pool configuration.
endif::ai[]
include::partial$dataSourceUserIntro.adoc[]


. Select the ‘Netezza’ data source, and press btn:[Create]. This opens the data source definition screen.
+
image::dataSourceNewNetezza2.png[alt="Netezza data source definition screen"]
...
+
image::dataSourceNetezza2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceNetezza3.png[alt="connection error icon"]
+
