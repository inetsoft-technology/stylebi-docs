...
// metadata for AI
ifdef::ai[]
Summary: How to connect to SQL Server Express as a data source in InetSoft. Covers entering Host Name, Port, Instance Name, Database Name, and SQL Server login credentials (not Windows domain credentials), or using a custom JDBC URL. Notes that Azure connections require a Database Name.
endif::ai[]
...
+
image::dataSourceNewSQLServerExpress2.png[alt="SQL Server Express data source definition screen"]
...
+
image::dataSourceSQLServerExpress2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceSQLServerExpress3.png[alt="connection error icon"]
+
