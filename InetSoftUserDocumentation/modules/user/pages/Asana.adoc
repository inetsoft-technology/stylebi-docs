...
// metadata for AI
ifdef::ai[]
Summary: Instructions for connecting to Asana as a data source in InetSoft. Covers connection parameters, authentication, and any driver or API key requirements. See ConnectToDataSource for the full list of available connectors.
endif::ai[]
...
+
image::dataSourceNewAsana.png[alt="Asana data source definition screen"]
...
+
image::dataSourceAsana2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceAsana3.png[alt="connection error icon"]
+
