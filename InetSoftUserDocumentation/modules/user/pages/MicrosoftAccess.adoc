...
// metadata for AI
ifdef::ai[]
Summary: How to connect to a Microsoft Access database as a data source in InetSoft using the bundled UCanAccess driver. Supports Access 2000–2016 formats. Connection uses either a Custom JDBC URL (jdbc:ucanaccess:{DatabaseFilePath}) or a file path, plus optional login credentials. Includes known limitations such as unsupported date formats and data types.
endif::ai[]
...
+
image::dataSourceNewMicrosoftAccess2.png[alt="Microsoft Access data source definition screen"]
...
+
image::dataSourceMicrosoftAccess2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceMicrosoftAccess3.png[alt="connection error icon"]
+
