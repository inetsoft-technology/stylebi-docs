...
// metadata for AI
ifdef::ai[]
Summary: Instructions for connecting to ClickHouse as a JDBC data source in InetSoft. Requires a third-party JDBC driver (driver class name and JDBC URL in the form jdbc:(ch|clickhouse)[:<protocol>]://endpoint[/<database>]). Covers setting the database type to Custom, entering driver class and JDBC URL, and providing login credentials. See ConnectToDataSource for the full list of available connectors.
endif::ai[]
include::partial$dataSourceUserIntro.adoc[]

. Select the ‘ClickHouse’ data source, and press btn:[Create]. This opens the data source definition screen.
+
image::dataSourceNewClickHouse.png[alt="ClickHouse data source definition screen"]
...
. Enter the ‘JDBC URL’ that should be used to connect to the database, which has the following form. (You can obtain this from the database administrator.)
 ...
 jdbc:(ch|clickhouse)[:<protocol>]://endpoint1[,endpoint2,...][/<database>]
...
+
image::dataSourceClickhouse2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceClickhouse3.png[alt="connection error icon"]
+
