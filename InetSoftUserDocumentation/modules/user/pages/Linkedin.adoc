...
// metadata for AI
ifdef::ai[]
Summary: Instructions for connecting to Linkedin as a data source in InetSoft. Covers connection parameters, authentication, and any driver or API key requirements. See ConnectToDataSource for the full list of available connectors.
endif::ai[]
...
+
image::dataSourceNewLinkedin2.png[alt="Linkedin data source definition screen"]
...
+
image::dataSourceLinkedin2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceLinkedin3.png[alt="connection error icon"]
+
