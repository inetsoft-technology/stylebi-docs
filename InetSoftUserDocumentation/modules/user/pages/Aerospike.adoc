...
// metadata for AI
ifdef::ai[]
Summary: How to connect to Aerospike as a data source in InetSoft using a custom JDBC driver. Requires a plugin driver, the driver class name, and a JDBC URL of the form jdbc:aerospike:localhost[:<host>][/<namespace>], plus login credentials.
endif::ai[]
...
+
image::connectAerospike.png[alt="Aerospike data source definition screen"]
...
+
image::dataSourceAerospike3.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceAerospike2.png[alt="connection error icon"]
+
