...
// metadata for AI
ifdef::ai[]
Summary: How to connect to YouTube Analytics as a data source in InetSoft. Requires a Google OAuth Client ID and Client Secret (created via Google Cloud project with YouTube Analytics API enabled). Covers both creating the connection and generating the required Google Cloud credentials.
endif::ai[]
...
+
image::dataSourceNewYouTubeAnalytics2.png[alt="YouTube Analytics data source definition screen"]
...
+
image::dataSourceYouTubeAnalytics3.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceYouTubeAnalytics4.png[alt="connection error icon"]
+
