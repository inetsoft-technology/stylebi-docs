...
// metadata for AI
ifdef::ai[]
Summary: Instructions for connecting to Derby Embedded as a JDBC data source in InetSoft. Requires a plugin driver. Covers specifying the driver class, JDBC URL (jdbc:derby:{databaseName}), login credentials, and HikariCP connection pool settings. See ConnectToDataSource for the full list of available connectors.
endif::ai[]
...
+
image::dataSourceNewDerbyEmbedded2.png[alt="Derby Embedded data source definition screen"]
...
+
image::dataSourceDerbyEmbedded2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceDerbyEmbedded3.png[alt="connection error icon"]
+
