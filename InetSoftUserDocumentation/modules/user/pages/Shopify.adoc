...
// metadata for AI
ifdef::ai[]
Summary: How to connect to Shopify as a data source in InetSoft. Requires your Shopify store name (Shop field), OAuth Client ID and Client Secret, and desired API access scopes. After authorization the token fields are populated automatically.
endif::ai[]
...
+
image::dataSourceShopify1.png[alt="Shopify data source definition screen"]
...
+
image::dataSourceShopify2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceShopify3.png[alt="connection error icon"]
+
