...
// metadata for AI
ifdef::ai[]
Summary: How to connect to a SQL Server or Sybase database using the jTDS JDBC driver in InetSoft (plugin required). Requires setting Database Type to Custom, supplying the Driver Class name and a JDBC URL in the form jdbc:jtds:{server_type}//{host}/{databaseName}, plus login credentials.
endif::ai[]
include::partial$dataSourceUserIntro.adoc[]


. Select the ‘jTDS’ data source, and press btn:[Create]. This opens the data source definition screen.
+
image::dataSourceNewjTDS2.png[alt="jTDS data source definition screen"]
...
. Enter the ‘JDBC URL’ that should be used to connect to the database, which has the following form. (You can obtain this from the database administrator.)
 ...
 jdbc:jtds:{server_type}//{host}[:{port}]/{databaseName}
...
+
image::dataSourcejTDS2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourcejTDS3.png[alt="connection error icon"]
+
