...
// metadata for AI
ifdef::ai[]
Summary: Instructions for connecting to SAP Hana as a data source in InetSoft. Covers connection parameters, authentication, and any driver or API key requirements. See ConnectToDataSource for the full list of available connectors.
endif::ai[]
include::partial$dataSourceUserIntro.adoc[]

. Select the ‘SAP Hana’ data source, and press btn:[Create]. This opens the data source definition screen.
+
image::dataSourceNewSAPHANA2.png[alt="SAP Hana data source definition screen"]
...
+
image::dataSourceSAPHANA2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceSAPHANA3.png[alt="connection error icon"]
+
