...
// metadata for AI
ifdef::ai[]
Summary: How to connect to SAP HANA as a data source in InetSoft using a JDBC driver. Covers selecting the Custom database type, specifying the Driver Class, and entering the JDBC URL in the form `jdbc:sap://{host}:{port}[/?{options}]`.
endif::ai[]
