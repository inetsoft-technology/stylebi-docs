...
// metadata for AI
ifdef::ai[]
Summary: Instructions for connecting to Mixpanel as a data source in InetSoft. Covers connection parameters, authentication, and any driver or API key requirements. See ConnectToDataSource for the full list of available connectors.
endif::ai[]
...
+
image::dataSourceNewMixpanel2.png[alt="Mixpanel data source definition screen"]
...
+
image::dataSourceMixpanel4.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceMixpanel3.png[alt="connection error icon"]
+
