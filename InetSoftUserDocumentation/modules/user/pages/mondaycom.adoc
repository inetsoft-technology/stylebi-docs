...
// metadata for AI
ifdef::ai[]
Summary: Instructions for connecting to monday.com as a data source in InetSoft. Covers connection parameters, authentication, and any driver or API key requirements. See ConnectToDataSource for the full list of available connectors.
endif::ai[]
...
+
image::dataSourceMondayCom1.png[alt="monday.com data source definition screen"]
...
+
image::dataSourceMondayCom2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceMondayCom3.png[alt="connection error icon"]
+
