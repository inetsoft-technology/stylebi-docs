...
// metadata for AI
ifdef::ai[]
How to schedule automatic Dashboard generation and delivery on a predefined cycle. There are two distinct entry points with very different feature sets:

QUICK-SCHEDULE (Dashboard toolbar Schedule button only): Opens a simple wizard. Supports bookmark selection (new or existing), output format (PDF, PowerPoint, Excel, or CSV — available formats depend on the export options configured for the Dashboard by an administrator), email recipients, recurrence (Daily/Weekly/Monthly) with time and time zone, and an optional Include Link option. Does NOT support Save to Disk, Notification, Creation Parameters, Alerts, Execute As, or any other advanced options.

ADVANCED SCHEDULED TASKS (Portal Schedule button → New Task): Full scheduling interface with Condition, Action, and Options tabs. Supports all output formats (PDF, PowerPoint, Excel, CSV), Notification of task status, Deliver to Emails with dynamic subject/filename parameters (MessageFormat-style {0}/{1,date} placeholders and creation parameter names in curly braces), Save to Disk (server filesystem or FTP/SFTP), Creation Parameters for passing Dashboard parameter values at generation time, Alerts (execute action only when highlight or gauge range conditions are met), and Schedule Options (enable/disable, date range, Execute As a specific user for VPM/row-level security filtering, locale). Execute As and all Options tab settings are available ONLY in advanced scheduled tasks, not in quick-schedule. Managing tasks (edit, run now, stop, delete, move to folder) also applies only to advanced tasks.
endif::ai[]
...
+
image::scheduleDashboard1.png[alt="select Create New Bookmark or Use Current Bookmark"]

. Press btn:[Next].

. In the ‘Format’ menu, select the format in which the Dashboard should be delivered. (Available formats depend on administrative settings).
+
PDF:: Generates the Dashboard as a PDF document.
PowerPoint:: Generates the Dashboard as a PowerPoint presentation.
Excel:: {exportexcel}
CSV:: {exportcsv}
...
+
image::scheduleDashboard2.png[alt="select Daily, Weekly, or Monthly schedule"]
...
+
image::schedule1.png[alt="press New Task button"]
+
This creates a new task with the name ‘Task1’, and opens the ‘Schedule Task’ page.
+
image::schedule2.png[alt="new Task1 in Schedule Task page"]
...
+
image::alert1.png[alt="press Select to specify Dashboard"]
...
+
image::alert2.png[alt="select highlight or range conditions for alert"]
...
image::schedule5.png[alt="scheduled tasks list in Portal"]
...
+
image::schedule6.png[alt="select task and press Move button"]
...
image::scheduleReport2.png[alt="schedule action options"]
...
image::scheduleDashboard3.png[alt="press Multiple Actions button"]
...
image::schedulerNotification2.png[alt="enter email addresses in Notify field"]
...
Insert Parameters:: The '`Subject`' line and '`Attachment Name`' can include parameters for automatic insertion of Dashboard name and time and date of generation. These parameters use the {JavaTextMessageFormat} syntax, where `pass:[{0}]` represents the Dashboard  alias (or name, if no alias is specified), and `pass:[{1,date}]` represents the time and date of Dashboard  generation. You can specify a format for the date parameter using the patterns described in xref:dashboard:DataFormat.adoc[]. Some examples of automatically generated names are shown below.
+
|===
...
+
image::schedulerSaveToServer.png[alt="choose format from Format menu"]
+
CSV:: {EXPORTCSV}
Excel:: {exportexcel}
...
image::schedule4.png[alt="Options tab settings"]
