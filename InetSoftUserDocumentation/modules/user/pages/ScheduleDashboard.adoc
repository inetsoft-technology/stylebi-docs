...
QUICK-SCHEDULE (Dashboard toolbar Schedule button only): Opens a simple wizard. Supports bookmark selection (new or existing), output format (CSV or Excel only — not PDF or PowerPoint), email recipients, recurrence (Daily/Weekly/Monthly) with time and time zone, and an optional Include Link option. Does NOT support PDF, PowerPoint, Save to Disk, Notification, Creation Parameters, Alerts, Execute As, or any other advanced options.
