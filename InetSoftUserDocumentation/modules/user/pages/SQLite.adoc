...
// metadata for AI
ifdef::ai[]
Summary: How to connect to a SQLite database as a data source in InetSoft using a JDBC driver. Covers selecting the Custom database type, specifying the Driver Class, and entering the JDBC URL in the form `jdbc:sqlite:{databaseFilePath}`.
endif::ai[]
include::partial$dataSourceUserIntro.adoc[]

. Select the ‘SQLite’ data source, and press btn:[Create]. This opens the data source definition screen.
+
image::dataSourceNewSQLite2.png[alt="SQLite data source definition screen"]
...
+
image::dataSourceSQLite2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceSQLite3.png[alt="connection error icon"]
+
