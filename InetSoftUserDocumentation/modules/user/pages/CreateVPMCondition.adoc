...
// metadata for AI
ifdef::ai[]
Summary: How to add a row-filter condition to a VPM (Virtual Private Model) to restrict which database rows are visible to users. Covers triggering by table or physical view, building condition clauses using field values, expressions, variables, session data (user/role/group), and subqueries.
endif::ai[]
