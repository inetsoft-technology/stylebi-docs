...
// metadata for AI
ifdef::ai[]
How to add a row-filter condition (row filter, data restriction, security filter, WHERE clause) to a VPM (Virtual Private Model) to restrict which database rows are visible to users. Covers triggering by table or physical view, building condition clauses using field values, Perl5 regular expressions, fixed values, variables, session data (user/role/group), and subqueries. Also covers managing condition clauses: deleting, reordering (Move Up/Down), changing logical operators (and/or), and indenting/unindenting clause groups for parenthetical grouping. Scripted condition triggers can also be added (see AddVPMConditionTrigger).
endif::ai[]
...
+
image::dataModel59.png[alt="select Conditions tab and press Add Condition"]

. Select the *Conditions* sub-tab.
+
image::dataModel60.png[alt="select Conditions sub-tab"]
...
+
image::dataModel61.png[alt="select Table option and press Choose Table"]
+
In the ‘Choose Table’ dialog box, select the database table that should trigger the VPM and press btn:[OK].
+
image::dataModel62.png[alt="select database table and press OK"]
...
+
image::dataModel63.png[alt="select Physical View option and press Choose Physical View"]
+
In the ‘Choose Physical View’ dialog box, select the physical view that should trigger the VPM and press btn:[OK].
+
image::dataModel64.png[alt="select physical view and press OK"]
****
. Press the btn:[Edit clause] button. This opens the ‘Conditions’ dialog box.
+
image::dataModel65.png[alt="press Edit clause button"]

. Specify the desired condition clause.
+
image::dataModel66.png[alt="specify condition clause"]
+
...
+
image::dataModel67.png[alt="use Change Value Type to change condition type"]
+
.Value Type Options
****
...
+
image::dataModel68.png[alt="use Browse Data to select value"]
...
Session Data:: Use the logged-in user’s name, roles, or groups in the condition. The available options are *User*, *Roles*, and *Groups* — these are the only session values available. If you need to filter by a user property not covered by these options (such as '`region`' or '`department`'), consider encoding that property into a role or group name, or use a VPM with a subquery or expression instead.
...
+
image::dataModel75.png[alt=”click operator and press Modify”]
...
+
image::dataModel76.png[alt=”click operator and press Indent or Unindent”]
+
...
+
image::dataModel69.png[alt="enter name for condition"]
