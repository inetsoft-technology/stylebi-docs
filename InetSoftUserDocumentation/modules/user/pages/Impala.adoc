...
// metadata for AI
ifdef::ai[]
Summary: How to connect to Apache Impala as a data source in InetSoft Style Intelligence using a JDBC driver (plugin driver required). You must supply a Driver Class name and a JDBC URL of the form jdbc:impala://{host}:{port}/{databaseName}, plus login credentials. After connecting, proceed to create a data set.
endif::ai[]
include::partial$dataSourceUserIntro.adoc[]

. Select the ‘Impala’ data source, and press btn:[Create]. This opens the data source definition screen.
+
image::dataSourceImpala1.png[alt="Impala data source definition screen"]
...
+
image::dataSourceImpala2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceImpala3.png[alt="connection error icon"]
+
