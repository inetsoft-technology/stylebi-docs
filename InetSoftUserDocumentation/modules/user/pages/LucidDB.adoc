...
// metadata for AI
ifdef::ai[]
Summary: How to connect to LucidDB as a data source in InetSoft. Requires uploading the LucidDB JDBC driver plugin and providing the Driver Class, JDBC URL (format: jdbc:luciddb:https://{host}:{port}), and login credentials. After a successful connection, you can proceed to define a data set.
endif::ai[]
include::partial$dataSourceUserIntro.adoc[]


. Select the ‘LucidDB’ data source, and press btn:[Create]. This opens the data source definition screen.
+
image::dataSourceNewLucidDB2.png[alt="LucidDB data source definition screen"]
...
+
image::dataSourceLucidDB2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceLucidDB3.png[alt="connection error icon"]
+
