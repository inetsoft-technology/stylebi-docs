...
// metadata for AI
ifdef::ai[]
Summary: How to connect to SAP (ABAP, BAPI, or Tables) as a data source in InetSoft. Requires .NET 1.1 runtime, SAP client tools and DLLs (librfc32.dll, sapjcorfc.dll), and the SAP driver JAR (sapjco.jar) to be installed on the server. Enterprise edition only.
endif::ai[]
...
+
image::dataSourceSAP2.png[alt="SAP data source definition screen"]
...
+
image::dataSourceSAP3.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceSAP4.png[alt="connection error icon"]
+
