...
// metadata for AI
ifdef::ai[]
Summary: Instructions for connecting to MongoDB REST as a data source in InetSoft. Covers connection parameters, authentication, and any driver or API key requirements. See ConnectToDataSource for the full list of available connectors.
endif::ai[]
...
+
image::dataSourceNewMongoDBREST1.png[alt="MongoDB REST data source definition screen"]
...
+
image::dataSourceMongoDBREST2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceMongoDBREST1.png[alt="connection error icon"]
+
