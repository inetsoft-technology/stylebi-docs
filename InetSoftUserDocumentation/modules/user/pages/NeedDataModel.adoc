...
// metadata for AI
ifdef::ai[]
Guidance on whether to create a data model for a relational database (not available for Hadoop Hive). Explains the benefits of data models for self-service access, reducing query maintenance, simplifying data access for Dashboard designers, enabling filtering on fields not in a Dashboard, and avoiding manual joins. Also describes the two components of a data model: the physical view (defines a subset of database tables and their join relationships) and the logical model (maps tables to entities and columns to attributes for a business-oriented representation).
endif::ai[]
...
image::user:entityAttribute.png[alt="data model entity attribute example"]
