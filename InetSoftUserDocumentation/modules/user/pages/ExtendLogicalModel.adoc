...
// metadata for AI
ifdef::ai[]
How to create a tenant-specific logical model extension in a multi-tenant environment. Covers linking the extended model to an extended physical view and a specific connection (one-to-one relationship), adding new entities/attributes or extending inherited ones, and hiding inherited attributes via the 'Visible' option (inherited properties cannot be modified, only hidden). Administrators expose the extended model to tenants by setting permissions on the corresponding connection. Enterprise only.
endif::ai[]
...
+
image::dataModel88.png[alt="press More Options, select Add Extended Model"]
...
+
image::dataModel89.png[alt="choose connection option and press OK"]
+
...
+
image::dataModel90.png[alt="extended model opens for editing"]
+
...
+
image::dataModel91.png[alt="press Add Element to add entities and attributes"]
