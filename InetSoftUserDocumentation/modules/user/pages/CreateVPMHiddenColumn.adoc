...
// metadata for AI
ifdef::ai[]
How to specify hidden columns (column masking, restrict column access) in a VPM to restrict user access to specific database table columns. Hidden columns are restricted from all users except those associated with roles listed in the "Grant to" pane. Tables with hidden columns are automatically added to the Lookup tab. Columns derived from hidden columns via expressions are accessible only to users who have permission for all columns used in the expression. Enterprise only. See AddDataSecurity for VPM overview.
endif::ai[]
...
+
image::dataModel70.png[alt="select Hidden Columns tab"]

. Select the column(s) that you want to hide, and press the {buttonAddArrow}.
+
image::dataModel71.png[alt="select columns and press Add"]
+
...
+
image::dataModel72.png[alt="add roles to Grant to pane"]
