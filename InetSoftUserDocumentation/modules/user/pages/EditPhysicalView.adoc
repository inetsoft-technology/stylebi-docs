...
// metadata for AI
ifdef::ai[]
How to edit an existing physical view in InetSoft, covering: adding inline SQL (embedded SQL views); automatically creating joins; resolving join cycles (loops) using weak joins or table aliases; creating a single-table alias to break a join cycle; using the auto-alias feature to alias multiple downstream tables at once; modifying join properties (type, order priority, cardinality, merging rule, weak link); and identifying query traps such as loop traps, higher-level aggregate traps, and higher-level join traps.
endif::ai[]
...
+
image::dataModel20.png[alt="click physical view to open for editing"]
...
+
image::dataModel6.png[alt="press Create Inline View button"]
...
+
image::dataModel7.png[alt="enter inline view name and SQL query"]

. Press btn:[OK]. This adds the inline view as a new table and places it into the physical view.
+
image::dataModel8.png[alt="inline view added as new table"]
...
+
image::dataModel9.png[alt="specify join construction method"]
...
+
image::dataModel10.png[alt="select key columns and press OK"]
...
+
image::dataModel11.png[alt="join path in red indicates join cycle"]
...
+
image::dataModel21.png[alt="select table with outgoing join"]
...
+
image::dataModel22.png[alt="select join condition and press Edit a join"]

. Set the desired join characteristics in the ‘Edit Join’ panel.
+
image::dataModel15.png[alt="set join characteristics in Edit Join panel"]
+
...
+
image::dataModel23.png[alt="right-click table and select Create Alias"]

. In the ‘Table Alias’ dialog box, provide a name for the alias table and press btn:[OK].
+
image::dataModel24.png[alt="name alias table and press OK"]
...
+
image::dataModel25.png[alt="alias table added to physical view"]
...
image::dataModel29.png[alt="join cycle example with PRODUCTS table"]
...
+
image::dataModel30.png[alt="right-click PRODUCTS and select Auto Alias"]

. Check the ‘Enable auto aliasing’ box.
+
image::dataModel31.png[alt="check Enable auto aliasing box"]
+
...
+
image::dataModel32.png[alt="select tables with incoming joins"]
+
...
+
image::dataModel33.png[alt=”set Keep Outgoing Links and table prefix”]
...
+
image::dataModel34.png[alt="aliased tables with cycle eliminated"]
