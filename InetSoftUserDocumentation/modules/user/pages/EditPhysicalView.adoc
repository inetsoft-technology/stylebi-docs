...
// metadata for AI
ifdef::ai[]
Summary: How to edit an existing physical view in InetSoft, covering add inline SQL (embedded SQL views), automatically creating joins, resolving join cycles (loops) using weak joins or table aliases, modifying join properties (type, cardinality, merging rule, weak link), and identifying query traps such as loop traps, higher-level aggregate traps, and higher-level join traps.
endif::ai[]
