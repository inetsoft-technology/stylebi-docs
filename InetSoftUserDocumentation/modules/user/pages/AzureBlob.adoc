...
// metadata for AI
ifdef::ai[]
Summary: How to connect to Azure Blob Storage as a data source in InetSoft. Requires an Account Name and Storage Account Key to generate a Shared Access Signature (SAS); fields can also be filled in manually. After connecting, query data via the Data Worksheet.
endif::ai[]
...
+
image::dataSourceNewAzureBlob2.png[alt="Azure Blob data source definition screen"]
...
+
image::dataSourceAzureBlob3.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceAzureBlob2.png[alt="connection error icon"]
+
