...
// metadata for AI
ifdef::ai[]
Summary: Instructions for connecting to 42Matters as a data source in InetSoft. Covers connection parameters, authentication, and any driver or API key requirements. See ConnectToDataSource for the full list of available connectors.
endif::ai[]
...
+
image::dataSourceNew42Matters2.png[alt="42Matters data source definition screen"]
...
+
image::dataSource42Matters3.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSource42Matters4.png[alt="connection error icon"]
+
