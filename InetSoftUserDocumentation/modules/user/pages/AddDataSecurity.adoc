...
// metadata for AI
ifdef::ai[]
Summary: Overview and navigation page for Virtual Private Model (VPM) security, which provides row- and column-level data filtering for relational data sources. Covers conditions, hidden columns, lookups, and their triggers. Enterprise only. See CreateNewVPM to begin.
endif::ai[]
