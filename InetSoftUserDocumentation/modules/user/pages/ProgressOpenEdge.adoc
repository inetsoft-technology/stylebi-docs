...
// metadata for AI
ifdef::ai[]
Summary: How to connect to Progress OpenEdge as a data source in InetSoft using a JDBC driver. Covers selecting the Custom database type, specifying the Driver Class, and entering the JDBC URL in the form `jdbc:datadirect:openedge://{host}:{port}`.
endif::ai[]
...
+
image::dataSourceNewProgressOpenEdge2.png[alt="Progress OpenEdge data source definition screen"]
...
+
image::dataSourceProgressOpenEdge2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceProgressOpenEdge3.png[alt="connection error icon"]
+
