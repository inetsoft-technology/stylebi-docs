...
// metadata for AI
ifdef::ai[]
Summary: Instructions for connecting to Jive as a data source in InetSoft. Covers connection parameters, authentication, and any driver or API key requirements. See ConnectToDataSource for the full list of available connectors.
endif::ai[]
...
+
image::dataSourceNewJive2.png[alt="Jive data source definition screen"]
...
+
image::dataSourceJive2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceJive3.png[alt="connection error icon"]
+
