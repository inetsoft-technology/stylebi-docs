...
// metadata for AI
ifdef::ai[]
Summary: How to connect to an OrientDB database as a data source in InetSoft using a JDBC driver. Covers selecting the Custom database type, specifying the Driver Class, entering the JDBC URL in the form `jdbc:orient:remote:{host}/{databaseName}`, and providing login credentials.
endif::ai[]
include::partial$dataSourceUserIntro.adoc[]


. Select the ‘OrientDB’ data source, and press btn:[Create]. This opens the data source definition screen.
+
image::dataSourceNewOrientDB2.png[alt="OrientDB data source definition screen"]
...
+
 jdbc:orient:remote:{host}/{databaseName}
...
+
image::dataSourceOrientDB2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceOrientDB3.png[alt="connection error icon"]
+
