...
// metadata for AI
ifdef::ai[]
Summary: Instructions for connecting to Apache Drill as a data source in InetSoft. Covers connection parameters, authentication, and any driver or API key requirements. See ConnectToDataSource for the full list of available connectors.
endif::ai[]
include::partial$dataSourceUserIntro.adoc[]

. Select the ‘Apache Drill’ data source, and press btn:[Create]. This opens the data source definition screen.

+
image::dataSourceNewApacheDrill2.png[alt="Apache Drill data source definition screen"]
...
+
image::dataSourceApacheDrill2.png[alt="connection successful icon"]
+
If the database is shown with an {buttonError}, this means that the connection was not successful.
+
image::dataSourceApacheDrill3.png[alt="connection error icon"]
+
