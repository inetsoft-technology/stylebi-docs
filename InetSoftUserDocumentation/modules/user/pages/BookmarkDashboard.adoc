...
// metadata for AI
ifdef::ai[]
Summary: How to save, manage, and share Dashboard bookmarks that preserve filter selections and other settings for later reuse. Covers creating, updating, deleting, and sharing bookmarks in the User Portal.
endif::ai[]
...
NOTE: Bookmarks store only settings, *not* data. See xref:dashboard:AddAnnotation.adoc[] to save annotations with a bookmark.
...
+
image::dashboard:AddBookmark.png[alt="press Bookmarks button"]
...
+
image::dashboard:AddBookmark2.png[alt="enter name for new bookmark"]
...
+
image::dashboard:Bookmark Filter.png[alt="press Filter to restrict bookmarks"]

. To filter the list of available bookmarks, type all or part of a Bookmark label in the ‘Search’ field.
+
image::dashboard:Bookmark Search.png[alt="type in Search field to filter bookmarks"]

. Select the bookmark you want to restore.
+
image::dashboard:Browse Bookmark Dialog.png[alt="select bookmark to restore"]
...
+
image::dashboard:Browse Bookmark Dialog.png[alt="select bookmark to update"]
...
NOTE: You can only delete a bookmark for which you are the owner. An administrator can also follow xref:administration:LoginAsDifferentUser.adoc[] to delete a bookmark. (The ‘Home’ bookmark cannot be deleted.)
...
+
image::dashboard:BookmarkDelete1.png[alt="press Remove Bookmarks button"]
...
+
image::dashboard:BookmarkDelete2.png[alt="select Older than or Not accessed since"]
...
+
image::dashboard:BookmarkProps.png[alt="press Edit to open Bookmark Properties"]
...
+
image::dashboard:Bookmark Default.png[alt="press Set as Default button"]
+
