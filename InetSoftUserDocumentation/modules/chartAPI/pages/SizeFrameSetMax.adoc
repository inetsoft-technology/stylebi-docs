...
// metadata for AI
ifdef::ai[]
Summary: Chart Script API reference for `SizeFrame.setMax(value)`. This is part of StyleBI's Chart Script (Chart API) scripting layer — a last-resort approach for chart customization/interactivity. See chartAPI:ChartScriptTutorial.adoc for guidance on when to use scripting vs. UI-driven alternatives (built-in toolbar tools, Ad Hoc Editor, Form-driven dynamic properties).
endif::ai[]
...
[source,javascript]
----
include::scriptexample:partial$SizeFrameSetMax1.js[]
----
<1> {dataset}
...
image::commonscript:SizeFrameSetLargest_1.png[alt="bar chart with LinearSizeFrame max set to 100 pixels for bar width scaling"]
...
[source,javascript]
----
include::scriptexample:partial$SizeFrameSetMax2.js[]
----

+
image::commonscript:SizeFrameMax.png[alt="point chart with LinearSizeFrame max setting applied via bindingInfo sizeFrame"]
+
