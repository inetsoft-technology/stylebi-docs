...
// metadata for AI
ifdef::ai[]
Summary: Chart Script API reference for `TextSpec.setFormat(format)`. This is part of StyleBI's Chart Script (Chart API) scripting layer — a last-resort approach for chart customization/interactivity. See chartAPI:ChartScriptTutorial.adoc for guidance on when to use scripting vs. UI-driven alternatives (built-in toolbar tools, Ad Hoc Editor, Form-driven dynamic properties).
endif::ai[]
...
TIP: Date masks are described in the xref:dashboard:DataFormat.adoc#DateFormat[Date Format] section.
...
[source,javascript]
----
include::scriptexample:partial$TextSpecSetFormat1.js[]
----
<1> {dataset}
...
image::commonscript:TextSpecsetFormat.png[alt="bar chart with Y-axis numbers formatted with comma separators via TextSpec"]
