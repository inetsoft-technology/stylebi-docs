...
// metadata for AI
ifdef::ai[]
Summary: Chart Script API reference for `EGraph.getScale(field)`. This is part of StyleBI's Chart Script (Chart API) scripting layer — a last-resort approach for chart customization/interactivity. See chartAPI:ChartScriptTutorial.adoc for guidance on when to use scripting vs. UI-driven alternatives (built-in toolbar tools, Ad Hoc Editor, Form-driven dynamic properties).
endif::ai[]
...
[source,javascript]
----
include::scriptexample:partial$EGraphGetScale1.js[]
----
<1> Obtain handle to `Scale` object.
<2> Apply operations to `Scale` object.

+
image::commonscript:EGraphgetScale.png[alt="EGraph with scale min and max set"]
