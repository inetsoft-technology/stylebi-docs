...
// metadata for AI
ifdef::ai[]
Summary: Chart Script API reference for `Change Chart Scaling`. This is part of StyleBI's Chart Script (Chart API) scripting layer — a last-resort approach for chart customization/interactivity. See chartAPI:ChartScriptTutorial.adoc for guidance on when to use scripting vs. UI-driven alternatives (built-in toolbar tools, Ad Hoc Editor, Form-driven dynamic properties).
endif::ai[]

[TIP]
====
If your Chart was created with the Chart Editor, use *simpler* xref:dashboardscript:MeasureAxisProperties.adoc[] script properties (such as xref:dashboardscript:MeasureAxisLogarithmic.adoc[], xref:dashboardscript:MeasureAxisMinimum.adoc[], xref:dashboardscript:MeasureAxisMaximum.adoc[], etc.) to modify axis scaling. The information below is useful when you need more advanced capabilities, or when you are building a Chart from scratch with the Chart API.
====
...
image::commonscript:ScalingChartAxes.png[alt="bar chart with default linear axis scale"]
...
+
image::commonscript:ScalingChartAxesFinal.png[alt="bar chart with logarithmic Y-axis scale"]
...
+
image::commonscript:Axis Range Example 1.png[alt="edit dimension button next to State field"]

. From the ‘Ranking’ option in the ‘Edit Dimension’ panel, specify ‘Top 5 of Sum(Total)’. Press the {buttonApply}.
+
image::commonscript:Axis Range Example 2.png[alt="ranking option set to top 5 of sum total"]
...
+
image::commonscript:Axis Range Example 3.png[alt="bar chart with Y-axis lower limit at 75 percent of minimum"]
+
...
+
image::commonscript:Axis Range Example 4.png[alt="Category selection list added to dashboard"]
...
image::commonscript:ScalingChartColorFrame.png[alt="bar chart with brightness color frame and default scale"]
...
+
image::commonscript:ScalingChartAxesFinal.png[alt="bar chart with custom color frame scale range"]
