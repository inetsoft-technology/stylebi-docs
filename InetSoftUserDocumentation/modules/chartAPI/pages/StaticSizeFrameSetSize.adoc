...
// metadata for AI
ifdef::ai[]
Summary: Chart Script API reference for `StaticSizeFrame.setSize(value)`. This is part of StyleBI's Chart Script (Chart API) scripting layer — a last-resort approach for chart customization/interactivity. See chartAPI:ChartScriptTutorial.adoc for guidance on when to use scripting vs. UI-driven alternatives (built-in toolbar tools, Ad Hoc Editor, Form-driven dynamic properties).
endif::ai[]
...
[source,javascript]
----
include::scriptexample:partial$StaticSizeFrameSetSize1.js[]
----
<1> {dataset}
...
+
image::commonscript:StaticSizeFrame2SetSize.png[alt="bar chart with StaticSizeFrame size 50 at max 100 for bar width"]
+
...
[source,javascript]
----
include::scriptexample:partial$StaticSizeFrameSetSize2.js[]
----

+
image::commonscript:StaticSizeFrame_1.png[alt="point chart with StaticSizeFrame size 30 set via bindingInfo sizeFrame size"]
+
