= LegendSpec.setBorderColor(value)
:navtitle: setBorderColor()
%experimental%
include::ROOT:partial$buttonSnippets.adoc[]

// metadata for AI
ifdef::ai[]
Summary: Chart Script API reference for `LegendSpec.setBorderColor(value)`. This is part of StyleBI's Chart Script (Chart API) scripting layer — a last-resort approach for chart customization/interactivity. See chartAPI:ChartScriptTutorial.adoc for guidance on when to use scripting vs. UI-driven alternatives (built-in toolbar tools, Ad Hoc Editor, Form-driven dynamic properties).
endif::ai[]

Specifies the legend border color.  The corresponding “getter” function is `getBorderColor()`.







== Parameter
A {javaawtcolor} object




== Example


.Chart Component Script
[source,javascript]
----
include::scriptexample:partial$LegendSpecSetBorderColor1.js[]
----
<1> {dataset}
<2> Create the xref:LegendSpec.adoc[] object.
<3> Use `setBorderColor` to assign the legend color.

image::commonscript:LegendSpecsetBorderColor.png[alt="legend with custom border color applied"]

include::chartAPI:partial$scriptThatModifiesGraph.adoc[]


[TIP]
=====



To change the property on a Chart that was *previously created* with the Chart Editor, use “getter” methods such as xref:chartAPI:EGraphGetElement.adoc[] ① and xref:chartAPI:GraphElementGetColorFrame.adoc[] ② to obtain a handle to the desired xref:chartAPI:GraphElement.adoc[] and xref:chartAPI:ColorFrame.adoc[]. For example:

[source,javascript]
----
var elem = graph.getElement(0); <1>
var frame = elem.getColorFrame(); <2>
// Compact syntax: var frame = graph.getElement(0).getColorFrame();

var spec = new LegendSpec();
spec.setBorderColor(java.awt.Color(0xff00ff));
frame.setLegendSpec(spec);

----
=====