...
To change the property on a Chart that was *previously created* with the Chart Editor, use “getter” methods such as xref:chartAPI:EGraph_getCoordinate.adoc[] ①, xref:chartAPI:RectCoordGetYScale.adoc[] ②, xref:chartAPI:ScaleGetAxisSpec.adoc[] ③, and xref:chartAPI:AxisSpecGetTextSpec.adoc[] ④ to obtain a handle to the desired xref:chartAPI:Coordinate.adoc[], xref:chartAPI:Scale.adoc[], xref:chartAPI:AxisSpec.adoc[], and xref:chartAPI:TextSpec.adoc[]. For example:
