...
// metadata for AI
ifdef::ai[]
Summary: Chart Script API reference for `CategoricalColorFrame.init(val[,color])`. This is part of StyleBI's Chart Script (Chart API) scripting layer — a last-resort approach for chart customization/interactivity. See chartAPI:ChartScriptTutorial.adoc for guidance on when to use scripting vs. UI-driven alternatives (built-in toolbar tools, Ad Hoc Editor, Form-driven dynamic properties).
endif::ai[]
...
[source,javascript]
----
include::scriptexample:partial$CategoricalColorFrameInit1.js[]
----
<1> {dataset}
...
image::commonscript:CategoricalColorFrameInit1.png[alt="scatter chart with color frame initialized by field names"]
...
[source,javascript]
----
include::scriptexample:partial$CategoricalColorFrameInit2.js[]
----
<1> {dataset}
<2> Create the xref:ColorFrame.adoc[] object.
...
image::commonscript:CategoricalColorFrameInit2.png[alt="scatter chart with color frame initialized with custom colors"]
