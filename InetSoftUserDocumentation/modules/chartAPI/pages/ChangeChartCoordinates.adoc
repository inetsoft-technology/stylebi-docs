...
// metadata for AI
ifdef::ai[]
Summary: Chart Script API reference for `Change Chart Coordinates`. This is part of StyleBI's Chart Script (Chart API) scripting layer — a last-resort approach for chart customization/interactivity. See chartAPI:ChartScriptTutorial.adoc for guidance on when to use scripting vs. UI-driven alternatives (built-in toolbar tools, Ad Hoc Editor, Form-driven dynamic properties).
endif::ai[]
...
image::commonscript:RectCoordExample.png[alt="scatter chart with rectangular coordinates and dot grid"]
...
image::commonscript:RectCoordExample_1.png[alt="scatter chart with explicit rectangular coordinates"]
...
image::commonscript:PolarCoordsExample.png[alt="scatter chart converted to polar coordinates"]
...
image::commonscript:PolarCoordsExampleTHETA.png[alt="polar chart with THETA angle-only mapping"]
...
image::commonscript:PieChartExample1_1.png[alt="basic bar chart before pie chart conversion"]
...
+
image::commonscript:PieChartExample2_1.png[alt="stacked bar with color coding by state"]
...
. Convert the Chart into a Stacked Bar Chart with just one bar by using the xref:chartAPI:GraphElementSetCollisionModifier.adoc[] property. Create a xref:chartAPI:StackRange.adoc[] object to make sure there is enough room for the stacked elements.
+
[source,javascript]
...
+
image::commonscript:PieChartExample3_1.png[alt="single stacked bar with symmetric stack modifier"]
...
graph.setCoordinate(polar);
+
image::commonscript:PieChartExample4_1.png[alt="stacked bar converted to pie chart with polar coordinates"]
...
+
image::commonscript:PieChartExample5_1.png[alt="pie chart with axis lines and labels hidden"]
...
+
image::commonscript:PieChartExample6_1.png[alt="exploded pie chart with state name labels"]
...
+
image::commonscript:ParallelCoordsTutorial.png[alt="parallel coordinates chart with color-coded lines"]
...
graph.addElement(elem);
+
image::commonscript:FacetCoordExample.png[alt="facet chart with inner and outer coordinate grid"]
+
...
image::commonscript:PlotSpecSetBackground.png[alt="bar chart with blue plot background color"]
...
* To assign a map background in script, use   xref:dashboardscript:ChartSetupGoogleMapsPlot.adoc[].
====
...
image::commonscript:PlotSpecSetBackgroundImageGoog.png[alt="scatter chart with map image as plot background"]
