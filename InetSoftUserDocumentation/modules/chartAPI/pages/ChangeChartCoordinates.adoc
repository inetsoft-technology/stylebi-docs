...
[TIP]
====
If needed, you can obtain a handle to the existing  RectCoord object by using xref:chartAPI:EGraph_getCoordinate.adoc[]:
[source,javascript]
...
To create polar coordinates, you first need to create a set of rectangular coordinates with the xref:chartAPI:RectCoord.adoc[] constructor or access existing rectangular coordinates with xref:chartAPI:EGraph_getCoordinate.adoc[]. Consider the script from the previous section, xref:#AssignRectangularCoordinatesAutomatically[].
...
. Use the Chart’s xref:chartAPI:EGraph_setCoordinate.adoc[] method to apply the new polar coordinates.
+
[source,javascript]
graph.setCoordinate(polar);
+
