...
// metadata for AI
ifdef::ai[]
Summary: Chart Script API reference for `AxisSpec.setTickVisible(Boolean)`. This is part of StyleBI's Chart Script (Chart API) scripting layer — a last-resort approach for chart customization/interactivity. See chartAPI:ChartScriptTutorial.adoc for guidance on when to use scripting vs. UI-driven alternatives (built-in toolbar tools, Ad Hoc Editor, Form-driven dynamic properties).
endif::ai[]
...
[source,javascript]
----
include::scriptexample:partial$AxisSpecSetTickVisible1.js[]
----
<1> {dataset}
<2> Create the `AxisSpec` objects.
<3> Use `setTickVisible` to hide ticks.

image::commonscript:AxisSpecsetTickVisible.png[alt="bar chart with axis tick marks hidden"]
