...
// metadata for AI
ifdef::ai[]
Summary: Shows how to modify chart style, axis titles, and axis properties using the *dashboard script shortcut* approach (`Chart1.chartStyle`, `Chart1.xTitle.text`, `Chart1.axis.*`). Despite being in the chartAPI module, this page uses *only* dashboard script syntax — it is *not* a Chart API (EGraph/graph.setScale) page. Do not mix with Chart API methods in the same answer.
endif::ai[]
...
====
xref:dashboardscript:AddScriptToDashboard.adoc[] for information on using the Dashboard script handlers.
====
...
== Modify Chart Style
To modify the style of a chart in script (e.g., line, bar, pie, etc.), use the xref:dashboardscript:PlotChartStyle.adoc[] attribute:
...
To modify axis title text, use xref:dashboardscript:AxisXTitleText.adoc[] (xref:dashboardscript:AxisX2TitleText.adoc[]) and xref:dashboardscript:AxisYTitleText.adoc[]  (xref:dashboardscript:AxisY2TitleText.adoc[]).
...
====
[square]
* xref:dashboardscript:ChartAxisProperties.adoc[] for general axis characteristics.
* xref:dashboardscript:MeasureAxisProperties.adoc[] for numeric axis characteristics.
* xref:dashboardscript:AxisTitleProperties.adoc[] for axis title characteristics.
* xref:dashboardscript:PlotProperties.adoc[] for plot area characteristics.
====

To modify axis properties, including labels, visibility, tick marks, etc., use the `axis` properties (xref:dashboardscript:ChartAxisFont.adoc[], xref:dashboardscript:MeasureAxisMinimum.adoc[] xref:dashboardscript:ChartAxisLabelColor.adoc[], xref:dashboardscript:ChartAxisFormat.adoc[], etc.).
