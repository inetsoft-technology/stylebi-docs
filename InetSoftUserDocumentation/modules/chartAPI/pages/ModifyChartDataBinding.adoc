...
// metadata for AI
ifdef::ai[]
Summary: Shows how to modify chart data binding using `Chart1.query` and `Chart1.bindingInfo.*` in *onRefresh script*. This is a dashboard script approach — it is *not* a Chart API (EGraph) approach. Do not mix with `graph.setScale()` or other Chart API methods in the same answer.
endif::ai[]
...
The Chart  xref:dashboardscript:ChartBindingInfo.adoc[]  property provides access to the data binding options in the Chart’s ‘Data Binding’ dialog box. This includes the binding of fields to the X-axis, Y-axis, and VisualFrames (Color, Shape, etc.), as well as grouping and summarization settings.
...
+
NOTE: Changes to the `bindingInfo` property must in general be made *prior to query execution*. Therefore, attach `bindingInfo` scripts to the *onRefresh* handler.
...
+
image::commonscript:ModifyingChartBinding.png[alt="bar chart with scripted data binding and color subseries"]
