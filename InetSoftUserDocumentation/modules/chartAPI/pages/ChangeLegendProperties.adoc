...
// metadata for AI
ifdef::ai[]
Summary: Chart Script API reference for `Change Legend Properties`. This is part of StyleBI's Chart Script (Chart API) scripting layer — a last-resort approach for chart customization/interactivity. See chartAPI:ChartScriptTutorial.adoc for guidance on when to use scripting vs. UI-driven alternatives (built-in toolbar tools, Ad Hoc Editor, Form-driven dynamic properties).
endif::ai[]
...
[TIP]
====
If your Chart was created with the Chart Editor, use  *simpler* xref:dashboardscript:PlotProperties.adoc[]   script (such as xref:dashboardscript:PlotLegendPosition.adoc[] or xref:dashboardscript:PlotLegendTitleFont.adoc[])  to modify the legend. The information below is useful when you need more advanced capabilities, or when you are  building a Chart from scratch with the Chart API.
====
...
image::commonscript:LegendSpecExample.png[alt="bar chart with CategoricalColorFrame distinguishing states by color"]
...
+
image::commonscript:LegendSpecExampleFinal.png[alt="bar chart with modified legend title font border and layout above chart"]
