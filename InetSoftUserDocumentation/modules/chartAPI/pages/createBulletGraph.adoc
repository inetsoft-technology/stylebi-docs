...
// metadata for AI
ifdef::ai[]
Summary: Chart Script API reference for `createBulletGraph(measure, ranges, target, color, xdims, ydims, opts)`. This is part of StyleBI's Chart Script (Chart API) scripting layer — a last-resort approach for chart customization/interactivity. See chartAPI:ChartScriptTutorial.adoc for guidance on when to use scripting vs. UI-driven alternatives (built-in toolbar tools, Ad Hoc Editor, Form-driven dynamic properties).
endif::ai[]
...
image::commonscript:BulletChartWS.png[alt="data worksheet with Target range1 range2 range3 expression columns"]
...
[source,javascript]
----
include::scriptexample:partial$createBulletGraph1.js[]
----
<1> See xref:Dataset.adoc[] for more information about setting the `dataset`.
<2> Create the bullet chart.
+
image::commonscript:BulletChart.png[alt="horizontal bullet graph with target and range bands by company and employee"]
+
