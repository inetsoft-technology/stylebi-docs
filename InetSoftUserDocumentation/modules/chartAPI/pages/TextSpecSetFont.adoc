...
[TIP]
=====
To change the property on a Chart that was *previously created* with the Chart Editor, use “getter” methods such as xref:chartAPI:EGraph_getXTitleSpec.adoc[] ① and xref:chartAPI:TitleSpec_getTextSpec.adoc[] ② to obtain a handle to the desired xref:chartAPI:TitleSpec.adoc[] and xref:chartAPI:TextSpec.adoc[]. For example:
