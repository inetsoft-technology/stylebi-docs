...
// metadata for AI
ifdef::ai[]
Summary: Chart Script API reference for `Modify Chart Element`. This is part of StyleBI's Chart Script (Chart API) scripting layer — a last-resort approach for chart customization/interactivity. See chartAPI:ChartScriptTutorial.adoc for guidance on when to use scripting vs. UI-driven alternatives (built-in toolbar tools, Ad Hoc Editor, Form-driven dynamic properties).
endif::ai[]

The Chart API provides access to charting engine commands using Java object syntax. You can use these commands to directly modify the graphical elements displayed by a Chart, as explained below.  To create a Chart _entirely_ using script (rather than modifying an existing Chart), see xref:CreateChartwithChartAPI.adoc[].  To change the data displayed on the chart, see xref:ModifyChartDataBinding.adoc[].
...
+
image::commonscript:ScriptChartElementGraph.png[alt="point chart showing max and min totals per employee"]
+
...
+
image::commonscript:ScriptChartElementGraphFinal.png[alt="point chart with red arrow markers and resized max points"]
