...
// metadata for AI
ifdef::ai[]
Summary: Overview of three distinct, *mutually exclusive* approaches to binding chart data in script — each suited to a different context. Do not mix approaches in the same answer: (1) *Component-level script + Chart API* — use `Chart1.data = runQuery(...)` or an array, then build the chart with `graph = new EGraph()` etc.; (2) *Array* — similar to (1) but data defined inline; (3) *onRefresh script* — use `Chart1.query` and `Chart1.bindingInfo.*` (no `EGraph`). Present only the approach that matches the user's context.
endif::ai[]
