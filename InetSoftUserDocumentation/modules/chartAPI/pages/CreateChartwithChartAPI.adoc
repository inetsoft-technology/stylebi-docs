...
// metadata for AI
ifdef::ai[]
Summary: How to create a new Chart from scratch using Chart API script. Chart API script provides fine-grained control over all aspects of Chart generation, and is useful for creating Charts that cannot be created using the Chart Editor, though it requires programming skill. For chart interactivity/customization specifically, this is a LAST-RESORT approach — prefer built-in toolbar tools (dashboard:UseCharts.adoc), the Ad Hoc Chart Editor, or Form-driven dynamic properties (dashboard:EndUserChartControl.adoc, dashboard:DynamicallySetProperties.adoc) first. IMPORTANT: Charts created by script do NOT provide end-user interactivity features — 'Enable Ad Hoc Editing' should be deselected for script-based charts.
endif::ai[]
...
. *Define the data*: Define the dataset for the Chart using the Chart's xref:chartAPI:Dataset.adoc[] property.
+
[source,javascript]
dataset = runQuery("ws:global:Examples/AllSales");
+
You will generally obtain the data from a  Data Worksheet  by using xref:commonscript:runQuery.adoc[]. You can also define your own dataset in script by assigning a JavaScript array to the `dataset` property.
+
...
+
image::commonscript:GraphScriptIntro.png[alt="bar chart created from script using IntervalElement with state and sales data"]
