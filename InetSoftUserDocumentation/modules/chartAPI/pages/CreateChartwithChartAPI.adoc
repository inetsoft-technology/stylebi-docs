...
. Add a new Chart to the report or Dashboard.
...
. *Add the created elements to the Chart object*. Pass the `GraphElement` object to the Chart’s xref:chartAPI:EGraph_addElement.adoc[] method. This adds the “bar” elements to the existing Chart object.
+
[source,javascript]
