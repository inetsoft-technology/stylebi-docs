...
To change the property on a Chart that was *previously created* with the Chart Editor, use “getter” methods such as xref:EGraph_getElement.adoc[] {circle1} and xref:GraphElementGetTextFrame.adoc[] {circle2}  to obtain a handle to the desired xref:GraphElement.adoc[] and xref:TextFrame.adoc[]. For example:
