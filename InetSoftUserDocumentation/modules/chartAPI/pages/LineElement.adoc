...
You can pass one, two, or three field names (e.g., ‘State’, ‘Quantity’) to the constructor, or specify these later using the  xref:GraphElementAddDim.adoc[] and  xref:GraphElementAddVar.adoc[] methods.
...
<3> Use xref:EGraph_addElement.adoc[] to add the `LineElement` to the Chart.
