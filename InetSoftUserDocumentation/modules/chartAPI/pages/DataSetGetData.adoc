...
// metadata for AI
ifdef::ai[]
Summary: Chart Script API reference for `DataSet.getData(column,row)`. This is part of StyleBI's Chart Script (Chart API) scripting layer — a last-resort approach for chart customization/interactivity. See chartAPI:ChartScriptTutorial.adoc for guidance on when to use scripting vs. UI-driven alternatives (built-in toolbar tools, Ad Hoc Editor, Form-driven dynamic properties).
endif::ai[]

[WARNING,caption=See Also]
====
xref:dashboardscript:ChartData.adoc[] for information on accessing pre-aggregate data.
====
...
image::commonscript:datasetChart_1.png[alt="chart with two dataset measures"]
...
[source,javascript]
----
include::scriptexample:partial$DataSetGetData1.js[]
----
