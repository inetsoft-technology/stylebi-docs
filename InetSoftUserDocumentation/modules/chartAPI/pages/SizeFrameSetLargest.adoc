...
// metadata for AI
ifdef::ai[]
Summary: Chart Script API reference for `SizeFrame.setLargest(value)`. This is part of StyleBI's Chart Script (Chart API) scripting layer — a last-resort approach for chart customization/interactivity. See chartAPI:ChartScriptTutorial.adoc for guidance on when to use scripting vs. UI-driven alternatives (built-in toolbar tools, Ad Hoc Editor, Form-driven dynamic properties).
endif::ai[]
...
[source,javascript]
----
include::scriptexample:partial$SizeFrameSetLargest1.js[]
----
<1> {dataset}
...
image::commonscript:SizeFrameSetLargest.png[alt="bar chart with LinearSizeFrame bar widths set to largest 50 pixels"]
...
[source,javascript]
----
include::scriptexample:partial$SizeFrameSetLargest2.js[]
----

+
image::commonscript:SizeFrame_2.png[alt="point chart with LinearSizeFrame largest setting applied via bindingInfo"]
+
