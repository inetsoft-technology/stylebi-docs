...
// metadata for AI
ifdef::ai[]
Summary: Chart Script API reference for `Represent Data with Shape, Color, Size`. This is part of StyleBI's Chart Script (Chart API) scripting layer — a last-resort approach for chart customization/interactivity. See chartAPI:ChartScriptTutorial.adoc for guidance on when to use scripting vs. UI-driven alternatives (built-in toolbar tools, Ad Hoc Editor, Form-driven dynamic properties).
endif::ai[]
...
[TIP]
====
If your Chart was created with the Chart Editor, use *simpler* xref:dashboardscript:ChartBindingInfoColorframe.adoc[], xref:dashboardscript:ChartBindingInfoShapeFrame.adoc[], and xref:dashboardscript:ChartBindingInfoSizeFrame.adoc[] properties in *onRefresh* script for data-driven color, shape, and size coding. The information below is useful when you need more advanced capabilities, or when you are building a Chart from scratch with the Chart API.
====
...
image::commonscript:VisualFrameExample1.png[alt="scatter chart showing State and Quantity without any VisualFrame applied"]
...
+
image::commonscript:VisualFrameExample2.png[alt="scatter chart with point sizes representing Total field via LinearSizeFrame"]
