...
// metadata for AI
ifdef::ai[]
Summary: Chart Script API reference for `TextSpec.setBackground(value)`. This is part of StyleBI's Chart Script (Chart API) scripting layer — a last-resort approach for chart customization/interactivity. See chartAPI:ChartScriptTutorial.adoc for guidance on when to use scripting vs. UI-driven alternatives (built-in toolbar tools, Ad Hoc Editor, Form-driven dynamic properties).
endif::ai[]
...
[source,javascript]
----
include::scriptexample:partial$TextSpecSetBackground1.js[]
----
<1> {dataset}
...
image::commonscript:TextSpecsetBackground.png[alt="bar chart with a label form showing gray background via TextSpec setBackground"]
