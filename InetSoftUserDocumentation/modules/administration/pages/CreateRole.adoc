...
// metadata for AI
ifdef::ai[]
Summary: How to create and manage roles in Enterprise Manager when using the default authentication provider, for assigning permissions to sets of users with similar functions.
endif::ai[]
...
+
image::server5.png[alt="new role created in navigation tree"]
...
+
image::securityDefineRole.png[alt="add users and groups to role assigned-to area"]
...
To copy the '`Assigned to`' or '`Inherit from`' identities from one role to another, follow the steps below:
...
. Press the btn:[Copy Identities] button below the '`Assigned to`' or '`Inherit from`' table to copy the current identities. You can then navigate to another role and paste those identities.

. Press the btn:[Paste Identities] button below the corresponding table to paste the identities you have copied.
+
TIP: The btn:[Paste Identities] button will display numbers in parentheses, such as btn:[Paste Identities (x of n)]. The *second value* (*n*) indicates the number of identities that were copied to the clipboard. The *first value* (*x*) indicates the number of identities that are compatible with the current role, and can actually be pasted.
