...
[#EmailProperties]
== Email Properties
Mail Host (Required):: Specifies one or more valid SMTP servers, separated by commas. The SMTP servers must be able to accept connections on the specified ports. The SMTP port can be configured with the xref:AllProperties.adoc#_mail_properties[mail.smtp.port] property. If the servers require secure protocol, enable the '`Secure SMTP`' option.
