...
// metadata for AI
ifdef::ai[]
Summary: How to configure the server email (SMTP) settings in Enterprise Manager for sending scheduled Dashboard deliveries and error notifications.
endif::ai[]
...
+
image::Figure 6.png[alt="select Email page under General settings"]
+
...
. Set ‘Mail Host’  to `smtp.gmail.com`  and enable '`Opportunistic TLS`'.
