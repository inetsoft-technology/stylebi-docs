...
// metadata for AI
ifdef::ai[]
Summary: Reference for scheduler task conditions in Enterprise Manager that determine when a scheduled task runs, covering the Daily, Weekly, Monthly, Hourly, and Run Once time-based conditions (with time zone and time range options), and the Chained condition that triggers on completion of another task or cycle.
endif::ai[]
...
image::schedulerAdminCondition1.png[alt="set condition for scheduled task on Condition tab"]
