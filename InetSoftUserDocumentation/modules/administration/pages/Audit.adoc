...
// metadata for AI
ifdef::ai[]
Summary: How to view audit data and configure the audit database in Enterprise Manager. Covers available audit datasets (bookmark history, logon history, modification history, asset dependencies, etc.) and how to configure a custom audit database via docker-compose.yaml environment variables (supported databases: Cosmos DB, DynamoDB, H2, Firestore, MongoDB). Enterprise only.
endif::ai[]
...
+
image::Figure 22.png[alt="press Auditing button in Enterprise Manager"]
