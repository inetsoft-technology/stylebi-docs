...
The audit feature tracks Dashboard and data access, as well as asset dependencies. When auditing is enabled, all Dashboard and database access is logged in the audit database and written to the server logs. (See xref:MonitorServerActivity.adoc#Logs[Logs] for more information.) Note that auditing requires you to  enable a security provider (see xref:SpecifySecurityProvider.adoc[]).
