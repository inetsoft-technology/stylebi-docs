...
Create an HTML page with the D3.js chart and place it into the _web-assets_ folder in the *SREE Home* location. The HTML should be similar to that below:
...
<head>
  ...
  <script src="js/d3-inetsoft.js"></script>
</head>
<body>
  <div id="#chart"></div>
  <script>
    function loadData(table) {
    // D3 code goes here, consuming the table data from the dashboard
  }
...
* The `d3-inetsoft.js` plugin should be included as an external script.
* Your D3.js code should be inserted in the `loadData()` function.
* The first parameter of `d3.inetsoft()` function is the ‘Name’ of the *data table* in the Dashboard. See Step 2 below for more information about the data table.
