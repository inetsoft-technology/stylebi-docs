...
// metadata for AI
ifdef::ai[]
Summary: How to integrate a D3.js chart into a Dashboard using the InetSoft D3 plugin, covering the two-step process of creating an HTML page with the D3.js chart (uploaded to server storage) and building a Dashboard with a Text component (to embed the HTML) and a data Table or Chart component that supplies data to the D3.js chart.
endif::ai[]
...
. Create a new dashboard. (See xref:dashboard:CreateNewDashboard.adoc[] for more details.)

. Add a Text component into the Dashboard. Set the content of the Text component to the URL of the HTML file from Step 1 above (e.g., `/d3.html`). (See xref:dashboard:AddTextOrHTML.adoc[] for more information about using a Text component.)

. Resize the Text component to  be able to display the D3.js chart.

. Add a Table or Chart component that contains the data to be used by the D3.js chart. (See xref:dashboard:AddTable.adoc[] or xref:dashboard:AddCrosstab.adoc[] for more details.)

. *_Optional:_* Set the table or chart to be *hidden* if you do not want users to see it. (See xref:dashboard:GeneralTab.adoc[] for information on how to hide a component.)

. Set the ‘Name’ of the Table or Chart to the ID used as the first parameter to the `d3.inetsoft()` function. (This is “D3DataTable“ in the HTML example above. See xref:dashboard:GeneralTab.adoc[] for more information about the ‘Name’ property.)