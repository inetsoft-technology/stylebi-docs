...
// metadata for AI
ifdef::ai[]
Summary: How to configure and manage materialized views (also called query cache, pre-aggregated data, performance cache, or cached query) in Enterprise Manager, which pre-aggregate Dashboard data in a cache to dramatically improve query performance for end users.
endif::ai[]
...
+
image::Figure 15.png[alt="select Repository tab in Content page"]
...
+
image::Materialized View Create page_1.png[alt="select MV tab for dashboard materialized view"]
...
+
image::Materialized View Apply VPM.png[alt="apply or bypass VPM options panel"]
...
+
image::Materialized View Candidates_1.png[alt="materialized view candidates list page"]
+
[NOTE]
====
[square]
...
+
image::Materialized View Candidates Select_1.png[alt="select dashboard assets in candidates table"]
...
. *_Optional:_*  Select ‘Generate Data’ if you want to immediately perform the full materialization process for the selected Dashboards upon pressing the btn:[Create/Update] button. (This may take a significant amount of time, during which the materialized view is constructed and data files are saved. The  data  file names are generated automatically and cannot be changed.)
...
+
image::Materialized View Notice_1.png[alt="materialized view generation completion notice"]
+
...
TIP: You can also access materialized view status from the Web API  endpoints.  See xref:integration:AccessInetSoftAPIMethods.adoc[] and xref:user:RESTJSON.adoc[] for details on how to access API endpoints from a datasource.
...
+
image::Materialized View Manage page_1.png[alt="materialized views management table listing"]
+
...
Size (MB)::	The combined size of all the materialized view data files, not including the back-up.
...
+
image::Materialized View Manage Filter_1.png[alt="filter materialized views by status cycle or user"]
...
+
image::Materialized View Manage Column Select.png[alt="select columns to display in MV table"]
...
+
image::Materialized View Node_1.png[alt="materialized views global options page"]
...
+
****
Create MV On-demand:: Set '`Create MV On-demand`' to specify that the ‘Dashboard Options’ dialog box in Visual Composer should offer the ‘Create MV On-demand’ option for individual Dashboards. When ‘Create MV On-demand’ is enabled for a Dashboard, a new materialized view is automatically created on-the-fly if the Dashboard does not have an existing materialized view (either because the materialized view was never populated with data or because the Dashboard underwent modifications that rendered an existing materialized view unusable). The materialized view will supply data to the Dashboard both at design time and at runtime. See xref:dashboard:DashboardOptions.adoc[]  for information on setting this option.
+
image::dashboard:Options Create MV on Demand_1.png[alt="create MV on-demand option in dashboard options"]
...
image::Materialized View All Users In Group.png[alt="create materialized view for all users in group option"]
...
image::MaterializedViewTransform1.png[alt="data worksheet with filters on precursor data blocks"]
...
image::MaterializedViewTransform2.png[alt="data worksheet with filters moved to resultant data block"]
====
