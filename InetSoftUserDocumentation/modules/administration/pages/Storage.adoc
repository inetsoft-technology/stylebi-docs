...
// metadata for AI
ifdef::ai[]
Summary: How to manage StyleBI storage in Enterprise Manager, including managing stored assets and files in the Content > Storage tab, uploading and managing database drivers and plugins, changing the backend storage method (MapDB, AWS DynamoDB/S3, Azure CosmosDB/Blob, Google Firestore/GCS, JDBC, MongoDB) via docker-compose.yaml, and backing up stored assets.
endif::ai[]
...
+
image::Figure 11.png[alt="view Storage configuration page in General settings"]
+
...
+
image::DataSpaceTab.png[alt="select Storage tab on Content page"]
...
+
image::Figure 12.png[alt="Storage Folder Definition panel"]
...
+
image::Figure 13.png[alt="Storage File Definition panel"]
+
[square]
...
+
image::Drivers_Plugins1.png[alt="Drivers and Plugins tab in Content page"]
...
+
image::Drivers_Plugins2.png[alt="upload plugin Zip file to server"]
+
The uploaded plugin is shown in the table below the buttons.
+
image::Drivers_Plugins3.png[alt="uploaded plugin shown in plugins table"]
...
+
image::Drivers_Plugins6.png[alt="select Upload option in Create Driver dialog"]
...
+
image::Drivers_Plugins7.png[alt="choose driver JAR files to package into plugin"]
...
+
image::Drivers_Plugins8.png[alt="enter Plugin ID Name and Version fields"]
...
+
image::Drivers_Plugins9.png[alt="enter Maven Coordinates in Create Driver dialog"]

.. Press btn:[Next], and choose the desired driver file or files to package into a plugin.
+
image::Drivers_Plugins10.png[alt="choose driver files from Maven repository to package"]
...
+
image::Drivers_Plugins5.png[alt="check plugin checkbox and press Uninstall button"]

. Restart the server.

[#ChangetheStorageMethod]
== Change the Storage Method

[WARNING,caption=see also]
====
If you are configuring the Scheduler to dispatch tasks to cloud-native infrastructure, see xref:ConfigureCloudRunner.adoc[].
====
...
IMPORTANT:  If you change the key-value storage from the default MapDB, you will also need to change the specified  audit database (see xref:Audit.adoc#ConfiguretheAuditDatabase[Configure the Audit Database]).
...
INETSOFTCONFIG_BLOB_CACHEDIRECTORY:'/var/lib/inetsoft/blobCache'
...
INETSOFTCONFIG_BLOB_S3_PATHSTYLEACCESS:true
...
INETSOFTCONFIG_KEYVALUE_DATABASE_JDBCURL:"jdbc:h2:/data_dir/kv/kv;
MODE=Derby;AUTO_SERVER=TRUE;AUTO_SERVER_PORT=8192;AUTO_RECONNECT=TRUE"
...
. Select the *General* page in the left panel, and select the *Storage* page in the right panel.
+
image::Figure 11.png[alt="select Storage page under General settings"]
...
+
image::Figure 14.png[alt="enter backup filename in Storage Backup dialog"]
...
+
image::scheduleBackup2.png[alt="open Tasks tab for asset file backup scheduled task"]
...
The number of backup copies to retain is controlled by the property `asset.backup.count` (default=3). {EMPropertyConfig} Note that large backup count may consume significant storage space and should be monitored regularly.
