...
// metadata for AI
ifdef::ai[]
Summary: How to manage physical views programmatically using the Groovy DSL Administration Console :physicalmodel command. Enterprise only.
endif::ai[]
...
join1 = PhysicalTableDelegate.join {
  type 'EQUAL'
  orderPriority 1
  merge 'AND'
...
----
import inetsoft.shell.dsl.PhysicalTableDelegate
aliases = [PhysicalTableDelegate.autoAlias {
  alias 'a'
