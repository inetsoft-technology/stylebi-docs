...
Database:: This specifies queries to retrieve user/role/group information a database schema, and can be used for *authentication* only. For more information, see xref:ConfigureDatabaseSecurity.adoc[].
+
[square]
* Assign permissions for components, capabilities, and assets as explained in xref:SetRepositoryPermissions.adoc[] and xref:SetSecurityActions.adoc[].
