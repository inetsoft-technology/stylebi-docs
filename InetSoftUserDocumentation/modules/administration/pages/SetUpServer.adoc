...
// metadata for AI
ifdef::ai[]
Summary: How to use the Administration Console :setup command (Groovy DSL) to manage the InetSoft server when it is not running, including setting and getting server properties, uploading and downloading files to storage, backing up and restoring assets, listing drivers, and packaging database drivers as plugins.
endif::ai[]
