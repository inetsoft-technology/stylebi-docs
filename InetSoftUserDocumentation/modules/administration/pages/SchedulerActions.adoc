...
// metadata for AI
ifdef::ai[]
Summary: Reference for scheduled task actions and options in StyleBI, covering the three action types (Dashboard, Backup, Batch), delivery options (email notification, deliver to emails, save to disk), creation parameters, schedule options, and how to insert dynamic parameters into email subjects and filenames.
endif::ai[]
...
image::user:schedulerCreationParamaters.png[alt="assign creation parameters for scheduled Dashboard action"]

IMPORTANT: If the generated Dashboard does not reflect the most *recent data*, the Scheduler may be using a cached dataset. In this case, you can either increase the separation of your scheduled tasks or reduce the '`Data Cache Timeout`'.  See xref:TailorServerPerformance.adoc[] for details about this property.
...
image::schedulerChooseBookmarks.png[alt="add bookmarks to scheduled Dashboard action"]
...
image::schedulerAdminNotification1.png[alt="configure notification email for scheduled task"]
...
image::schedulerAdminDeliverToEmails_dashboard.png[alt="configure deliver to emails action for Dashboard"]
...
image::schedulerAdminSaveToServer.png[alt="configure save to disk action for scheduled Dashboard"]
...
image::Schedule4.png[alt="configure scheduler task options tab settings"]
...
== Backup Action
IMPORTANT: The backup action is available only in Enterprise Manager.
...
+
image::scheduleBackup1.png[alt="select assets to add to backup task"]
...
+
image::scheduleBatch1.png[alt="select scheduled task for batch action"]
...
+
image::scheduleBatch2.png[alt="add parameter in embedded batch parameters dialog"]
...
+
image::scheduleBatch4.png[alt="choose Data Worksheet for batch query parameters"]
+
...
+
image::scheduleBatch3.png[alt="Data Worksheet with batch parameter value sets"]
...
+
image::scheduleBatch5.png[alt="map columns to batch parameter names from data block"]
+
...
+
image::scheduleBatch6.png[alt="parametrize email properties in query parameters dialog"]
