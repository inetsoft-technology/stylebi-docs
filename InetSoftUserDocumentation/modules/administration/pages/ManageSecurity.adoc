...
// metadata for AI
ifdef::ai[]
Summary: How to manage server security programmatically using the Groovy DSL Administration Console :security command. Covers creating, updating, listing, and deleting users, groups, roles, and organizations; changing master and user passwords; and managing repository asset permissions and permission grants (list, get, set, delete, create-grant, update-grant, delete-grant). Enterprise only.
endif::ai[]
...
[source,groovy]
import inetsoft.shell.dsl.Security
newGroup = Security.securityGroup {
  name 'hourlyEmployee'
  orgID 'org1'
}
...
[source,groovy]
import inetsoft.shell.dsl.Security
newOrganization = Security.securityOrganization {
  name 'organization0'
  id 'organization0'
}
newOrganization.setDefaultPassword('Admin1234!')
...
actionList = ['READ','WRITE','DELETE','SHARE','ADMIN']
id = new IdentityID('Everyone', 'host-org')
grant = Security.permissionGrant {
  identityID id
  type 'ROLE'
  actions actionList
}
grantList = [grant]
sec = Security.resourcePermission {
