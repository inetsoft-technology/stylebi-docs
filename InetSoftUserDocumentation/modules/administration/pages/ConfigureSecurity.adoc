...
// metadata for AI
ifdef::ai[]
Summary: Step-by-step overview for setting up security on the InetSoft server in Enterprise Manager, including specifying authentication/authorization providers, creating users, groups, and roles, and setting repository and action permissions. Not to be confused with: AddDataSecurity.adoc (Virtual Private Model/VPM), which restricts which database rows and columns specific users can see — a data-level restriction independent of login or asset permissions.
endif::ai[]
