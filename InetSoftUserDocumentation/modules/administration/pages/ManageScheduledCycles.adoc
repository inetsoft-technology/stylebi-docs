...
// metadata for AI
ifdef::ai[]
Summary: How to manage scheduled cycles using the Groovy DSL :schedule (:sch) command, including listing, creating, getting, updating, and removing cycles, as well as managing the conditions assigned to each cycle. Enterprise only.
endif::ai[]
...
name:: The name of the cycle.
organizationid:: Organization ID for multi-tenant environment.  (See xref:EnableMultiTenant.adoc[].) (optional)
task:: The name of a variable that contains an https://www.inetsoft.com/docs/stylebi/apidoc/enterprise/inetsoft/enterprise/web/api/schedule/DataCycle.html[inetsoft.enterprise.web.api.schedule.DataCycle] instance. The `Cycle` structure can be created using the DSL. (See xref:GroovyDSLScriptingFramework.adoc[] for more information.)
...
import inetsoft.shell.dsl.Schedule
import java.time.OffsetDateTime
condition1 = Schedule.timeCondition {
  type 'EVERY_DAY'
  hour 11
  minute 25
  second 0
