...
// metadata for AI
ifdef::ai[]
Summary: How to upload a CSS file to the server in Enterprise Manager to apply to all Dashboards or to specific users, groups, or roles. See CreateCSSDefinitionFile to create the file first.
endif::ai[]
...
+
image::PresentationGlobalSettings.png[alt="select Global Settings tab on Presentation page"]
+
...
+
image::PresentationOrgSettings.png[alt="select Organization Settings tab and choose organization"]
+
...
+
image::Global Look and Feel.png[alt="select Look and Feel page in right panel"]
...
+
image::uploadCSSFile.png[alt="uncheck Default Dashboard CSS and click to upload custom file"]
