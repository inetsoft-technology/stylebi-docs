...
// metadata for AI
ifdef::ai[]
Summary: How to monitor and manage scheduled tasks using the Groovy DSL Administration Console :schedule command, including adding, editing, running, removing, and checking task status. Enterprise only.
endif::ai[]
...
import inetsoft.shell.dsl.Schedule
import inetsoft.sree.security.IdentityID
ownerID = new IdentityID("admin", "host-org")
task1 = Schedule.task {
  name 'New Name'
  owner ownerID
...
import inetsoft.shell.dsl.Schedule
import java.time.OffsetDateTime
condition = Schedule.timeCondition {
  type 'AT'
  date OffsetDateTime.parse('2019-01-01T00:00:00Z')
}
...
ems = ['sarapeng@inetsoft.com']
action2 = Schedule.viewsheetAction {
