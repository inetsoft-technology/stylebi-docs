...
// metadata for AI
ifdef::ai[]
Summary: How to enable multi-tenancy in Enterprise Manager to support multiple organizations (tenants) with isolated data connections and assets. Covers the Domain and Path URL routing options for identifying the tenant organization, including sample HAProxy configuration. Enterprise only.
endif::ai[]
...
+
image::EM_Security_Multitenant1.png[alt="enable multi-tenancy on security page"]

. Select one of the options from the '`Pass Organization By`' menu.
+
image::EM_Security_Multitenant2.png[alt="select pass organization by option"]
