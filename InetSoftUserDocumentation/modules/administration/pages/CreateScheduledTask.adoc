...
// metadata for AI
ifdef::ai[]
Summary: How to create and manage scheduled tasks, data cycles, alerts, and automated alerts in Enterprise Manager. Covers defining task conditions and actions, scheduler options (Execute As, locale, date range), creating data cycles for materialized view automation, configuring highlight-based or script-based alerts, organizing tasks into folders, and monitoring task status. For portal users scheduling their own Dashboard delivery, see ScheduleDashboard.adoc in the user module.
endif::ai[]
...
+
image::schedulerTasksTab.png[alt="select Tasks tab on Schedule page"]
...
. Select the *Conditions* tab and press the btn:[Add] button to specify a condition to determine when the task will execute. See xref:SchedulerConditions.adoc[] for details.  To copy an existing condition, select the condition in the list and press the btn:[Copy] button.
+
image::scheduleTaskCopy.png[]
+
TIP: To run a task *on demand*, see xref:#MonitorScheduledTasks[] below.

. Select the *Actions* tab and press the btn:[Add] button to specify the type of action to take when the scheduled task executes. See xref:SchedulerActions.adoc[] for details. To copy an existing action, select the action in the list and press the btn:[Copy] button.
+
image::scheduleActionCopy.png[]
...
+
image::schedulerDataCyclesTab.png[alt="select Data Cycles tab on Schedule page"]

include::partial$selectOrganization.adoc[]

. Press the btn:[New Cycle] button to create a new cycle.
+
image::newCycleLink.png[alt="press New Cycle button to create a cycle"]
...
+
image::Figure 35.png[alt="set cycle name and time condition"]
...
+
To *copy* the cycle`'s current permissions, press the btn:[Copy Permissions] button below the permissions table.  You can then navigate to another cycle and paste the permissions to that cycle. To paste permissions you have already copied from another cycle, press the btn:[Paste Permissions] button.
+
TIP: The btn:[Paste Permissions] button will display numbers in parentheses, such as btn:[Paste Permissions (x of n)].  The *second  value* (*n*) indicates the number of permissions that were copied to the clipboard.  The *first  value* (*x*) indicates the  number of permissions that  are compatible  with the   current cycle, and can actually be pasted.
...
+
image::multipleCycles.png[alt="multiple cycle conditions listed with Add button"]
...
* xref:dashboardscript:ChartHighlighted.adoc[Chart.highlighted], xref:dashboardscript:TableHighlighted.adoc[Table.highlighted], xref:dashboardscript:ImageHighlighted.adoc[Image.highlighted], xref:dashboardscript:TextHighlighted.adoc[Text.highlighted] to determine in Dashboard script whether a highlight has been applied or not.
====
...
+
image::Alert2_1.png[alt="select Dashboard for alert on Actions tab"]
...
+
TIP: See xref:dashboardscript:AddScriptToDashboard.adoc[] for more information on where to add script.
+
...
image::schedulerFolder1.png[alt="select New Folder from more options menu"]
...
+
image::schedulerFolder2.png[alt="select task and press Move Task button"]

. Choose the desired folder into which to place the task, and press ‘OK’.
+
image::schedulerFolder3.png[alt="choose destination folder for task"]
...
image::Figure 34.png[alt="scheduled tasks table showing status and run times"]

To temporarily disable or enable a task, use the checkbox under the ‘Enabled’ column.

The '`Distribution`' chart provides a breakdown of daily tasks, including both enabled  and disabled tasks.

image::ScheduleTaskDistribution.png[alt="Distribution chart showing daily task breakdown"]

You can click a bar on the chart to zoom in.  Press the {buttonBack}  button at the top-right to return to the original view.

image::ScheduleTaskDistribution2.png[alt="zoomed distribution chart view after clicking a bar"]
...
image::schedulerTaskRunNow.png[alt="select Run Now from task more options menu"]
