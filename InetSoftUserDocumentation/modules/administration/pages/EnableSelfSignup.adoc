...
// metadata for AI
ifdef::ai[]
Summary: How to enable the self-signup feature in Enterprise Manager that lets users create their own accounts from the login page, adding them to a dedicated Self Organization. Enterprise only.
endif::ai[]
...
image::selfsignup3.png[alt="create an account link on login page"]
...
+
image::Figure 23_1.png[alt="enable security on security settings page"]
...
+
image::selfsignup1.png[alt="enable self signup checkbox on security page"]

. To enable users to sign in with Google, select the *Sign In With Google* tab.
+
image::selfsignup2.png[alt="sign in with Google tab settings"]
