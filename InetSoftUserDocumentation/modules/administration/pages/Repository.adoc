...
// metadata for AI
ifdef::ai[]
Summary: How to manage the repository in Enterprise Manager, covering sorting repository assets, creating folders, deleting and restoring assets, configuring Dashboard properties and security, pinning Pinboards for users, and managing Data Worksheet properties.
endif::ai[]
...
+
image::Figure 15.png[alt="repository tab in content page showing asset tree"]
...
+
image::RepositoryNewFolder.png[alt="new folder option in repository new circle menu"]
...
+
image::Figure 18.png[alt="folder property tab with name and description fields"]
...
+
image::reportSecurity2.png[alt="add permissions to folder security tab"]
...
+
image::archiveRestore.png[alt="restore button to recover asset from recycle bin"]
...
To add a Dashboard to the Repository, use Visual Composer to save the Dashboard into a global scope (see xref:dashboard:SavePublicDashboard.adoc[]) or import the Dashboard (see xref:ImportExportAssets.adoc[]).To modify properties of a Dashboard, follow the steps below:
...
+
image::Viewsheet Definition Page.png[alt="dashboard tab with name alias and description fields"]
+
...
+
image::reportSecurity4.png[alt="add permissions to dashboard security tab"]
...
+
image::Viewsheet MV Page.png[alt="MV tab for creating dashboard materialized view"]
+
...
A *Pinboard* is a Dashboard that has been added to the {buttonDashboard} in the User Portal for quick, repeated access. To pin a Dashboard, follow the steps below:
...
. To add a Dashboard *for all users*, select the ‘Pinboard Tab’ folder on the tree. To add a Dashboard for a *particular user only*, select the ‘`User Pinboard Tab`’ folder, and choose the desired user.

. Press the {buttonNewCircle} above the tree, and select ‘New Dashboard’.
+
image::newDashboard.png[alt="new dashboard option in repository new circle menu"]
...
+
image::Dashboard Properties Dialog VS_1.png[alt="dashboard property tab with name and description fields"]

. Choose a Dashboard from the ‘Select Dashboard’ list.
+
image::Edit Dashboard Admin VS.png[alt="select dashboard from list to pin as Pinboard"]
...
The new Dashboard is now listed in the ‘Pinboard Tab’ folder, and will be visible to the users who have appropriate permissions. To delete a Dashboard, select the Dashboard and press the {buttonDelete} above the repository list.

[#ManageDataWorksheets]
...
+
image::Materialized View Worksheet1.png[alt="expand data worksheets node and select worksheet"]
