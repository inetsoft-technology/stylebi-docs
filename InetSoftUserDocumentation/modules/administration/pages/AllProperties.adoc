...
schedule.task.timeout:: The maximum duration in milliseconds for  scheduled tasks and materialized view creation. Default is `600000` (ten minutes); an invalid (non-numeric) value also falls back to this default. Set to `0` for no timeout. Tasks dispatched via the cloud runner are an exception: a value of `0` or less is clamped to the `600000` default instead of waiting unbounded, since cloud runner execution consumes billed cloud compute. The timeout ensures that a task cannot persist   indefinitely.  When the timeout is reached, the job  is terminated regardless of whether the task is in a failed state or running successfully.
...
security.cache:: Caches identity lookups (such as user roles and group memberships) for improved performance. This applies to any security provider (LDAP, database, or custom), not just LDAP.

security.allow.iframe:: Set to `false` (default) to add the `X-Frame-Options` header (`SAMEORIGIN`) to server responses generally, preventing the application -- not only the Enterprise Manager and User Portal login pages -- from being displayed within an IFrame. Set to `true` to omit that header, which may be necessary when you want to embed StyleBI within an IFrame; this should be used in conjunction with Single Sign-On (SSO), and the server must be using SSL in this case. See xref:integration:ConfigureSingleSignOn.adoc[] in xref:integration:AdvancedServerIntegration.adoc[]   for more information.
+
Setting this to `true` also changes session cookie behavior: the session cookie's `SameSite` attribute is set to `None` (rather than the default `Lax`) so the cookie can still be sent with the cross-site requests an embedded IFrame produces.
