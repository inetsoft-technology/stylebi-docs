...
// metadata for AI
ifdef::ai[]
Summary: Reference page in Enterprise Manager for viewing and editing all server configuration properties. Explains how to find, add, edit, and delete properties using the All Properties table. Covers chart, data source, and export properties, and notes that file path values can be absolute or relative to the repository directory.
endif::ai[]
:availableDataSources: JDBC, Microsoft Access, Oracle, SQL Server, MySQL, DB2, jTDS, Ingres, Sybase, Derby, Derby Embedded, Informix, SQL Anywhere, PostgreSQL, Dremio, Impala, Vertica, Teradata, SAP HANA, Snowflake, Actian, Amazon Aurora/RDS MySQL, Amazon Aurora/RDS PostgreSQL, AWS EMR, AWS Redshift, Aster Data nCluster, Cisco Info Server, Exasol, Google Cloud SQL, IBM AS400, MapR, MariaDB, MemSQL, MonetDB, Pivotal Greenplum, Progress OpenEdge, FileMaker, LucidDB, Netezza, SQLite, SQL Server Express, Google BigQuery, Siebel CRM (Oracle), Siebel CRM (SQL Server), HPCC, OrientDB, Aerospike, Apache Drill, Presto, PHOENIX, Google Spreadsheets, SAP ABAP, SAP BAPI, SAP Tables, R, Facebook Ad Insights, Google Analytics, Sharepoint Online, Apache Hive, Data.gov JSON, OData, MongoDB, Cassandra, Couchbase, Redis, Cosmosdb, Salesforce, REST JSON, REST XML, Rest.ActiveCampaign, Rest.Airtable, Rest.Appfigures, Rest.Asana, Rest.AzureBlob, Rest.AzureSearch, Rest.CampaignMonitor, Rest.Chargebee, Rest.Chargify, Rest.ChartMogul, Rest.ConstantContact, Rest.Copper, Rest.DataDotWorld, Rest.Firebase, Rest.FortyTwoMatters, Rest.Freshdesk, Rest.Freshsales, Rest.Freshservice, Rest.GitHub, Rest.GitLab, GoSquared, Rest.GoogleCalendar, GraphQL, Rest.Harvest, Rest.HelpScoutDocs, Rest.HubSpot, Rest.InfluxDB, Rest.Insightly, Rest.Intervals, Rest.Jira, Rest.Jive, Rest.Lighthouse, Rest.Linkedin, Rest.LiveAgent, Rest.Mailchimp, Rest.Mixpanel, monday.com, Rest.Nicereply, Rest.Pipedrive, Rest.PipelineDeals, Rest.Prometheus, Rest.SEOmonitor, Rest.SalesforceReports, Rest.SendGrid, Rest.ServiceNow, Shopify, Rest.Smartsheet, Rest.Square, Rest.Stripe, Rest.SurveyMonkey, Rest.TeamDesk, Rest.Toggl, Rest.Twilio, Rest.Twitter, Rest.WordPress, Rest.Xero, Rest.YouTubeAnalytics, Rest.Zendesk, Rest.ZendeskSell, Rest.ZohoCRM

To set a property manually, follow the steps below.   File paths  in properties can be absolute, or relative to the repository directory. Relative paths can use the `sree.home` environment variable to specify the storage root, for example, `$(sree.home)/filename.xml`.
...
+
image::allProperties.png[alt="select All Properties page in left panel"]
...
graph.area.maxcount:: The maximum number of dimension values that can be displayed in an Area Chart (default is 20,000).
graph.axis.inplot:: Specifies whether the maximum measure value is included within the plot region. (Default is `false`.) See also xref:dashboardscript:PlotInPlot.adoc[].
graph.axislabel.maxcount:: The maximum number of labels to display on a Chart axis (default is 2000).
graph.bar.maxcount:: The maximum number of dimension values that can be displayed in a Bar Chart (default is 100,000).
graph.candle.maxcount:: The maximum number of dimension values that can be displayed in a Candle Chart (default is 5,000).
...
graph.radar.maxcount:: The maximum number of dimension values that can be displayed in a Radar Chart (default is 5,000).
...
graph.stock.maxcount:: The maximum number of dimension values that can be displayed in a Stock Chart (default is 5,000).
...
map.mapping.dir:: Directory where map files are stored. Default is *_map_* in Storage root.  See xref:Storage.adoc[] for more information.
...
datasource.registry.file:: List of `.xml` files that are used to define all the available data sources.
...
query.registry.file:: List of `.xml` files that are used to define all the available queries.
...
format.date:: Format used for all date data. See xref:dashboard:DataFormat.adoc[] for more information.
...
format.time:: Format used for all time data (Date). See xref:dashboard:DataFormat.adoc[] for more information.
...
always.warn.joinMaxRows:: If set to `false` (default), the user will be shown a single warning for the session when the `join.table.maxrows` limit is violated. (See below.) Set to `true` to warn the user for each individual violation.
...
crossJoin.maxCellCount:: Sets the limit for number of cells in  the result set  of a cross join.  (Default is 500000000.)
...
query.runtime.timeout:: Sets the global timeout (seconds) for all queries during Dashboard runtime execution.  If the timeout is too short, this can cause a data-read error in some cases.
...
selectionList.maxrow:: The maximum number of items to be displayed in a Selection List or Selection Tree (default 50000).
...
table.output.maxcol:: Sets the global maximum column limit for all Tables, Crosstabs, and Freehand Tables during Dashboard runtime execution. Valid range is 1–500. Values outside this range are clamped to the nearest bound. Default is 500.
...
mail.from.address:: ‘From’ email address used in email messages. Default is `reportserver@inetsoft.com`.
mail.from.enabled:: Set to `true` to allow users to enter a ‘From’ address that is different from their own email address. Default is `false`.
...
mail.smtp.ssl.protocols:: Specifies an alternate SMTP protocol. Example:  `mail.smtp.ssl.protocols=TLSv1.2`.
...
mv.cache.data:: Set to `false` to stop materialized view result sets from being cached and shared across Dashboards. Caching is on by default.
...
font.ratio.x:: A list of ratios used to adjust horizontal character spacing. This is mostly used when the font width information is different between Java and PDF. The ratio can be used to adjust the spacing so that the generated PDF matches the actual font size used in PDF viewers. It is a list of semicolon-separated ratios. Each ratio is specified by a font name and, optionally, a font style, followed by the ratio, e.g., `MS Hei:1.1;Algerian-bolditalic:1.02`.
...
pdf.output.attachment:: Default is `embed`, which causes a PDF request to return an HTML page with an `<embed>` tag so the PDF displays inline in the browser. Any other value -- including `true` or `false` -- returns the PDF as a downloadable attachment instead; the property is not a boolean switch, only `embed` is treated as a distinct value.
...
share.email.enabled:: Enables the {buttonSocialEmail} under the {buttonSocialSharing}  in the Dashboard toolbar.  Default is `true`.

share.facebook.enabled:: Enables the {buttonFacebook} under the {buttonSocialSharing}  in the Dashboard toolbar.  Default is `true`.

share.linkedin.enabled:: Enables the {buttonLinkedIn} under the {buttonSocialSharing}  in the Dashboard toolbar.  Default is `true`.

share.twitter.enabled:: Enables the {buttonTwitter} under the {buttonSocialSharing}  in the Dashboard toolbar.  Default is `true`.

share.link.enabled:: Enables the {buttonCopyLink} under the {buttonSocialSharing}  in the Dashboard toolbar.  Default is `true`.

share.slack.enabled:: Enables the {buttonSlack} under the {buttonSocialSharing}  in the Dashboard toolbar.  Default is `false`. Requires `share.slack.url` to also be set, otherwise the option is shown disabled.

share.slack.url:: The Slack incoming webhook URL for a channel. When a user shares the Dashboard using the {buttonSlack}, the server POSTs a JSON SlackMessage to this URL containing  the Dashboard link.

share.googlechat.enabled:: Enables the {buttonGoogleHangoutsChat} under the {buttonSocialSharing}  in the Dashboard toolbar.  Default is `false`. Requires `share.googlechat.url` to also be set, otherwise the option is shown disabled.

share.googlechat.url:: The Google Hangouts Chat incoming webhook URL. When a user shares the Dashboard using the {buttonGoogleHangoutsChat}, the server POSTs a card-style JSON message to this URL containing a `<users/all>` mention and a clickable '`Click here to view`' link.

share.opengraph.title::  The fallback `og:title` `<meta>` tag injected into the User Portal's HTML `<head>` for  social sharing previews. Default is "`Business Intelligence Software | Dashboard Reporting`".

share.opengraph.description:: The fallback `og:description` `<meta>` tag injected into the User Portal's HTML `<head>` for  social sharing previews. Default is "`InetSoft is a pioneer in the field of data intelligence…`".

share.opengraph.sitename:: The fallback `og:site_name` `<meta>` tag injected into the User Portal's HTML `<head>` for  social sharing previews. Default is "`InetSoft.com`".

share.opengraph.image:: The fallback `og:image` URL `<meta>` tag injected into the User Portal's HTML `<head>` for  social sharing previews. Default is `/images/logo.png`.  (Relative paths are resolved to the server`'s base URL.)
...
portal.home.link:: Sets the URL of the ‘Home’ button in the User Portal. Default is `..`, one level above the Portal URL. This can be an absolute URL such as https://www.google.com, or a path relative to `/app/` such as  `composer`.

em.home.link:: Sets the URL of the ‘Home’ button in Enterprise Manager. Default is `..`, one level above the Enterprise Manager URL. This can be an absolute URL such as https://www.google.com, or a path relative to `/app/` such as  `composer`. See  xref:Presentation.adoc#PortalIntegration[Portal Integration] for more information.

portal.customLoadingText:: Specifies text to display on the loading screen when the user logs into the User Portal. The message is displayed under the "Please wait, the application is loading" message and logo -- but that built-in message and logo are shown only while `portal.customLoadingLogo` is empty. If a custom loading logo is configured, this text is the only text shown on the splash screen.

portal.customLogo.enabled:: When set to `true`, this reveals the '`Default Favicon`' property  xref:Presentation.adoc#LookandFeel[Look and Feel] page.  Default is `false`.  Uncheck the '`Default Favicon`' property to upload a custom favicon.

portal.logout.url:: The URL to which the user is directed when they  select '`Log Out`'. If  SSO is in effect, this redirect occurs after  the user is directed to  `sso.logout.url`.
...
wallboarding.enabled:: When set to `true`, prevents Dashboards with ‘Server-Side Update’ enabled from timing out due to inactivity. This is useful for Dashboards that are publicly displayed (as  “wallboards”) for long periods of time without any user interaction. See xref:dashboard:DashboardOptions.adoc[] for information about the ‘Server-Side Update’ Dashboard option.

script.cursor.top:: When set to `true`, places the cursor at the top of the script when the Script Editor is opened.  When `false` (default) the cursor appears at the bottom of the script when the Script Editor is opened.
...
schedule.java.opts:: Specifies additional Java options and parameters to be passed to the Scheduler instance.
schedule.auto.start::  Set to `true` (default) to allow the scheduler to be started automatically on initialization of the repository servlet. This property applies only to a development environment running from *source*; it has no effect in a container deployment, where StyleBI's supported configuration is a separate scheduler container rather than a second long-lived process inside the main container. In a container, this property is set to `false` at first storage initialization regardless of the `defaults.properties` default of `true`, and the `INETSOFTENV_SCHEDULE_AUTO_START` environment variable does not override this. If you need to specify additional parameters to the Java executable when using this  auto-start option, add a `schedule.java.opts` property. (See above.)
...
schedule.task.timeout:: The maximum duration in milliseconds for  scheduled tasks and materialized view creation. Default is `600000` (ten minutes). Set to `0` for no timeout. When the timeout is reached, the job  is terminated regardless of whether the task is in a failed state or running successfully.
+
[IMPORTANT]
Tasks dispatched via the *cloud runner* are an *exception*: A value of `0` or less is clamped to the `600000` default instead of waiting unbounded. This ensures that a cloud-billed task cannot persist   indefinitely.
...
openid.property.provider:: Fully-qualified class name of an implementation of `inetsoft.enterprise.sso.OpenIDPropertyProvider` that provides the OpenID Connect configuration for a given HTTP request.
openid.postprocessor:: Fully-qualified class name of an implementation of `inetsoft.enterprise.sso.OpenIDPostprocessor` that modifies the SRPrincipal created by the SSO filter given the set of claims returned from the IdP.
role.administrator:: Only users with this role can log into the EM and administer the  server.
security.cache:: Caches identity lookups (such as user roles and group memberships) for improved performance. This applies to any security provider (LDAP, database, or custom).

security.allow.iframe:: Set to `false` (default) to add the `X-Frame-Options` header `SAMEORIGIN` to server responses generally, preventing the StyleBI application (not only the Enterprise Manager and User Portal login pages) from being displayed within an IFrame. Set to `true` to omit that header, which may be necessary when you want to embed StyleBI within an IFrame; this should be used in conjunction with Single Sign-On (SSO), and the server must be using SSL in this case. See xref:integration:ConfigureSingleSignOn.adoc[] in xref:integration:AdvancedServerIntegration.adoc[]   for more information.
+
Setting this to `true` also changes session cookie behavior: the session cookie's `SameSite` attribute is set to `None` (rather than the default `Lax`) so the cookie can still be sent with the cross-site requests produced by an embedded IFrame.

sso.logout.url:: The URL to which the user is directed when they  select '`Log Out`'. After SSO logout occurs, the user is redirected to the URL defined at `portal.logout.url`.
...
security.exposedefaultorgtoall:: When set to `true`, indicates that *all* organizations can see the Host Organization`'s Dashboards.  (The '`Host Organization`' is the default organization that is administered by the Site Administrator. See xref:CreateOrganization.adoc[] for more details.)  When set to `true` for a specific organization, e.g., `security.org0.exposedefaultorgtoall=true`, only the specific organization (`org0`) can see the Host Organization`'s Dashboards. The default  is `false`, which means other organizations cannot see the Host Organization`'s Dashboards.
...
asset.backup.count:: Number of copies of the storage backup to retain. (Default=3.) See xref:Storage.adoc[] for more information.  Note that large backup count may consume significant storage space and should be monitored regularly.
...
html.image.directory:: Location to which images should be uploaded for  the  xref:dashboard:ImageProperties.adoc#GeneralTab[Static Image] property of Image components. The  path should be specified relative to the root storage location.  For example,  specify the value `serverImage/myImages` to force images to upload into the _serverImage/myImages_ folder in server storage.  The specified location must already exist on the server.  See xref:Storage.adoc#ManageStoredAssets[Manage Stored Assets] for information on how to create a folder in server storage.
...
mysql.server.timezone:: Set to a valid Java time zone ID to indicate the StyleBI server time zone (e.g., `US/Eastern`). If not defined, the default time zone is used.
...
ai.assistant.visible:: When enabled (default) displays the {buttonAssistant} in the navigation bar of the User Portal.

chat.app.internal.url:: When specified, this enables the reverse-proxy mode for the AI assistant so that the browser never communicates to the AI Assistant directly. Communications are directed through the proxy URL which lets you deploy the AI Assistant at an internal network address that browsers cannot reach. The internal URL is never exposed to the client. Leave this setting blank when using the direct mode (see `chat.app.server.url`).

chat.app.server.url:: When a proxy URL is NOT provided for `chat.app.internal.url`, this provides the public-facing URL of the AI Assistant service. StyleBI returns this URL directly to the browser, which then connects to the AI Assistant server directly ("direct mode"). If `chat.app.internal.url` is specified, this URL is ignored.

chat.app.title:: Custom name to display for the AI Assistant in the history sidebar.

chat.app.vendor.name:: Custom name to display for the AI Assistant in the AI Assistant header.

chat.app.logo.url:: URL for a custom logo image to display in the AI Assistant header. This can also specify a server storage path to an internally saved image (e.g., `portal/shapes/logo.png`).