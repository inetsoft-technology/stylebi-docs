...
schedule.auto.start::  Set to `true` (default) to allow the scheduler to be started automatically on initialization of the repository servlet. (This property is only utilized when running from *source*). If you need to specify additional parameters to the Java executable when using this  auto-start option, add a `schedule.java.opts` property. (See above.)
...
schedule.task.timeout:: The maximum duration in milliseconds for  scheduled tasks and materialized view creation. Default is 0 (no timeout). The timeout ensures that a task cannot persist   indefinitely.  When the timeout is reached, the job  is terminated regardless of whether the task is in a failed state or running successfully.
