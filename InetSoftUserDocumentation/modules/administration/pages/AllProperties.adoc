...
datasource.selectedSource:: Specifies the datasource for which to refresh metadata.
...
driver.directory:: A comma-separated list of directory paths to search for driver files. Default is `sree.home/drivers`. Only the final directory in the list can be edited (add or remove driver files) in Enterprise Manager. Other directories on the list are read-only in Enterprise Manager.
...
excel.close.factor:: For Excel export, a factor determining whether nearby  elements are placed in adjacent cells or spaced apart. A smaller factor creates more space between elements in the exported spreadsheet.
excel.colwidth.max:: For export to Excel (best data editing), specifies the maximum permitted Excel column width. If an element width exceeds the maximum, it is represented in Excel by merging multiple columns with widths given by `excel.colwidth.normal`.
excel.colwidth.normal:: For export to Excel (best data editing), specifies the column width to use for merged columns when an element’s width exceeds `excel.colwidth.max`. A value of `-1` indicates that no merging should be performed.
excel.export.grid.show:: When `true`, specifies that gridlines will be shown in the Excel export, and that default text backgrounds are transparent. When `false`, gridlines are suppressed and default text backgrounds are white.
excel.font.ratio:: Sets the proportion by which font size is globally reduced for Excel exports. (Font size reduction can often improve the appearance of Excel exports.) For example, setting `excel.font.ratio=0.85` uniformly reduces font sizes within the export to 85% of their size in the original. To set this proportion on a font-by-font basis, use the property `template excel.font.<fontname>.ratio`, e.g., `excel.font.arial.ratio=0.80`.
excel.rowheight.max:: For export to Excel (best data editing), specifies the maximum permitted Excel row height. If an element height exceeds the maximum, it is represented in Excel by merging multiple rows with heights given by `excel.rowheight.normal`.
excel.rowheight.normal:: For export to Excel (best data editing), specifies the row height to use for merged rows when an element’s height exceeds `excel.rowheight.max`. A value of `-1` indicates that no merging should be performed.
...
export.csv.quote:: Quote character used when exporting a CSV file.
export.Excel.isHeaderFooterInPageSetup:: If `true`, top and bottom margins, and headers and footers will be set in Excel’s “Page Setup”.
export.menu.options:: A comma separated list of option numbers. Only the options on the list are added to the export menu. e.g: `PDF, EXCEL, EXCEL_SHEET, EXCEL_DATA, RTF, RTF_LAYOUT, HTML, HTML_NO_PAGINATION, HTML_BUNDLE, HTML_BUNDLE_NO_PAGINATION, CSV, SVG, TEXT, REPORT.`
...
html.export.embed:: Set to `true` to embed the output of the export (from the ‘Export’ dialog box) into the same frame and replace the existing contents.
...
image.type:: `PNG` or `JPEG`. PNG can only be used with version 4.0 or newer browsers. Generally PNG results in an image with a smaller file size, and better quality than JPEG.
html.image.count:: Specifies the maximum number of images cached on the  server. Reducing the number of cached images can help alleviate certain server memory issues.
html.image.scale:: Set to `true` to scale images using x/y ratio; otherwise, natural size is used, regardless of the size setting.
html.image.xratio:: When generating HTML, images are converted to JPEG using a resizing ratio of 1.0. This sometimes causes the image to be distorted. This default can be changed by setting this property.
html.image.yratio:: When generating HTML, images are converted to JPEG using a resizing ratio of 1.0. This sometimes causes the image to be distorted. This default can be changed by setting this property.
rtf.image.scale:: Boolean that determines whether the image is exported in its original size in RTF format.
stylereport.dither:: True to dither image when printing to black/white printer. Default is `false`.
stylereport.ditherImage:: `true` to force all images to be dithered before printing. Default is `false`.
...
excel.sheet.maxrows:: Sets the integer row limit for every table within an Excel export. A message is displayed below each limited table to indicate that the data in the table has been truncated.
http.session.maxActive:: Maximum number of active sessions (default=10000). If a new session is created when the number of active sessions exceeds this property, the oldest session is invalidated.  You must restart the server after changing this property.
...
http.session.timeout:: HTTP session timeout in seconds (default =1800 seconds). If the session timeout is set in the _web.xml_ file, that setting takes precedence over this property.  You must restart the server after changing this property.
...
log.view.maximum:: The maximum number of lines to return when the ‘All’ option is selected on the ‘Log View’ page.
log.custom.config:: Enables custom logging, equivalent to ‘Use Custom Log Configuration’. See xref:Logging.adoc[] for more information.
log.level.com.hazelcast:: Sets log level for `com.hazelcast`.
...
mail.attachment.max:: Maximum size for a PDF attachment on an email. If the PDF file is larger than the maximum size, the PDF is split into multiple parts.
...
mail.split.message:: Message used when a PDF file is split into multiple parts.
...
system.admin.address:: Email address of system administrator for the  error page.
...
default.browser:: This defines the default browser used on Unix machines.  It defaults to Mozilla.

dhtml.error.page:: Defines the  error page.

exceed.session.page:: Contains a path to an HTML file that contains a message to display when the maximum number of concurrent users has been exceeded. If this is not set, the default message is used.
...
html.offset.x:: The distance of the generated HTML contents to the left edge of the browser.

html.offset.y:: The distance from the generated HTML contents to the top of the browser.


html.page.button:: Set to `false` to remove ‘pass:[Page ____ of ____]’.

html.page.frame:: Frame name for page view (only used if `html.page.window` is set to `false`).


html.page.window:: Determines whether a new window is created when a Dashboard is launched from the repository view (`true` or `false`).
...
html.repository.footer:: HTML codes to insert at the end of the repository window.

html.repository.header:: HTML codes to insert at the beginning of the Repository window. If set to `none`, no welcome message will be displayed above the Repository listing.

html.repository.help:: Set to `false` to suppress display of the ‘Help’ link below the Repository tree.

html.repository.organize:: Set to `false` to suppress display of the ‘Organize’ link below the Repository tree.

html.repservlet.title:: The Portal page title. If unspecified, a default title  will be used.



html.tblheading.hide:: Set to `true` to include table and section grouping headers in the TOC.


html.toolbar.background:: Specifies the background color of the toolbar. The value is a Hex triplet, prefixed by a `#`.

html.toolbar.font:: Specifies the foreground color of the toolbar. The value is a Hex triplet, prefixed by a `#`.

html.toolbar.location:: Specifies the location of the toolbar on the HTML page: `Top`, `Bottom`, `Left`, `Right`, or `Top_and_Bottom`.

html.toolbar.text:: Set to `true` to use text-based toolbar. Default to use image buttons.

html.viewer.frameset:: Set to `true` to use frames to display the repository tree and assets in the same window.

hyperlink.indicator:: When set to `true`, automatically displays hyperlinks with underlining. When set to `false` (default), displays hyperlinks without underlining. (Users can manually format the hyperlinks as desired.)
...
search.pagelocation.perpage:: Specifies the number of search results displayed per page. The default value is 30.

search.pagelocation.brieflength:: Specifies the length of the description for each search result. The default value is 45.
...
schedule.abnormity.notify:: Specifies a comma-separated list of emails to receive notification in the event that the Scheduler application becomes unresponsive.
...
schedule.cycle.interval:: Interval between batch cycles (in milliseconds).
...
schedule.reload.auto:: Set to true to automatically reload schedule file when it is modified.
schedule.reload.interval:: Interval (in milliseconds) for checking schedule file changes. Default is 2000.
schedule.task.file:: Schedule a XML file path.
...
em.user.max:: For LDAP security, the maximum number of users that will be listed in the user tree (under the *User* tab or *Security* tabs). The default is 1000.
...
role.administrator:: Only users with this role can log into the EM and administer the  server. For an LDAP/AD security provider it maps to a Security Group. It defaults to ‘Administrator’. Example: `role.administrator=SRAdmin`.
security.acl.file:: Access the control file used by the default security service.
...
security.ldap.groupbase:: The base DN for groups. For iPlanet, default is `ou=Groups`, and for ADS, default is `cn=Users`.
security.ldap.rolebase:: The base DN for roles. For iPlanet, default is `ou=People`, and for ADS, default is `cn=Users`.
security.ldap.subtree:: Set to `true` to enable recursive search of specified LDAP user base. Default is `false`.
security.ldap.userbase:: The base DN for users. For iPlanet, default is `ou=People`, and for ADS, default is `cn=Users`.
security.use.cookie:: Set to `false` to disable the use of cookies in security service.
sso.default.roles:: Sets the default roles for new user logging in via SSO.
...
cluster.defaultProxy:: Specifies a default value for an empty ‘Repository Servlet URL’ field on the ‘Server Status’ page.
cluster.servers.https:: Specifies a comma-separated list of cluster nodes that should use HTTPS. Values should be specified as `host\:port`. For example, `cluster.servers.https=192.168.6.111\:8443`.
...
dependency.checker.enabled:: `true` (default) for automatic detection and re-synchronization of  dependencies.
dependency.checker.interval:: Interval between checks for changes in  dependencies, in milliseconds (default=30000).
html.directory:: Directory used to hold HTML temporary files (images).
html.retry.interval:: Milliseconds between retry, when waiting for a Dashboard.
...
soap.data.service.timeout:: Specifies the request timeout (in milliseconds) for a `SoapDataService` request. Default is 2 hours.
sree.bundle:: The name of the  localization bundle.
stylereport.library.path:: List of _.srl_ files specified using the file’s full path. Use this when specifying multiple _.srl_ files.
stylereport.locale.encode:: Load the locale property file used by Catalog, so other language text can be properly loaded.
stylereport.locale.properties:: Locale mapping property file name.
stylereport.locale.resource:: Locale mapping file resource name.
stylereport.useCustomDriver:: `true` to use custom driver. Default is `false`.
...
fiscal.start.day:: The day in the `fiscal.start.month` on which the fiscal year begins.
fiscal.start.month:: The month in which the fiscal year begins.
local.timezone:: Set to a valid Java time zone ID to indicate the StyleBI server time zone (e.g., `US/Eastern`). If not defined, the default time zone is used.
use.locale.weekGrouping:: When set to `true`, charts and crosstabs use Monday as the first day of the week for the ‘Week’ date grouping level. When set to `false` (default), charts and crosstabs use Sunday as the first day of the week for the ‘Week’ date grouping level. This setting applies *only* for user locales in which Monday is considered the first day of the week. It does not apply to the ‘Day of Week’ date grouping level, which automatically uses Monday as the first day of the week in appropriate locales.
