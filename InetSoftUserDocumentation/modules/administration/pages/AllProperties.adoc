...
schedule.task.timeout:: The maximum duration in milliseconds for  scheduled tasks and materialized view creation. Default is `600000` (ten minutes). Set to `0` for no timeout, except for tasks executed via the cloud runner, which are always capped at `600000` regardless of this setting, since a cloud-billed task must never wait unbounded. The timeout ensures that a task cannot persist   indefinitely.  When the timeout is reached, the job  is terminated regardless of whether the task is in a failed state or running successfully.
...
security.allow.iframe:: Set to `false` (default) to prevent the Enterprise Manager and User Portal login pages from being displayed within an IFrame. This is equivalent to setting the HTTP header property `X-Frame-Options` to `SAMEORIGIN`, and should be used in conjunction with Single Sign-On (SSO). See xref:integration:ConfigureSingleSignOn.adoc[] in xref:integration:AdvancedServerIntegration.adoc[]   for more information.
+
Set to `true` to prevent the `X-Frame-Options` header from being added, which may be necessary when you want to embed the login pages within an IFrame.  (The server must be using SSL in this case.)
