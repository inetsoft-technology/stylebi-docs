...
portal.customLoadingText:: Specifies text to display on the loading screen when the user logs into the User Portal, replacing the built-in "`*Loading...*`" message. This text is displayed below the loading logo -- either the built-in animated logo, or the optional custom image set by `portal.customLoadingLogo`.
