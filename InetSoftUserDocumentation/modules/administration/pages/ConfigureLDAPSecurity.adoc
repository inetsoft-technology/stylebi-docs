...
// metadata for AI
ifdef::ai[]
Summary: How to configure LDAP (including Active Directory) as the authentication and authorization provider in Enterprise Manager, mapping LDAP users, groups, and roles to InetSoft security.
endif::ai[]
...
+
image::LDAP Settings.png[alt="configure LDAP server connection settings"]
...
image::Figure 25.png[alt="select Active Directory from LDAP Server menu"]
...
image::ActiveDirectorySampleSchema.png[alt="sample Active Directory schema structure"]
...
+
image::ActiveDirectorySampleSchema5.png[alt="set Root DN field in security provider"]
...
+
image::ActiveDirectorySampleSchema1.png[alt="set User and Group Search Base to Departments unit"]

Role Search Base:: To search the ‘InetSoftRoles’ unit, set ‘Role Search Base’ to `ou=InetSoftRoles,ou=Roles`.
+
image::ActiveDirectorySampleSchema2.png[alt="set Role Search Base to InetSoftRoles unit"]
...
+
image::ActiveDirectorySampleSchema3.png[alt="enter Administrator ID distinguished name"]

The ‘Security Provider’ page for these setting appears as follows:

image::ActiveDirectorySampleSchema4.png[alt="completed Security Provider page for sample Active Directory"]
====
...
image::Figure 26.png[alt="configure Generic LDAP server settings"]
