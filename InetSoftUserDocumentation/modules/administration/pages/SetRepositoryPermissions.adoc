...
// metadata for AI
ifdef::ai[]
Summary: How to set, copy, and paste repository permissions in Enterprise Manager for Dashboards, folders, data sources, and other repository assets. Covers assigning READ, WRITE, DELETE, SHARE, and ADMIN permissions to users, roles, or groups, and explains the derive-from-parent inheritance model. NOTE: This page covers asset-level security only — i.e., who can open or manage a dashboard. To restrict which DATA users see within a dashboard (row-level or column-level filtering), use a VPM instead. See AddDataSecurity.adoc.
endif::ai[]
...
+
image::dashboardSecurityTab.png[alt="select Security tab for repository asset"]
...
+
image::dashboardSecurityTab2.png[alt="press Add button to open Add Permission dialog"]
...
To copy permissions from one asset to another, follow the steps below:
...
. Press the btn:[Copy Permissions] button below the permissions table to copy the asset's current permissions.  You can then navigate to another asset and paste the permissions to that asset.

. Press the btn:[Paste Permissions] button below the permissions table to paste permissions you have already copied from another asset.
+
TIP: The btn:[Paste Permissions] button will display numbers in parentheses, such as btn:[Paste Permissions (x of n)].  The *second  value* (*n*) indicates the number of permissions that were copied to the clipboard.  The *first  value* (*x*) indicates the  number of permissions that  are compatible  with the   current asset, and can actually be pasted.
