...
// metadata for AI
ifdef::ai[]
Summary: Reference table of CSS properties and values available for styling Dashboard non-tabular components (charts, gauges, filters, etc.) in a CSS definition file.
endif::ai[]

The table below lists the available properties and values for styling Dashboard non-tabular components. (To apply CSS styles to Tables, Crosstabs, and Freehand Tables, see xref:CSSTableStyleProperties.adoc[].) For a list of Dashboard component regions that can be styled using these properties, see xref:dashboard:VisualFormat.adoc[] and  xref:CSSDashboardRegions.adoc[].
