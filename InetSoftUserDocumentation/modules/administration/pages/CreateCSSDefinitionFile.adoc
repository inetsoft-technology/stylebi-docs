...
// metadata for AI
ifdef::ai[]
Summary: How to author a CSS definition file (UTF-8 text, no BOM) that applies styles to Dashboard components. Covers the specificity order (global default, Type, Class, ID), syntax for each selector type (Global Default, CSS Type, CSS Class, CSS ID, CSS Table Style, CSS Chart Palette), combining selectors, defining and using CSS variables, and suppressing CSS styles on export using Viewsheet[print=false]. Uploading the finished file is covered in UploadCSSFile; examples are in ExampleCSSDefinitionFile.
endif::ai[]
...
For Dashboard components, the ‘Type’ is displayed in the *CSS* pop-up in the *Format* panel. See xref:dashboard:VisualFormat.adoc[] for a list of all available CSS component types.

image::dashboard:CSS Table Detail.png[alt="view CSS type in Format panel"]
...
image::cssColorPaletteGradient.png[alt="gradient color palette applied to chart"]
...
----
See xref:dashboard:VisualFormat.adoc[] in xref:dashboard:VisualizeYourData.adoc[] for information on how to assign a CSS Class or ID to a Dashboard component.
