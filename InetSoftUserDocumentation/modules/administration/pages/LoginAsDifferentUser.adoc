...
// metadata for AI
ifdef::ai[]
Summary: How to enable and use the Login as Different User feature to impersonate another user without knowing their password. Configuration is done in Enterprise Manager (enabling the login.loginAs property and assigning permissions on the Security page), and the actual impersonation is performed from the User Portal login screen using a special 'Login as' field.
endif::ai[]
...
+
image::LoginAs.png[alt="login screen with login as field"]
