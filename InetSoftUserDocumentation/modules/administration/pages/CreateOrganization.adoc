...
// metadata for AI
ifdef::ai[]
Summary: How to create and edit a tenant organization in Enterprise Manager for multi-tenant deployments. Covers cloning from an existing organization, setting the organization name and ID, assigning a portal theme, adding roles as members, configuring inherited permissions and administrator permissions, and setting organization-level properties such as max row count, max column count, max cell size, and max user count. Enterprise only.
endif::ai[]
...
+
image::EM_Security_Multitenant1.png[alt="select Enable Multi-Tenancy option"]
...
+
image::organization3.png[alt="press New Organization link below navigation tree"]
...
+
image::organization2.png[alt="Clone From Existing Organization menu selection"]
...
+
image::server18.png[alt="new organization with default name in security tree"]
...
+
image::server19.png[alt="add roles to organization Members panel"]
...
+
image::server20.png[alt="Properties panel with Add button for user limits"]
+
This opens the '`Property Editor`', where you can set the following properties for the organization:
+
image::server21.png[alt="Property Editor showing max row column and user count settings"]
+
[cols="1,4"]
|===
