...
// metadata for AI
ifdef::ai[]
Summary: How to configure Presentation settings in Enterprise Manager, covering general date/time formats, export menu options, look and feel (fonts, favicon, custom shapes), themes (CSS variables for Portal and Enterprise Manager), welcome page, login banner, composer messages, Dashboard settings and toolbar, portal integration tabs, time settings, PDF settings and font mapping, data source visibility, web map backgrounds (Mapbox/Google Maps), social sharing, and AI Assistant integration.
endif::ai[]
...
image::PresentationGlobalSettings.png[alt="global settings tab for presentation options"]
...
image::PresentationOrgSettings.png[alt="organization settings tab with select organization menu"]
...
+
image::Figure 40.png[alt="general format page with date and time format settings"]
...
+
image::Figure 43.png[alt="export menu page with dashboard export format options"]
...
+
image::Global Look and Feel.png[alt="look and feel page with portal appearance settings"]
...
+
image::Global Look and Feel2.png[alt="user fonts dialog for uploading custom fonts"]
+
You can add multiple font variations under the same name so that, for example, a bold or italic version of the font will be available when the user applies bold or italic formatting. Verify that each parent folder for the desired fonts is listed in the '`CID Font Path`' (see  xref:#PDFSettings[]). Note that fonts for   *asset exports* will not be available unless the font file is located in a '`CID Font Path`' directory.

+
TIP: You *do not* need to restart the app after adding fonts, but you should advise users to clear their browser cache in order to see the new fonts.





Default Favicon:: Uncheck the '`Default Favicon`' option to upload a custom favicon in PNG format.




Custom Shapes::  To upload custom images for use with a Chart (see xref:dashboard:BasicChartingSteps.adoc[]), press the btn:[Custom Shapes] button to open the '`Custom Shapes`' dialog box.  To upload a Zip file containing images, first enable the '`Extract archive files`' option.  Press  btn:[Select], and choose an  image file (png, gif, jpg, svg) or archive. Press btn:[Upload] to upload the image or archive to the _portal/shapes_ directory. (See  xref:administration:Storage.adoc[] for   information about managing uploads.)
+
image::CustomShapes.png[alt="custom shapes dialog for uploading chart images"]
+
...
+
image::theme0.png[alt="themes tab listing available portal themes"]
...
====
+
image::theme4.png[alt="apply CSS changes to theme"]
...
+
image::theme5.png[alt="enter theme name and JAR file for import"]
...
image::theme6.png[alt="download button to export theme as JAR file"]

[#DeleteTheme]
=== Delete a Theme
To delete a theme,  select the theme that you want to delete, and press the {buttonDelete}.

image::theme7.png[alt="delete button to remove selected theme"]
...
+
image::welcomePage.png[alt="welcome page settings with URI and resource options"]
...
+
image::loginBanner2.png[alt="login banner page with text and HTML options"]
...
image::loginBanner.png[alt="login banner message displayed below password field"]
...
image::ComposerMessages2.png[alt="visual composer right panel with default help messages"]
...
+
image::ComposerMessages.png[alt="composer messages page with custom message fields"]
...
+
image::DashboardSettings.png[alt="dashboard settings page with enable dashboards option"]

. Select ‘Enable Dashboards’ to enable the {buttonDashboard} in the User Portal.

. *_Optional:_* Uncheck ‘Dashboard Tabs Top’ to display the tabs for multiple Dashboards at the bottom of the screen rather than at the top (default).

. *_Optional:_* Select '`Drill Tabs Top`' to display tabs of drill-down Dashboards at the top of the screen.  (See xref:dashboard:DrillDownIntoData.adoc#DrillwithChartHyperlinks[Drill with Chart Hyperlinks] for more details on drill-down features.)
...
+
image::ViewsheetToolbarOptions.png[alt="dashboard toolbar page with visible button options"]
...
+
image::Figure 41.png[alt="portal integration page with tab and link settings"]
...
+
image::UserPortalLinks.png[alt="user portal feature toggles for help logout and home"]
...
. *_Optional_*: Enter a '`Home Link`' and '`EM Home Link`' to set the URL of the {buttonHome}  in the User Portal and Enterprise Manager, respectively. (The default is the `index.html` landing page). This can be an absolute URL such as https://www.google.com, or a path relative to `/app/` such as  `composer`.
...
image::TimeSettings.png[alt="time settings page with week start day and schedule options"]
...
+
image::Figure 45.png[alt="PDF settings page with compress and font options"]
...
+
image::fontMapping.png[alt="font mapping page for CJK TrueType to CID font mapping"]
...
+
image::DataSourceTypeVisibility.png[alt="data source visibility page with visible and hidden sections"]
...
+
image::DataSourceTypeVisibility2.png[alt="select data source to add to visible types list"]
...
+
image::DataSourceTypeVisibility3.png[alt="select data source to add to hidden types list"]
...
+
image::EM_WebMap.png[alt="web map page with Mapbox and Google Maps options"]
...
+
image::socialSharing1.png[alt="social sharing page with sharing option checkboxes"]
...
image::socialSharing2.png[alt="webhook URL field for Google Hangouts or Slack sharing"]
...
[#AIIntegration]
== AI Integration
To customize the integration of the AI Assistant feature, follow the steps below:
...
. Select the *AI Integration* page in the right panel.
+
image::AIIntegration1.png[alt="configure AI Integration in Enterprise Manager"]

. Adjust the settings listed below:
+
Show AI Assistant Chat Link in Navigation Bars:: When enabled (default) displays the {buttonAssistant} in the navigation bar of the User Portal.  The equivalent property is `ai.assistant.visible`.

AI Assistant Internal URL:: When specified, this enables the reverse-proxy mode for the AI assistant so that the browser never communicates  to the AI Assistant directly.  Communications are directed through the proxy URL which lets you deploy the AI Assistant at  an internal network address that browsers cannot reach. The internal URL is never exposed to the client.  Leave this setting blank when using the direct mode (below).  The equivalent property is `chat.app.internal.url`.

AI Assistant Server URL:: When a proxy URL is NOT provided for '`AI Assistant Internal URL`', the '`AI Assistant Server URL`' provides the public-facing URL of the AI Assistant service. StyleBI returns this URL directly to the browser, which then connects to the AI Assistant server directly ("`direct mode`").  If the '`AI Assistant Internal URL`' above is specified, this URL is ignored. The equivalent property is `chat.app.server.url`.

AI Assistant Title:: Custom  name to display for the AI Assistant in the history sidebar. The equivalent property is `chat.app.title`.

AI Assistant Vendor Name:: Custom  name to display for the AI Assistant in the AI Assistant header. The equivalent property is `chat.app.vendor.name`.

AI Assistant Logo URL:: URL for a custom logo image to display in the AI Assistant header. This can   also specify a server storage path to an  internally saved image (e.g., `portal/shapes/logo.png`). The equivalent property is `chat.app.logo.url`.
