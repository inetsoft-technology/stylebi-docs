...
. Set a desired image format. The ‘Image Format’ option determines the encoding format for graphics, such as painters (charts) displayed in the browser. PNG images generally have better quality and a smaller file size than JPEG images.
...
|--inet-report-panel-margin|CSS margin property of the main panel, used to control the position of the panel in the User Portal. Example: `0 0` positions the panel to the left.
...
. You can specify a URL (same domain as InetSoft server) or a specific resource file such as an HTML or JSP file. HTML content can be localized by using the following tag:
...
Local Timezone:: Set to a valid Java time zone ID to indicate the StyleBI server time zone (e.g., `US/Eastern`). If not defined, the default time zone is used. (Equivalent property: `local.timezone`.)
