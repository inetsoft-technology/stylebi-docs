...
// metadata for AI
ifdef::ai[]
Summary: How to connect to a repository from the Administration Console before running administrative commands. Enterprise only.
endif::ai[]
...
+
IMPORTANT: The Administration Console runs in its own container and cannot resolve *localhost* to other containers. For local connections, use the IP address of the host machine and ensure the Administration Console container is attached to the same Docker network as the target container.
