...
// metadata for AI
ifdef::ai[]
Summary: How to use the Monitoring section of Enterprise Manager to track server activity. The Summary page shows server performance charts (heap memory, CPU, GC, uptime, cache, swapping, top users) and JVM details. Other pages cover executing Dashboards, running queries, data cache statistics, active user sessions, cluster nodes, and server/scheduler logs. Covers how to terminate Dashboards, queries, or user sessions from the monitoring pages.
endif::ai[]
...
image::Monitoring.png[alt="monitoring button at top of enterprise manager"]
...
Use the *Summary* Monitoring page  to track server performance. To access this page, press the {buttonMonitoring} at the top of Enterprise Manager, then select *Summary* from the left panel. In a clustered environment, select a machine from ‘Cluster Node’ menu to view the monitoring statistics for that machine. Click on a chart to view the detailed information provided by the corresponding ‘Monitoring’ page.


image::Monitoring Summary.png[alt="summary monitoring page with server performance charts"]
...
|Version Number|StyleBI server version|None
...
image::JVM.png[alt="JVM details panel at bottom of summary page"]


== Dashboards

Use the *Dashboards* Monitoring page to access key information about *executing* Dashboards that are currently being processed by the server, as well as *open* Dashboards that are active in memory but are not currently being processed. To access this page, press the {buttonMonitoring} at the top of Enterprise Manager, then select *Dashboards* from the left panel.


In a clustered environment, select a machine from ‘Cluster Node’ menu to view the monitoring statistics for that machine.

image::Monitoring Viewsheets.png[alt="dashboards monitoring page with executing and open dashboards"]
...
== Queries
Use the *Queries* Monitoring page  to access key information about queries currently executing on the server. To access this page, press the {buttonMonitoring} at the top of Enterprise Manager, then select *Queries* from the left panel. In a clustered environment, select a machine from ‘Cluster Node’ menu to view the monitoring statistics for that machine.

image::Monitoring Queries.png[alt="queries monitoring page with active query list"]
...
Use the *Cache* Monitoring page  to access key information for  the data cache. To access this page, press the {buttonMonitoring} at the top of Enterprise Manager, then select *Cache* from the left panel. In a clustered environment, select a machine from ‘Cluster Node’ menu to view the monitoring statistics for that machine.

image::Monitoring Cache.png[alt="cache monitoring page with data cache statistics"]
...
Use the *Users* Monitoring page  to access key information about user logins, and allows you to terminate a given user session. To access this page, press the {buttonMonitoring} at the top of Enterprise Manager, then select *Users* from the left panel. In a clustered environment, select a machine from ‘Cluster Node’ menu to view the monitoring statistics for that machine.

image::Monitoring Users.png[alt="users monitoring page with active session list"]
...
Use the *Cluster* Monitoring page  to view cluster nodes. To access this page, press the {buttonMonitoring} at the top of Enterprise Manager, then select *Cluster* from the left panel.

image::Monitoring Cluster.png[alt="cluster monitoring page showing cluster nodes"]
...
Use the *Logs* Monitoring page  to view and download server and scheduler logs. To access this page, press the {buttonMonitoring} at the top of Enterprise Manager, then select *Logs* from the left panel.

image::Figure 36.png[alt="logs monitoring page with log file viewer"]
...
+
image::LogNumberOfLines.png[alt="select number of log lines to display"]
...
+
image::LogDownload.png[alt="download button to export zip of all log files"]
