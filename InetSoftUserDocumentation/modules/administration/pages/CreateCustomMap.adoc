...
// metadata for AI
ifdef::ai[]
Summary: How to create a custom geographic map for use in Dashboard map charts by processing shapefiles with the OpenJUMP GIS application and InetSoft plugin, then uploading the exported mapdata.xml file to server storage. Covers installing OpenJUMP, exporting and simplifying map layers, adding additional layers, overlaying shapes, and editing existing custom maps.
endif::ai[]
...
+
image::OpenJUMPExp1.png[alt="open shapefile in OpenJUMP application"]
...
+
image::OpenJUMPExp2.png[alt="Consolidate Features dialog box in OpenJUMP"]
...
+
image::OpenJUMPExp3.png[alt="consolidated map listed in OpenJUMP project tree"]
...
+
image::OpenJUMPExp4.png[alt="Area Histogram dialog with min max and interval settings"]
+
...
+
image::OpenJUMPExp5.png[alt="press Output Window button in OpenJUMP toolbar"]
+
...
+
image::OpenJUMPExp6.png[alt="area histogram showing polygon removal percentages"]
+
...
+
image::OpenJUMPExp7.png[alt="Remove Small Polygons dialog box"]
+
...
+
image::OpenJUMPExp8.png[alt="new map layer after polygon removal in project tree"]
...
+
image::OpenJUMPExp9.png[alt="polygon removal operation report in Output Window"]
...
+
image::OpenJUMPExp10.png[alt="Find Precision dialog box in OpenJUMP"]
...
+
image::OpenJUMPExp11.png[alt="current map coordinate precision in Output Window"]
+
...
+
image::OpenJUMPExp12.png[alt="Noding dialog with layer and precision settings"]
...
+
image::OpenJUMPExp13.png[alt="Point Threshold Histogram dialog with layer and interval settings"]
+
...
.. To view the histogram, press the ‘Output Window’ button in the OpenJUMP toolbar.
+
image::OpenJUMPExp14.png[alt="boundary point significance histogram in Output Window"]
+
...
+
image::OpenJUMPExp15.png[alt="Simplify dialog with layer algorithm and threshold settings"]
...
+
image::OpenJUMPExp16.png[alt="point removal operation report in Output Window"]
...
+
image::OpenJUMPExp17.png[alt="Export Layer dialog box in OpenJUMP"]
...
+
image::OpenJUMPExp18.png[alt="Export Layer dialog showing map type and layer name fields"]
+
See xref:dashboard:MapChart.adoc[] in xref:dashboard:VisualizeYourData.adoc[] for more details on using the Chart Editor in a Dashboard.
...
+
image::OpenJUMPExp21.png[alt="upload mapdata.xml to root level of server storage"]
...
+
image::CustomMapNewLayer1.png[alt="set layer name for new map layer in Export Layer dialog"]
...
+
image::CustomMapNewLayer2.png[alt="press View/Edit Attributes button in OpenJUMP toolbar"]
+
...
+
image::CustomMapNewLayer3.png[alt="right-click table and select Editable from context menu"]
...
+
image::CustomMapNewLayer4.png[alt="choose Combine Selected Features for selected map regions"]
+
...
+
image::CustomMapNewLayer5.png[alt="select View/Edit Schema from table context menu"]
+
This opens the ‘Edit Schema’ dialog box.

.. Click in the table, and then press the ‘Insert’ (+) button to create a new column.
+
image::CustomMapNewLayer6.png[alt="press Insert button to create new column in Edit Schema"]

.. Enter the name and type of the new column (e.g., “CensusDivisions”).
+
image::CustomMapNewLayer7.png[alt="enter name and type for new column in schema"]
...
+
image::CustomMapNewLayer8.png[alt="enter labels in blank cells of Attributes table"]
+
...
+
image::CustomMapNewLayer9.png[alt="select Feature ID and Display name from new column"]
...
+
image::CustomMapOverlay.png[alt="Export Layer dialog with Feature ID and Display name selected"]
...
+
image::CustomMapOverlay2.png[alt="exported overlay files including CSV and mapdata.xml"]
...
. Create a Map Chart  based on the custom map. (See xref:dashboard:MapChart.adoc[].) Observe that the specified regions and labels are overlaid on the map as desired.
+
image::CustomMapOverlay3.png[alt="map chart with Census Region shapes overlaid on states"]
...
+
image::OpenJUMPExp19.png[alt="select InetSoft Map Layer in OpenJUMP open dialog"]
...
+
image::OpenJUMPExp20.png[alt="select map type and layer name to edit"]
