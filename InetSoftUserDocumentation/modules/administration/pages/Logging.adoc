...
// metadata for AI
ifdef::ai[]
Summary: How to configure server logging settings in Enterprise Manager, including log file location, maximum log file size, log rotation, default log detail level (Debug/Info/Warning/Error), and custom per-asset log levels.
endif::ai[]
...
+
image::Figure 10.png[alt="logging settings page in Enterprise Manager"]
...
. Press the btn:[Add] button below the '`Custom Log Levels`' table. This adds a new log level.
+
image::CustomLogLevelAdd.png[alt="add new custom log level entry"]

. In the ‘Context’ menu, select the type of asset (e.g.,  Dashboard, query, etc.) to which you want to assign a distinct log level.
+
image::CustomLogLevelType.png[alt="select asset type in context menu"]

. In the ‘Name’ field, enter the name of the asset for which you want to set a custom log level.

. If multi-tenancy is  enabled (see xref:EnableMultiTenant.adoc[] for more information), select the desired '`Organization`'.


. Select the desired logging level for this particular asset. (See above for level information.)
+
image::CustomLogLevelLevel.png[alt="select logging level for custom log entry"]
