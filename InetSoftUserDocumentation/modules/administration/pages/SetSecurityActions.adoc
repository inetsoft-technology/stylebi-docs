...
// metadata for AI
ifdef::ai[]
Summary: How to configure Action Permissions (Security Actions) in Enterprise Manager to restrict which features users can access in the User Portal, covering built-in roles (Everyone, Designer, Advanced, Administrator), setting permissions for actions like bookmarks, chart types, portal tabs, scheduling, and Visual Composer, and how to copy and paste permissions between actions.
endif::ai[]

This section explains how to restrict the actions that a user can perform in the User Portal based on user, group, or role.

== Built-In Role Permissions

StyleBI includes the following built-in roles with predefined permissions. You can create additional roles and customize permissions as needed. See xref:CreateRole.adoc[] and the sections below for more information.
...
a|
[square]
* Repository tree (browse and view Dashboards)
* Ad-hoc editing of Dashboards
* Bookmarks
* Print, email, and export Dashboards

|*Designer*
a|
[square]
* *Everyone* role permissions (above)
* Dashboard tab (Pinned Dashboards)
* Visual Composer (create and edit Dashboards and Data Worksheets)
...
[square]
* No restrictions.
* Full access to the User Portal and Enterprise Manager.
...
+
image::securityActions.png[alt="select Actions tab on Security page"]
...
Social Sharing:: Access to the various options under the {buttonSocialSharing} in the Portal. (See xref:dashboard:ShareToSocialNetwork.adoc[] for more information.)
...
AI Assistant:: Access to the {buttonAssistant}  in the Portal and Visual Composer.
...
Edit Mobile Devices:: Permission to edit mobile device screen layouts. (See xref:dashboard:CreateDeviceLayout.adoc[] for more information about device layouts.)
...
Materialize Assets:: Access to the ‘Materialize’ menu option for Dashboards in the Portal Repository tree, as well as for Enterprise Manager. (See xref:dashboard:ImprovePerformance.adoc[] and xref:administration:MaterializedView.adoc[]  for more information.)
...
====
+
image::server7.png[alt="select action in security actions tree"]
+
...
+
image::server7a.png[alt="Add Permission dialog for security action"]

. {buttonCtrl}-click to select the desired users, groups, or roles, and press {buttonEMAdd}.
+
image::Report Permissions.png[alt="select users groups or roles to add permission"]
...
* The ‘Use Parent Permissions’ setting is significant only for the asset components listed under the ‘Data Worksheet’ node. If you set permissions for the ‘Global’ node, they can be inherited by all the assets under this node.
====


[#CopyandPastePermissions]
== Copy and Paste Permissions

IMPORTANT: You cannot copy and paste permissions across different organizations or  different security providers.



The procedure for copying and pasting object permissions (Actions) is the same for all components.  For example, to copy  or paste permissions for the ‘Bookmark’ feature, follow the steps below:
...
. Press the btn:[Copy Permissions] button below the permissions table to copy the action`'s current permissions.  You can then navigate to another action and paste the permissions to that action.

. Press the btn:[Paste Permissions] button below the permissions table to paste permissions you have already copied from another action.
+
TIP: The btn:[Paste Permissions] button will display numbers in parentheses, such as btn:[Paste Permissions (x of n)].  The *second  value* (*n*) indicates the number of permissions that were copied to the clipboard.  The *first  value* (*x*) indicates the  number of permissions that  are compatible  with the   current action, and can actually be pasted.
