...
// metadata for AI
ifdef::ai[]
Summary: How to configure server performance options in Enterprise Manager on the General > Performance page, including query timeouts, maximum row counts, dataset caching settings, and related safeguard properties. Also covers the list of properties available to limit query size and execution time.
endif::ai[]
...
+
image::Figure 9.png[alt="select Performance page under General settings"]
+
