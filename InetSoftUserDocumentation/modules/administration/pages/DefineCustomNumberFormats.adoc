...
// metadata for AI
ifdef::ai[]
Summary: How to define custom number formats, including using built-in K, M, B suffixes to scale values and creating custom suffixes and multipliers by editing the userformat.xml file and uploading it via the Presentation page in Enterprise Manager.
endif::ai[]
...
image::dashboard:Format Number KMB.png[alt="number format with K M B suffix options"]
...
image::commonscript:Format Number Custom Multiplier.png[alt="custom multiplier suffixes in format presets"]
