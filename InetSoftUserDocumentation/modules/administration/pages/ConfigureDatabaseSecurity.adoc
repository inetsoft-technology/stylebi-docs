...
// metadata for AI
ifdef::ai[]
Summary: How to configure a database as the authentication provider in Enterprise Manager, including connection settings (driver, URL, credentials, hash algorithm) and SQL query configuration for users, groups, roles, emails, and organizations (multi-tenant). Also covers administrator role settings. Enterprise only.
endif::ai[]
...
image::server22.png[alt="configure database authentication settings"]
...
== User, Group, and Role Query Settings
...
== Organization Query Settings (Multi-Tenant)
