...
// metadata for AI
ifdef::ai[]
Summary: If set to `true` (default), when ‘Group all others together’ is enabled for Top/Bottom ranking in the Group ‘Edit’ panel of the Freehand Table Editor, this places the ‘Others’ group _after_ all the ranked groups, regardless of the specified dimension sorting. (See xref:viewsheet:AddFreehandTable.adoc#SortandRank[Sort and Rank] for information about dimension ranking in a Freehand Table.)
endif::ai[]
