...
// metadata for AI
ifdef::ai[]
Summary: Specifies a tooltip to be displayed on the Text component. Use placeholder `pass:[{0}]` to insert the data value from the Text, and format the inserted data using number formats. (See xref:viewsheet:DataFormat.adoc[] for more details on format strings.)
endif::ai[]
