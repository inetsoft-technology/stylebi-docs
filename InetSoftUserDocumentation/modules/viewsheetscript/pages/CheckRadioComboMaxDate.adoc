...
// metadata for AI
ifdef::ai[]
Summary: Specifies the latest selectable date when the ComboBox is configured to use a *calendar interface* for Date or TimeInstant type data. (See xref:viewsheet:RadioCheckBoxComboProperties.adoc[] for information on setting the ‘Calendar’ option.) This property should be set in the *onInit* handler (See xref:AddScriptToDashboard.adoc#AddDashboardScript[Add Dashboard Script] for more information about handlers.)
endif::ai[]
