...
// metadata for AI
ifdef::ai[]
Summary: Sets the zoom level for a xref:viewsheet:MapChart.adoc[] that uses a default or Mapbox background. If the value is an integer, this is equivalent to the number of times the {buttonZoomIn} is pressed on the map.
endif::ai[]
