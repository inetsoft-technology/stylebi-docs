...
// metadata for AI
ifdef::ai[]
Summary: Enables the Text component to scale when the Dashboard is automatically scaled to screen. (See xref:viewsheet:ArrangeComponents.adoc[].) Set to `false` to prevent the Text from scaling, which may be appropriate when the Text is used as a label for components that do not scale, such as Radio Button.
endif::ai[]
