...
----
<1> See xref:viewsheetscript:CreateScriptFunction.adoc[] for more details on how to define a function.
...
If a Dashboard generates errors, you can use the Visual Composer Console (see xref:viewsheet:VisualComposerConsole.adoc[]) to analyze the errors.  Errors will also be recorded in the server logs. See xref:administration:Logging.adoc[] for more information.


When you attempt to debug a Dashboard script, it is often helpful to view the current values of variables and objects. To do this, use the xref:viewsheetscript:SpecialFunctionsAlert.adoc[] function, which opens a "`toast`" message box to display a specified string.
[source,javascript]
...
The `alert()` dialog box is also useful for displaying critical information to a user (for example, if the user’s selections have resulted in an empty dataset).  You can also use the  xref:viewsheetscript:SpecialFunctionsConfirm.adoc[] function to display information to a user, which also prompts the user to either continue or abort the operation.
