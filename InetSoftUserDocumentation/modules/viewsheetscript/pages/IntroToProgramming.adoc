...
Note that these operators *cannot* be used to compare Date objects. Use date functions instead, such as `dateDiff()` or `CALC.dateValue()`. See xref:commonscript:DateObjectFunctions.adoc[] for more information about date functions.
...
=== Conditionals
The `if/else` statement tests a Boolean expression to decide between two alternative actions.  The `if` conditional structure executes the statements within the ‘if’ block only when the specified condition is `true`. Alternative conditions can be tested by using one or more optional ‘else if’ blocks (similar to  xref:#Switch[]). An optional final ‘else’ block will be executed if all other conditions evaluate `false`.
