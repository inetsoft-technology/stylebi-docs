...
// metadata for AI
ifdef::ai[]
Summary: When `true`, apply the '`Trend and Comparison`' setting to automatically-computed grand totals as well as to the specified aggregates. See xref:viewsheet:TrendCompareData.adoc[] for more information about the available settings. Default is `false`.
endif::ai[]
