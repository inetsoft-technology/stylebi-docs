...
xref:viewsheetscript:CommonSelectObject.adoc[], xref:viewsheetscript:CommonSelectLabel.adoc[]:: The selected value or label of the component: Applies to *single-value* input elements (Slider, Spinner, RadioButton, ComboBox) and the tabs of co-located components in a Tabbed Interface. For example, to return the value or label selected in a Radio Button control, use the following syntax:
+
...
xref:viewsheetscript:CommonSelectObjects.adoc[], xref:viewsheetscript:CommonSelectLabels.adoc[]::
...
xref:WriteBackToDatabase.adoc#AccessUserModifiedData[Access  User-Modified Data] in xref:WriteBackToDatabase.adoc[] information on how to access an *Editable Table*.
