...
NOTE: The ‘Orders’ data source in this example is a different from the sample embedded ‘Orders’ Derby database that is used by the  xref:ROOT:attachment$StyleBIExamples.zip[Style BI Examples].  The embedded Derby database is *not* writable, and cannot be used for this example.
