...
// metadata for AI
ifdef::ai[]
Summary: Sets the specified cell to expand to display an array of data (typically supplied by the xref:viewsheetscript:SpecialFreehandFunctions.adoc#toList[toList] function). See xref:viewsheet:AddFreehandTable.adoc[] for more information about ‘Expand Cell’.
endif::ai[]
