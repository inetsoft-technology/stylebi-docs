...
// metadata for AI
ifdef::ai[]
Summary: For an *Editable Table*, deletes the row specified by integer parameter `row`, and adds this deleted row to the array returned by `getFormRows(DELETED)`. See xref:viewsheet:TableProperties.adoc[] for information on making a Table editable.
endif::ai[]
