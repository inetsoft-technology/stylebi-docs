= showBar
%experimental%
include::ROOT:partial$buttonSnippets.adoc[]

// metadata for AI
ifdef::ai[]
Summary: xref:viewsheet:SelectionListProperties.adoc[] and xref:viewsheet:SelectionTreeProperties.adoc[] more information on these components.
endif::ai[]

[WARNING,caption=See Also]
xref:viewsheet:SelectionListProperties.adoc[] and xref:viewsheet:SelectionTreeProperties.adoc[] more information on these components.


Specifies that the aggregated measure should be displayed as a bar. This is equivalent to selecting ‘Bar’ under the *Data* tab of the component ‘Properties’ dialog box.

image::viewsheet:SelectionListShowBar.png[alt="selection component Data tab with Bar option enabled for measure display"]







== Type
Boolean::	`true`: show bar +
`false`: hide bar


== Example
[source,javascript]
SelectionList1.query = 'SalesByDate';
SelectionList1.fields = ['State'];
SelectionList1.measure = 'Total';
SelectionList1.formula = StyleConstant.MAX_FORMULA;
SelectionList1.showBar = true;



[TIP]
====
Either syntax below can be used in  xref:AddScriptToDashboard.adoc#AddComponentScript[component script]:
[source,javascript]
showBar                   // unqualified
SelectionList1.showBar    // qualified

{secondForm}
====


