...
// metadata for AI
ifdef::ai[]
Summary: How to group and aggregate (summarize) data in a Data Worksheet in Visual Composer, equivalent to a SQL GROUP BY or aggregate query, including choosing group-by columns, aggregate functions, named groups (custom grouping), and deciding between Data Worksheet vs. Dashboard-level aggregation. IMPORTANT: this Worksheet-vs-Dashboard choice applies only to aggregation. Joining separate data blocks/tables/data models (see JoinData.adoc) has no Dashboard-level counterpart — joins must always happen in the Data Worksheet. Do not generalize the "either level" framing here to joins.
endif::ai[]

Grouping and aggregation,  also known as summarization, is an essential part of presenting information to business audiences.  By use of effective grouping and aggregation, you can reduce massive datasets to a handful of key trends and indicators, facilitating business  insight and action.  The sections below explain how to effectively aggregate your data.

== Where to Group and Aggregate
You can group and aggregate data at the  data pipeline level (Data Worksheet) or presentation level (Dashboard).

Data Worksheet Level::
To group and aggregate data at the Data Worksheet level, see the sections below.
+
[square]
* xref:#GroupAggregateRegularDataBlock[]
* xref:#GroupandAggregateCrosstabDataBlock[]
* xref:#GroupDatabyNumericRange[]
* xref:#GroupDatabyDateRange[]
* xref:#NamedGrouping[]

+
When you perform grouping and aggregation in a Data Worksheet, *_only_* the aggregated data will be available for further manipulation and presentation. This approach may be most efficient when you want to use the aggregated data across many Dashboards.


Dashboard Level::
To group and aggregate data at the Dashboard level, see the following sections in xref:dashboard:VisualizeYourData.adoc[]. This provides the user with the greatest flexibility to adjust the grouping and aggregation to suit their needs.
[square]
* To group and aggregate data in  Chart, see xref:dashboard:GroupDataByDimension.adoc[].
* To group and aggregate data in a  crosstab, see xref:dashboard:AddCrosstab.adoc[].
* To group and aggregate data as a  KPI, see xref:dashboard:AddAKPI.adoc[].
...
[#GroupAggregateRegularDataBlock]
...
+
image::Grouping_WS1.png[alt="press Group and Aggregate button in the bottom panel"]
...
====
+
image::grouping2_1.png[alt="Group and Aggregate panel with grouping fields and summary fields selected"]
+
...
+
image::grouping3.png[alt="data block after grouping showing only grouping and summary columns"]

. To view the grouped and aggregated data, press the {buttonChangeView} in bottom panel, and choose {buttonLiveDataView}.
+
image::grouping4.png[alt="live data view showing grouped and aggregated data in the data block"]
...
NOTE: You can also create a crosstab (with more than one column header) in a Dashboard. See xref:dashboard:AddTable.adoc[] in xref:dashboard:VisualizeYourData.adoc[].
...
+
image::groupAndAggregateButton_1.png[alt="press Group and Aggregate button in the bottom panel"]
+
This opens the *Group and Aggregate* panel.

. In the ‘Group and Aggregate’ panel, press btn:[Switch to Crosstab].
+
image::CrosstabDialog2.png[alt="press Switch to Crosstab button in the Group and Aggregate panel"]
+
The ‘Group and Aggregate’ panel changes to the crosstab view.
+
image::CrosstabDialog.png[alt="Group and Aggregate panel changed to crosstab view"]
...
+
image::CrosstabDialog3.png[alt="Row Header and Column Header fields configured in the crosstab panel"]
...
+
image::groupAndAggregateButtonExp.png[alt="press Group and Aggregate button in the bottom panel for the Sales1 block"]

. Press the btn:[Switch to Crosstab] button at the bottom of the panel.
+
image::groupAndAggregateButtonExp2.png[alt="press Switch to Crosstab button to display crosstab binding interface"]
+
...
+
image::Unpivot Crosstab1.png[alt="crosstab configured with Category Company row headers Date column header and Total measure"]
...
+
image::dragCrosstab.png[alt="crosstab in live data view with category company row headers and year column headers"]
...
+
image::Unpivot Crosstab3.png[alt="right-click data block and select Unpivot to open Unpivot Data dialog"]
...
+
image::Unpivot Crosstab5.png[alt="Unpivot Data dialog with value of 2 entered for levels of row headers"]

. Press btn:[OK]. This creates a new data block containing the unpivoted (flattened) data.
. In the bottom panel, press the {buttonChangeView} and select {buttonLiveDataView} from the menu. (See xref:PreviewData.adoc[].)
+
image::Unpivot Crosstab4.png[alt="unpivoted flat table with Dimension column from crosstab column headers"]
...
+
image::numericRangeDialog_2.png[alt="right-click numeric column header and select New Range Column"]
+
This opens the ‘Numeric Range Option’ panel. Here you can specify the different benchmarks defining the range.
+
image::numericRangeDialog.png[alt="Numeric Range Option panel to specify benchmark values and range settings"]
...
+
image::rangeColumnExp1.png[alt="right-click Total column and select New Range Column from context menu"]
...
+
image::numericRangeDialogFilled.png[alt="Numeric Range Option dialog with four thresholds entered for Revenue Range"]
+
...
+
image::numericRangeDialogRelable.png[alt="ranges relabeled as Low Medium and High in the Numeric Range Option dialog"]
...
+
image::numericRangeDialogResult.png[alt="data block preview showing Revenue Range column with Low Medium High labels"]
...
+
image::numericRangeDialogReopen.png[alt="press Numeric Range button in range column header to reopen the dialog"]
.. Select the ‘Greater than Max’ option, enter a label, and press btn:[OK].
+
image::numericRangeDialogLessThanMin.png[alt="select Greater than Max option and enter label for values above maximum"]
...
+
image::dateRangeDialog1.png[alt="right-click date column header and select New Range Column"]
+
This opens the ‘Date Range Option’ panel. Here you can specify the date grouping.
+
image::dateRangeDialog2.png[alt="Date Range Option panel showing date grouping options including Year Quarter Month and Day"]
. Enter a name for the new range column.
+
include::ROOT:partial$duplicateNames.adoc[]
...
NOTE: You can also create a custom grouping in a Chart or Crosstab at the Dashboard level. See xref:dashboard:GroupDataByDimension.adoc[] and xref:dashboard:AddTable.adoc[] in xref:dashboard:VisualizeYourData.adoc[].
...
+
image::newObjectGrouping.png[alt="press Create Object button and select Grouping from toolbar"]
...
This opens the ‘Grouping’ dialog box.
+
image::NewGroupingDialog.png[alt="Grouping dialog box with Name Type and Only For fields"]
...
+
image::AddConditionList.png[alt="Add Condition List dialog box with Group Name field and condition menus"]
...
+
image::WestCoastConditions.png[alt="condition this is one of WA OR CA entered for the WestCoast group"]
. Press  btn:[OK]  to exit the ‘Add Condition List’ dialog box and return to the ‘Grouping’ dialog.
+
image::WestCoastGroupingDialog.png[alt="Grouping dialog showing WestCoast group added to the Geographic Grouping"]
. Press btn:[OK].
. Right-click on the ‘Geographic Grouping’ group and select ‘Set as Primary’. {rightClickOptions}
+
image::WestCoastSetPrimary.png[alt="right-click Geographic Grouping and select Set as Primary"]
...
+
image::saveWestCoast.png[alt="Save as Data Worksheet dialog with Geographic Grouping entered as asset name"]
...
+
image::WestCoastinTree.png[alt="Geographic Grouping shown in left panel with group icon"]
****
