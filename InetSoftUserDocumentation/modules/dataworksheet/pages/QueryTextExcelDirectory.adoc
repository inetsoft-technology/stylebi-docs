...
// metadata for AI
ifdef::ai[]
Summary: How to create a Data Worksheet query in Visual Composer from a Text/Excel Directory data source, including selecting a file or folder, choosing a sheet (for Excel), setting encoding and delimiter options, and configuring import settings such as Unpivot Data (for crosstab tables), First Row as Header, and Remove Quotation Marks.
endif::ai[]
...
+
image::dataSourceTextExcel5.png[alt="select Text Excel Directory Query button from the toolbar"]
+
This opens the ‘Text/Excel Directory  Query’ dialog box.
+
image::dataSourceTextExcel6.png[alt="Text Excel Directory Query dialog box for configuring the data query"]
+
