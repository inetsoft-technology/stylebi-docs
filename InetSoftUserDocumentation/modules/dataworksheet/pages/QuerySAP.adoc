...
// metadata for AI
ifdef::ai[]
Summary: How to create a Data Worksheet query in Visual Composer from an SAP data source, covering three SAP connection types: SAP ABAP (selecting Infoset, User Group, and Query Name then loading parameters), SAP BAPI (selecting Object Type, Function Name, and Table then loading parameters), and SAP Tables (filtering by type and table name). This is an Enterprise-only feature.
endif::ai[]
...
+
image::dataSourceSAP5.png[alt="SAP Query dialog box for configuring the data query"]
...
+
image::dataSourceSAP7.png[alt="SAP ABAP query parameters loaded with values entered"]
...
.. *_Optional:_*  To add additional parameters, press the btn:[Add] button. To remove a parameter, check the corresponding box and press the btn:[Remove] button.
+
image::dataSourceSAP8.png[alt="SAP BAPI query configured with object type function and table"]
...
+
image::dataSourceSAP9.png[alt="SAP Tables query with desired table selected in Table Name menu"]
