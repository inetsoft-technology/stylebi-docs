...
// metadata for AI
ifdef::ai[]
Summary: How to create a Data Worksheet query from a GraphQL connection, including writing GraphQL query syntax, using variables, enabling pagination with a pagination variable and JSON Path expression, choosing GET or POST request type, and specifying a JSON Path to extract results (with examples for a bare array, a named-field-wrapped array, and a top-level array of mixed content). Also covers the optional Expand Arrays feature for normalizing nested array data.
endif::ai[]


To create a new data set from an existing GraphQL  connection, follow the steps below:

. If you have not yet configured the database connection, see xref:user:GraphQL.adoc[]. Then proceed with the steps below.
...
. Press the {buttonObject}, and select {buttonGraphQLQuery}. This opens the ‘GraphQL  Query’ dialog box.

+
image::graphQL1.png[alt="GraphQL Query dialog box for configuring the data query"]
+
