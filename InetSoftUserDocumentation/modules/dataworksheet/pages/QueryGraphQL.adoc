...
// metadata for AI
ifdef::ai[]
Summary: How to create a Data Worksheet query from a GraphQL connection in Visual Composer, including writing GraphQL query syntax, using variables, enabling pagination with a pagination variable and JSON Path expression, choosing GET or POST request type, and specifying a JSON Path to extract results. Also covers the optional Expand Arrays feature for normalizing nested array data.
endif::ai[]
