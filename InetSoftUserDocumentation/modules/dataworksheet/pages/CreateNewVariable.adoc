...
// metadata for AI
ifdef::ai[]
Summary: How to define a Variable in a Data Worksheet and use it in filter conditions to create parameterized queries where the user supplies the value at runtime. The Variable Properties dialog has a 'Default Value' field (labeled exactly "Default Value") where you can enter a literal default value (text, number, or date depending on the Variable's Type); expressions are not supported in this field — only literal values. Variables defined here are automatically available as parameters in any Dashboard that uses the worksheet — accessible via the parameter.variableName syntax in Dashboard script, or controllable via Dashboard Form components.
endif::ai[]
...
TIP: Variables defined in a Data Worksheet are automatically available as parameters in any Dashboard that uses the Worksheet. You can read or write them in Dashboard script using the xref:dashboardscript:parameter.adoc[] property (e.g., `parameter.variableName`). You can also control their values using Dashboard Form components. See xref:dashboard:DynamicallySetProperties.adoc[] for details.
...
+
image::variableButtonToolbar.png[alt="press Create Object button and select Variable from toolbar"]
+
This opens the ‘Variable Properties’ dialog box.
+
image::parameterDialog.png[alt="Variable Properties dialog box with name label and type fields"]
...
. Select the data type of the Variable from the ‘Type’ menu. This is the data type of the values that the Variable will provide to the filtering condition.  The following types are available:
+
include::ROOT:partial$DataTypeTable.adoc[]
+
. *_Optional:_* In the ‘Default Value’ field, enter a literal default value for the Variable (text, number, or date, matching the selected '`Type`'). This value is used when the user does not supply a value at runtime.
...
+
image::SelectionList_2.png[alt="Embedded List Values dialog box to enter label and value pairs"]
+
...
+
image::querySelectionList.png[alt="Table dialog showing value column and label column selection for variable"]
+
...
+
image::parameterCA.png[alt="Variable Properties dialog with state variable configured and CA as default value"]
. Press btn:[OK].
. Right-click on the new Variable in the Data Worksheet and select ‘Set as Primary’. {rightClickOptions}
+
image::statePrimary.png[alt="right-click variable and select Set as Primary from context menu"]
. Press the {buttonSave} in the top toolbar. This opens the ‘Save as Data Worksheet’ dialog.
. Specify “State” as the asset name.
+
image::saveStateWorksheet.png[alt="Save as Data Worksheet dialog with State entered as the asset name"]
...
+
image::StateinTree.png[alt="State variable shown in left panel with variable icon"]
...
+
image::DragStateWorksheet.png[alt="State variable dragged from left panel into the Data Worksheet"]
+
...
+
image::SwitchtoVariableCondition.png[alt="press Change Value Type button and select Variable in condition panel"]
. From the ‘Variable Name’ menu, select ‘state’.
+
image::Stateequaltostate.png[alt="condition configured with State field equal to state variable"]
. Press btn:[OK]. You will be prompted for the value of the ‘state’ Variable.
. Enter “FL” and press btn:[OK].
+
image::specifyStateParameter.png[alt="parameter prompt dialog asking user to enter state value FL"]
. Preview the data block (see xref:PreviewData.adoc[]), and note that only the sales for the state of Florida are now shown.
+
image::Variable in Condition End.png[alt="data block preview showing only sales records for Florida"]
****