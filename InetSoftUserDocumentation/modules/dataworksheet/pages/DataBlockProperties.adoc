...
// metadata for AI
ifdef::ai[]
Summary: Reference for Data Worksheet data block properties, including description, maximum rows retrieved, and handling of duplicate rows.
endif::ai[]
...
+
image::dataworksheet:TableProps.png[alt="Table Properties panel with description max rows and other settings"]

The  properties are described below.

Description:: The description text is displayed as a tooltip when the user hovers the mouse  over the data block in the Worksheet canvas or over the  {buttonTableIcon} in the table’s title row.
Max Rows:: The maximum number of rows to retrieve from the database at runtime.
+
TIP: If a Dashboard Chart, Table, or Crosstab bound to this data block shows fewer rows than expected, check this setting first. It silently caps the data at the source, before any Dashboard-level filters, Top-N settings, or aggregation are applied.
