...
// metadata for AI
ifdef::ai[]
Summary: How to filter data in a Data Worksheet in Visual Composer. Filtering is done by selecting an existing data block and pressing the Define Condition button — there is no separate "filter data block" or "filter object"; the condition is added directly to the data block. Conditions are also called query filters, row filters, or WHERE clauses. Covers pre-aggregate and post-aggregate conditions, ranking (top-N/bottom-N) filters, dynamic conditions using variables, session data, field comparisons, expressions, and subqueries, plus materialized view update conditions. Not to be confused with: FilterDashboard.adoc, which covers user-controlled filter UI components (Selection Lists, Range Sliders, Calendars) that appear on a Dashboard.
endif::ai[]
...
[square]
* To filter at the Dashboard level, see xref:dashboard:FilterDashboard.adoc[]. This provides the user with the greatest flexibility to adjust the filtering,
...
To filter the detail (pre-aggregate) data in a data block, follow the steps below. Note: If you want Dashboard users to dynamically filter the data in a Dashboard, see xref:dashboard:FilterDashboard.adoc[] instead.
...
+
image::filtering2.png[alt=”press Define Condition button in the bottom panel to open Condition panel”]
. Select the ‘Advanced Conditions’ check box.
+
image::AdvancedConditionsBlank.png[alt=”Advanced Conditions checkbox toggles between simple and advanced condition modes”]
+
...
====
+
image::filtering3.png[alt=”specify pre-aggregate condition using menus in the Condition panel”]
+
...
+
image::commonscript:Ad Hoc Filtering10_ws.png[alt="compound condition with AND or OR operator added to condition list"]
...
+
image::commonscript:Ad Hoc Filtering11_ws.png[alt="use Indent and Unindent buttons to specify evaluation order for compound conditions"]
+
...
You can now use this Data Worksheet as the data source for a Dashboard. See xref:dashboard:VisualizeYourData.adoc[] to create a Dashboard.
...
+
image::importWorksheet_2.png[alt="Sales Explore Data Worksheet dragged into the Data Worksheet panel"]

. Click the data block to select it.
. Press the {buttonDefineCondition} in the bottom panel. This opens the ‘Conditions’ panel.
+
image::ConditionExp.png[alt="Condition panel opened in the bottom panel for the selected data block"]

. Press the btn:[More] button to create a new condition.
. Enter the following condition: `[Total] [is] [greater than] [1000]`.
+
image::newCondition.png[alt="condition configured as Total is greater than 1000"]
...
+
image::previewConditionedtable.png[alt="data block preview showing only totals greater than 1000"]
...
NOTE: If you want Dashboard users to dynamically filter data in a Dashboard, see xref:dashboard:FilterDashboard.adoc[]  in instead.
...
In the bottom panel press the {buttonDefineCondition}. This opens the *Condition* panel.
+
image::filterAggregate1.png[alt="press Define Condition button to open Condition panel for grouped data block"]
. Select the ‘Advanced Conditions’ check box.
+
image::AdvancedConditionsBlank.png[alt="Advanced Conditions checkbox toggles between simple and advanced condition modes"]
+
...
====
+
image::filtering4.png[alt="specify post-aggregate condition using menus in the Condition panel"]
...
+
image::filterAggregate2.png[alt="compound post-aggregate condition with multiple clauses added"]
...
. *_Optional:_* To specify the order of evaluation for a compound condition, use the btn:[Indent] and btn:[Unindent] buttons. To indent a set of clauses, select the ‘and’ or ‘or’ operator the condition box, and then press btn:[Indent]. Use btn:[Unindent] to reverse indentation.
+
image::filterAggregate3.png[alt="indented compound condition showing order of evaluation"]
+
Clauses are evaluated in order of indentation, the most indented clauses being evaluated first.
. To delete a clause in the condition, select the clause and press btn:[Delete]. To delete ALL clauses in the condition, press btn:[Clear]. To reposition a clause in the condition, select the clause and press btn:[Up] or btn:[Down].
. To modify  a clause in the condition, select the clause and then specify the clause using the menus at the top (as described above). Press btn:[Modify] to change the existing condition to a new condition matching the menu selections.
...
NOTE: If you want to configure ranking in a specific Chart or Crosstab, see xref:dashboard:FilterDashboard.adoc[] instead.
...
+
image::RankingCondition2.png[alt="press More button and specify ranking condition using the menus"]
+
If the condition you specify references data model attributes that do not already exist in the data block, these attributes are automatically added to the data block as hidden columns.
. Press btn:[Append] to add the condition.
...
NOTE: If you want to apply dynamic conditions at the Dashboard level (rather than at the data level), see xref:dashboard:FilterDashboard.adoc[].
...
. Open the Dashboard for editing. (See xref:dashboard:EditADashboard.adoc[].)

. Add the desired Form component to the Dashboard. (See xref:dashboard:DynamicallySetProperties.adoc[] for more information about Form components.)
...
+
image::dashboard:formControlVariable.png[alt="open Target menu and select Variable node to link form control to variable"]
. Press btn:[OK] to exit the ‘Properties’ panel.

The Form control is now linked to the Data Worksheet Variable, and changes the user makes using the control will alter the values in the  Variable. To set the options displayed by the Form control, see xref:dashboard:RadioCheckBoxComboProperties.adoc[].
====

. Click to select the data block that you want to filter.
. Press the {buttonDefineCondition} button in the bottom panel.
+
image::ConditionButton_Field.png[alt="press Define Condition button in the bottom panel"]
+
This opens the ‘Condition’ panel.
+
image::ConditionDialog.png[alt="Condition panel showing filter options for the selected data block"]
...
+
image::Variable in Condition.png[alt="press Change Value Type and select Variable to use variable in condition"]
+
...
** Type the name of a Dashboard Form component. (See xref:dashboard:DynamicallySetProperties.adoc[] for information about using these components.)
+
image::Variable in ConditionTyped.png[alt="variable name typed into the combo box on the right side of condition"]
. Press ‘OK’ to close the  ‘Condition’ panel.

The Data Worksheet will now prompt for the value of the variable/parameter, and will continue to do so each time the Data Worksheet is freshly  accessed.  The user will also be prompted for the parameter value by any Dashboard that utilizes the Data Worksheet, _unless_ the Dashboard contains a Form component that sets the value of the parameter. If you need to *suppress* the parameter prompt in a Dashboard (for example, if you are setting the parameter using script), use the ‘Prompt for Parameters’ property in the xref:dashboard:DashboardOptions.adoc[] dialog box.

[IMPORTANT]
====
If *no value* is provided for a variable (parameter), it will have a *null* value.  This will cause the condition to be ignored, which generally results in *all* records  being returned.  If this is not the desired behavior, you can either test for null parameter values using xref:commonscript:isNull.adoc[] in  *onRefresh* script (see xref:dashboardscript:AddScriptToDashboard.adoc#AddDashboardScript[Add Dashboard Script]) and substitute a non-null value, or you can add an `[is not][equal to][null]` condition clause, as shown below:

image::nullVariable.png[alt="condition with is not equal to null clause added to handle null variable"]
...
+
image::Session Data in Condition.png[alt="press Change Value Type and select Session Data in the condition panel"]

. In the menu, choose one of ‘User’, ‘Roles’, or ‘Groups’. These parameters return information about the user who is currently accessing the Data Worksheet; respectively, the username, the array of roles to which the user belongs, and the array of groups to which the user belongs. The adjacent menu automatically chooses the `[equal to]` or `[one of]` operator to match your selection.
+
image::SessionData in ConditionChosen.png[alt="session data condition with User Roles or Groups selected from menu"]
+
NOTE: These three options are the only Session Data values available. Custom session attributes cannot be added to this menu.
...
+
image::Field in Condition.png[alt="press Change Value Type and select Field to compare two columns"]
. Select the column containing the values to test.
+
image::Field in Condition2.png[alt="select column containing values to compare on right side of condition"]
+
...
+
image::Expression in Condition.png[alt="press Change Value Type and select Expression to enter expression in condition"]
...
+
image::Expression in Condition2.png[alt="press Edit Formula button to launch Formula Editor for expression entry"]
+
...
+
image::SubQueryMenu.png[alt="press Change Value Type and select Subquery to display the Subquery button"]
. Press the ‘Subquery’ button to open the ‘Subquery’ dialog box.
+
image::SubQueryDialog.png[alt="Subquery dialog box with subquery in column and table column menus"]
...
+
image::twoDWSQueries_3.png[alt="Fast Go Game and Wireless Mouse data blocks added to the worksheet"]
...
. Press the {buttonDefineCondition} button in the bottom panel.
+
image::SubQueryConditionButton.png[alt="press Define Condition button for the Wireless Mouse data block"]
+
This opens the ‘Condition’ panel.
+
image::ConditionDialog.png[alt="Condition panel showing filter options for the selected data block"]
...
+
image::SubQueryExampleList.png[alt="Customers is not one of condition with Subquery button shown"]
...
+
image::SubQueryExampleDialog.png[alt="Subquery dialog with Fast Go Game1 selected and Customers FGG in column menu"]
...
+
image::SubQueryExampleResult.png[alt="Wireless Mouse data filtered to exclude companies that purchased Fast Go Game"]
...
+
image::SubQueryExampleRowDep1.png[alt="Category and Total columns dragged from Order Model to create data block"]
+
[NOTE]
...
+
image::SubQueryExampleRowDep2.png[alt="press Define Condition in bottom panel to open Condition panel for sales this week"]
.. Press the btn:[More] button and enter the following condition: `[Order Date] [is] [in range] [This week]`.
+
image::SubQueryExampleRowDep3.png[alt="condition Order Date is in range This week entered for sales this week block"]
.. Press btn:[OK].
.. Press the {buttonGroupAndAggregate} in the bottom panel to open the *Group and Aggregate* panel.
+
image::SubQueryExampleRowDep5.png[alt="press Group and Aggregate button to configure grouping for the data block"]
...
+
image::SubQueryConditionRDButton.png[alt="press Define Condition button on sales this week data block to reopen Condition panel"]
. Press the btn:[More] button.
...
+
image::SubQueryExampleDialog2.png[alt="press Change Value Type and select Subquery to open subquery button"]
...
+
image::changeSubQuerySettings.png[alt="Subquery dialog with in column subquery column and current table column configured"]
. Press btn:[OK] to close the ‘Subquery’ dialog box to return to the *Condition* panel.
+
image::SubQueryExampleRowDep4.png[alt="Condition panel showing row-dependent subquery condition configured"]
...
+
image::TablewithSubQuery.png[alt="two data blocks connected by subquery link in the worksheet canvas"]
...
+
image::SubQueryExampleRowDep6.png[alt="sales this week data block showing only categories with higher sales than last week"]
...
+
image::ConditionButton.png[alt="press Define Condition button to open the Condition panel"]

. Select the *MV Update* tab.
+
image::MVUpdateTab.png[alt="select MV Update tab in the Condition panel"]
...
+
image::ConditionDialogMV.png[alt="Conditions dialog box for specifying materialized view update conditions"]
If you select the ‘Expression’ option for the right side of the condition, follow the additional steps below:
+
image::MVUpdateExpressionOption.png[alt="Expression option selected on right side of materialized view condition"]
...
+
image::MVUpdateMaxValue.png[alt="MV MaxValue represents maximum value of field specified in left-side menu"]
