...
// metadata for AI
ifdef::ai[]
Summary: How to create new calculated data fields, including expression columns (JavaScript or SQL) in a Data Worksheet data block, and Dashboard calculated fields for detail-level or aggregate-level calculations. Covers formula syntax, data types, and referencing existing columns.
endif::ai[]
:experimental:
...
+
image::dataworksheet:agileTransform3.png[alt="press Create Expression button in the bottom panel"]
+
...
+
include::ROOT:partial$DataTypeTable.adoc[]
+
...
+
image::dataworksheet:agileTransform5.png[alt="Formula Editor with expression entered in the script area"]
+
...
+
image::dataworksheet:DateParts2.png[alt="date type fields showing available date parts in Script mode"]
+
TIP: To compute date parts  using a *fiscal calendar*,  use  fiscal functions such as xref:commonscript:CALC_fiscalweek.adoc[CALC.fiscalweek] and xref:commonscript:CALC_fiscalmonth.adoc[CALC.fiscalmonth] in the Formula Editor.
+
. Press btn:[OK] to close the Editor and create the new column.
+
image::dataworksheet:agileTransform6.png[alt="new expression column added to the data block"]
+
...
. Edit the Dashboard to which you want to add an expression field. See xref:dashboard:EditADashboard.adoc[].
...
+
image::dashboard:NewCalculatedField1.png[alt="right-click data block in Toolbox and select New Calculated Field"]
+
...
+
image::dashboard:NewCalculatedField2.png[alt="select Detail or Aggregate option in Calculated Field panel"]
+
...
+
. Press btn:[OK] to close the Formula Editor. This adds the new calculated field to the data source at the top of the *Toolbox* panel.
+
image::dashboard:NewCalculatedField4.png[alt="new calculated field added to data source in Toolbox panel"]


You can display this calculated field in Output Components (see xref:dashboard:AddAKPI.adoc[]) or Data View Components (see xref:dashboard:AddAChart.adoc[] and xref:dashboard:AddTable.adoc[]) in the same way that would display any other data field.
...
To create a detail-level calculation, follow the steps below.
...
This opens the ‘Calculated Field’ panel.
+
. In the ‘Name’ field, enter a name for the new calculated field. (See xref:#CreateCalcField[] above for full details about naming.)

. Select the ‘Detail’ option.

. Enter  the desired expression. The available detail fields are listed in the top-left panel. Click on an available field in the panel to add it to the expression. See xref:#ExcelStyleFunctions[] below for common operators in detail calculations.
+
image::dashboard:NewCalculatedField3.png[alt="Edit Calculated Field dialog showing detail fields available for expression"]
+
NOTE:  *Date*-type fields supply the following date parts: Year, QuarterOfYear, MonthOfYear,             DayOfMonth, and DayOfWeek. *Time*-type fields supply the following date parts: HourOfDay, MinuteOfHour,            SecondOfMinute. *TimeInstant*-type fields supply all the previous date parts.
...
Many expressions are similar to formulas that you might use in Microsoft Excel^®^. Use these CALC functions  whenever your calculation can be expressed as a single formula.  Some common examples are shown below.  (See xref:commonscript:CalcObjectFunctions.adoc[] for a more complete listing.)
...
****
The Formula Editor may already contain the desired aggregate fields in the top-left panel. Click on an available field in the panel to add it to the expression.

To create an aggregate-level calculation, follow the steps below.
...
. Select the ‘Aggregate’ option.  If  the Dashboard already contains aggregating components such as Charts or Crosstabs, you may see predefined aggregates already available in the left panel.

. To add a new aggregate to the list, follow the steps below:

.. Press the ‘New Aggregate’ link in the top-left panel. This opens the ‘Aggregate’ dialog box.
+
image::dashboard:CalcFieldExample8.png[alt="press New Aggregate link to add a new aggregate to the list"]
...
+
image::dashboard:CalcFieldExample9.png[alt="select field to aggregate in the Aggregate dialog box"]
+
...
+
image::dashboard:CalcFieldExample10.png[alt="newly defined aggregate appears in the top-left panel of Formula Editor"]
+
. *_Optional:_*  Press the ‘New Aggregate’ button again, and repeat the above steps to add as many additional aggregates as desired. To *delete* an aggregate from the list, press the {buttonActions} to the right of the aggregate, and select '`Delete Column(s)`'.

. Once you have defined the required aggregates in the top-left panel (or have the predefined aggregates that you want), enter the desired calculation (using the aggregate fields) in the formula field at the bottom of the dialog box.
+
NOTE: Click an aggregate field in the top-left panel to add it to the expression.
+
image::dashboard:CalcFieldExample11.png[alt="enter calculation using aggregate fields in the formula area"]
...
image::dashboard:galleryBubble2.png[alt="Global Visual Analysis dashboard sample using calculated fields for map colors"]
****
