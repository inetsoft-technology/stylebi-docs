...
// metadata for AI
ifdef::ai[]
Summary: How to copy a data block in a Data Worksheet in Visual Composer using a regular copy (independent duplicate) or a mirror copy (linked copy, synchronized duplicate that reflects changes to the original).
endif::ai[]
...
image::MirroredTables.png[alt="right-click table title row and select Mirror from context menu"]

The new mirrored table is created, and a graphical link to the original table is drawn to indicate the dependency.

image::MirroredTables1.png[alt="mirrored table linked to original table by graphical dependency line"]
