...
// metadata for AI
ifdef::ai[]
Summary: How to create a Data Worksheet query from a monday.com data source using GraphQL, including writing the query with optional variables, configuring pagination (Pagination Variable and Pagination JSON Path), choosing a Request Type (GET or POST), specifying a JSON Path to extract fields from the response (with examples for a bare array, a named-field-wrapped array, and a top-level array of mixed content), and optionally enabling the Expand Arrays option.
endif::ai[]
