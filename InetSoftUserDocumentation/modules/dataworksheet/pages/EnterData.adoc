...
// metadata for AI
ifdef::ai[]
Summary: How to create and manage an Embedded Table (also called an editable grid, editable table, or data entry grid) in a Data Worksheet by manually typing data, including creating the table, editing column headers and data cells, adding or removing rows, converting an existing data block to an Embedded Table, importing data from a CSV or Excel file, and configuring a Dashboard Form component or Dashboard Table to modify Embedded Table values from a Dashboard.
endif::ai[]
...
+
image::newEmbeddedTableToobar.png[alt="press Create Object button and select Embedded Table from toolbar"]
...
+
image::EmbeddedTableCreate.png[alt="Embedded Table dialog to specify table name rows and columns"]
...
+
image::embeddedTableEnterData.png[alt="click a data cell in bottom panel to enter text directly"]
...
+
image::EmbeddedTableCreate2.png[alt="Embedded Table dialog creating table with three rows and three columns"]
...
+
image::EditableEmbeddedTable.png[alt="double-click column header to switch cell to editing mode"]
...
+
image::filledEmbeddedTable.png[alt="embedded table with columns A B C and data values entered in cells"]
****
...
+
image::insertRowEmbedded.png[alt="right-click table row and select Insert Row or Append Row"]
+
...
+
image::SalesbyCategoryLiveData.png[alt="Sales data block in live data view showing sales by category"]
...
+
image::converttoEmbedded2.png[alt="select Query1 data block and press Change View to select Edit action"]

. Click on any of the cells in the bottom panel. Notice that all the cells are now editable.
+
image::EmbeddedTableEditable.png[alt="embedded table with all cells now editable after conversion"]
****

== Import Data into an Embedded Table


To upload text or Excel data into a new Embedded Table, see xref:UploadData.adoc[]. To upload text or Excel data into an existing Embedded Table, follow the steps below.
...
+
image::importCSVButton.png[alt="press Import Data File button in the bottom panel"]
+
This opens the ‘Import Data File’ dialog box.
+
image::embeddedTable4.png[alt="Import Data File dialog with file encoding delimiter and header options"]
...
+
image::dashboard:Input Browse Embedded Table.png[alt="embedded table representation displayed for selecting target cell"]
