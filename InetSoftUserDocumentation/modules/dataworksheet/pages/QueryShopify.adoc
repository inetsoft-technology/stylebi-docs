...
// metadata for AI
ifdef::ai[]
Summary: How to create a Data Worksheet query from a Shopify data source, including entering a GraphQL query, specifying a JSON Path expression to extract fields from the response (with examples for a bare array, a named-field-wrapped array, and a top-level array of mixed content), and optionally enabling the Expand Arrays option to flatten array records into multiple rows.
endif::ai[]
...
+
image::shopify1.png[alt="Shopify Query dialog box for configuring the data query"]
