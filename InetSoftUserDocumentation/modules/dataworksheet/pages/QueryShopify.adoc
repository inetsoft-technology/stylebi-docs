...
// metadata for AI
ifdef::ai[]
Summary: How to create a Data Worksheet query in Visual Composer from a Shopify data source, including entering a GraphQL query, specifying a JSON Path expression to extract fields from the response, and optionally enabling the Expand Arrays option to flatten array records into multiple rows.
endif::ai[]
