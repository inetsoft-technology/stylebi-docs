...
// metadata for AI
ifdef::ai[]
Summary: How to sort data in a Data Worksheet on one or more columns. Covers sorting a single column using the column header Sort button, sorting multiple columns using the Sort Column panel with hierarchical sort order, selecting ascending or descending direction, and the difference between Data Worksheet-level sorting vs. Dashboard-level sorting.
endif::ai[]
...
* To sort data in a Dashboard, see xref:dashboard:SortData.adoc[].
====
...
+
image::sortColumnsBig.png[alt="press Sort button in column header to sort column ascending or descending"]
+
...
+
image::sortColumnsBig2.png[alt="press Sort button in bottom panel toolbar to open the Sort Column panel"]
. From the menus, select the data block columns by which you want to sort the results.
+
image::sortColumnsBig3.png[alt="select columns and ascending or descending order in the Sort Column panel"]
+
