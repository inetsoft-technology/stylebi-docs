...
// metadata for AI
ifdef::ai[]
Summary: How to create and manage a concatenated data block in a Data Worksheet by combining two or more data blocks vertically using union, intersection, or difference (minus) operations. Covers creating each concatenation type, changing the concatenation type (including keeping duplicate rows for union), reordering tables, and adding tables to an existing concatenation using the Concatenation Editor.
endif::ai[]

Concatenations allow you to combine data from different data blocks, which may represent different database tables, queries, or data models.  A _concatenated data block_ is a *vertical composition* of two or more data blocks with a specified relationship (union, intersection, difference) between the rows of the base data blocks.
...
+
image::ConcatenateTableButton2.png[alt="press Concatenate button in toolbar to select concatenation type"]
...
+
image::IntersectConcat2.png[alt="intersect concatenation applied to two selected data blocks"]
+
...
+
image::IntersectPreview.png[alt="preview of intersection result showing common customers"]
+
...
+
image::DragQueryforUnion_1.png[alt="union concatenation applied to Wireless Mouse and Fast Go Game data blocks"]
+
This creates a new data block, ‘Query1’.

. Click the ‘Query1’ data block to select it. In the bottom panel, press the {buttonChangeView} and choose {buttonLiveDataView}.
+
image::UnionConcatPreview.png[alt="preview of union result combining customers from both data blocks"]
+
...
+
image::MinusConcat2.png[alt="minus concatenation applied to Fast Go Game and Wireless Mouse data blocks"]
. Click the ‘Query1’ data block to select it. In the bottom panel, press the {buttonChangeView} and choose {buttonLiveDataView}.
+
image::MinusPreview.png[alt="preview of minus result showing unique customers from first table"]
+
...
+
image::editConcatenation.png[alt="right-click concatenated table and select Edit Concatenation"]
+
...
+
image::ConnectorLine.png[alt="Concatenation Editor showing connector line between data blocks"]
. Select a new concatenation type.
+
image::ConnectorLine2.png[alt="select new concatenation type in Concatenation Type panel"]
...
+
image::Concatenate Reorder 1.png[alt="press Reorder Tables button to open the Reorder Tables dialog"]
...
+
image::Concatenate Reorder 2.png[alt="reorder tables dialog with arrow buttons to change table order"]
. Press btn:[Done] to close the Editor.
...
+
image::editConcatenation2.png[alt="drag additional table to concatenation border in the editor"]
. Press btn:[Done] to close the Editor.
