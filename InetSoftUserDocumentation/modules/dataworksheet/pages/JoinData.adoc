...
TIP: To construct a dataset from a single data source, consider using xref:QueryData.adoc[] instead. To combine different tables in ways that are not described by formal joins (e.g., referencing arbitrary cells), use a Freehand Table (see xref:dashboard:AddFreehandTable.adoc[]).
