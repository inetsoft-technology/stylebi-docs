...
To join data blocks using an inner join, outer join, or inequality join, follow the steps below:

[.videotitle]
.Watch Video
[%collapsible]
=====
.Create an inner join, outer join, inequality join...
...
. To join data blocks *by dragging fields*,  follow these steps:
...
+
This creates a new joined data block and opens the Join Editor.
+
image:JoiningTablesActionResult.png[]
.. Proceed to define the joins in the Join Editor as explained below.

. To  join data blocks by using  the *{buttonJoin}*, first follow these steps:
+
.. Select the data blocks you want to join together. (You can hold the {buttonShift} key and click on the data blocks to select both, or drag with the mouse to “lasso” the data blocks.)
...
image:mashup3_1.png[]
+
NOTE: Some joins may have been created by default. To delete a join, click  the join symbol in the link and select ‘Remove Join Condition’ from the context menu.
+
...
image:mashup3_3.png[]
+
NOTE: An *outer join* is a generalization of the inner join: In addition to the matching rows preserved by the equi-join, the left outer join returns all the rows in the left table, while the right outer join returns all the rows in the right table. The full outer join returns all the rows of both tables. Note: When you specify an outer join, the additional rows included from a table (beyond those selected by the equi-join) do not have matching rows in the other table. Therefore, outer-join tables generally exhibit empty cells corresponding to these unmatched rows.
...
. To view the data in the joined data block, click to select the joined data block ('Query1') and press the {buttonChangeView} in the bottom panel. Select {buttonLiveDataView}.
+
image:mashup5.png[]
+
...
. Right-click the joined data block (‘Orders and Returns’), and select ‘Set As Primary’ from the context menu. {rightClickOptions}
+
image:mashup8_1.png[]
+
...
To change the join type of a joined data block, follow these steps. (You can change the join type for all joined data blocks *_except_* for merge-joins, because merge-joins do not define a relationship.)
