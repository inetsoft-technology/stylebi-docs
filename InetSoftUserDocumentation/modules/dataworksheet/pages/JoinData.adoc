...
// metadata for AI
ifdef::ai[]
Summary: How to join (SQL join, table join, merge tables) two or more data blocks horizontally in a Data Worksheet, defining equality or inequality join conditions to combine data from different tables, queries, or data models. Covers inner join, outer join, inequality join, cross join, and merge join, as well as editing joins (changing join type, reorder tables, add tables, hide duplicate columns). Joining is one of the techniques performed while mashing up data in a Data Worksheet — it is not an alternative to "mashup," which is the general term for the whole drag-and-drop worksheet workflow. See MashupData.adoc. This is the recommended, simpler approach — rather than xref:dashboard:AddFreehandTable.adoc[Freehand Table] — for a request like "a table where cells pull from different data sources plus a custom calculation," as long as the sources share a common row/key: join them here, add the calculation as a Calculated Field (see CreateNewDataField.adoc), and bind a regular Table or Crosstab to the joined result. IMPORTANT: unlike aggregation (see GroupAggregateData.adoc), which can be performed at either the Data Worksheet or Dashboard level, joining has no Dashboard-level equivalent. A Chart/Crosstab/KPI can aggregate its single bound data block at the Dashboard level, but nothing at the Dashboard level combines separate data blocks/tables/data models the way a join does — that must happen here, in the Data Worksheet, before the data reaches a Dashboard. Do not describe Dashboard-level binding, chart/crosstab properties, filter components, or component script as a way to "join" data at the Dashboard level.
endif::ai[]

[WARNING,caption=see also]
xref:ConcatenateData.adoc[] to create a *vertical composition* of two or more data blocks.

Joins allow you to combine data from different data blocks, which may represent different database tables, queries, or data models. A _joined data block_ is a *horizontal composition* of two or more data blocks with a specified relationship (equality or inequality) between the rows of the base data blocks. The following sections explain how to create and edit joins.

== When to Use
This approach may produce the most efficient queries when you need to combine data from different data sources, or when you are using a data model.

TIP: To construct a dataset from a single data source, consider using xref:QueryData.adoc[] instead. To combine different tables in ways that are not accomplished by formal joins (e.g., referencing arbitrary cells), use a Freehand Table (see xref:dashboard:AddFreehandTable.adoc[]).
...
+
image::joinHandles.png[alt="join handles displayed on each side of the selected data block"]
. Click on one of the join handles (left or right) and drag it onto a join handle on the data block that you want to join.
+
image::JoiningTablesAction.png[alt="drag join handle from one data block onto join handle of the other"]

This creates a new joined data block and opens the Join Editor. Proceed to xref:#UsetheJoinEditor[] to define the joins.

image::JoiningTablesActionResult.png[alt="new joined data block created after connecting join handles"]
...
+
image::mashup2_1.png[alt="press Join button in toolbar and select Inner Outer Join"]
...
+
image::mashup3_1.png[alt="drag join field from one data block onto corresponding field in the other"]
+
...
image::mashup3_2.png[alt="two join conditions linking Name and Order Num columns between Orders and Returns"]
****
+
. *_Optional:_* To change an inner join to an outer join, right-click on the join link and choose the desired outer join.
+
image::mashup3_3.png[alt="right-click join link and choose outer join type from context menu"]
+
...
+
image::JoiningTablesAction4.png[alt="drag additional table from left panel into the Join Editor"]

. Press btn:[Done].
+
image::mashup3_4.png[alt="press Done to close the Join Editor and create the joined data block"]
+
This creates the joined data block ('`Query1`').
+
image::mashup4_1.png[alt="Query1 joined data block created in the Data Worksheet canvas"]
...
+
image::InnerJoinExp1.png[alt="Sales1 and customers1 data blocks added to the Data Worksheet"]
...
+
image::InnerJoinExp2.png[alt="drag join handle from customers1 onto join handle of Sales1 data block"]
+
...
+
image::CreatingJoin.png[alt="drag Company column from Sales1 onto company name column of customers1"]
. Press btn:[Done] to close the Join Editor and create the new joined data block, ‘Query1’.
+
image::JoinedTable.png[alt="Query1 joined data block created combining sales and customer data"]
...
+
image::JoinedPreview.png[alt="preview of Query1 joined data block showing combined sales and customer columns"]
****
...
+
image::CrossJoinOperation.png[alt="press Join button in toolbar and select Cross Join option"]
+
...
+
image::CrossJoinExp2.png[alt="new embedded table Query1 created with two rows and two columns"]
...
+
image::CrossJoinExp3.png[alt="column headers renamed to First Name and Last Name in embedded table"]
.. In the table cells below the columns headers, enter the following names:
+
image::CrossJoinExp4.png[alt="salesperson names entered in First Name and Last Name table cells"]
...
+
image::CrossJoining.png[alt="press Join button and select Cross Join to create cross-joined data block"]
+
This creates a new joined data block, ‘Query2’.
+
image::CrossJoined.png[alt="Query2 cross-joined data block created from customers and salesperson data blocks"]
...
+
image::previewCrossJoin.png[alt="preview showing cross-product of customers and salesperson data blocks"]
+
...
+
image::MergeJoinToolbar.png[alt="press Join button in toolbar and select Merge Join option"]
+
This creates the new merge-joined data block.
+
image::MergeJoinToolbar2.png[alt="new merge-joined data block created horizontally combining both data block columns"]
+
...
+
image::twoDWSQueries_2.png[alt="Wireless Mouse and Fast Go Game data blocks added to the worksheet"]
...
+
image::MergeJoinButton.png[alt="press Join button in toolbar and select Merge Join for the two data blocks"]
+
This creates a new data block, ‘Query1’.
+
image::MergeJoin.png[alt="Query1 merge-joined data block combining Wireless Mouse and Fast Go Game columns"]
...
+
image::previewMergeJoin.png[alt="preview of merge-joined data block showing both data sets side by side"]
+
...
image::ChangeJoinTypeAction.png[alt="right-click joined data block and select Edit Join from context menu"]
...
image::JoinTypeWorksheetExplorer.png[alt="right-click join link and select desired join type from context menu"]
...
+
image::JoinCustomerIDs.png[alt="inner join created between All Sales Company field and Fast Go Game Customers field"]
...
+
image::previewJoinedCustomerIDs.png[alt="inner join preview showing only companies that purchased Fast Go Game"]
+
...
+
image::ChangeJoinCustomerIDType.png[alt="click join link and select Right Outer Join from context menu"]
. Press btn:[Done] to close the Join Editor.
. Preview the joined data block again.
+
image::previewLeftJoinedCustomerIDs.png[alt="right outer join preview showing all records from All Sales including unmatched rows"]
+
...
+
image::Join Reorder Tables.png[alt="press Reorder Tables button to open the Reorder Tables dialog"]
...
+
image::Join Reorder Tables2.png[alt="Reorder Tables dialog with arrow buttons to set table join order"]
...
+
image::ChangeJoinTypeAction2.png[alt="drag additional table from left panel into the Join Editor"]
...
image::mashup5.png[alt="select joined data block and press Change View to see live data"]
...
image::mashup6_1.png[alt="press Show Hide button on duplicate columns to hide them from view"]
...
image::mashup7.png[alt="Table Properties panel with Name field for renaming the joined data block"]
...
image::mashup8_1.png[alt="right-click joined data block and select Set As Primary from context menu"]
...
+
image::worksheetNavigation.png[alt="hold Shift and Ctrl and hover over column to highlight its source data block"]
