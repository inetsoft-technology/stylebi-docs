...
// metadata for AI
ifdef::ai[]
Summary: How to create a REST JSON query in a Data Worksheet from an existing REST JSON connection, including entering a URL suffix and JSON Path expression, optionally expanding arrays, selecting a pagination type, and configuring Lookup Queries to resolve values from URLs embedded in JSON fields. Also covers the optional Tabular selection and query timeout settings. Includes JSON Path examples for filtering objects by a field value and for indexing into a specific numbered position of a top-level array (e.g. a response shaped as [metadata, data]), using $.[1][*] to select and expand the data element.
endif::ai[]
...
+
image::dataSourceREST5.png[alt="REST JSON Query dialog box for configuring the data query"]
...
+
image::dataSourceREST6.png[alt="URL Suffix and JSON Path fields entered for the REST query"]
+
****
.Example: JSON Path...
[%collapsible]
...
image::JSON_LookupQuery.png[alt="Lookup Query configured to query URL from the base query field"]
====
