...
// metadata for AI
ifdef::ai[]
Summary: How to create a Data Worksheet query in Visual Composer from an R connection by entering an R script (data set name or analysis script), with optional Pre-execution JavaScript to pass Worksheet data blocks and variables into the R session, and optional Post-execution JavaScript to write R session data frames or variables back into Worksheet Embedded Tables.
endif::ai[]
...
+
image::dataSourceR5.png[alt="select R Query button from the toolbar"]
+
This opens the ‘R  Query’ dialog box.
+
image::dataSourceR6.png[alt="R Query dialog box for configuring the data query"]
