...
// metadata for AI
ifdef::ai[]
Summary: How to create a Data Worksheet query in Visual Composer using the Database Query dialog box, supporting both basic and advanced SQL query modes, including column selection, joins, filter conditions, grouping, sorting, SQL string editing, and adding query variables or materialized view update conditions.
endif::ai[]
...
* You are *not* joining data from multiple datasources
* You have already created queries for other applications, and want to copy over the SQL strings intact

If you *are* using a data model, or need to join diverse datasources, it may be simpler to use the approach described in xref:JoinData.adoc[].
