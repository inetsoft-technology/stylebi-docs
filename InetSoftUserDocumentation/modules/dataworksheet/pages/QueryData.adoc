...
// metadata for AI
ifdef::ai[]
Summary: How to create a Data Worksheet query using the Database Query dialog box, writing SQL directly against a relational database for cases where visual data block assembly is insufficient.
endif::ai[]

// metadata for AI
ifdef::ai[]
Summary: How to create a Data Worksheet query from an existing Query Data connection, including browsing available fields and configuring query parameters. See QuerySpecificDataSource for the full list of available data source query types.
endif::ai[]
...
If you *are* using a data model, or need to mashup diverse datasources, it may be simpler to use the approach described in xref:MashupData.adoc[].
...
+
image::newWorksheet3.png[alt="New Data Worksheet dialog showing options including query for the data source"]
...
image::databaseQuery1.png[alt="press Create Object button and select Database Query from toolbar"]

If you do not see the ‘_Database_ Query’ option, this indicates that you do not have '`Physical Table`' permissions.  See xref:administration:SetSecurityActions.adoc[] for more information.
====
...
+
image::databaseQuery2.png[alt="new data block created from the specified database query"]

. To edit the query further, press the {buttonEdit} in the bottom panel. This reopens the ‘Database Query’ dialog box.
+
image::databaseQuery3_1.png[alt="press Edit button in bottom panel to reopen the Database Query dialog"]
...
+
image::agileData11_1.png[alt="choose database in the data source menu in the Database Query dialog"]

. Select the database fields that you want to retrieve by dragging them from the left panel into the ‘Columns’ area.
+
image::agileData12_1.png[alt="drag database fields from left panel into the Columns area"]
...
+
image::agileData13_1.png[alt="Add Join dialog with table and join field menus and join operator"]
+
...
+
image::agileData20_1.png[alt="join displayed in the Joins area after pressing OK in Add Join dialog"]
+
...
+
image::agileData21_1.png[alt="multiple joins displayed in the Joins area of the query definition"]

.. To change an existing join, press the {buttonEdit} next to the join.
+
image::agileData22_1.png[alt="press Edit button next to an existing join to modify it"]
.. To delete an existing join, press the {buttonDelete} button next to the join.
+
image::agileData23_1.png[alt="press Delete button next to a join to remove it from the query"]
...
+
image::agileData14_1.png[alt="press Edit button next to Conditions area to open the Conditions dialog"]
+
...
====
+
image::agileData15_1.png[alt="press Edit button next to SQL area to open the SQL string for manual editing"]
+
...
Then create a runtime script to replace the variable `var` with a table name such as `customers` or `prospects`.  You can place this script in the *onInit* handler  (see xref:dashboardscript:AddScriptToDashboard.adoc#AddDashboardScript[Add Dashboard Script]).

IMPORTANT: Parameters in the query that have a *null* value may cause  a SQL error.  You should therefore check   (for example, using  Dashboard xref:dashboardscript:AddScriptToDashboard.adoc#AddDashboardScript[onLoad] script) to ensure that all parameters referenced in the query have legitimate values.
...
+
image::advancedQuery1.png[alt="enable the Advanced Query option in the Database Query dialog"]

. Enter a '`Name`' for the query.

. Under the *Links* tab, drag the desired database tables from the left panel to the right panel.
+
image::advancedQuery2.png[alt="drag database tables from left panel to right panel under the Links tab"]
+
. Expand the tables if desired by pressing the {buttonShowColumns}  in the title bar.
+
image::advancedQuery3.png[alt="press Show Columns button to expand the table and view fields"]
...
+
image::advancedQuery4.png[alt="drag join handle from one table to the join handle of another table"]
+
...
+
image::advancedQuery5.png[alt="Join Editor opened showing automatically created join between the tables"]
...
+
image::advancedQuery6.png[alt="drag join field from one table onto corresponding field in the other table"]
+
...
image::mashup3_3.png[alt="right-click join link and choose right outer join from context menu"]
...
+
image::advancedQuery7.png[alt="all tables joined with their relationships defined in the Links tab"]
...
+
image::advancedQuery8.png[alt="drag or double-click fields to move them to the Query Fields panel under Fields tab"]
...
+
image::advancedQuery9.png[alt="Add Expression dialog with SQL expression entered to define a derived column"]
...
+
image::advancedQuery10.png[alt="select expression column and type alias in the Field Alias box"]
...
+
image::advancedQuery11.png[alt="Conditions tab with filtering condition menus and Change Value Type button"]
...
+
image::advancedQuery12.png[alt="select sort field and press Sort button to choose ascending or descending order"]
...
+
image::advancedQuery13.png[alt="SQL String tab showing the SQL statement representing the current query definition"]
